...
{
    public partial class frm_QuanlyBA_Noikhoa : Form
    {
        ...
        public frm_QuanlyBA_Noikhoa(string lstLoaiBA)
        {
            InitializeComponent();
        ...
        void InitEvents()
        {
            ...
            Load += frm_QuanlyBA_Noikhoa_Load;
            KeyDown += frm_QuanlyBA_Noikhoa_KeyDown;
        ...
        private void frm_QuanlyBA_Noikhoa_Load(object sender, EventArgs e)
        {
        ...
        private void TimKiemThongTin()
        {
            ...
            m_dtData = SPs.EmrBaNoikhoaLaydanhsachBA(tungay, denngay, ma_luotkham, ma_BA, ten_benhnhan, id_khoadieutri).GetDataSet().Tables[0];
        ...
        private void frm_QuanlyBA_Noikhoa_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void cmdUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (MauBA == LoaiBA.BA_NOIKHOA)//Bệnh án nội khoa
            {
                ...
                EmrBaNoikhoa bant = EmrBaNoikhoa.FetchByID(Utility.Int64Dbnull(grdList.GetValue("id_ba")));
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!Utility.Coquyen("EMR_XOA_BENH_AN"))
                {
                 ...
                 objEmrBa = EmrBaNoikhoa.FetchByID(Utility.Int64Dbnull(grdList.GetValue(EmrBaNoikhoa.Columns.IdBa)));
                if (objEmrBa == null)
                {
                ...
                if (Utility.AcceptQuestion(string.Format("Bạn có chắc chắn muốn xóa Bệnh án với mã {0} của người bệnh {1} hay không?", grdList.GetValue(EmrBaNoikhoa.Columns.MaBa).ToString(), grdList.GetValue("ten_benhnhan").ToString()), "Xác nhận xóa bệnh án", true))
                {
                    try
                    {
                        ...
                        DataRow[] arrDr = m_dtData.Select(string.Format("{0}={1}", EmrBaNoikhoa.Columns.IdBa, grdList.GetValue(EmrBaNoikhoa.Columns.IdBa)));
                        if (arrDr.Length > 0)
        ...
        void _OnCreated(long id,string ma_ba, action m_enAct)
        {
            try
            {
                DataTable dt_temp = SPs.EmrBaNoikhoaLaydanhsachBA(new DateTime(1900, 1, 1), new DateTime(1900, 1, 1), "", ma_ba, "", -1).GetDataSet().Tables[0];
        ...
        bool DeleteMe()
        {
            try
            {
                using (var scope = new TransactionScope())
                {
                    using (var dbscope = new SharedDbConnectionScope())
                    {
                        new Delete().From(EmrBaNoikhoa.Schema).Where(EmrBaNoikhoa.Columns.IdBa).IsEqualTo(Utility.Int32Dbnull(grdList.GetValue(EmrBaNoikhoa.Columns.IdBa), -1)).Execute();
                    }
                    scope.Complete();
        ...
        private void cmdPrint_Click(object sender, EventArgs e)
        {
            ...
            //    string ma_luotkham = grdList.GetValue(EmrBaNoikhoa.Columns.MaLuotkham).ToString();
            //    long id_phieu = Utility.Int64Dbnull(grdList.GetValue(EmrBaNoikhoa.Columns.IdPhieu));
            //    DataTable dtData =
      ...
      void InitData4Print()
        ...
        {
            frm_BenhAn_NoiKhoa BenhAn_NoiKhoa = new frm_BenhAn_NoiKhoa("01/BV1");
