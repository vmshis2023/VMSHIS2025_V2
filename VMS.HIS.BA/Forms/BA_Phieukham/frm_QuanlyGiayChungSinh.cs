...
{
    public partial class frm_QuanlyGiayChungSinh : Form
    {
        ...
        public frm_QuanlyGiayChungSinh()
        {
            InitializeComponent();
        ...
        void InitEvents()
        {
            ...
            Load += frm_QuanlyGiayChungSinh_Load;
            KeyDown += frm_QuanlyGiayChungSinh_KeyDown;
        ...
        private void frm_QuanlyGiayChungSinh_Load(object sender, EventArgs e)
        {
        ...
        private void TimKiemThongTin()
        {
            ...
            m_dtData = SPs.EmrGiayChungsinhLaydanhsach(-1,tungay, denngay, "",100,-1, ma_luotkham,ten_benhnhan,"","",100).GetDataSet().Tables[0];
            Utility.SetDataSourceForDataGridEx(grdList, m_dtData, true, true, "1=1", "ngaycap_giaychungsinh,ngaysinh_be,hoten_be");
            ModifyCommand();
        }
        ...
        private void frm_QuanlyGiayChungSinh_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            try
            {
                EmrGiayChungsinh objGiayXacnhan = EmrGiayChungsinh.FetchByID(Utility.Int32Dbnull(grdList.GetValue(EmrGiayChungsinh.Columns.Id), -1));
                if (objGiayXacnhan == null)
                {
                    Utility.ShowMsg(string.Format("Giấy chứng sinh của bé {0} con của sản phụ {1} có thể đã bị người khác xóa ở chức năng khác. Vui lòng kiểm tra lại bằng cách nhấn nút tìm kiếm", grdList.GetValue("hoten_be").ToString(), grdList.GetValue("ten_benhnhan").ToString()));
                    return;
                }
        ...
        private void cmdPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    EmrGiayChungsinh _phieu = new Select().From(EmrGiayChungsinh.Schema)
                           .Where(EmrGiayChungsinh.Columns.Id).IsEqualTo(Utility.Int64Dbnull(grdList.GetValue(EmrGiayChungsinh.Columns.Id)))
                           .ExecuteSingle<EmrGiayChungsinh>();
                    if (_phieu.Id <= 0)
                    {
                        Utility.ShowMsg("Bạn cần lưu thông tin Giấy chứng sinh trước khi thực hiện in phiếu");
                        return;
                    }
