...
{
    public partial class frm_QuanlyBienBanHoiChanThongQuaMo : Form
    {
        ...
        public frm_QuanlyBienBanHoiChanThongQuaMo()
        {
            InitializeComponent();
        ...
        void InitEvents()
        {
            ...
            Load += frm_QuanlyBienBanHoiChanThongQuaMo_Load;
            KeyDown += frm_QuanlyBienBanHoiChanThongQuaMo_KeyDown;
        ...
        private void frm_QuanlyBienBanHoiChanThongQuaMo_Load(object sender, EventArgs e)
        {
        ...
        private void TimKiemThongTin()
        {
            ...
            m_dtData = SPs.EmrPt01BienbanhoichanthongquamoLaydanhsach(-1,tungay, denngay, "", ma_luotkham,ten_benhnhan).GetDataSet().Tables[0];
            Utility.SetDataSourceForDataGridEx(grdList, m_dtData, true, true, "1=1", "sngay_hoichan,ten_benhnhan");
            ModifyCommand();
        }
        ...
        private void frm_QuanlyBienBanHoiChanThongQuaMo_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            frm_BienBanHoiChanThongQuaMo _bienban = new frm_BienBanHoiChanThongQuaMo();
        ...
        void _OnCreated(long id, action m_enAct)
        {
            try
            {
                DataTable dt_temp = SPs.EmrPt01BienbanhoichanthongquamoLaydanhsach(id, new DateTime(1900, 1, 1), new DateTime(1900, 1, 1),"","","").GetDataSet().Tables[0];
                if (m_enAct == action.Delete)
                {
                    if (DeleteMe())
                    {
                        DataRow[] arrDr = m_dtData.Select(string.Format("{0}={1}", EmrPt01Bienbanhoichanthongquamo.Columns.Id, grdList.GetValue(EmrPt01Bienbanhoichanthongquamo.Columns.Id)));
                        if (arrDr.Length > 0)
                ...
                if (m_enAct == action.Update && m_dtData != null && m_dtData.Columns.Count > 0 && dt_temp.Rows.Count > 0)
                {
                    ...
                    if (arrDr.Length > 0)
                    {
                        arrDr[0]["sngay_bienban"] = dt_temp.Rows[0]["sngay_bienban"];
                        arrDr[0]["sngay_hoichan"] = dt_temp.Rows[0]["sngay_hoichan"];
                        arrDr[0]["sngay_dukien"] = dt_temp.Rows[0]["sngay_dukien"];
                ...
                m_dtData.AcceptChanges();
                Utility.GotoNewRowJanus(grdList, EmrPt01Bienbanhoichanthongquamo.Columns.Id, id.ToString());
            }
            catch (Exception ex)
            {
        ...
        bool DeleteMe()
        {
            try
            {
                using (var scope = new TransactionScope())
                {
                    ...
                    {
                        long IdPhieu = Utility.Int32Dbnull(grdList.GetValue(EmrPt01Bienbanhoichanthongquamo.Columns.Id), -1);
                        new Delete().From(EmrPt01Bienbanhoichanthongquamo.Schema).Where(EmrPt01Bienbanhoichanthongquamo.Columns.Id).IsEqualTo(IdPhieu).Execute();
                        emrdoc.DeleteDocument(IdPhieu, Loaiphieu_HIS.BIENBANHOICHAN_THONGQUAMO, Loaiphieu_HIS.BIENBANHOICHAN_THONGQUAMO);
                    }
                    scope.Complete();
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            try
            {
                EmrPt01Bienbanhoichanthongquamo obj_bienban = EmrPt01Bienbanhoichanthongquamo.FetchByID(Utility.Int32Dbnull(grdList.GetValue(EmrPt01Bienbanhoichanthongquamo.Columns.Id), -1));
                if (obj_bienban == null)
                {
                    Utility.ShowMsg(string.Format("Biên bản hội chẩn thông qua mổ của người bệnh {0} có thể đã bị người khác xóa ở chức năng khác. Vui lòng kiểm tra lại bằng cách nhấn nút tìm kiếm", grdList.GetValue("ten_benhnhan").ToString()));
                    return;
                }
                
                if (Utility.AcceptQuestion(string.Format("Bạn có chắc chắn muốn xóa phiếu {0} của người bệnh {1} hay không?", grdList.GetValue(EmrPt01Bienbanhoichanthongquamo.Columns.MaPhieu).ToString(), grdList.GetValue("ten_benhnhan").ToString()), "Xác nhận xóa", true))
                {
                    if (DeleteMe())
                    {
                        Utility.ShowMsg(string.Format("Xóa Biên bản hội chẩn thông qua mổ cho người bệnh {0} thành công", grdList.GetValue("ten_benhnhan").ToString()));
                        DataRow[] arrDr = m_dtData.Select(string.Format("{0}={1}", EmrPt01Bienbanhoichanthongquamo.Columns.Id, grdList.GetValue(EmrPt01Bienbanhoichanthongquamo.Columns.Id)));
                        if (arrDr.Length > 0)
                            m_dtData.Rows.Remove(arrDr[0]);
                        m_dtData.AcceptChanges();
        ...
        private void cmdPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    long Id = Utility.Int64Dbnull(grdList.GetValue(EmrPt01Bienbanhoichanthongquamo.Columns.Id));

                    EmrPt01Bienbanhoichanthongquamo _phieu = new Select().From(EmrPt01Bienbanhoichanthongquamo.Schema)
                       .Where(EmrPt01Bienbanhoichanthongquamo.Columns.Id).IsEqualTo(Id)
