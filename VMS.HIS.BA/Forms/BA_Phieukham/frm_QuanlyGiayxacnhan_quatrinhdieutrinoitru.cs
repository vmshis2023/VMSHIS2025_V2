...
{
    public partial class frm_QuanlyGiayxacnhan_quatrinhdieutrinoitru : Form
    {
        ...
        public frm_QuanlyGiayxacnhan_quatrinhdieutrinoitru()
        {
            InitializeComponent();
        ...
        void InitEvents()
        {
            ...
            Load += frm_QuanlyGiayxacnhan_quatrinhdieutrinoitru_Load;
            KeyDown += frm_QuanlyGiayxacnhan_quatrinhdieutrinoitru_KeyDown;
        ...
        private void frm_QuanlyGiayxacnhan_quatrinhdieutrinoitru_Load(object sender, EventArgs e)
        {
        ...
        private void TimKiemThongTin()
        {
            ...
            m_dtData = SPs.Tt25GiayxacnhanQuatrinhdieutrinoitruLaydanhsach(-1,tungay, denngay, "", ma_luotkham,ten_benhnhan).GetDataSet().Tables[0];
        ...
        private void frm_QuanlyGiayxacnhan_quatrinhdieutrinoitru_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            frm_giayxacnhan_quatrinhdieutrinoitru giayxacnhan = new frm_giayxacnhan_quatrinhdieutrinoitru();
        ...
        void _OnCreated(long id, action m_enAct)
        {
            try
            {
                DataTable dt_temp = SPs.Tt25GiayxacnhanQuatrinhdieutrinoitruLaydanhsach(id, new DateTime(1900, 1, 1), new DateTime(1900, 1, 1),"","","").GetDataSet().Tables[0];
                if (m_enAct == action.Delete)
                {
                    if (DeleteMe())
                    {
                        DataRow[] arrDr = m_dtData.Select(string.Format("{0}={1}", Tt25GiayxacnhanQuatrinhdieutrinoitru.Columns.Id, grdList.GetValue(Tt25GiayxacnhanQuatrinhdieutrinoitru.Columns.Id)));
                        if (arrDr.Length > 0)
        ...
        bool DeleteMe()
        {
            try
            {
                using (var scope = new TransactionScope())
                {
                    ...
                    {
                        long IdPhieu = Utility.Int32Dbnull(grdList.GetValue(Tt25GiayxacnhanQuatrinhdieutrinoitru.Columns.Id), -1);
                        new Delete().From(Tt25GiayxacnhanQuatrinhdieutrinoitru.Schema).Where(Tt25GiayxacnhanQuatrinhdieutrinoitru.Columns.Id).IsEqualTo(IdPhieu).Execute();
                        emrdoc.DeleteDocument(IdPhieu, Loaiphieu_HIS.TT25_GIAYXACNHAN_QUATRINHDIEUTRINOITRU, Loaiphieu_HIS.TT25_GIAYXACNHAN_QUATRINHDIEUTRINOITRU);
                    }
                    scope.Complete();
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            try
            {
                Tt25GiayxacnhanQuatrinhdieutrinoitru objGiayXacnhan = Tt25GiayxacnhanQuatrinhdieutrinoitru.FetchByID(Utility.Int32Dbnull(grdList.GetValue(Tt25GiayxacnhanQuatrinhdieutrinoitru.Columns.Id), -1));
                if (objGiayXacnhan == null)
                {
                    Utility.ShowMsg(string.Format("Giấy xác nhận quá trình điều trị nội trú của người bệnh {0} có thể đã bị người khác xóa ở chức năng khác. Vui lòng kiểm tra lại bằng cách nhấn nút tìm kiếm", grdList.GetValue("ten_benhnhan").ToString()));
                    return;
                }
                
                if (Utility.AcceptQuestion(string.Format("Bạn có chắc chắn muốn xóa Giấy xác nhận quá trình điều trị nội trú {0} của người bệnh {1} hay không?", grdList.GetValue(Tt25GiayxacnhanQuatrinhdieutrinoitru.Columns.SoHoso).ToString(), grdList.GetValue("ten_benhnhan").ToString()), "Xác nhận xóa", true))
                {
                    if (DeleteMe())
                    {
                        Utility.ShowMsg(string.Format("Xóa Giấy xác nhận quá trình điều trị nội trú cho người bệnh {0} thành công", grdList.GetValue("ten_benhnhan").ToString()));
                        DataRow[] arrDr = m_dtData.Select(string.Format("{0}={1}", Tt25GiayxacnhanQuatrinhdieutrinoitru.Columns.Id, grdList.GetValue(Tt25GiayxacnhanQuatrinhdieutrinoitru.Columns.Id)));
                        if (arrDr.Length > 0)
                            m_dtData.Rows.Remove(arrDr[0]);
                        m_dtData.AcceptChanges();
            ...
            try
            {
                ...
                try
                {
                    Tt25GiayxacnhanQuatrinhdieutrinoitru giayxacnhan = new Select().From(Tt25GiayxacnhanQuatrinhdieutrinoitru.Schema)
                           .Where(Tt25GiayxacnhanQuatrinhdieutrinoitru.Columns.Id).IsEqualTo(Utility.Int64Dbnull( grdList.GetValue(Tt25GiayxacnhanQuatrinhdieutrinoitru.Columns.Id)))
                           .ExecuteSingle<Tt25GiayxacnhanQuatrinhdieutrinoitru>();
                    if (giayxacnhan==null)
                    {
                        Utility.ShowMsg("Giấy xác nhận quá trình điều trị nội trú không tồn tại.\nCó thể đã bị người khác xóa trong khi bạn đang thao tác.\nVui lòng kiểm tra lại");
