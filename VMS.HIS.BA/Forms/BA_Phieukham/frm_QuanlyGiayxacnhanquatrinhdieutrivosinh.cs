...
{
    public partial class frm_QuanlyGiayxacnhanquatrinhdieutrivosinh : Form
    {
        ...
        public frm_QuanlyGiayxacnhanquatrinhdieutrivosinh()
        {
            InitializeComponent();
        ...
        void InitEvents()
        {
            ...
            Load += frm_QuanlyGiayxacnhanquatrinhdieutrivosinh_Load;
            KeyDown += frm_QuanlyGiayxacnhanquatrinhdieutrivosinh_KeyDown;
        ...
        private void frm_QuanlyGiayxacnhanquatrinhdieutrivosinh_Load(object sender, EventArgs e)
        {
        ...
        private void TimKiemThongTin()
        {
            ...
            m_dtData = SPs.Tt25GiayxacnhanquatrinhdieutrivosinhLaydanhsach(-1,tungay, denngay, "", ma_luotkham,ten_benhnhan).GetDataSet().Tables[0];
        ...
        private void frm_QuanlyGiayxacnhanquatrinhdieutrivosinh_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            frm_giayxacnhanquatrinhdieutrivosinh giayxacnhan = new frm_giayxacnhanquatrinhdieutrivosinh();
        ...
        void _OnCreated(long id, action m_enAct)
        {
            try
            {
                DataTable dt_temp = SPs.Tt25GiayxacnhanquatrinhdieutrivosinhLaydanhsach(id, new DateTime(1900, 1, 1), new DateTime(1900, 1, 1),"","","").GetDataSet().Tables[0];
                if (m_enAct == action.Delete)
                {
                    if (DeleteMe())
                    {
                        DataRow[] arrDr = m_dtData.Select(string.Format("{0}={1}", Tt25Giayxacnhanquatrinhdieutrivosinh.Columns.Id, grdList.GetValue(Tt25Giayxacnhanquatrinhdieutrivosinh.Columns.Id)));
                        if (arrDr.Length > 0)
        ...
        bool DeleteMe()
        {
            try
            {
                using (var scope = new TransactionScope())
                {
                    ...
                    {
                        long IdPhieu = Utility.Int32Dbnull(grdList.GetValue(Tt25Giayxacnhanquatrinhdieutrivosinh.Columns.Id), -1);
                        new Delete().From(Tt25Giayxacnhanquatrinhdieutrivosinh.Schema).Where(Tt25Giayxacnhanquatrinhdieutrivosinh.Columns.Id).IsEqualTo(IdPhieu).Execute();
                        emrdoc.DeleteDocument(IdPhieu, Loaiphieu_HIS.TT25_GIAYXACNHAN_QUATRINHDIEUTRIVOSINH, Loaiphieu_HIS.TT25_GIAYXACNHAN_QUATRINHDIEUTRIVOSINH);
                    }
                    scope.Complete();
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            try
            {
                Tt25Giayxacnhanquatrinhdieutrivosinh objGiayXacnhan = Tt25Giayxacnhanquatrinhdieutrivosinh.FetchByID(Utility.Int32Dbnull(grdList.GetValue(Tt25Giayxacnhanquatrinhdieutrivosinh.Columns.Id), -1));
                if (objGiayXacnhan == null)
                {
                    Utility.ShowMsg(string.Format("Giấy xác nhận quá trình điều trị vô sinh của lao động nữ của người bệnh {0} có thể đã bị người khác xóa ở chức năng khác. Vui lòng kiểm tra lại bằng cách nhấn nút tìm kiếm", grdList.GetValue("ten_benhnhan").ToString()));
                    return;
                }
                
                if (Utility.AcceptQuestion(string.Format("Bạn có chắc chắn muốn xóa Giấy xác nhận quá trình điều trị vô sinh của lao động nữ {0} của người bệnh {1} hay không?", grdList.GetValue(Tt25Giayxacnhanquatrinhdieutrivosinh.Columns.SoHoso).ToString(), grdList.GetValue("ten_benhnhan").ToString()), "Xác nhận xóa", true))
                {
                    if (DeleteMe())
                    {
                        Utility.ShowMsg(string.Format("Xóa Giấy xác nhận quá trình điều trị vô sinh của lao động nữ cho người bệnh {0} thành công", grdList.GetValue("ten_benhnhan").ToString()));
                        DataRow[] arrDr = m_dtData.Select(string.Format("{0}={1}", Tt25Giayxacnhanquatrinhdieutrivosinh.Columns.Id, grdList.GetValue(Tt25Giayxacnhanquatrinhdieutrivosinh.Columns.Id)));
                        if (arrDr.Length > 0)
                            m_dtData.Rows.Remove(arrDr[0]);
                        m_dtData.AcceptChanges();
            ...
            try
            {
                ...
                try
                {
                    Tt25Giayxacnhanquatrinhdieutrivosinh giayxacnhan = new Select().From(Tt25Giayxacnhanquatrinhdieutrivosinh.Schema)
                           .Where(Tt25Giayxacnhanquatrinhdieutrivosinh.Columns.Id).IsEqualTo(Utility.Int64Dbnull( grdList.GetValue(Tt25Giayxacnhanquatrinhdieutrivosinh.Columns.Id)))
                           .ExecuteSingle<Tt25Giayxacnhanquatrinhdieutrivosinh>();
                    if (giayxacnhan==null)
                    {
                        Utility.ShowMsg("Giấy xác nhận quá trình điều trị vô sinh của lao động nữ không tồn tại.\nCó thể đã bị người khác xóa trong khi bạn đang thao tác.\nVui lòng kiểm tra lại");
                    ...
                    WordPrinter.InPhieu(dtData, "TT25_GIAYCHUNGNHAN_TAINANTHUONGTICH.doc", "");
