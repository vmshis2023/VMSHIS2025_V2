...
{
    public partial class frm_QuanlyGiayxacnhan_nghiduongthai : Form
    {
        ...
        public frm_QuanlyGiayxacnhan_nghiduongthai()
        {
            InitializeComponent();
        ...
        void InitEvents()
        {
            ...
            Load += frm_QuanlyGiayxacnhan_nghiduongthai_Load;
            KeyDown += frm_QuanlyGiayxacnhan_nghiduongthai_KeyDown;
        }
        /// <summary>
        ...
        private void frm_QuanlyGiayxacnhan_nghiduongthai_Load(object sender, EventArgs e)
        {
        ...
        private void TimKiemThongTin()
        {
            ...
            m_dtData = SPs.Tt25GiayxacnhanNghiduongthaiLaydanhsach(-1,tungay, denngay, "", ma_luotkham,ten_benhnhan).GetDataSet().Tables[0];
        ...
        private void frm_QuanlyGiayxacnhan_nghiduongthai_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            frm_giayxacnhan_nghiduongthai giayxacnhan = new frm_giayxacnhan_nghiduongthai();
        ...
        void _OnCreated(long id, action m_enAct)
        {
            try
            {
                DataTable dt_temp = SPs.Tt25GiayxacnhanNghiduongthaiLaydanhsach(id, new DateTime(1900, 1, 1), new DateTime(1900, 1, 1),"","","").GetDataSet().Tables[0];
                if (m_enAct == action.Delete)
                {
                    if (DeleteMe())
                    {
                        DataRow[] arrDr = m_dtData.Select(string.Format("{0}={1}", Tt25GiayxacnhanNghiduongthai.Columns.Id, grdList.GetValue(Tt25GiayxacnhanNghiduongthai.Columns.Id)));
                        if (arrDr.Length > 0)
        ...
        bool DeleteMe()
        {
            try
            {
                using (var scope = new TransactionScope())
                {
                    ...
                    {
                        long IdPhieu = Utility.Int32Dbnull(grdList.GetValue(Tt25GiayxacnhanNghiduongthai.Columns.Id), -1);
                        new Delete().From(Tt25GiayxacnhanNghiduongthai.Schema).Where(Tt25GiayxacnhanNghiduongthai.Columns.Id).IsEqualTo(IdPhieu).Execute();
                        emrdoc.DeleteDocument(IdPhieu, Loaiphieu_HIS.TT25_GIAYXACNHAN_NGHIDUONGTHAI, Loaiphieu_HIS.TT25_GIAYXACNHAN_NGHIDUONGTHAI);
                    }
                    scope.Complete();
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            try
            {
                Tt25GiayxacnhanNghiduongthai objGiayXacnhan = Tt25GiayxacnhanNghiduongthai.FetchByID(Utility.Int32Dbnull(grdList.GetValue(Tt25GiayxacnhanNghiduongthai.Columns.Id), -1));
                if (objGiayXacnhan == null)
                {
                    Utility.ShowMsg(string.Format("Giấy Xác nhận nghỉ dưỡng thai của người bệnh {0} có thể đã bị người khác xóa ở chức năng khác. Vui lòng kiểm tra lại bằng cách nhấn nút tìm kiếm", grdList.GetValue("ten_benhnhan").ToString()));
                    return;
                }
                
                if (Utility.AcceptQuestion(string.Format("Bạn có chắc chắn muốn xóa Giấy xác nhận nghỉ dưỡng thai {0} của người bệnh {1} hay không?", grdList.GetValue(Tt25GiayxacnhanNghiduongthai.Columns.SoHoso).ToString(), grdList.GetValue("ten_benhnhan").ToString()), "Xác nhận xóa", true))
                {
                    if (DeleteMe())
                    {
                        Utility.ShowMsg(string.Format("Xóa Giấy xác nhận nghỉ dưỡng thai cho người bệnh {0} thành công", grdList.GetValue("ten_benhnhan").ToString()));
                        DataRow[] arrDr = m_dtData.Select(string.Format("{0}={1}", Tt25GiayxacnhanNghiduongthai.Columns.Id, grdList.GetValue(Tt25GiayxacnhanNghiduongthai.Columns.Id)));
                        if (arrDr.Length > 0)
                            m_dtData.Rows.Remove(arrDr[0]);
                        m_dtData.AcceptChanges();
            ...
            try
            {
                ...
                try
                {
                    Tt25GiayxacnhanNghiduongthai giayxacnhan = new Select().From(Tt25GiayxacnhanNghiduongthai.Schema)
                           .Where(Tt25GiayxacnhanNghiduongthai.Columns.Id).IsEqualTo(Utility.Int64Dbnull( grdList.GetValue(Tt25GiayxacnhanNghiduongthai.Columns.Id)))
                           .ExecuteSingle<Tt25GiayxacnhanNghiduongthai>();
                    if (giayxacnhan==null)
                    {
                        Utility.ShowMsg("Giấy Xác nhận nghỉ dưỡng thai không tồn tại.\nCó thể đã bị người khác xóa trong khi bạn đang thao tác.\nVui lòng kiểm tra lại");
