    ...
    public partial class frm_BenhAn_NoiKhoa : Form
    {
        ...
        void txtMaBenhAn_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    objEmrBa = new Select().From(EmrBaNoikhoa.Schema).Where(EmrBaNoikhoa.Columns.MaBa).IsEqualTo(Utility.DoTrim(txtMaBenhAn.Text)).ExecuteSingle<EmrBaNoikhoa>();
                    if (objEmrBa == null)
                        ClearControl();
                    else
                    {
        ...
        void txtIDBenhAn_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    objEmrBa = EmrBaNoikhoa.FetchByID(Utility.Int64Dbnull(txtIDBenhAn.Text));
                    if (objEmrBa == null)
                        ClearControl();
                    else
                    {
        ...
        void LuuBA(int trangthai)
        {
            try
            {
                ...
                dtDataBA = SPs.EmrBaNoikhoaLaythongtin(-1, "", objLuotkham.IdBenhnhan, objLuotkham.MaLuotkham).GetDataSet().Tables[0];
                _isSuccess = true;
            }
            catch (Exception ex)
            {
        ...
        void ThongbaoSaiBenhAn(EmrBaNoikhoa objEmrBa)
        {
        ...
        //    }
        //} 
        private EmrBaNoikhoa TaoEmrBa()
        {
            if (objEmrBa == null) objEmrBa = new EmrBaNoikhoa();
            try
            {
        ...
        void FillData4Update()
        {
            try
            {
                ...
                SqlQuery sqlQuery = new Select().From<EmrBaNoikhoa>()
                    .Where(EmrBaNoikhoa.Columns.MaLuotkham)
                    .IsEqualTo(objLuotkham.MaLuotkham)
                    .And(EmrBaNoikhoa.Columns.IdBenhnhan)
                ...
                if (objEmrBa == null || (objEmrBa.IdBenhnhan != objLuotkham.IdBenhnhan && objEmrBa.MaLuotkham != objLuotkham.MaLuotkham))
                    objEmrBa = sqlQuery.ExecuteSingle<EmrBaNoikhoa>();
                //Autofill Data
                ...
                if (objEmrBa != null && (Utility.Coquyen("EMR_XOA_BA") || globalVariables.UserName == objEmrBa.NguoiTao))
                {
                    if (Utility.AcceptQuestion("Bạn có muốn xóa thông tin bệnh án đang chọn không ?", "Thông báo", true))
                    {
                        using (var Scope = new TransactionScope())
                        {
                            using (var dbScope = new SharedDbConnectionScope())
                            {
                               new Delete().From(EmrBaNoikhoa.Schema)
                                     .Where(EmrBaNoikhoa.Columns.IdBa).IsEqualTo(objEmrBa.IdBa)
                                     .And(EmrBaNoikhoa.Columns.LoaiBa).IsEqualTo(objEmrBa.LoaiBa)
                                     .And(EmrBaNoikhoa.Columns.MaCoso).IsEqualTo(objEmrBa.MaCoso)
                                     .Execute();
