...
{
    public partial class frm_QuanlyBA_SanKhoa : Form
    {
        ...
        public frm_QuanlyBA_SanKhoa(string lstLoaiBA)
        {
            InitializeComponent();
        ...
        void InitEvents()
        {
            ...
            Load += frm_QuanlyBA_SanKhoa_Load;
            KeyDown += frm_QuanlyBA_SanKhoa_KeyDown;
        ...
        private void frm_QuanlyBA_SanKhoa_Load(object sender, EventArgs e)
        {
        ...
        private void TimKiemThongTin()
        {
            ...
            m_dtData = SPs.EmrBaSankhoaLaydanhsachBA(tungay, denngay, ma_luotkham, ma_BA, ten_benhnhan, id_khoadieutri).GetDataSet().Tables[0];
        ...
        private void frm_QuanlyBA_SanKhoa_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void cmdUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                frm_BenhAn_SanKhoa BA = new frm_BenhAn_SanKhoa(MauBA);
                EmrBaSankhoa bant = EmrBaSankhoa.FetchByID(Utility.Int64Dbnull(grdList.GetValue("id_ba")));
                BA.objEmrBa = bant;
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!Utility.Coquyen("EMR_XOA_BENH_AN"))
                {
                 ...
                 objEmrBa = EmrBaSankhoa.FetchByID(Utility.Int64Dbnull(grdList.GetValue(EmrBaSankhoa.Columns.IdBa)));
                if (objEmrBa == null)
                {
                ...
                if (Utility.AcceptQuestion(string.Format("Bạn có chắc chắn muốn xóa Bệnh án với mã {0} của người bệnh {1} hay không?", grdList.GetValue(EmrBaSankhoa.Columns.MaBa).ToString(), grdList.GetValue("ten_benhnhan").ToString()), "Xác nhận xóa bệnh án", true))
                {
                    try
                    {
                        using (var Scope = new TransactionScope())
                        {
                            ...
                            {
                                new Delete().From(EmrBaSankhoa.Schema)
                                      .Where(EmrBaSankhoa.Columns.IdBa).IsEqualTo(objEmrBa.IdBa)
                                      .And(EmrBaSankhoa.Columns.LoaiBa).IsEqualTo(objEmrBa.LoaiBa)
                                      .And(EmrBaSankhoa.Columns.MaCoso).IsEqualTo(objEmrBa.MaCoso)
                                      .Execute();
                        ...
                        DataRow[] arrDr = m_dtData.Select(string.Format("{0}={1}", EmrBaSankhoa.Columns.IdBa, grdList.GetValue(EmrBaSankhoa.Columns.IdBa)));
                        if (arrDr.Length > 0)
        ...
        void _OnCreated(long id,string ma_ba, action m_enAct)
        {
            try
            {
                DataTable dt_temp = SPs.EmrBaSankhoaLaydanhsachBA(new DateTime(1900, 1, 1), new DateTime(1900, 1, 1), "", ma_ba, "", -1).GetDataSet().Tables[0];
        ...
        bool DeleteMe()
        {
            try
            {
                using (var scope = new TransactionScope())
                {
                    using (var dbscope = new SharedDbConnectionScope())
                    {
                        new Delete().From(EmrBaSankhoa.Schema).Where(EmrBaSankhoa.Columns.IdBa).IsEqualTo(Utility.Int32Dbnull(grdList.GetValue(EmrBaSankhoa.Columns.IdBa), -1)).Execute();
                    }
                    scope.Complete();
        ...
        private void cmdPrint_Click(object sender, EventArgs e)
        {
            ...
            //    string ma_luotkham = grdList.GetValue(EmrBaSankhoa.Columns.MaLuotkham).ToString();
            //    long id_phieu = Utility.Int64Dbnull(grdList.GetValue(EmrBaSankhoa.Columns.IdPhieu));
            //    DataTable dtData =
      ...
      void InitData4Print()
        ...
        {
            frm_BenhAn_SanKhoa BA = new frm_BenhAn_SanKhoa("05/BV1");
