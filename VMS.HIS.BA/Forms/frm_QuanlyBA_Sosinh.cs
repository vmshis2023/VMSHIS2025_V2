...
{
    public partial class frm_QuanlyBA_Sosinh : Form
    {
        ...
        public frm_QuanlyBA_Sosinh(string lstLoaiBA)
        {
            InitializeComponent();
        ...
        void setquyen()
        {
            try
            {
                cmdInsert.Visible = Utility.Coquyen("EMR_THEM_BA");
                cmdUpdate.Visible = Utility.Coquyen("EMR_SUA_BA");
                cmdDelete.Visible = Utility.Coquyen("EMR_XOA_BA");
                cmd_khoa_ki.Visible = Utility.Coquyen("EMR_TRUONGKHOA_KI");
                cmd_gui_khth.Visible = Utility.Coquyen("EMR_GUI_KHTH_PHEDUYET");
                cmd_duyet.Visible = Utility.Coquyen("EMR_DUYET_BA");
                cmd_tuchoi.Visible = Utility.Coquyen("EMR_TUCHOI_BA");

                cmd_giamdoc_ki.Visible = Utility.Coquyen("EMR_GIAMDOC_KI_BA");
                cmd_dong_BA.Visible = Utility.Coquyen("EMR_DONG_BA");
            }
            catch (Exception ex)
            {
        ...
        void InitEvents()
        {
            ...
            Load += frm_QuanlyBA_Sosinh_Load;
            KeyDown += frm_QuanlyBA_Sosinh_KeyDown;
        ...
        private void frm_QuanlyBA_Sosinh_Load(object sender, EventArgs e)
        {
        ...
        private void InitData()
        {
            ...
            DataBinding.BindDataCombobox(cbo_khoa_noitru,THU_VIEN_CHUNG.Laydanhmuckhoa("NOI","KHOA",-1), DmucKhoaphong.Columns.IdKhoaphong, DmucKhoaphong.Columns.TenKhoaphong);
        }
        ...
        private void TimKiemThongTin()
        {
            ...
            int id_khoadieutri = Utility.Int32Dbnull(cbo_khoa_noitru.SelectedValue);
        ...
        private void frm_QuanlyBA_Sosinh_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void cmdUpdate_Click(object sender, EventArgs e)
        {
            ...
            //{
                frm_BenhAn_Sosinh BA = new frm_BenhAn_Sosinh(MauBA);
        ...
        private void lnkDelete_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            txtmaBA.Clear();
            txtmaBA.Focus();
            cbo_khoa_noitru.SelectedIndex = -1;
        ...
        {
            frm_BenhAn_Sosinh _BenhAn = new frm_BenhAn_Sosinh("06/BV1");
            _BenhAn.m_enAct = action.Insert;
            _BenhAn._OnCreated += _OnCreated;
            _BenhAn.ShowDialog();
        }

        private void cmd_phieutheodoisosinh_Click(object sender, EventArgs e)
        {

        }

        private void cmd_gui_khth_Click(object sender, EventArgs e)
        {
            try
            {
                objLuotkham = Utility.getKcbLuotkham(grdList.CurrentRow);
                if (objLuotkham == null)
                {
                    Utility.ShowMsg("Bạn cần chọn bệnh án để thực hiện thao tác gửi kế hoạch tổng hợp");
                    return;
                }
                if (objLuotkham.TrangthaiNoitru < 3)
                {
                    Utility.ShowMsg("Chưa xác nhận ra viện cho người bệnh nên không cho phép gửi kế hoạch tổng hợp");
                    return;
                }
                EmrBa objBa = EmrBa.FetchByID(Utility.Int64Dbnull(grdList.GetValue("id_ba")));

                if(objBa==null)
                {
                    Utility.ShowMsg("Vui lòng chọn Bệnh án trước khi gửi kế hoạch tổng hợp");
                    return;
                }
                if (objBa.TrangThai==2)
                {
                    Utility.ShowMsg("Bệnh án cần hoàn tất thông tin trước khi gửi kế hoạch tổng hợp. Vui lòng chọn Bệnh án-->Bấm nút cập nhập-->Nhập đầy đủ các thông tin và nhấn út Hoàn tất BA");
                    return;
                }
                new Update(EmrBa.Schema)
                    .Set(EmrBa.Columns.TrangThai).EqualTo(3)
                    .Where(EmrBa.Columns.IdBa).IsEqualTo(objBa.IdBa)
                    .Execute();
                //SqlQuery sqlQuery = new Select().From<EmrHosoluutru>()
                //       .Where(EmrHosoluutru.Columns.MaLuotkham)
                //       .IsEqualTo(objLuotkham.MaLuotkham)
        ...
        private void cmd_duyet_Click(object sender, EventArgs e)
        {
            try
            {
                EmrBa objBa = EmrBa.FetchByID(Utility.Int64Dbnull(grdList.GetValue("id_ba")));
                ...
                if (objBa == null)
                {
                    Utility.ShowMsg("Vui lòng chọn Bệnh án trước khi Duyệt HSBA");
                    return;
                }
                if (objBa.TrangThai != 3)
                {
                    Utility.ShowMsg("Bệnh án cần ở trạng thái gửi KHTH(3) mới được phép Duyệt/Nhận hồ sơ. Vui lòng nhấn F5 để xem lại trạng thái mới nhất của HSBA");
                    return;
                }
                frm_ChonNguoiNhanHoSo _ChonNguoiNhanHoSo = new frm_ChonNguoiNhanHoSo(objBa);
                _ChonNguoiNhanHoSo.ShowDialog();
            }
            catch (Exception ex)
            {
                Utility.CatchException(ex);
        ...
        private void cmd_tuchoi_Click(object sender, EventArgs e)
        {
            ...
            if (objBa == null)
            {
                Utility.ShowMsg("Vui lòng chọn Bệnh án trước khi Từ chối HSBA");
                return;
            }
            if (objBa.TrangThai !=3)
            {
                Utility.ShowMsg("Bệnh án cần ở trạng thái chờ Tổng hợp phê duyệt mới được phép hủy từ chối. Vui lòng nhấn F5 để xem lại trạng thái mới nhất của HSBA");
                return;
            }
            frm_NhaplydoTuchoi_BA _NhaplydoTuchoi_BA = new frm_NhaplydoTuchoi_BA(objBa);
            _NhaplydoTuchoi_BA.ShowDialog();
        }
        int num = 0;
        private void cmd_khoa_ki_Click(object sender, EventArgs e)
        {
            try
            {
                EmrBa objBa = EmrBa.FetchByID(Utility.Int64Dbnull(grdList.GetValue("id_ba")));
                ...
                if (objBa.TrangThai <=2)
                {
                    Utility.ShowMsg("Bệnh án cần ở trạng thái Hoàn tất trước khi trưởng khoa thực hiện kí. Vui lòng chọn Bệnh án-->Bấm nút cập nhập-->Nhập đầy đủ các thông tin và nhấn út Hoàn tất BA");
                    return;
                }
                InitData4Print();
               string docFile= clsInBA.InBA(objEmrBa.IdBa, objEmrBa.MaBa, objEmrBa.LoaiBa, objLuotkham, dtkhoanhapvien, dtkhoachuyen, dt_tssk, dtPhieuPttt, 1, true);
                using (var scope = new TransactionScope())
                {
                    using (var dbscope = new SharedDbConnectionScope())
                    {
                    ...
                    .Set(EmrBa.Columns.TrangThai).EqualTo(3)
                    .Where(EmrBa.Columns.IdBa).IsEqualTo(objBa.IdBa)
                    .And(EmrBa.Columns.TrangThai).IsEqualTo(2)
                    .Execute();
                    }
                    scope.Complete();
                }
