...
namespace VNS.HIS.UI.BA
{
    public partial class frm_dmuc_gay : Form
    {
        ...
        bool m_blnAllowCurrentCellChanged = false;
        ...
        public frm_dmuc_gay(string p_strArgs)
        {
            InitializeComponent();
        ...
        void InitEvents()
        {
            ...
            {
                //Form load
                this.Load += new EventHandler(frm_dmuc_gay_Load);
                //Bắt sự kiện KeyDown trên form
                this.KeyDown += new KeyEventHandler(frm_dmuc_gay_KeyDown);
        ...
        private void GrdPhieu_RowCheckStateChanged(object sender, RowCheckStateChangeEventArgs e)
        {
            try
            {
                if (Utility.isValidGrid(grdList) && Utility.isValidGrid(grdPhieu))
                {
                    ...
                    {
                        phan_loai = Utility.sDbnull(grdList.GetValue("ma"));
                         num = new Update(DmucChung.Schema).Set(DmucChung.Columns.Phanloai).EqualTo(phan_loai).Where(DmucChung.Columns.Loai).IsEqualTo(dmuc_con).And(DmucChung.Columns.Ma).IsEqualTo(Utility.sDbnull(grdPhieu.GetValue("ma"))).Execute();
        ...
        void AnalyzeArguments(string p_strArgs)
        {
            try
            {
                string[] ChildType = p_strArgs.Split('@');
                if (ChildType.Length > 1)
                {
                    dmuc_con = ChildType[1];
                    dtChild = m_BusRules.dsGetList(dmuc_con).Tables[0];
                    //Gán dữ liệu vào lưới
                    Utility.SetDataSourceForDataGridEx(grdPhieu, dtChild, true, true, "1=1", "STT_HTHI,TEN");
                }

                string[] arrValues = ChildType[0].Split('|');
        ...
        /// <param name="e"></param>
        void frm_dmuc_gay_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        /// <param name="e"></param>
         ...
         void frm_dmuc_gay_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void LoadQuanHeGayPhieu()
        {
            ...
            foreach (Janus.Windows.GridEX.GridEXRow gridExRow in grdPhieu.GetDataRows())
            {
                gridExRow.BeginEdit();
                string phan_loai = Utility.sDbnull(gridExRow.Cells[DmucChung.Columns.Phanloai].Value);
               
                if (Utility.sDbnull(grdList.GetValue(DmucChung.Columns.Ma))== phan_loai)
                {
