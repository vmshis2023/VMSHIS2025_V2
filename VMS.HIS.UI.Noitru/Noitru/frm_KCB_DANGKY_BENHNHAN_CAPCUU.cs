...
namespace VNS.HIS.UI.NOITRU
{
   ...
   /// </summary>
    public partial class frm_KCB_DANGKY_BENHNHAN_CAPCUU : Form
    {
        ...
        string huong_dieu_tri = "3";
        private Logger log;
        public bool updateonly = false;
        public frm_KCB_DANGKY_BENHNHAN_CAPCUU(string huong_dieu_tri)
        {
            InitializeComponent();
            this.huong_dieu_tri = huong_dieu_tri;
            ...
            try
            {
                ...
                dtNgayNhapVien.Value = globalVariables.SysDate.AddMinutes(1);
        ...
        void InitEvents()
        {
            this.FormClosing += new System.Windows.Forms.FormClosingEventHandler(this.frm_KCB_DANGKY_BENHNHAN_CAPCUU_FormClosing);
            this.Load += new System.EventHandler(this.frm_KCB_DANGKY_BENHNHAN_CAPCUU_Load);
            this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.frm_KCB_DANGKY_BENHNHAN_CAPCUU_KeyDown);
            this.Shown += frm_KCB_DANGKY_BENHNHAN_CAPCUU_Shown;
            ...
            txtMaKcbbd.LostFocus += txtNoiDKKCBBD_LostFocus;
            ...
            txtMaNoiCaptheBHYT.TextChanged += new EventHandler(txtNoiCaptheBHYT_TextChanged);
            txtMaNoiCaptheBHYT.KeyDown += txtNoiCaptheBHYT_KeyDown;
        ...
        //    chkGiayBHYT.CheckedChanged += chkGiayBHYT_CheckedChanged;
            ...
            //this.KeyUp += frm_KCB_DANGKY_BENHNHAN_CAPCUU_KeyUp;
            ...
            chknothe.CheckedChanged += Chknothe_CheckedChanged;
            txtdoituongnothe._OnEnterMe += Txtdoituongnothe__OnEnterMe;
            txt_MaloaiDoituongKCB._OnEnterMe += Txt_MaloaiDoituongKCB__OnEnterMe;
        ...
        private void TxtGiuong__OnEnterMe()
        {
            LoadGiaGiuong();
        }
        ...
        void LoadGiaGiuong()
        {
            DataTable dtGia = new dmucgiagiuong_busrule().dsGetLoaigiuong(Utility.Int16Dbnull(cbo_giuong.SelectedValue, -1)).Tables[0];
        ...
        private void TxtBuong__OnEnterMe()
        {
            ChonBuong();
        }
        private void ChonBuong()
        {
            DataTable dtGiuong = THU_VIEN_CHUNG.NoitruTimkiemgiuongTheobuong(Utility.Int32Dbnull(cbo_khoadieutri.SelectedValue), Utility.Int32Dbnull(cbo_buong.SelectedValue), 0);
            DataBinding.BindDataCombobox(cbo_giuong, dtGiuong, NoitruDmucGiuongbenh.Columns.IdGiuong, NoitruDmucGiuongbenh.Columns.TenGiuong);
            if (dtGiuong.Rows.Count <= 0)
            {
                //xóa trắng phần giường và giá
                cbo_giuong.SelectedIndex = -1;
                txtGia.SetId(-1);
                txtGia.RaiseEnterEvents();
                return;
            }
            cbo_giuong_SelectedIndexChanged(cbo_giuong, new EventArgs());
        }
        private void TxtKhoanoitru__OnEnterMe()
        {
            LoadBuong();
        }
        void LoadBuong()
        {
           DataTable dtBuong = THU_VIEN_CHUNG.NoitruTimkiembuongTheokhoa(Utility.Int32Dbnull(cbo_khoadieutri.SelectedValue));
            DataBinding.BindDataCombobox(cbo_buong, dtBuong, NoitruDmucBuong.Columns.IdBuong, NoitruDmucBuong.Columns.TenBuong);
           
            if (dtBuong.Rows.Count <= 0)
            {
                //xóa trắng phần giường và giá
                cbo_giuong.SelectedIndex = -1;
                txtGia.SetId(-1);
        ...
        private void Txt_MaloaiDoituongKCB__OnEnterMe()
        {
            lastCode = txt_MaloaiDoituongKCB.MyCode;
        }

        private void Txtdoituongnothe__OnEnterMe()
        {
            SinhMatheBHYT_DoituongNothe();
        }
        ...
        void SinhMatheBHYT_DoituongNothe()
        {
            try
            {
                if (chknothe.Checked && THU_VIEN_CHUNG.IsBaoHiem(_objDoituongKcb.IdLoaidoituongKcb) && Utility.sDbnull(txtdoituongnothe.MyCode, "") != "")
                {
                    txtmathebhyt.Text = string.Format("{0}{1}{2}", Utility.sDbnull(txtdoituongnothe.MyCode, ""), Utility.sDbnull(txtdoituongnothe.MyCode, "").Contains("QN") ? "97" : globalVariables.gv_strNoicapBHYT,
                                                       globalVariables.SysDate.ToString("yyMMddHHmm"));
                    txtMaNoiCaptheBHYT.Text = globalVariables.gv_strNoicapBHYT;
                    txtMaKcbbd.Text = globalVariables.gv_strNoiDKKCBBD;
                }
                else
                {
        ...
        {
           if(THU_VIEN_CHUNG.IsBaoHiem(_objDoituongKcb.IdLoaidoituongKcb) && Utility.sDbnull(txtDiachi_bhyt.Text).Length > 0 )
            {
                if(Utility.sDbnull(txtDiachi.Text).Length > 0)
        ...
        void frm_KCB_DANGKY_BENHNHAN_CAPCUU_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        void chkCapCuu_CheckedChanged(object sender, EventArgs e)
        {
            if (chkCapCuu.Checked)
            {
                ...
                txt_MaloaiDoituongKCB.Init( m_dtDoiTuong_KCB,new List<string>() { DmucChung.Columns.Ma, DmucChung.Columns.Ma, DmucChung.Columns.Ten });
                txt_MaloaiDoituongKCB.SetCode( lastCode);
            }
            LayLydoVaovien();
        }
        ...
        void frm_KCB_DANGKY_BENHNHAN_CAPCUU_Shown(object sender, EventArgs e)
        {
       ...
       void dtpBOD_TextChanged(object sender, EventArgs e)
        ...
        void cmdInlaihoadon_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                new INPHIEU_THANHTOAN_NGOAITRU().InBienlai(false, Payment_Id,-1, _objLuotkham,0);
            }
            catch (Exception ex)
            {
                Utility.ShowMsg("Lỗi:" + ex.Message);
        ...
        /// <summary>
        /// Cần dành thời gian xem lại mục đích hàm này
        /// </summary>
                ...
                if (objLuotkham != null)
                {
                     ...
                     if (!NotPayment(txtIdBenhnhan.Text.Trim(), ref ma_luotkhamgannhatchuathanhtoan, ref ngayKham, ref tenkhoa))//Đã thanh toán-->Có thể thê lần khám. Nhưng kiểm tra ngày hẹn xem có được phép khám tiếp
                    {
                        ...
                        if (_Info != null && _Info.SongayDieutri != null)
                        {
                            ...
                            else if (kt < soNgayDieuTri)
                            {
                                DialogResult dialogResult =
                                    MessageBox.Show(
                                        "Bác sĩ hẹn :  " + soNgayDieuTri + @"ngày" + "\r\nNgày khám gần nhất:  " +
                                        ngaykhamcu + "\r\nCòn: " + kt1 + @" ngày nữa mới được tái khám" +
                                        "\r\nBạn có muốn tiếp tục thêm lần khám?\nNhấn Yes để thêm lần khám mới. Nhấn No để về trạng thái cập nhật", @"Thông Báo", MessageBoxButtons.YesNo);
                        ...
                        else//Chưa thăm khám-->kiểm tra xem action Add có bị đổi hay không
                        {
                            ...
                            else if (oldAct == action.Update)//Kiểm tra khác ngày thì thêm lần khám, cùng ngày thì giữ nguyên trạng thái cập nhật hoặc thêm mới
                            {
                                if (objLuotkham.NgayTiepdon.ToString("yyyyMMdd") != globalVariables.SysDate.ToString("yyyyMMdd"))
                                    m_enAct = action.Add;
                            }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frm_KCB_DANGKY_BENHNHAN_CAPCUU_Load(object sender, EventArgs e)
        {
            ...
            DataBinding.BindDataCombobox(cbo_khoadieutri, mDtKhoaNoitru, DmucKhoaphong.Columns.IdKhoaphong, DmucKhoaphong.Columns.TenKhoaphong);
        ...
        private void LoadMe()
        {
            try
            {
               ...
               // if (this.Args.Split('-')[0] == "KTC") lblBATC.Visible = txtSoBATCQG.Visible = true;
                ...
                txtTuoi.Enabled = dtpBOD.CustomFormat == @"yyyy";
                lblLoaituoi.Visible = dtpBOD.CustomFormat != @"yyyy";
                dtpBOD_TextChanged(dtpBOD, new EventArgs()) ;
                XoathongtinBHYT(true);
                AddAutoCompleteDiaChi();
                Get_DanhmucChung();
                AutocompleteBenhvien();
                ...
                ChangeObjectRegion();
                RefreshData(false);
                //Hiển thị các thông tin nhập viện
                LoadthongtinNhapvien();
                AllowTextChanged = true;
            }
            catch (Exception ex)
            {
        ...
        void LoadthongtinNhapvien()
        {
            try
            {
                if (_objLuotkham == null) return;
                NoitruPhieunhapvien objPhieuNv = new Select().From(NoitruPhieunhapvien.Schema)
               .Where(NoitruPhieunhapvien.Columns.IdBenhnhan).IsEqualTo(_objLuotkham.IdBenhnhan)
               .And(NoitruPhieunhapvien.Columns.MaLuotkham).IsEqualTo(_objLuotkham.MaLuotkham)
               .ExecuteSingle<NoitruPhieunhapvien>();
                if (objPhieuNv != null)
                {
                    txtSovaovien.Text = Utility.sDbnull(objPhieuNv.SoVaovien);
                    dtNgayNhapVien.Value = objPhieuNv.NgayNhapvien.Value;
                    txtChovaokhoa.Text = Utility.sDbnull(objPhieuNv.ChovaoKhoa);
                    cbo_khoadieutri.SelectedValue = objPhieuNv.IdKhoanoitru;
                    txtChandoanbandau._Text = Utility.sDbnull(objPhieuNv.ChandoanVaovien);
                    txtQuanlybenhly.Text = Utility.sDbnull(objPhieuNv.QuatrinhBenhly);
                    autoBanthan.Text = Utility.sDbnull(objPhieuNv.TsuBanthan);
                    autoGiadinh.Text = Utility.sDbnull(objPhieuNv.TsuGiadinh);
                    autoLydovv._Text = Utility.sDbnull(objPhieuNv.LydoNhapvien);
                    autoToanthan.Text = Utility.sDbnull(objPhieuNv.KhamToanthan);

                    txtCacbophan.Text = Utility.sDbnull(objPhieuNv.BphanKhac);
                    txtTTKQLamSang.Text = Utility.sDbnull(objPhieuNv.TtKquaLamsang);
                    txtDaXuLy.Text = Utility.sDbnull(objPhieuNv.DaXuly);
                    txtMach.Text = Utility.sDbnull(objPhieuNv.Mach);
                    txtNhiptho.Text = Utility.sDbnull(objPhieuNv.Nhiptho);
                    txtha.Text = Utility.sDbnull(objPhieuNv.Huyetap);
                    txtNhietDo.Text = Utility.sDbnull(objPhieuNv.Nhietdo);
                    txtChieucao.Text = Utility.sDbnull(objPhieuNv.Chieucao);
                    txtCannang.Text = Utility.sDbnull(objPhieuNv.Cannang);
                    txtSPO2.Text = Utility.sDbnull(objPhieuNv.SPO2);
                    txtBmi.Text = Utility.sDbnull(objPhieuNv.Bmi);

                }
                NoitruPhanbuonggiuong objBG = new Select().From(NoitruPhanbuonggiuong.Schema)
                   .Where(NoitruPhanbuonggiuong.Columns.IdBenhnhan).IsEqualTo(_objLuotkham.IdBenhnhan)
                   .And(NoitruPhanbuonggiuong.Columns.MaLuotkham).IsEqualTo(_objLuotkham.MaLuotkham)
                   .And(NoitruPhanbuonggiuong.Columns.Id).IsEqualTo(_objLuotkham.IdRavien)
                   .ExecuteSingle<NoitruPhanbuonggiuong>();
                if (objBG != null)
                {
                    cbo_khoadieutri.SelectedValue = objBG.IdKhoanoitru;
                    cbo_khoadieutri_SelectedIndexChanged(cbo_khoadieutri, new EventArgs());
                    cbo_buong.SelectedValue = objBG.IdBuong;
                    cbo_buong_SelectedIndexChanged(cbo_buong, new EventArgs());
                    cbo_giuong.SelectedValue = objBG.IdGiuong;
                    cbo_giuong_SelectedIndexChanged(cbo_giuong, new EventArgs());
                    cboGia.SelectedIndex = Utility.GetSelectedIndex(cboGia, Utility.sDbnull(objBG.IdGia));
                }
            }
            catch (Exception)
            {
        ...
        {
            dt_Congkhamthiluc = THU_VIEN_CHUNG.LayDsach_Dvu_KCB(_maDoituongKcb, "ALL", -1, -1, 1);
        ...
        private void LoadThongtinBenhnhan()
        {
            ...
            _ptramBhytGocCu = 0m;
            AllowTextChanged = false;
            objBenhnhan = KcbDanhsachBenhnhan.FetchByID(Utility.Int64Dbnull( txtIdBenhnhan.Text));
            if (objBenhnhan != null)
            {
                ...
                txtNhommau.SetCode(objBenhnhan.NhomMau);
                ...
                if (_objLuotkham != null)
                {
                    ...
                    _allowAgeChanged = true;//Để tự tính tuổi
                    ...
                    if (THU_VIEN_CHUNG.IsBaoHiem(_objDoituongKcb.IdLoaidoituongKcb) && !string.IsNullOrEmpty(_objLuotkham.MatheBhyt))//Thông tin BHYT
                    {
                        ...
                        txtMaNoiCaptheBHYT.Text = Utility.sDbnull(_objLuotkham.NoiDongtrusoKcbbd);
                       ...
                       txt_MaloaiDoituongKCB.SetCode( _objLuotkham.MaDoituongKcbBhyt);
                        lastCode = _objLuotkham.MaDoituongKcbBhyt;
                        txtMaKcbbd.Text = Utility.sDbnull(_objLuotkham.MaKcbbd);
        ...
        private bool IsValidTheBhyt_more()
        {
            ...
            if (txt_MaloaiDoituongKCB.MyID=="-1")
            {
                Utility.ShowMsg("Bạn phải chọn mã đối tượng KCB", "Thông báo");
                txt_MaloaiDoituongKCB.Focus();
                return false;
            }
            year = Utility.Int32Dbnull(Microsoft.VisualBasic.DateAndTime.DateDiff(Microsoft.VisualBasic.DateInterval.Year, dtpBOD.Value, dtpNgaytiepdon.Value));
        ...
        private void txtNoiCaptheBHYT_TextChanged(object sender, EventArgs e)
        {
            ...
            if (_hasjustpressBackKey && txtMaNoiCaptheBHYT.Text.Length <= 0)
            {
                txtOthu6.Focus();
            ...
            if (!IsValidTheBhyt()) return;
            GetNoiDangKy();
            LoadClinicCode();//Load lại vì nơi đăng ký kcbbd có nhiều cái có thể trùng nhau và khác nơi cấp(mã tỉnh/tp)
            txtMaKcbbd.Focus();
            txtMaKcbbd.SelectAll();
        }
        ...
        private void GetNoiDangKy()
        {
            ...
            if (objdiachinh != null)
            {
                txtTenNoiCapTheBHYT.Text = Utility.sDbnull(objdiachinh["ten_diachinh"]);
        ...
        private bool KT_20_Ky_Tu_BHYT(string IdBenhnhan, string sobhyt)
        {
            ...
            if (patientExam != null)
            {
                ...
                if (txtMaNoiCaptheBHYT.Text.ToUpper() != Utility.sDbnull(patientExam.NoiDongtrusoKcbbd).ToUpper()
                       || txtMaKcbbd.Text.ToUpper() != Utility.sDbnull(patientExam.MaKcbbd).ToUpper())
                {
        ...
        private void ClearControl()
        {
          ...
          //  txtDantoc.Clear();
            ...
            SetActionStatus();
            DmucDichvukcb objDmucDichvukcb = DmucDichvukcb.FetchByID(-1);
        ...
        bool Checktrungthongtin()
        {
            if (Utility.sDbnull(txtTEN_BN.Text).Length > 0 && Utility.sDbnull(txtCMT.Text).Length > 0)
            {
                ...
                if (dttrungthongtin.Rows.Count > 0)
                {
                    ...
                    {
                        frm_DSACH_BN_TKIEM Timkiem_Benhnhan = new frm_DSACH_BN_TKIEM("ALL", 0);
        ...
        bool ChecktrungCMT()
        {
            ...
            {
                //Check trùng 3 thông tin Họ tên+ CCCD+Ngày tháng năm sinh
                string sql = string.Format("select 1 from kcb_danhsach_benhnhan where upper(cmt)='{0}'", Utility.sDbnull(txtCMT.Text));
                DataTable dttrungthongtin = Utility.ExecuteSql(sql, CommandType.Text).Tables[0];
                if (dttrungthongtin.Rows.Count > 0)
                {
                    if (Utility.AcceptQuestion("Hệ thống phát hiện đã tồn tại người bệnh trong hệ thống trùng thông tin CCCD/CMT như bạn đang nhập.\r\nNhấn yes để hiển thị danh sách người bệnh trùng thông tin.\r\nNhấn No để tiếp tục tiếp đón", "Thông báo trùng thông tin", true))
                    {
                        frm_DSACH_BN_TKIEM Timkiem_Benhnhan = new frm_DSACH_BN_TKIEM("ALL", 0);
                        Timkiem_Benhnhan.AutoSearch = true;
                        Timkiem_Benhnhan.FillAndSearchData(false, "", "", "", Utility.sDbnull(txtCMT.Text), new DateTime(1900,1,1), Utility.ByteDbnull(cboPatientSex.SelectedValue, 100), "", "-1");
                        if (Timkiem_Benhnhan.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                        {
                            txtMaKcbbd.Clear();
                            txtNoiphattheBHYT.Clear();
        ...
        private void cmdSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (THU_VIEN_CHUNG.Laygiatrithamsohethong("BHYT", "0", false) == "1"
                    ...
                    && !chknothe.Checked )
                {
                    ...
                    if (m_enAct != action.Update
                       ...
                       && globalVariables.ACCOUNTCLINIC.Substring(0, 2) != txtMaNoiCaptheBHYT.Text.Trim()
                       && globalVariables.ACCOUNTCLINIC.Substring(2, 3) != txtMaKcbbd.Text.Trim())
                    {
                        if (!Utility.AcceptQuestion(string.Format("Nơi đăng ký KCBBĐ: {0}{1} không phải tại bệnh viện (Mã KCBBĐ của viện: {2}). \r\nBạn có muốn tiếp tục?", txtMaNoiCaptheBHYT.Text, txtMaKcbbd.Text, globalVariables.ACCOUNTCLINIC), "Nơi ĐK KCB ban đầu không phải tại bệnh viện", true))
                        {
                            return;
                        }
                        else
                            Utility.Log(this.Name, globalVariables.UserName, string.Format("Người dùng {0} đồng ý tiếp đón bệnh nhân {1} có nơi đăng ký KCBBĐ: {2}{3} không phải tại bệnh viện", globalVariables.UserName, txtTEN_BN.Text, txtMaNoiCaptheBHYT.Text, txtMaKcbbd.Text));
                    }
                }
        ...
        private bool IsValidData()
        {
            ...
            if (THU_VIEN_CHUNG.IsBaoHiem(_idLoaidoituongKcb))
            {
                if(chknothe.Checked && txtdoituongnothe.MyCode=="-1")
                {
                    Utility.ShowMsg("Bạn cần chọn đối tượng nợ thẻ");
                    txtdoituongnothe.Focus();
                    txtdoituongnothe.SelectAll();
                    return false;
                }    
            ...
            {
                //if (THU_VIEN_CHUNG.Laygiatrithamsohethong("BHYT_BATNHAPNOICHUYENDEN", "0", false) == "1")
                //{
            ...
            if (Utility.sDbnull(txtCMT.Text, "").Length <= 0 && Utility.Int32Dbnull(txtTuoi.Text, 0) >= 12)
            {
            ...
            //    return false;
            //}
            //if (THU_VIEN_CHUNG.Laygiatrithamsohethong("KCB_BATNHAP_NGUONGT", "0", false) == "1")
            //{
      ...
      bool isValidNhapVien()
        {
            if (Utility.Int32Dbnull(cbo_khoadieutri.SelectedValue, -1) == -1)
            {
                Utility.ShowMsg("Bạn phải chọn khoa nội trú");
                cbo_khoadieutri.Focus();
                return false;
            }
            if(dtpNgaytiepdon.Value> dtNgayNhapVien.Value)
            {
                Utility.ShowMsg("Thời gian nhập viện phải sau thời gian tiếp đón. Vui lòng kiểm tra lại");
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frm_KCB_DANGKY_BENHNHAN_CAPCUU_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void frm_KCB_DANGKY_BENHNHAN_CAPCUU_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
        ...
        {
            lblStatus.Text = m_enAct == action.Insert ? "THÊM MỚI" : (m_enAct==action.Add?"THÊM LẦN KHÁM":"CẬP NHẬT");
        }
        ...
        void InPhieu()
        {
            try
            {
                ...
                if (objBenhnhan != null && _objLuotkham != null && objCongkham != null && AllowQMS && THU_VIEN_CHUNG.Laygiatrithamsohethong("QMSPK_ENABLE", "0", false) == "1")
        ...
        private void ThemMoiLanKhamVaoLuoi()
        {
            ...
            dr[KcbLuotkham.Columns.MaKcbbd] = Utility.sDbnull(txtMaKcbbd.Text, "");
            dr[DmucNoiKCBBD.Columns.TenKcbbd] = txtTenKcbbd.Text;// lblClinicName.Text;
            ...
            dr["Noicap_KCBBD"] = Utility.sDbnull(txtMaNoiCaptheBHYT.Text + txtMaKcbbd.Text, "");
            ...
            if (!b_HasKham)
            {
               ...
               // KcbQm objQMS = KcbQm.FetchByID(IdQMS);
                ...
                if (objDichvuKcb != null)
                {
                    ...
                    objCongkham.ThanhtoanCongkhamsau = _objLuotkham.ThanhtoanCongkhamsau;
        ...
        {
            txtSovaovien.Text = THU_VIEN_CHUNG.LaysoVaovien();
            NoitruPhieunhapvien objPhieuNv = new Select().From(NoitruPhieunhapvien.Schema)
                .Where(NoitruPhieunhapvien.Columns.IdBenhnhan).IsEqualTo(_objLuotkham.IdBenhnhan)
                .And(NoitruPhieunhapvien.Columns.MaLuotkham).IsEqualTo(_objLuotkham.MaLuotkham)
                .ExecuteSingle<NoitruPhieunhapvien>();
            if (objPhieuNv == null) objPhieuNv = new NoitruPhieunhapvien();
            ...
            else
            {
                ...
                objPhieuNv.IsNew = false;
                objPhieuNv.MarkOld();
            }    
            objPhieuNv.ChovaoKhoa = Utility.sDbnull(txtChovaokhoa.Text);
            objPhieuNv.NgayNhapvien = dtNgayNhapVien.Value;
            objPhieuNv.MaLuotkham = "";
            objPhieuNv.IdBenhnhan = -1;
            objPhieuNv.IdKhoanoitru =Utility.Int32Dbnull(cbo_khoadieutri.SelectedValue);
        ...
        private NoitruPhanbuonggiuong TaoBuonggiuong()
        {
            ...
            objNoitruPhanbuonggiuong.MaLuotkham = "";
            objNoitruPhanbuonggiuong.IdBenhnhan = -1;
            objNoitruPhanbuonggiuong.IdKhoanoitru = Utility.Int16Dbnull(cbo_khoadieutri.SelectedValue, -1);
            objNoitruPhanbuonggiuong.NgayTao = globalVariables.SysDate;
            objNoitruPhanbuonggiuong.NgayVaokhoa = dtNgayNhapVien.Value;
            objNoitruPhanbuonggiuong.IdKham = -1;
            ...
            objNoitruPhanbuonggiuong.IdBuong = Utility.Int16Dbnull(cbo_buong.SelectedValue);
            objNoitruPhanbuonggiuong.IdGiuong = Utility.Int16Dbnull(cbo_giuong.SelectedValue);
            objNoitruPhanbuonggiuong.TinhChiphi = 1;
            objNoitruPhanbuonggiuong.IsGhepGiuong = 0;
            objNoitruPhanbuonggiuong.IdGia = Utility.Int32Dbnull(txtGia.MyID, -1);
        ...
        private void InsertPatient()
        {
            ...
            long v_id_kham = -1;
            string msg = "";
            NoitruPhieunhapvien objPhieuNv = Taophieunhapvien();
            errorProvider1.Clear();
            ActionResult actionResult = _kcbDangky.ThemmoiBenhnhan_CapCuu(this.myTrace, objBenhnhan, _objLuotkham,ref objPhieuNv, TaoBuonggiuong(), dtNgayNhapVien.Value, ref v_id_kham, ref msg);
            ...
            switch (actionResult)
            {
                case ActionResult.Success:
                    ...
                    cmdSave.Enabled = false;
                    MBlnCancel = false;
                    if(chk_InphieukhamNhapvienSauLuu.Checked)
                    {
                        IN_PHIEU_KHAM_VAO_VIEN(objPhieuNv);
                    }    
        ...
        private void IN_PHIEU_KHAM_VAO_VIEN(NoitruPhieunhapvien objPhieuNv)
        {
            DataTable dsTable =
               new noitru_nhapvien().NoitruLaythongtinInphieunhapvien(_objLuotkham.MaLuotkham, Utility.Int32Dbnull(_objLuotkham.IdBenhnhan));
            ...
            SqlQuery sqlQuery = new Select().From(KcbChandoanKetluan.Schema)
                  .Where(KcbChandoanKetluan.Columns.MaLuotkham).IsEqualTo(_objLuotkham.MaLuotkham)
                  .And(KcbChandoanKetluan.Columns.IdBenhnhan).IsEqualTo(_objLuotkham.IdBenhnhan)
            ...
            chandoan += "," + ten_benhcp;
            DataSet ds = new noitru_nhapvien().KcbLaythongtinthuocKetquaCls(_objLuotkham.MaLuotkham, Utility.Int32Dbnull(_objLuotkham.IdBenhnhan), (byte)0);
        ...
        private void UpdatePatient()
        {
            ...
            errorProvider1.Clear();
            NoitruPhieunhapvien objPhieuNv = Taophieunhapvien();
            ActionResult actionResult = _kcbDangky.CapNhatBenhnhanCapcuu(this.myTrace, objBenhnhan, _objLuotkham,ref objPhieuNv,TaoBuonggiuong(), _ptramBhytCu, _ptramBhytGocCu, ref msg);
        ...
        private KcbDanhsachBenhnhan TaoBenhnhan()
        {
            ...
            objBenhnhan.NhomMau = txtNhommau.myCode;
            if (m_enAct == action.Insert)
            {
        ...
        private KcbLuotkham TaoLuotkham()
        {
            try
            {
                ...
                _objLuotkham.TrangthaiNoitru = 1;
                ...
                _objLuotkham.HuongDieutri = huong_dieu_tri;
                if (sudungsonha)
                {
                ...
                if (THU_VIEN_CHUNG.IsBaoHiem(_idLoaidoituongKcb))
                {
                    ...
                    _objLuotkham.MaDoituongKcbBhyt = Utility.sDbnull(txt_MaloaiDoituongKCB.MyCode);
                ...
                //Thông tin nhập viện
                _objLuotkham.IdBacsiNhapvien = Utility.Int16Dbnull(txtBacsi.MyID);
                _objLuotkham.IdKhoanoitru = Utility.Int16Dbnull(cbo_khoadieutri.SelectedValue, -1);
                _objLuotkham.NgayNhapvien = dtNgayNhapVien.Value;
                _objLuotkham.SoVaovien = Utility.sDbnull(txtSovaovien.Text);
                _objLuotkham.MotaNhapvien = Utility.DoTrim(autoGhichuvaovien.Text);
                _objLuotkham.IdCongkhamNhapvien = -1;
        ...
        private void cboMaKhuvuc_SelectedIndexChanged_1(object sender, EventArgs e)
        {
        ...
        private void cbo_khoadieutri_SelectedIndexChanged(object sender, EventArgs e)
        {
            LoadBuong();
        }

        private void cbo_buong_SelectedIndexChanged(object sender, EventArgs e)
        {
            ChonBuong();
        }

        private void cbo_giuong_SelectedIndexChanged(object sender, EventArgs e)
        {
            LoadGiaGiuong();
        }
