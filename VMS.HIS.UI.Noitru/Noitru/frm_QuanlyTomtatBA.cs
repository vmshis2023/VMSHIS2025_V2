...
namespace VNS.HIS.UI.NOITRU
{
    public partial class frm_QuanlyTomtatBA : Form
    {
        ...
        void InitEvents()
        {
            ...
            Load += frm_QuanlyTomtatBA_Load;
            KeyDown += frm_QuanlyTomtatBA_KeyDown;
        ...
        private void frm_QuanlyTomtatBA_Load(object sender, EventArgs e)
        {
        ...
        private void TimKiemThongTin()
        {
            ...
            string ma_PhieuTTBA=Utility.DoTrim(txtmaBBHC.Text);
            ...
            m_dtData = SPs.KcbTomtatBATimkiem(-1,tungay, denngay, -1, ma_luotkham, ten_benhnhan).GetDataSet().Tables[0];
        ...
        private void frm_QuanlyTomtatBA_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        void _PhieuTTBA__OnCreated(long id,action m_enAct)
        {
            try
            {
                DataTable dt_temp = SPs.KcbTomtatBATimkiem(id, new DateTime(1900, 1, 1), new DateTime(1900, 1, 1), -1, "", "").GetDataSet().Tables[0];
                ...
                if (m_enAct == action.Update && m_dtData != null && m_dtData.Columns.Count > 0 && dt_temp.Rows.Count > 0)
                {
                    ...
                    if (arrDr.Length > 0)
                    {
                        arrDr[0]["quatrinhbenhly_dienbienlamsang"] = dt_temp.Rows[0]["quatrinhbenhly_dienbienlamsang"];
                        arrDr[0]["tiensu_benh"] = dt_temp.Rows[0]["tiensu_benh"];
                        arrDr[0]["tomtat_kqcls"] = dt_temp.Rows[0]["tomtat_kqcls"];
                        arrDr[0]["ghi_chu"] = dt_temp.Rows[0]["ghi_chu"];
                    }
                    else
                        m_dtData.ImportRow(dt_temp.Rows[0]);
        ...
        private void cmdUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                frm_TomtatBA _PhieuTTBA = new frm_TomtatBA();
                _PhieuTTBA._OnCreated += _PhieuTTBA__OnCreated;
                _PhieuTTBA.ttba = KcbTomtatBA.FetchByID(Utility.Int64Dbnull(grdList.GetValue("Id")));
        ...
        private void cmdXoa_Click(object sender, EventArgs e)
        {
            try
            {
                if (Utility.Coquyen("noitru_PhieuTTBA_xoa") || globalVariables.UserName == grdList.GetValue("nguoi_tao"))
                {
                }
                else
                {
                    Utility.thongbaokhongcoquyen("noitru_PhieuTTBA_xoa", "xóa Tóm tắt hồ sơ bệnh án");
                    return;
                }
                if (!Utility.isValidGrid(grdList))
                {
                    Utility.ShowMsg("Bạn cần chọn một phiếu Tóm tắt hồ sơ bệnh án trên danh sách để xóa. Vui lòng kiểm tra lại");
                    return;
                }
        ...
        private void cmdPrint_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                KcbTomtatBA ttba = KcbTomtatBA.FetchByID(Utility.Int64Dbnull(grdList.GetValue("id")));
                ...
                //DataTable dtData = SPs.KcbTomtatBAIn(ttba.Id, Utility.Int64Dbnull(grdList.GetValue("id_benhnhan")), Utility.sDbnull(grdList.GetValue("ma_luotkham"))).GetDataSet().Tables[0];
                ...
                //dicMF.Add("gioitinh_nam",grdList.GetValue("id_gioitinh").ToString() == "0" ? "1" : "0");
                //dicMF.Add("gioitinh_nu", grdList.GetValue("id_gioitinh").ToString() == "0" ? "0" : "1");
                ...
                //               Path.GetFileNameWithoutExtension(PathDoc), "TOMTAT_BA", Utility.sDbnull(grdList.GetValue("ma_luotkham")), Utility.sDbnull(ttba.Id), Guid.NewGuid().ToString(), Path.GetExtension(PathDoc));
