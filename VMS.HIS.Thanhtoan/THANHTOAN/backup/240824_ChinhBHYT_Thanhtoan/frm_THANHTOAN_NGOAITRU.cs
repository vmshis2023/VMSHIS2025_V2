...
namespace  VNS.HIS.UI.THANHTOAN
{
    public partial class frm_THANHTOAN_NGOAITRU : Form
    {
        ...
        void grdPayment_UpdatingCell(object sender, UpdatingCellEventArgs e)
        {
            ...
            //    long _IdChitietdonthuoc = Utility.Int64Dbnull(grdPayment.GetValue("id_phieu_chitiet"));
            //    string ten_thuoc = grdPayment.GetValue("ten_chitietdichvu").ToString();
            //    string ten_dvt = grdPayment.GetValue("ten_donvitinh").ToString();
            //    KcbDonthuocChitiet objChitiet = KcbDonthuocChitiet.FetchByID(_IdChitietdonthuoc);
            //    //decimal so_luong = Utility.DecimaltoDbnull(grdPresDetail.GetValue(KcbDonthuocChitiet.Columns.SoLuong));
            //    //if (so_luong<=)
            //    //{
            ...
            //    decimal num = CommonLoadDuoc.SoLuongTonTrongKho(-1L, Utility.Int32Dbnull(objChitiet.IdKho, -1), Utility.Int32Dbnull(objChitiet.IdThuoc, -1), Utility.Int64Dbnull(objChitiet.IdThuockho),
            //      Utility.Int32Dbnull(THU_VIEN_CHUNG.Laygiatrithamsohethong("KIEMTRATHUOC_CHOXACNHAN", "1", false), 1), (byte)0);
            //    //Kiểm tra nếu số lượng tồn > số lượng kê thì cho cập nhật
            //    num = num + Utility.DecimaltoDbnull(e.InitialValue);
            //    if (num >= Utility.DecimaltoDbnull(e.Value))
            //    {
            //        //Bắt đầu cập nhật số lượng cho dòng chi tiết và trong bảng tạm kê
            //        objChitiet.SluongSua = Utility.DecimaltoDbnull(e.Value, objChitiet.SoLuong);
            ...
            //            decimal thanhtien = Utility.DecimaltoDbnull(e.Value) *
            //                                (Utility.DecimaltoDbnull(grdPayment.GetValue("don_gia")) +
            //                                 Utility.DecimaltoDbnull(grdPayment.GetValue("phu_thu")));
            ...
            //                       string.Format(
            //                           "Bán thuốc tại quầy: Sửa số lượng thuốc của bệnh nhân {0} có mã lần khám {1} và mã bệnh nhân là: {2} từ {3} sang {4}", objBenhnhan.TenBenhnhan,
            //                           objLuotkham.MaLuotkham, objLuotkham.IdBenhnhan, e.InitialValue.ToString(), objChitiet.SluongSua.ToString()),
            //                       newaction.Update, this.GetType().Assembly.ManifestModule.Name);
            //    }
            //    else
            //    {
            //        Utility.ShowMsg(string.Format("Thuốc {0} đang có {1} {2}.\nSố lượng bạn mua ={3} nhiều hơn số tồn thuốc kho thực tế. Đề nghị điều chỉnh lại số lượng mua hợp lý.\nChú ý:Hệ thống đang kiểm tra theo Id-thuốc kho={4} Có thể còn thuốc này ở các lô nhập khác, bạn có thể thực hiện thêm đơn thuốc tại quầy nếu người bệnh muốn mua thêm", ten_thuoc, num, ten_dvt, e.Value, objChitiet.IdThuockho));
            //        e.Cancel = true;
            //    }
        ...
        private void PerformAction()
        {
            try
            {
                ...
                if (objLuotkham != null)
                {
                    ...
                    ActionResult actionResult = _THANHTOAN.ThanhtoanChiphiDvuKcb(v_objThanhtoan, objLuotkham,
                        ...
                        case ActionResult.Success:
                            ...
                            if (THU_VIEN_CHUNG.Laygiatrithamsohethong("KCB_COKHAM_DOTHILUC", "0", false) == "1" && THU_VIEN_CHUNG.Laygiatrithamsohethong("QMSPK_ENABLE", "0", false) == "1")
                            {
                                ...
                                if (lstck.Any())//Nếu có thanh toán công khám-->Tìm công khám đo thị lực để sinh số QMS
                                {
                                    ...
                                    if (objCongkham != null && objCongkham.TrangThai!=1)//Chưa khám thị lực mới sinh số QMS cho phòng đo thị lực
                                    {
        ...
        private void HuyPhieuchi(GridEX grd)
        {
            ...
            if (grd.CurrentRow != null)
            {
                ...
                if (objThanhtoan != null)
                {
                    ...
                    {
                        Utility.ShowMsg(string.Format("Ngày lập phiếu chi {0} - Ngày hủy phiếu chi {1}. Hệ thống không cho phép bạn hủy phiếu chi đã quá {2} ngày. Cần liên hệ quản trị hệ thống để được trợ giúp", objThanhtoan.NgayThanhtoan.ToString("dd/MM/yyyy"), globalVariables.SysDate.ToString("dd/MM/yyyy"), KCB_THANHTOAN_SONGAY_HUYPHIEUCHI.ToString()));
                        return;
                    }
            ...
            if (e.Column.Key == "tile_chietkhau" || e.Column.Key == "tien_chietkhau")
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        grdThongTinChuaThanhToan.CurrentRow.Cells["tile_chietkhau"].Value = (Utility.DecimaltoDbnull(e.Value, 0) / Utility.DecimaltoDbnull(grdThongTinChuaThanhToan.CurrentRow.Cells["tt"].Value, 0)) * 100;
                        tile_chietkhau = Utility.DecimaltoDbnull(grdThongTinChuaThanhToan.CurrentRow.Cells["tile_chietkhau"].Value);
                    }
                    grdThongTinChuaThanhToan.CurrentRow.Cells["thuc_thu"].Value = Utility.DecimaltoDbnull(grdThongTinChuaThanhToan.CurrentRow.Cells["tt"].Value, 0) - Utility.DecimaltoDbnull(grdThongTinChuaThanhToan.CurrentRow.Cells["tien_chietkhau"].Value, 0);
                    //Cập nhật luôn vào bảng trong CSDL để in bảng kê chi phí cho người bệnh xem trước khi thanh toán
