...
namespace  VNS.HIS.UI.THANHTOAN
{
    public partial class frm_THANHTOAN_NGOAITRU : Form
    {
                    ...
                    if (chkPercent.Checked)
                    {
                        ...
                        foreach (DataRow dr in m_dtChiPhiThanhtoan.Rows)
                        {
                            if (Utility.sDbnull(dr["trangthai_thanhtoan"], "0") == "0" && Utility.sDbnull(dr["colChon"], "0") == "1")
                            {
                                ...
                                dr["thuc_thu"] = Utility.DecimaltoDbnull(dr["TT_BN_KHONG_PHUTHU"], 0) - Utility.DecimaltoDbnull(dr["tien_chietkhau"], 0);
        ...
        private KcbThanhtoan TaophieuThanhtoan()
        {
            ...
            objPayment.TtoanThuoc =Utility.Byte2Bool( ThanhtoanQuayThuoc);//0= thanh toán các loại dịch vụ;1= thanh toán đơn thuốc tại quầy
            ...
            if (grd.CurrentRow != null)
            {
                ...
                if (objPayment != null)
                {
                    ...
                    if (PropertyLib._ThanhtoanProperties.Hienthihuyphieuchi)
                    {
                        ...
                        frm.objLuotkham = objLuotkham;
                        frm.v_Payment_Id = Utility.Int32Dbnull(objPayment.IdThanhtoan, -1);
                        frm.objKcbThanhtoan = objPayment;
                        frm.Chuathanhtoan = _chuathanhtoan;
                    ...
                    else
                    {
                        ...
                        if (Utility.Bool2Bool(objPayment.TtoanThuoc) && objPayment.KieuThanhtoan==1)
                            actionResult = _THANHTOAN.HuyPhieuchiTrathuoctaiquay_Rieng(objPayment, objLuotkham, lydo_huy);
                        else
                            actionResult = _THANHTOAN.HuyPhieuchi(objPayment, objLuotkham, lydo_huy);
                        switch (actionResult)
                        {
                            case ActionResult.Success:
