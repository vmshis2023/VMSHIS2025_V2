...
using VMS.HIS.Thanhtoan.THANHTOAN;
using VMS.HIS.Bus.BHYT;
using VMS.HIS.Bus.QRPay;
using VMS.Invoice;
...
namespace  VNS.HIS.UI.THANHTOAN
{
    public partial class frm_THANHTOAN_NGOAITRU : Form
    {
        ...
        MisaInvoice _MisaInvoices = new MisaInvoice();
        ...
        string _help = "";
        byte ThanhtoanQuayThuoc = 0;
        /// <summary>
        ///     05-11-2013
        /// </summary>
        #region "khai báo biến "
        private DataTable m_dtDataTimKiem = new DataTable();
        private DataTable m_dtPayment, m_dtPhieuChi,m_dtTamthu = new DataTable();
        ...
        public frm_THANHTOAN_NGOAITRU(string pv_sArgs)
        {
            ...
            _MisaInvoices._OnStatus += _MInvoice__OnStatus;
            grdPayment.CellSelectionMode = CellSelectionMode.SingleCell;
            ...
            ThanhtoanQuayThuoc = Utility.ByteDbnull(pv_sArgs.Split('-')[3], 0);
            ucThuchikhac1.v_bytNoitru = v_bytNoitru;
            //cmdChuyenCLS.Visible = cmdDungChuyenCLS.Visible = !this.Args.Contains("KN");
            ...
            //Utility.grdExVisiableColName(grdPayment, "TAIHOADON", globalVariables.IsAdmin || Utility.Coquyen("HOADONDIENTU_DOWNLOAD"));
            //Utility.grdExVisiableColName(grdPayment, "XUATHOADON", globalVariables.IsAdmin || Utility.Coquyen("HOADONDIENTU_UPLOAD"));
            ...
            Utility.grdExVisiableColName(grdPayment, "sluong_sua",v_bytNoitru==0 && (lst_IDLoaithanhtoan.Contains("3") ||lst_IDLoaithanhtoan.Contains("5"))  );
        ...
        private void _MInvoice__OnStatus(string status, bool isErr)
        {
            UIAction.SetTextStatus(lblTrangthainoitru, status, isErr);
        }
        ...
        void InitEvents()
        {
          ...
          //  cmdChuyenCLS.Click += new EventHandler(cmdChuyenCLS_Click);
            ...
            optDvuAll.CheckedChanged += OptDvuAll_CheckedChanged;
            optDvuChuathanhtoan.CheckedChanged += OptDvuAll_CheckedChanged;
            optDvuDathanhtoan.CheckedChanged += OptDvuAll_CheckedChanged;
            optLoaiChiphiAll.CheckedChanged += OptLoaiChiphiAll_CheckedChanged; 
            optLoaiChiphiBHYT.CheckedChanged += OptLoaiChiphiAll_CheckedChanged;
            optLoaiChiphiDvu.CheckedChanged += OptLoaiChiphiAll_CheckedChanged;
            //mnuTutuc.Click += mnuTutuc_Click;
            mnuHuychonTatca.Click += mnuHuychonTatca_Click;
            mnuTamung.Click += MnuTamung_Click;

        }

        private void MnuTamung_Click(object sender, EventArgs e)
        {
            Tamthu();
        }

        private void OptLoaiChiphiAll_CheckedChanged(object sender, EventArgs e)
        {
            FilterData();
        }

        private void OptDvuAll_CheckedChanged(object sender, EventArgs e)
        {
            FilterData();
        }
        void FilterData()
        {
            string rowFilter = "";
            if(optDvuAll.Checked)
                rowFilter = "trangthai_thanhtoan in (0,1)";
            else if (optDvuChuathanhtoan.Checked)
                rowFilter = "trangthai_thanhtoan =0";
            else if (optDvuDathanhtoan.Checked)
                rowFilter = "trangthai_thanhtoan =1";

            if (optLoaiChiphiAll.Checked)
                rowFilter = rowFilter+ "";
            else if (optLoaiChiphiBHYT.Checked)
                rowFilter = rowFilter + " and bhyt_gia_tyle >0";
            else if (optLoaiChiphiDvu.Checked)
                rowFilter = rowFilter + " and bhyt_gia_tyle <=0";
            //rowFilter = rowFilter + " and trangthai_huy = 0";
        ...
        /// <summary>
        /// tile>0 có nghĩa là tăng thì làm phép *, tile<0 = giảm là phép chia cho 1+ tỉ lệ/100
        /// </summary>
        /// <param name="_row"></param>
        /// <param name="tile"></param>
        /// <param name="don_giamoi"></param>
        ...
        bool CapnhatDongia(GridEXRow _row, decimal tile, decimal don_giamoi, bool manual)
        {
            ...
            if (Utility.sDbnull(_row.Cells["trangthai_thanhtoan"].Value, "0") == "0")
            {
                ...
                //    Utility.DecimaltoDbnull(_row.Cells[KcbChidinhclsChitiet.Columns.DonGia].Value, 0) *
                //    (Utility.DecimaltoDbnull(_row.Cells[KcbChidinhclsChitiet.Columns.TyleTt].Value, 0) / 100) *
                //    Utility.DecimaltoDbnull(_row.Cells[KcbChidinhclsChitiet.Columns.SoLuong].Value, 0);
        ...
        void txtptramtanggia_nguoinuocngoai_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    string ask = string.Format("Bạn có chắc chắn muốn {0} giá {1} % cho toàn bộ các dịch vụ đang được chọn?", tile > 0 ? "tăng" : "giảm", tile.ToString());
        ...
        void grdPayment_UpdatingCell(object sender, UpdatingCellEventArgs e)
        {
            ...
            else if (e.Column.Key == "mota_them")
            {
                SuaMoTaThem();
            }
        ...
        void SuaMoTaThem()
        {
            try
            {
                ...
                if (!Utility.AcceptQuestion(string.Format("Bạn có chắc chắn muốn sửa thông tin mô tả của chứng từ đang chọn từ:\n{0} \nthành:\n{1}", oldValue, newValue), "Xác nhận thay đổi", true)) return;
        ...
        void updateQtyinDataTable(long IdChitietdonthuoc, decimal newQty, decimal TT)
        {
            try
            {
                DataRow[] arrDr = m_dtChiPhiThanhtoan.Select(string.Format("id_phieu_chitiet={1}",  IdChitietdonthuoc));
                if (arrDr.Length > 0)
                {
        ...
        /// <summary>
        /// Nếu muốn miễn giảm cả phụ thu thì sau này thay cột TT_BN_KHONG_PHUTHU=TT_BN
        /// </summary>
                    ...
                    if (chkPercent.Checked)
                    {
                        ...
                        foreach (DataRow dr in m_dtChiPhiThanhtoan.Rows)
                        {
                            if (Utility.sDbnull(dr["trangthai_thanhtoan"], "0") == "0" && Utility.sDbnull(dr["colChon"], "0") == "1")
                            {
                                ...
                                if (THU_VIEN_CHUNG.Laygiatrithamsohethong("THANHTOAN_CHIETKHAU_TUDONGLUU", "1", true) == "1") CapnhatChietkhau_DonGia(Convert.ToByte(0), id_loaithanhtoan, kieu_chietkhau, tile_chietkhau, tien_chietkhau, id_phieu, id_phieuchitiet);
                            }
                        }
                    }
            ...
            if (Utility.AcceptQuestion("Bạn có chắc chắn muốn chuyển toàn bộ chi phí từ nguồn giới thiệu thành chiết khấu cho Bệnh nhân hay không?", "Xác nhận chuyển chi phí nguồn", true))
            {
                ...
                foreach (GridEXRow row in grdThongTinChuaThanhToan.GetCheckedRows())
                {
                    if (Utility.sDbnull(row.Cells["tinh_chkhau"].Value, "0") == "1")
                    {
                        ...
                        row.Cells["tien_chietkhau"].Value = Utility.DecimaltoDbnull(row.Cells["tt"].Value, 0) * Utility.DecimaltoDbnull(objLuotkham.ChiphiGioithieu, 0) / 100;
        ...
        void grdPayment_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            try
            {
                string kieu_thanhtoan = Utility.sDbnull(grdPayment.GetValue("kieu_thanhtoan"));

                if (kieu_thanhtoan == "1")
                    CallPhieuChi(grdPayment);
                else if (kieu_thanhtoan == "10")//Ghi nợ
                {
                    //Tự điền các dịch vụ ghi nợ vào các dịch vụ chờ thanh toán
                    TuChonGhiNo();
                }
                else if (kieu_thanhtoan == "20")//Tạm ứng
                {
        ...
        void TuChonGhiNo()
        {
            try
            {
                long id_ghino = Utility.Int64Dbnull(grdPayment.GetValue(KcbThanhtoan.Columns.IdThanhtoan));
                foreach (GridEXRow exRow in grdThongTinChuaThanhToan.GetDataRows())
                {
                    exRow.BeginEdit();
                    exRow.IsChecked = false;

                    if (Utility.Int32Dbnull(exRow.Cells["id_tamthu"].Value, 0) == id_ghino &&  Utility.Int32Dbnull(exRow.Cells["trangthai_huy"].Value, 0) == 0 && Utility.Int32Dbnull(exRow.Cells["trangthai_thanhtoan"].Value, 0) == 0)
                    {
                        exRow.IsChecked = true;
                    }
                    else
                    {
                        exRow.IsChecked = false;
        ...
        void cmdHoanung_Click(object sender, EventArgs e)
        {
            ...
            if (cmdHoanung.Tag.ToString() == "0")//Hoàn ứng
            {
                ...
                SPs.NoitruHoanung(-1,-1l,maphieu, objLuotkham.MaLuotkham, objLuotkham.IdBenhnhan,DateTime.Now,
            ...
            else
            {
                ...
                SPs.NoitruHuyhoanung(objLuotkham.MaLuotkham, objLuotkham.IdBenhnhan,-1, -1l, (byte)0).Execute();
                cmdHoanung.Tag = "0";
        ...
        bool isValidICD()
        {
            ...
            {
                Utility.ShowMsg("Mã bệnh chính bạn nhập không tồn tại trong hệ thống\r\n Bạn có thể kiểm tra danh mục ICD 10 và thêm mã này vào.");
            }
            return true;
        }
        ...
        void cmdInBienlaiTonghop_Click(object sender, EventArgs e)
        {
            try
            {
                cmdInBienlaiTonghop.Enabled = false;
                int _Payment_ID = Utility.Int32Dbnull(grdPayment.GetValue(KcbThanhtoan.Columns.IdThanhtoan), -1);
                new INPHIEU_THANHTOAN_NGOAITRU().InBienlai(true, _Payment_ID, -1, objLuotkham, v_bytNoitru);
            }
            catch (Exception ex)
            {
                Utility.CatchException(ex);
            ...
            {
                cmdInBienlaiTonghop.Enabled = true;
            }
        }
        ...
        void cmdInBienlai_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (kieuthanhtoan == 10 )//Ghi nợ
                {
                    if (!Utility.isValidGrid(grdPayment)) return;
                    long id_ghino = Utility.Int64Dbnull(grdPayment.GetValue(KcbThanhtoan.Columns.IdThanhtoan));
                    InBienLaiGhiNo(id_ghino);
                }
                else if (kieuthanhtoan == 20)//Tạm ứng
                {
                    InPhieuTU();
                }
                else//Phiếu chi
        ...
        void InPhieuTU()
        {
            try
            {
                ...
                long id_tamung = Utility.Int64Dbnull(grdPayment.GetValue(KcbThanhtoan.Columns.IdThanhtoan));
        ...
        private void CauHinh()
        {
            try
            {
               ...
               mnuLayhoadondo.Visible = Utility.Coquyen("thanhtoan_quyen_laysobienlai") &&  RedInvoice;
                uiTabPageTamung.TabVisible =globalVariables.isSuperAdmin || THU_VIEN_CHUNG.Laygiatrithamsohethong("KCB_THANHTOAN_KICHHOAT_TAMUNG_NGOAITRU", "0", false) == "1";
        ...
        private void ModifyCommand()
        {
            try
            {
                ...
                cmdTraLaiTien.Enabled = Utility.isValidGrid(grdList) && grdThongTinDaThanhToan.GetCheckedRows().Length > 0 && objLuotkham != null && (chotratienngoaitrukhidangnoitru || objLuotkham.TrangthaiNoitru <= 0);
                ...
                pnlBHYT.Visible = Utility.isValidGrid(grdList) && objLuotkham != null && objLuotkham.MaDoituongKcb == "BHYT" ;
                //cmdInphoiBHYT.Visible = Utility.isValidGrid(grdList) && _chuathanhtoan <= 0 && Utility.DecimaltoDbnull(txtSoTienCanNop.Text) <= 0 && objLuotkham != null && objLuotkham.MaDoituongKcb == "BHYT" && grdPayment.GetDataRows().Length > 0 ;
                cmdInphoiBHYT.Visible = Utility.isValidGrid(grdList) && objLuotkham != null && objLuotkham.MaDoituongKcb == "BHYT";
                pnlInBHYT.Height = cmdInphoiBHYT.Visible ? 43 : 0;
                //cmdInphieuDCT.Visible = Utility.isValidGrid(grdList) && objLuotkham != null && objLuotkham.MaDoituongKcb == "BHYT" && grdPayment.GetDataRows().Length > 0;
        ...
        private void ClearControl()
        {
            try
            {
                ...
                txtThongtinMG.Clear();
                txtptramtanggia_nguoinuocngoai.Clear();
                chkDoituongnguoinuocngoai.Checked = false;
        ...
        private void GetData()
        {
            try
            {
                ketchuyen_tamthu = THU_VIEN_CHUNG.Laygiatrithamsohethong("THANHTOAN_TUDONGKETCHUYEN_TAMTHU","0",true)=="1";
                ...
                if (mDtThongTin.Rows.Count > 0)
                {
                    ...
                    if (objLuotkham != null)
                    {
                        ...
                        {
                            ucTamung1.pnlActTamung.Enabled = true;
                            ucThuchikhac1.pnlFunctions.Enabled = true;
                        }
                        ...
                        txtICD.SetCode(objLuotkham.MabenhChinh) ;
        ...
        private void TuybiennutchuyenCLS()
        {
            try
            {
                if (objLuotkham != null)
                {
                    ...
                    mnuHuychuyenCLS.Enabled = cmdDungChuyenCLS.Enabled;
                    ...
                    mnuChuyenCLS.Enabled = cmdChuyenCLS.Enabled;
                }
            }
            catch (Exception ex)
            {
                cmdDungChuyenCLS.Enabled = cmdChuyenCLS.Enabled = mnuHuychuyenCLS.Enabled = mnuChuyenCLS.Enabled = true;
            }
        }
        public static void UpdateChietKhau(DataTable dtNguon, DataTable dtDich)
        {
            if (dtNguon == null || dtDich == null) return;

            // Tạo dictionary để tra nhanh các dòng trong dtDich
            var map = dtDich.AsEnumerable().ToDictionary(
                r => $"{r["id_loaithanhtoan"]}_{r["id_phieu"]}_{r["id_phieu_chitiet"]}_{r["id_dichvu"]}_{r["id_chitietdichvu"]}",
                r => r
            );

            // Duyệt từng dòng của bảng nguồn (dtNguon)
            foreach (DataRow drNguon in dtNguon.Rows)
            {
                string key = $"{drNguon["id_loaithanhtoan"]}_{drNguon["id_phieu"]}_{drNguon["id_phieu_chitiet"]}_{drNguon["id_dichvu"]}_{drNguon["id_chitietdichvu"]}";

                if (map.TryGetValue(key, out DataRow drDich))
                {
                    drDich["tien_chietkhau"] = drNguon["tien_chietkhau"];
                    drDich["tile_chietkhau"] = drNguon["tile_chietkhau"];
                }
            }
        }
        private void GetDataChiTiet()
        {
            try
            {
                ...
                if(IsSimpleRefresh)
                {
                    dtTemp = m_dtChiPhiThanhtoan.Copy();
                }    
                m_dtChiPhiThanhtoan =
                ...
                if (IsSimpleRefresh)//Tự update lại các thông tin miễn giảm
                {
                    UpdateChietKhau(dtTemp, m_dtChiPhiThanhtoan);
                }
                ...
                m_dtChiPhiThanhtoan.AcceptChanges();
                    Utility.SetDataSourceForDataGridEx(grdThongTinChuaThanhToan, m_dtChiPhiThanhtoan, false, true, "trangthai_huy=0" , "");
               
                GetChiPhiDaThanhToan();
                FilterData();
                ////tạm khóa 240830 vì đã gọi ở hàm FilterData phía trên. Xóa sau
                //UpdateTuCheckKhiChuaThanhToan();
                //SetSumTotalProperties();
            }
            catch (Exception ex)
        ...
        private void SetSumTotalProperties()
        {
            try
            {
                ...
                _chuathanhtoan = 0m;
                ptram_BHYT_moi = Utility.DecimaltoDbnull(objLuotkham.PtramBhyt, 0);
                txtPtramBHChiTra.Text =string.Format("{0} %", Ptram_bhyt_cu.ToString());
                foreach (DataRow drv in m_dtChiPhiThanhtoan.Rows)//Không tính theo default view để luôn hiển thị đúng phần liên quan BHYT khi thực hiện lọc dữ liệu theo trạng thái thanh toán và loại chi phí
                {
                    if (Utility.Int32Dbnull(drv["tinh_chiphi"], 0) == 1 && Utility.Int32Dbnull(drv["trangthai_huy"], 0) == 0 )//&& Utility.Int32Dbnull(drv["tthai_tamthu"], 0) == 0)
                    {
                        ...
                        {
                            tt_chon += Utility.DecimaltoDbnull(drv["TT_BN"], 0);
                        }
                ...
                if (THU_VIEN_CHUNG.IsBaoHiem(objLuotkham.IdLoaidoituongKcb))
                {
                    ...
                    objLuotkham.MucHuongBhyt =Utility.Int16Dbnull( ptram_BHYT_moi);
                    if (Ptram_bhyt_cu != ptram_BHYT_moi)
                    {
                        txtPtramBHChiTra.Text = string.Format("{0} %", ptram_BHYT_moi.ToString());
                        ...
                        foreach (DataRow dr in m_dtChiPhiThanhtoan.Rows)
                        {
                            if (Utility.Int32Dbnull(dr["tinh_chiphi"], 0) == 1 && Utility.Int32Dbnull(dr["trangthai_huy"], 0) == 0 && Utility.DecimaltoDbnull(dr["bhyt_gia_tyle"], 0) > 0)//&& Utility.Int32Dbnull(drv["tthai_tamthu"], 0) == 0)
                            {
                                ...
                                THU_VIEN_CHUNG.Bhyt_PhantichGia(objLuotkham, dr, ptram_BHYT_moi);
                            }
                        }
                ...
                txtNguonkhac.Text = Utility.sDbnull(tt_khac);
        ...
        decimal tongtien_thuathieu = 0;
        void ThongtinTamung()
        {
            globalVariables.KhongHoanUng = false;
            chk_Botamung.Checked = false;
            //chk_Botamung.Font.Strikeout = false;
            ...
            decimal tongTamung_hienthi = 0;
            txtTongTU.Clear();
            if (uiTabPageTamung.TabVisible)
            {
                if (ucTamung1.m_dtTamung != null)
                {
                    tongTamung_hienthi = Utility.DecimaltoDbnull(ucTamung1.m_dtTamung.Compute("SUM(so_tien)", "tthai_huy=0 or tthai_huy is null"), 0);
            ...
            txtTongTU.Text = tongTamung_hienthi.ToString();
            //Kiểm tra nếu chỉ thanh toán thuốc thì ko đưa tạm ứng vào tính toán
            List<string> lstThuocVT = new List<string>() { "3","5"};
            List<string> lstIdLoaiThanhtoan = grdThongTinChuaThanhToan.GetCheckedRows().Where(c => !lstThuocVT.Contains( Utility.sDbnull(c.Cells["id_loaithanhtoan"].Value))).Select(c => Utility.sDbnull(c.Cells["id_loaithanhtoan"].Value)).Distinct().ToList<string>();
            if (lstIdLoaiThanhtoan.Count <= 0)
            {
                tongTamung = 0;
                globalVariables.KhongHoanUng = true;
                chk_Botamung.Checked = true;
                //chk_Botamung.Font.Strikeout = true;
            }
            ...
            if (tongTamung == 0)
            {
                ...
                lblThuathieu.ForeColor = Color.DarkBlue;
                tongtien_thuathieu = Utility.DecimaltoDbnull(txtThuathieu.Text);
            }
        ...
        private void TinhToanSoTienPhaithu()
        {
            try
            {
                ...
                List<GridEXRow> query1 = (from thanhtoan in grdThongTinChuaThanhToan.GetCheckedRows()
                                        ...
                                        // && Utility.Int32Dbnull(thanhtoan.Cells["tthai_tamthu"].Value) == 0
                                         select thanhtoan).ToList();
                List<GridEXRow> sqlKhuyenMai = (from thanhtoan in grdThongTinChuaThanhToan.GetCheckedRows()
                                          where Utility.Int32Dbnull(thanhtoan.Cells["trangthai_huy"].Value) == 0
                                         // && Utility.Int32Dbnull(thanhtoan.Cells["tthai_tamthu"].Value) == 0
                                          && Utility.Int32Dbnull(thanhtoan.Cells["tthai_khuyenmai"].Value) == 1
                                          select thanhtoan).ToList();
                decimal thanhtien = query.Sum(c => Utility.DecimaltoDbnull(c.Cells["TT_BN_KHONG_PHUTHU"].Value))+ query.Sum(c => Utility.DecimaltoDbnull(c.Cells["TT_PHUTHU"].Value));//thay 20240830 vì tính sai với đối tượng BHYT. Check sau nếu muốn
                decimal Chietkhauchitiet = query1.Sum(c => Utility.DecimaltoDbnull(c.Cells["tien_chietkhau"].Value));
                decimal tongtien_khuyenmai = sqlKhuyenMai.Sum(c => Utility.DecimaltoDbnull(c.Cells["tien_chietkhau"].Value));
                ...
                txtTienKhuyenmai.Text = Utility.sDbnull(tongtien_khuyenmai);
                ModifyCommand();
            }
            catch (Exception ex)
            {
        ...
        {
            LoadUserConfigs();
             cmdTamthu.Visible =  mnuTamthu.Visible = THU_VIEN_CHUNG.Laygiatrithamsohethong("THANHTOAN_CHOPHEP_TAMTHU", "0", true) == "1";
            //cmdKetchuyen.Visible = mnuKetChuyen.Visible = mnuTamthu.Visible;
            pnlTangGiamDonGia.Enabled = Utility.Coquyen("thanhtoan_tanggiam_tile_dongia");
            InitData();
            log.Trace("InitData finished");
            LoadPtttNganhang();
            mnuCheckError.Enabled = globalVariables.isSuperAdmin;
            InitPTTTColumns();
            setProperties();
        ...
        void LoadChuongtrinhKhuyenmai()
        {
            try
            {
                DataTable dtCTKM = SPs.GoiLaydsachChuongtrinhKhuyenmai(dtPaymentDate.Value).GetDataSet().Tables[0];
                DataBinding.BindDataCombobox(cboChuongtrinhKM, dtCTKM, GoiDanhsach.Columns.IdGoi, GoiDanhsach.Columns.TenGoi, "---Chọn KM---", true);
            }
            catch (Exception ex)
            {
                Utility.CatchException(ex); 
        ...
        bool LaDvuKhuyenMai(DataRow dr)
        {
            if (dtChitietKM == null || dtChitietKM.Rows.Count == 0)
                return true;
            int id_loaithanhtoan = Utility.Int32Dbnull(dr["id_loaithanhtoan"], 200);
            int id_dichvu = Utility.Int32Dbnull(dr["id_dichvu"], 200);
            int id_chitietdichvu = Utility.Int32Dbnull(dr["id_chitietdichvu"], 200);
            string filter = string.Format("id_dichvu={0} and id_chitietdichvu={1} and loai_dvu={2}", id_dichvu, id_chitietdichvu, id_loaithanhtoan);
            DataRow[] arrDr = dtChitietKM.Select(filter);
            return arrDr.Length > 0;
        }
        DataTable dtChitietKM;
        GoiDanhsach objCTKM = null;
        private void cboChuongtrinhKM_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (!m_blnHasloaded || objLuotkham == null || m_dtChiPhiThanhtoan==null) return;
            try
            {
                objCTKM = GoiDanhsach.FetchByID(Utility.Int32Dbnull(cboChuongtrinhKM.SelectedValue, -1));
                if (!m_dtChiPhiThanhtoan.Columns.Contains("tthai_khuyenmai")) m_dtChiPhiThanhtoan.Columns.Add(new DataColumn("tthai_khuyenmai", typeof(byte)));
                cmdXemchitietkhuyenmai.Visible = objCTKM != null;
                if (objCTKM != null && !Utility.Bool2Bool( objCTKM.KhuyenmaiTong))//Khuyến mại tổng sẽ xử lý khi bấm thanh toán để tự động đưa vào form nhập lý do G
                {
                   
                     dtChitietKM = new Select().From(GoiChitiet.Schema).Where(GoiChitiet.Columns.IdGoi).IsEqualTo(objCTKM.IdGoi).ExecuteDataSet().Tables[0];
                    
                        decimal tile = Utility.DecimaltoDbnull(objCTKM.SoTien, 0);
                       
                        if (objCTKM.KieuChietkhau == "%")//Miễn giảm bằng % cho các dịch vụ
                        {
                        string ask = string.Format("Bạn có chắc chắn muốn miễn giảm {0} % cho toàn bộ các dịch vụ đang được chọn?", Convert.ToInt16(tile).ToString());
                        if (!Utility.AcceptQuestion(ask, "Xác nhận miễn giảm cho các dịch vụ đang chọn", true)) return;
                        if (tile > 100m)
                            ...
                            foreach (DataRow dr in m_dtChiPhiThanhtoan.Rows)
                            {
                                if (Utility.sDbnull(dr["trangthai_thanhtoan"], "0") == "0" && Utility.sDbnull(dr["colChon"], "0") == "1")
                                {
                                    if (Utility.Byte2Bool(objCTKM.KhuyenmaiTatcadvu) || LaDvuKhuyenMai(dr))
                                    {
                                        dr["tien_chietkhau"] = Utility.DecimaltoDbnull(dr["tt"], 0) * tile / 100;
                                        dr["tile_chietkhau"] = tile;
                                        dr["tthai_khuyenmai"] = 1;
                                        //Cập nhật luôn vào bảng trong CSDL để in bảng kê chi phí cho người bệnh xem trước khi thanh toán
                                        byte id_loaithanhtoan = Utility.ByteDbnull(dr["id_loaithanhtoan"]);
                                        string kieu_chietkhau = "%";
                                        decimal tile_chietkhau = Utility.DecimaltoDbnull(dr["tile_chietkhau"], 0);
                                    ...
                                    else//Chọn gói khuyến mãi khác(giả sử chi tiết KM khác gói lần trước vừa chọn)
                                    {
                                        if (Utility.Int32Dbnull(dr["tthai_khuyenmai"], 0) == 1)
                                        {
                                            dr["tthai_khuyenmai"] = 0;
                                            dr["tien_chietkhau"] = 0;
                                            dr["tile_chietkhau"] = 0;
                                        }
                                    }
                        ...
                        else//Chương trình miễn giảm bằng tiền
                        {
                        string ask = string.Format("Bạn có chắc chắn muốn miễn giảm {0} vđn cho toàn bộ các dịch vụ đang được chọn?", Convert.ToDecimal(tile).ToString());
                        if (!Utility.AcceptQuestion(ask, "Xác nhận miễn giảm cho các dịch vụ đang chọn", true)) return;
                        foreach (DataRow dr in m_dtChiPhiThanhtoan.Rows)
                            {
                                if (Utility.sDbnull(dr["trangthai_thanhtoan"], "0") == "0" && Utility.sDbnull(dr["colChon"], "0") == "1")
                ...
                else//Không chọn khuyến mãi nữa thì reset lại các giá trị đã khuyến mãi trước đó
                {
                    if (m_dtChiPhiThanhtoan != null && m_dtChiPhiThanhtoan.Columns.Count > 0)
                    {
                        foreach (DataRow dr in m_dtChiPhiThanhtoan.Rows)
                        {
                            if (Utility.sDbnull(dr["trangthai_thanhtoan"], "0") == "0" && Utility.sDbnull(dr["colChon"], "0") == "1" && Utility.Int32Dbnull(dr["tthai_khuyenmai"], 0) == 1)
                            {
                                dr["tthai_khuyenmai"] = 0;
                                dr["tien_chietkhau"] = 0;
                                dr["tile_chietkhau"] = 0;
        ...
        private void LaydanhsachLichsuthanhtoan_phieuchi()
        {
            try
            {
             ...
             DataTable   dtData =
                    _THANHTOAN.LaythongtinCacLanthanhtoan(Utility.sDbnull(txtPatient_Code.Text, ""),
                        Utility.Int32Dbnull(txtPatient_ID.Text, -1), -1, v_bytNoitru, ThanhtoanQuayThuoc,
                        globalVariables.MA_KHOA_THIEN,lst_IDLoaithanhtoan,-1);
                m_dtPayment = dtData.Copy();
                ...
                arrDR = dtData.Select("Kieu_ThanhToan = 5");
                ...
                //Ghép phần tạm ưng, ghi nợ theo yc bệnh viện Hồng phát
                if (THU_VIEN_CHUNG.Laygiatrithamsohethong("THANHTOAN_HIENTHI_TAMUNG_GHINO_THUCHI", "0", true) == "1")
                {
                    DataRow[] arrTU_Noitru = ucTamung1.m_dtTamung.Select(string.Format("noitru={0}", this.v_bytNoitru));
                    foreach (DataRow dr in arrTU_Noitru)
                    {
                        DataRow drNew = dtData.NewRow();
                        drNew["kieu_thanhtoan"] = 20;
                        drNew["id_thanhtoan"] = dr["id"];
                        drNew["ma_thanhtoan"] = dr["code"];
                        drNew["ngay_thanhtoan"] = dr["ngay_tamung"];
                        drNew["tt_bn"] = dr["so_tien"];
                       
                        drNew["THUC_THU"] = dr["so_tien"];
                        drNew["TONGTIEN_GOC"] = dr["so_tien"];
                        drNew["ma_pttt"] = dr["ma_pttt"];
                        drNew["Ten_pttt"] = dr["Ten_pttt"];
                        drNew["ma_nganhang"] = dr["ma_nganhang"];
                        drNew["ten_nganhang"] = dr["ten_nganhang"];
                        drNew["ghichu"] = dr["ghi_chu"];
                        drNew["nguoi_tao"] = dr["nguoi_tao"];
                        drNew["ngay_tao"] = dr["ngay_tao"];
                        drNew["ten_trangthai"] = "Đã nộp tiền";
                        dtData.Rows.Add(drNew);
                    }
                    //Ghi nợ
                    DataTable dt_ghino = SPs.KcbThanhtoanGhinoLaydanhsach(new DateTime(1900, 1, 1), new DateTime(1900, 1, 1), objLuotkham.IdBenhnhan, objLuotkham.MaLuotkham,this.v_bytNoitru).GetDataSet().Tables[0];
                    foreach (DataRow dr in dt_ghino.Rows)
                    {
                        DataRow drNew = dtData.NewRow();
                        drNew["kieu_thanhtoan"] = 10;
                        drNew["id_thanhtoan"] = dr["id"];
                        drNew["ma_thanhtoan"] = dr["ma_ghino"];
                        drNew["ngay_thanhtoan"] = dr["ngay_ghino"];
                        drNew["tt_bn"] = Utility.DecimaltoDbnull(dr["so_tien"]) + Utility.DecimaltoDbnull(dr["tongtien_chietkhau_hoadon"]) + Utility.DecimaltoDbnull(dr["tongtien_chietkhau_chitiet"]);// dr["so_tien"];
                        drNew["tongtien_chietkhau"] = dr["tongtien_chietkhau"];
                        drNew["tongtien_chietkhau_hoadon"] = dr["tongtien_chietkhau_hoadon"];
                        drNew["tongtien_chietkhau_chitiet"] = dr["tongtien_chietkhau_chitiet"];
                        drNew["THUC_THU"] = dr["so_tien"];
                        drNew["TONGTIEN_GOC"] = drNew["tt_bn"];
                        drNew["ghichu"] = dr["lydo_ghino"];
                        drNew["nguoi_tao"] = dr["nguoi_tao"];
                        drNew["ngay_tao"] = dr["ngay_tao"];
                        drNew["ten_trangthai"] = "Ghi nợ";  
                        dtData.Rows.Add(drNew);
                    }
                }
                grdPayment.Visible = dtData.Rows.Count > 0;
                Utility.SetDataSourceForDataGridEx(grdPayment, dtData, false, true, "trang_thai=0 or trang_thai is null", "");
                //Utility.SetDataSourceForDataGridEx(grdPayment, m_dtPayment, false, true, "1=1", "");
                Utility.SetDataSourceForDataGridEx(grdPhieuChi, m_dtPhieuChi, false, true, "trang_thai=0 or trang_thai is null", "");
                ...
                decimal tongtien_tralai = m_dtPhieuChi == null || m_dtPhieuChi.Columns.Count <= 0 ? 0m : Utility.DecimaltoDbnull(m_dtPhieuChi.Compute("SUM(THUC_THU)", "noi_tru=0"), 0);
                txt_tongtien_tralai.Text = "(" + String.Format(Utility.FormatCurrecy(), Math.Abs(tongtien_tralai)) + ")";
                //RefreshMienGiam();
            }
            catch (Exception exception)
            {
        ...
        private void PerformAction()
        {
            try
            {
                ...
                if (objLuotkham != null)
                {
                    ...
                    MaLdoCk = "";
                    Lydo_chietkhau = "";
                    if (!chkChietkhauthem.Checked) chkChietkhauthem.Checked = objCTKM != null && Utility.Bool2Bool(objCTKM.KhuyenmaiTong);//Nếu đang chọn CT khuyến mãi tổng tiền/hóa đơn thì tự check để bung lên thông tin lý do MG
                    if (chkChietkhauthem.Checked || TongtienCkChitiet > 0)
                    {
                        frm_ChietkhauTrenHoadon chietkhauTrenHoadon = new frm_ChietkhauTrenHoadon(objCTKM);
                    ...
                    if (CoDichvu0dong)
                    {
                        if (!Utility.AcceptQuestion("Trong các hạng mục bạn chọn thanh toán có hạng mục giá 0 đồng. Bạn có chắc chắn muốn tiếp tục thực hiện thanh toán?\nNhấn Yes để tiếp tục thanh toán.\nNhấn No để dừng thanh toán và kiểm tra lại", "Cảnh báo hạng mục thanh toán có giá <=0", true))
                            return;
                    }
                    ...
                    v_objThanhtoan.ThanhtoanGhino = Utility.Bool2byte(lstGhino.Count > 0);
                    bool TamUngHoanUng = true;
                    List<string> lstThuocVT = new List<string>() { "3", "5" };
                    List<string> lstIdLoaiThanhtoan = grdThongTinChuaThanhToan.GetCheckedRows().Where(c => !lstThuocVT.Contains(Utility.sDbnull(c.Cells["id_loaithanhtoan"].Value))).Select(c => Utility.sDbnull(c.Cells["id_loaithanhtoan"].Value)).Distinct().ToList<string>();
                    if (lstIdLoaiThanhtoan.Count <= 0)//Nếu chỉ thanh toán VTTH+Thuốc thì không thực hiện tính tiền tạm ứng vào chi phí thanh toán và không thực hiện tự hoàn ứng theo các thông số khác
                    {
                        TamUngHoanUng = false;
                        chk_Botamung.Checked = true;
                        //chk_Botamung.Font.Strikeout = true;
                    ...
                    ActionResult actionResult = _THANHTOAN.ThanhtoanChiphiDvuKcb(v_objThanhtoan, objLuotkham,lstIdTamthu,
                        ...
                        ref ttbnChitrathucsu, ref ErrMsg, TamUngHoanUng);
                    IN_HOADON = ttbnChitrathucsu > 0;
                    switch (actionResult)
                    {
                        case ActionResult.Success:
                            Utility.Log(this.Name, globalVariables.UserName, string.Format("Thanh toán tiền cho bệnh nhân ID={0}, PID={1}, Tên={2}, sô tiền={3} thành công ", objLuotkham.IdBenhnhan.ToString(), objLuotkham.MaLuotkham, objBenhnhan.TenBenhnhan, v_objThanhtoan.TongTien.ToString()), newaction.Insert, this.GetType().Assembly.ManifestModule.Name);
                            //Sinh số QMS cho phòng khám nếu luồng của BV mắt
                            ...
                            {
                                var lstck = from p in lstItems where p.IdLoaithanhtoan == 1 select p;
                                if (lstck.Any())//Nếu có thanh toán công khám-->Tìm công khám đo thị lực để sinh số QMS
                                {
                                    ...
                                    //Chưa khám thị lực mới sinh số QMS cho phòng đo thị lực
                                    if (objCongkham != null && objCongkham.TrangThai!=1 && THU_VIEN_CHUNG.Laygiatrithamsohethong("QMSPK_ENABLE", "0", false) == "1")
                                    {
                                        ...
                                        new Update(KcbDangkyKcb.Schema).Set(KcbDangkyKcb.Columns.SttKham).EqualTo(STT_QMS).Where(KcbDangkyKcb.Columns.IdKham).IsEqualTo(objCongkham.IdKham).Execute();
                ...
                //Check nếu chọn Pttt=phân bổ và số dòng phân bổ <1 thì tự động hiển thị form phân bổ
                            ...
                            if (globalVariables.MaphieuHoanung != "" && THU_VIEN_CHUNG.Laygiatrithamsohethong("KCB_THANHTOAN_INPHIEUHOANUNG_SAUKHITHANHTOAN", "1", false) == "1")
                                Inphieuhoanung();
        ...
        bool CoDichvu0dong = false;
        
        /// <summary>
        ///     hàm thực hiện mảng của chi tiết thanh toán chi tiết
        ///     Kịch bản gây lỗi tiền chiết khấu
        ///     1. Chỉ định số lượng là 3
///2. Thu ngân chọn ca bệnh và chiết khấu 20% nên tạo ra tiền chk 12750000(Hệ thống update thẳng vào CSDL)
///3. Chưa kịp thanh toán thì chắc người bệnh thay đổi ý định nên bảo em làm 1 lần thôi bên báo cho ông bác sỹ sửa về 1. Nhưng chỗ sửa số lượng phần mềm ko update lại đc cái tiền ck nên nó giữ nguyên cái tiền ck trên bước 2
///4. TNV chọn lại ca bệnh thấy số lượng 1 mà tiền CK vẫn y xì nhưng ko để ý và thực hiện thanh toán luôn
            ...
            try
            {
                ...
                foreach (GridEXRow row in grdThongTinChuaThanhToan.GetCheckedRows())
                {
                   ...
                   KcbThanhtoanChitiet newItem = new KcbThanhtoanChitiet();
                    ...
                    if (objLuotkham.PtramBhyt != null) newItem.PtramBhyt = Utility.DecimaltoDbnull(objLuotkham.MucHuongBhyt, objLuotkham.PtramBhyt.Value);
                    if (objLuotkham.PtramBhytGoc != null) newItem.PtramBhytGoc = objLuotkham.PtramBhyt.Value;
                    ...
                    newItem.BhytGiaTyle = Utility.DecimaltoDbnull(row.Cells[KcbThanhtoanChitiet.Columns.BhytGiaTyle].Value, 0);
                    newItem.BnCct = Utility.DecimaltoDbnull(row.Cells[KcbThanhtoanChitiet.Columns.BnCct].Value, 0);
                    newItem.BnTtt = Utility.DecimaltoDbnull(row.Cells[KcbThanhtoanChitiet.Columns.BnTtt].Value, 0);
                    newItem.BhytNguonKhac = Utility.ByteDbnull(row.Cells[KcbThanhtoanChitiet.Columns.BhytNguonKhac].Value, 0);
                    newItem.TthaiKhuyenmai = Utility.ByteDbnull(row.Cells["tthai_khuyenmai"].Value, 0);
                    newItem.TienKhuyenmai = Math.Round(Utility.DecimaltoDbnull(row.Cells[KcbThanhtoanChitiet.Columns.TienChietkhau].Value, 0m), 3);//Tiền khuyến mãi thường bằng tiền chiết khấu
                    newItem.NgoaiGio = Utility.Byte2Bool(row.Cells["ngoai_gio"].Value);
                    newItem.DonGia = Utility.DecimaltoDbnull(row.Cells["don_gia_thanhtien"].Value, 0);
                    newItem.GiaGoc = Utility.DecimaltoDbnull(row.Cells[KcbThanhtoanChitiet.Columns.DonGia].Value, 0);
                    ...
                    newItem.IdChitietdichvu = newItem.IdLoaithanhtoan ==3 || newItem.IdLoaithanhtoan==5? Utility.Int16Dbnull(row.Cells["id_thuockho"].Value, -1): Utility.Int16Dbnull(row.Cells["Id_Chitietdichvu"].Value, -1);
                    ...
                    newItem.IdGoi= Utility.Int32Dbnull(row.Cells["id_goi"].Value, 0);
                    newItem.IdDangky = Utility.Int32Dbnull(row.Cells["id_dangky"].Value, 0);
                    newItem.TrongGoi = Utility.ByteDbnull(row.Cells["trong_goi"].Value, 0);
                    newItem.TongTralai = 0;
                    newItem.MultiCancel = 0;
                    newItem.IdThe = newItem.IdLichsuDoituongKcb;
                    ...
                    newItem.HienThi= Utility.ByteDbnull(row.Cells["hien_thi"].Value, 0);
                    ...
                    {//Kiểm tra trạng thái thanh toán tránh việc thanh toán 2 lần(2 user cùng chọn và sau đó từng người bấm nút thanh toán)
        ...
        {
            KcbThanhtoan objThanhtoan = new KcbThanhtoan();
            //Lấy maxdate của các dịch vụ để đưa vào bảng thanh toán phục vụ mục đích chặn sửa ngày thanh toán < ngày tạo dịch vụ
           
            objThanhtoan.IdThanhtoan = -1;
            objThanhtoan.MaLuotkham = Utility.sDbnull(txtPatient_Code.Text, "");
            objThanhtoan.IdBenhnhan = Utility.Int32Dbnull(txtPatient_ID.Text, -1);
            objThanhtoan.NgayThanhtoan = dtPaymentDate.Value;
            objThanhtoan.IdNhanvienThanhtoan = globalVariables.gv_intIDNhanvien;
            objThanhtoan.KieuThanhtoan = 0;//0=Thanh toán thường;1= trả lại tiền;2= thanh toán bỏ viện
            objThanhtoan.MaKhoaThuchien = globalVariables.MA_KHOA_THIEN;
            objThanhtoan.NoiTru = v_bytNoitru;
            objThanhtoan.TrangthaiIn = 0;
            objThanhtoan.NgayIn = null;
            objThanhtoan.TtoanThuoc =Utility.Byte2Bool( ThanhtoanQuayThuoc);//0= thanh toán các loại dịch vụ;1= thanh toán đơn thuốc tại quầy
            objThanhtoan.NguoiIn = string.Empty;
            objThanhtoan.MaPttt = cboPttt.SelectedValue.ToString();
            objThanhtoan.MaNganhang = cboNganhang.Enabled ?  Utility.sDbnull( cboNganhang.SelectedValue,"")  : "-1";
            objThanhtoan.NgayTonghop = null;
            objThanhtoan.NguoiTonghop = string.Empty;
            objThanhtoan.NgayChot = null;
            objThanhtoan.TrangthaiChot = 0;
            objThanhtoan.TongTien = Utility.DecimaltoDbnull(txtSoTienCanNop.Text, 0);
            objThanhtoan.Ghichu = Utility.DoTrim(txtGhichu.Text);
            //2 mục này được tính lại ở Business
            objThanhtoan.BnhanChitra = -1;
            objThanhtoan.BhytChitra = -1;
            objThanhtoan.PtramBhyt =Utility.Int16Dbnull( objLuotkham.PtramBhyt);
            objThanhtoan.MucHuongBhyt = objLuotkham.MucHuongBhyt;
           
            objThanhtoan.TileChietkhau = TileChietkhau;
            objThanhtoan.KieuChietkhau = "T";
            objThanhtoan.TongtienChietkhau = TongtienCk;
            objThanhtoan.TongtienChietkhauChitiet = TongtienCkChitiet;
            objThanhtoan.TongtienChietkhauHoadon = TongtienCkHoadon;
            if (chkLayHoadon.Checked &&THU_VIEN_CHUNG.Laygiatrithamsohethong("KCB_THANHTOAN_SUDUNGHOADONDO", "0", false)=="1")
            {
                objThanhtoan.MauHoadon =Utility.DoTrim( txtMauHD.Text);
                objThanhtoan.KiHieu =Utility.DoTrim( txtKiHieu.Text);
                objThanhtoan.IdCapphat = Utility.Int32Dbnull(grdHoaDonCapPhat.GetValue(HoadonCapphat.Columns.IdCapphat), -1);
                objThanhtoan.MaQuyen = Utility.DoTrim(txtMaQuyen.Text);
                objThanhtoan.Serie = Utility.DoTrim(txtSerie.Text);
            }
            objThanhtoan.MaVoucher=Utility.sDbnull(txtMaVoucher.Text,"");
            objThanhtoan.IdCtrinhKhuyenmai=Utility.Int32Dbnull(cboChuongtrinhKM.SelectedValue,-1);
            objThanhtoan.TienChietkhauVoucher = Utility.DecimaltoDbnull(txtTienKhuyenmai.Text, 0);
            objThanhtoan.MaLydoChietkhau = MaLdoCk;
            objThanhtoan.LydoChietkhau = Lydo_chietkhau;
            objThanhtoan.NgayTao = globalVariables.SysDate;
            objThanhtoan.NguoiTao = globalVariables.UserName;
            objThanhtoan.IpMaytao = globalVariables.gv_strIPAddress;
            objThanhtoan.TenMaytao = globalVariables.gv_strComputerName;
            objThanhtoan.ThuaThieu = tongtien_thuathieu;
            return objThanhtoan;
        }
        ...
        private bool IsValidata()
        {
            ...
            if (THU_VIEN_CHUNG.Laygiatrithamsohethong("THANHTOAN_CHOPHEP_0_DONG", true) == "0")
            {
                ...
                {
                    Utility.ShowMsg("Hệ thống chỉ cho phép thanh toán khi số tiền thu phải > 0. Liên hệ IT bệnh viện chỉnh tham số THANHTOAN_CHOPHEP_0_DONG để có thể thanh toán với tiền thu=0", "Thông báo", MessageBoxIcon.Warning);
                    return false;
                }
            }
            ...
            if (THU_VIEN_CHUNG.IsBaoHiem(objLuotkham.IdLoaidoituongKcb) && THU_VIEN_CHUNG.Laygiatrithamsohethong("BHYT_THANHTOAN_NHIEULAN", "0", false) == "0")
            {
                KcbThanhtoan objthanhtoan = new Select().From(KcbThanhtoan.Schema)
                    ...
                    .And(KcbThanhtoan.Columns.KieuThanhtoan).IsEqualTo(0)
                    .And(KcbThanhtoan.Columns.BhytChitra).IsGreaterThan(0)
        ...
        private void grdPayment_ColumnButtonClick(object sender, ColumnActionEventArgs e)
        {
            try
            {
                ...
                else if (e.Column.Key == "cmdHUY_PHIEUTHU")
                {
                    if (kieu_thanhtoan == "1")
                        HuyPhieuchi(grdPayment);
                    else if (kieu_thanhtoan == "10")//Ghi nợ
                    {
                        mnu_Lichsu_Congno.PerformClick();
                    }
                    else if (kieu_thanhtoan == "20")//Tạm ứng
                    {
                        tabThongTinCanThanhToan.SelectedTab = uiTabPageTamung;
                    }
                    else
                    {
                        ...
                        if (objThanhtoan.KieuThanhtoan == 0)
                            HuyThanhtoan(objThanhtoan);
                        else
                            HuyTamthu(objThanhtoan);
                    }
                }
                else if (e.Column.Key == "XUATHOADON")
                {
                    if (kieu_thanhtoan == "0")
                    {
                        if (!Utility.Coquyen("HOADONDIENTU_UPLOAD"))
                        {
                            Utility.thongbaokhongcoquyen("HOADONDIENTU_UPLOAD", " tải về hóa đơn điện tử");
                            return;
                        }
                        string paymentId = Utility.sDbnull(grdPayment.GetValue("id_thanhtoan"));
                        string eMessage = "";
                        bool kt = _MisaInvoices.phathanh_hoadon(paymentId, 0, "-1", ref eMessage);
                        if (kt)
                        {
                    ...
                    {
                        Utility.ShowMsg("Chỉ cho xuất HĐĐT với chứng từ thu. Vui lòng kiểm tra lại chứng từ đang chọn");
                    }    
                }
                else if (e.Column.Key == "TAIHOADON")
                {
                    if (kieu_thanhtoan == "0")
                    {
                        try
                        {
                            ...
                            KcbThanhtoan objThanhtoan = KcbThanhtoan.FetchByID(Utility.Int64Dbnull(grdPayment.GetValue("id_thanhtoan")));
                            if (objThanhtoan != null)
                            {
                                string TransactionID = Utility.sDbnull(objThanhtoan.TransactionId, "");
                                if (TransactionID.Length <= 0 || TransactionID == "")
                                {
                                    Utility.ShowMsg("Phiếu thanh toán chưa được phát hành Hóa đơn điện tử nên không thể tải về.\nCần xuất Hóa đơn điện tử trước khi thực hiện tính năng tải hóa đơn");
                                    return;
                                }
                                string eMessge = "";
                                if (Utility.Bool2Bool(objThanhtoan.TthaiXuatHddt) && TransactionID.Length > 0)
                                {
                                    ...
                                    Utility.Log(this.Name, globalVariables.UserName, string.Format("Tải thành công hóa đơn transaction_id: {0}, id_thanhtoan: {1}", TransactionID, objThanhtoan.IdThanhtoan), newaction.Download, "UI");
                                }
                                else
                                {
                                    Utility.ShowMsg("Phiếu thu bạn chọn chưa phát hành hóa đơn hoặc đã bị hủy hóa đơn nên không thể tải về. Vui lòng kiểm tra lại");
                                    return;
                                }
                            }
                        }
                    ...
                    {
                        Utility.ShowMsg("Chỉ cho Tải HĐĐT với chứng từ thu. Vui lòng kiểm tra lại chứng từ đang chọn");
                    }
                }
        ...
        //HoaDonDienTu _hoaDonDienTu = new HoaDonDienTu();
        //private void XUAT_HD()
        //{
        //    try
        //    {
        //        decimal tongBntt = 0; //grdPayment.GetDataRows().Sum(); 
        //        string paymentid = "";
        //        tongBntt = tongBntt + Utility.DecimaltoDbnull(grdPayment.CurrentRow.Cells["SO_TIEN_THANHTOAN"].Value, 0);
        //        paymentid = Utility.sDbnull(grdPayment.CurrentRow.Cells["Payment_Id"].Value, 0);
        //        bool daGui = Utility.sDbnull(grdPayment.CurrentRow.Cells[LHoadonLog.Columns.DaGui].Value, 0) == "0";
        //        string returnMessage = "";
        //        if (tongBntt > 0 && daGui)
        //        {
        //            log.Trace("--Begin: Gui hoa don dien tu cho lan kham: " + objPatientExam.PatientCode + " tong tien = " + tongBntt);
        //            bool thanhcong = false;
        //            thanhcong = BusinessHelper.Laygiatrithamsohethong("INVOICE_HOADON_SIGN", "0", false) == "0"
        //                        ? _hoaDonDienTu.CreateInvoiceUnSign(
        //                            Utility.sDbnull(paymentid), "", 1, ref returnMessage)
        //                        : _hoaDonDienTu.CreateInvoice(
        //                            Utility.sDbnull(paymentid), "", 1, ref returnMessage);
        //            if (!thanhcong)
        //            {
        //                log.Trace("--End: gui hoa don cho ma lan kham: " + objPatientExam.PatientCode + " cho lan lan toan " + paymentid + ": Error " + returnMessage);
        //            }
        //            else
        //            {
        //                log.Trace("--End: gui hoa don cho ma lan kham: " + objPatientExam.PatientCode + " cho lan lan toan " + paymentid + ": Susscess " + returnMessage);
        //            }
        //            LayThongtinThanhToanNgoaiTru();
        //            //if (Utility.AcceptQuestion("Bạn có muốn in hóa đơn luôn không?", "Thông báo", true))
        //            //{
        //            //    cmddownloadinvoices.PerformClick();
        //            //}
        //        }
        //        else
        //        {
        //            Utility.ShowMsg("Hóa đơn đã được gửi hoặc có tổng tiền <=0", "Cảnh báo", MessageBoxIcon.Warning);
        //            log.Trace("--End: gui hoa don cho ma lan kham: " + objPatientExam.PatientCode + " cho lan lan toan " + paymentid + " Tong tien chi tra <= 0 ");
        //        }
        //        log.Trace("----------------------------------------");
        //    }
        //    catch (Exception ex)
        ...
        //        paymentid = Utility.sDbnull(grdPayment.CurrentRow.Cells["Payment_Id"].Value, 0);
        //        bool thanhcong = false;
        //        DataSet dshoadonlog = SPs.SpGetinvoiceLog(paymentid, 1).GetDataSet();
        //        foreach (DataRow row in dshoadonlog.Tables[1].AsEnumerable())
        //        {
        //            if (Utility.sDbnull(row[LHoadonLog.Columns.DaGui], 0) == "1" && Utility.sDbnull(row[LHoadonLog.Columns.InvInvoiceAuthId], "") != "")
        //            {
        //                bool kt = _hoaDonDienTu.SaveFileInvoice("", Utility.sDbnull(row[LHoadonLog.Columns.InvInvoiceAuthId], ""), false, ref eMessge);
        //                if (kt)
        //                {
        //                    StoredProcedure sp =
        ...
        //                else
        //                {
        //                    Utility.ShowMsg(eMessge, "Cảnh báo", MessageBoxIcon.Warning);
        //                }
        //            }
        //            else
        //            {
        //                Utility.ShowMsg("Hóa đơn chưa được gửi hóa đơn", "Cảnh báo", MessageBoxIcon.Warning);
        //            }
        //        }
        //    }
        //    catch (Exception ex)
        ...
        string ma_lydohuy = "";
        string lydo_huy = "";
        private void HuyThanhtoan(KcbThanhtoan objThanhtoan)
        {
            try
            {
                ma_lydohuy = "";
                    ...
                    if (objThanhtoan != null)
                    {
                        ...
                        int kcbThanhtoanSongayHuythanhtoan =
                            Utility.Int32Dbnull(
                                THU_VIEN_CHUNG.Laygiatrithamsohethong("THANHTOAN_SONGAY_HUYTHANHTOAN", "0", true), 0);
                        var chenhlech =
                            (int) Math.Ceiling((globalVariables.SysDate.Date - objThanhtoan.NgayThanhtoan.Date).TotalDays);
                        if (chenhlech > kcbThanhtoanSongayHuythanhtoan)
                        {
                            Utility.ShowMsg(
                               string.Format( "Ngày thanh toán: {0}. Hệ thống không cho phép bạn hủy thanh toán khi đã quá {1} ngày. Cần liên hệ quản trị hệ thống để được trợ giúp",objThanhtoan.NgayThanhtoan.Date.ToString("dd/MM/yyyy"),kcbThanhtoanSongayHuythanhtoan.ToString()));
                            return;
                        }
                        if (Utility.Byte2Bool(objThanhtoan.TrangthaiChot))
                        {
                            Utility.ShowMsg(
                        ...
                        else
                        {
                                ...
                                if (
                                    !Utility.AcceptQuestion(
                                        string.Format("Bạn có muốn hủy lần thanh toán với Mã thanh toán {0}",
                                            objThanhtoan.IdThanhtoan), "Thông báo", true))
                                {
                                    return;
                                }
                            if (
                            ...
                            ActionResult actionResult = _THANHTOAN.HuyThanhtoan(objThanhtoan, objLuotkham, lydo_huy,
        ...
        private void HuyTamthu(KcbThanhtoan objThanhtoan)
        {
            try
            {
                ma_lydohuy = "";
                ...
                if (grdPayment.CurrentRow != null)
                {
                    ...
                    if (objThanhtoan != null)
                    {
                        ...
                        {
                            Utility.ShowMsg(
                               string.Format("Ngày tạm thu: {0}. Hệ thống không cho phép bạn hủy tạm thu khi đã quá {1} ngày. Cần liên hệ quản trị hệ thống để được trợ giúp", objThanhtoan.NgayThanhtoan.Date.ToString("dd/MM/yyyy"), kcbThanhtoanSongayHuythanhtoan.ToString()));
                            return;
                        }
                        if (Utility.Byte2Bool(objThanhtoan.TrangthaiChot))
                        {
                        ...
                        else
                        {
                            ...
                            bool huythanhtoanHuybienlai =
                                THU_VIEN_CHUNG.Laygiatrithamsohethong("THANHTOAN_HUYTHANHTOAN_HUYBIENLAI", "1", true) == "1";
                            ActionResult actionResult = _THANHTOAN.HuyTamthu(objThanhtoan, objLuotkham, lydo_huy,
                                idHdonLog, huythanhtoanHuybienlai);
                            switch (actionResult)
                            {
                                case ActionResult.Success:
        ...
        private void cmdInphoiBHYT_Click(object sender, EventArgs e)
        {
            ...
            objLuotkham = Utility.getKcbLuotkham(objLuotkham);//Lấy lại thông tin mới nhất
            ...
            if (objLuotkham.MaDoituongKcbBhyt == "2")
            {
                ...
                if (sqlQuery.GetRecordCount() <= 0)
                {
                    Utility.ShowMsg("Bệnh nhân phải ra viện mới được phép in phôi BHYT\r\n Mời bạn xem lại", "Thông báo", MessageBoxIcon.Warning);
                    return ;
                }
            }
        ...
        private void CapNhatNgayThanhToan(KcbThanhtoan objThanhtoan,DateTime newDate)
        {
            ...
            if (objThanhtoan != null)
            {
                ...
                //{
                //    if (objThanhtoan.MaxNgayTao.HasValue && newDate.Date < objThanhtoan.MaxNgayTao.Value.Date)
                //    {
                //        Utility.ShowMsg(string.Format("Bạn cần chọn ngày thanh toán cần >= {0}", objThanhtoan.MaxNgayTao.Value.ToString("dd/MM/yyyy")), "Thông báo");
                //        return;
                //    }
                //}
                string noi_dung = string.Format("Đổi ngày thanh toán của hóa đơn Id thanh toán ={0}, id bệnh nhân={1}, Mã lượt khám ={2} từ {3} thành {4} thành công", objThanhtoan.IdThanhtoan, objThanhtoan.IdBenhnhan, objThanhtoan.MaLuotkham, objThanhtoan.NgayThanhtoan, newDate);
                objThanhtoan.NgayThanhtoan = newDate;
                ActionResult actionResult = _THANHTOAN.UpdateNgayThanhtoan(objThanhtoan, noi_dung);
                switch (actionResult)
                {
                    case ActionResult.Success:
                        ...
                        break;
                    case ActionResult.Error:
                        Utility.ShowMsg("Lỗi trong quá trình đổi ngày thanh toán", "Thông báo lỗi", MessageBoxIcon.Error);
                        break;
                }
            }
        ...
        private void UpdatePhieuChiPaymentDate()
        {
                    ...
                    "Thông báo", true))
            {
                ...
                if (objThanhtoan != null)
                {
                    ...
                    string noi_dung = string.Format("Đổi ngày Phiếu chi Id phiếu ={0}, id bệnh nhân={1}, Mã lượt khám ={2} từ {3} thành {4} thành công", objThanhtoan.IdThanhtoan, objThanhtoan.IdBenhnhan, objThanhtoan.MaLuotkham, objThanhtoan.NgayThanhtoan, newDate);
                    objThanhtoan.NgayThanhtoan = newDate;
                    ActionResult actionResult = _THANHTOAN.UpdateNgayThanhtoan(objThanhtoan,noi_dung);
                    switch (actionResult)
                    {
                        case ActionResult.Success:
                            Utility.ShowMsg("Bạn chỉnh sửa ngày Phiếu chi thành công", "Thông báo");
                            break;
                        case ActionResult.Error:
                            Utility.ShowMsg("Lỗi trong quá trình sửa ngày Phiếu chi", "Thông báo lỗi", MessageBoxIcon.Error);
                            break;
                    }
                }
            }
        ...
        private void HuyPhieuchi(GridEX grd)
        {
            ...
            if (grd.CurrentRow != null)
            {
                ...
                if (objThanhtoan != null)
                {
                    ...
                    int Chenhlech = (int)Math.Ceiling((globalVariables.SysDate.Date - objThanhtoan.NgayThanhtoan.Date).TotalDays);
                    if (Chenhlech > KCB_THANHTOAN_SONGAY_HUYPHIEUCHI)
                    {
                        Utility.ShowMsg(string.Format("Ngày lập phiếu chi {0} \n- Ngày hủy phiếu chi {1}.\nHệ thống không cho phép bạn hủy phiếu chi đã quá {2} ngày.\nCần liên hệ quản trị hệ thống để được trợ giúp", objThanhtoan.NgayThanhtoan.ToString("dd/MM/yyyy"), globalVariables.SysDate.ToString("dd/MM/yyyy"), KCB_THANHTOAN_SONGAY_HUYPHIEUCHI.ToString()));
                        return;
                    }
                    if (PropertyLib._ThanhtoanProperties.Hienthihuyphieuchi)
                    {
                        ...
                        frm.objLuotkham = objLuotkham;
                        frm.v_Payment_Id = Utility.Int32Dbnull(objThanhtoan.IdThanhtoan, -1);
                        frm.objKcbThanhtoan = objThanhtoan;
                        frm.Chuathanhtoan = _chuathanhtoan;
                    ...
                    else
                    {
                        ...
                        if (Utility.Bool2Bool(objThanhtoan.TtoanThuoc) && objThanhtoan.KieuThanhtoan==1)
                            actionResult = _THANHTOAN.HuyPhieuchiTrathuoctaiquay_Rieng(objThanhtoan, objLuotkham, lydo_huy);
                        else
                            actionResult = _THANHTOAN.HuyPhieuchi(objThanhtoan, objLuotkham, lydo_huy);
                        switch (actionResult)
                        {
                            case ActionResult.Success:
        ...
        private void grdThongTinChuaThanhToan_UpdatingCell(object sender, UpdatingCellEventArgs e)
        {
            ...
            {
                long id_dangky = Utility.Int64Dbnull(grdThongTinChuaThanhToan.GetValue("id_dangky"));
                if (id_dangky > 0)
                {
                    ...
                    if (Utility.AcceptQuestion(string.Format("Bạn có chắc chắn muốn đổi giá dịch vụ cls {0} từ {1} thành {2} hay không?", grdThongTinChuaThanhToan.GetValue("ten_chitietdichvu"), Utility.FormatCurrencyHIS(Utility.DecimaltoDbnull(e.InitialValue, 0)), Utility.FormatCurrencyHIS(Utility.DecimaltoDbnull(e.Value, 0))), "Xác nhận đổi giá", true))
                    {
                        if (CapnhatDongia(grdThongTinChuaThanhToan.CurrentRow, 0, dongia_moi, true))
                            Utility.Log(this.Name, globalVariables.UserName, string.Format("Form Thanh Toán --> Sửa đơn giá của dịch vụ {0} từ {1} thành {2} thành công ", grdThongTinChuaThanhToan.GetValue("ten_chitietdichvu"), Utility.FormatCurrencyHIS(Utility.DecimaltoDbnull(e.InitialValue, 0)), Utility.FormatCurrencyHIS(Utility.DecimaltoDbnull(e.Value, 0))), newaction.Update, this.GetType().Assembly.ManifestModule.Name);
                        else
                        {
                            e.Value = e.InitialValue;
                            return;
                ...
                {
                    Utility.ShowMsg("Hệ thống chỉ đang cho phép sửa Đơn giá của các dịch vụ trong Gói dịch vụ. Không cho phép sửa giá các dịch vụ ngoài gói");
                    e.Value = e.InitialValue;
                    return;
                }    
            }
            ...
            {
                //Chỉ cho phép sửa số lượng với thuốc và VTTH
                byte id_loaithanhtoan = Utility.ByteDbnull(grdThongTinChuaThanhToan.CurrentRow.Cells["id_loaithanhtoan"].Value);
                if(id_loaithanhtoan ==3 || id_loaithanhtoan==5)
                {

                }   
                else
                {
                    Utility.ShowMsg(string.Format("Chỉ cho phép sửa số lượng với Thuốc hoặc VTTH. Vui lòng kiểm tra lại"));
                    e.Value = e.InitialValue;
                    return;
                }    
                //Check các trạng thái
                long _IdChitietdonthuoc = Utility.Int64Dbnull(grdThongTinChuaThanhToan.GetValue("id_phieu_chitiet"));
                string ten_thuoc = grdThongTinChuaThanhToan.GetValue("ten_chitietdichvu").ToString();
                string ten_dvt = grdThongTinChuaThanhToan.GetValue("ten_donvitinh").ToString();
                KcbDonthuocChitiet objChitiet = KcbDonthuocChitiet.FetchByID(_IdChitietdonthuoc);
                decimal sl_ke = Utility.DecimaltoDbnull(grdThongTinChuaThanhToan.GetValue(KcbDonthuocChitiet.Columns.SoLuong));
                //if (so_luong<=)
                //{
                ...
                if (sluong_ton >= sluongdieuchinh && sluong_ton > sl_ke)
                {
                    ...
                    if (record > 0)
                    {
                        ...
                        //                    (Utility.DecimaltoDbnull(grdThongTinChuaThanhToan.GetValue("don_gia")) +
                        //                     Utility.DecimaltoDbnull(grdThongTinChuaThanhToan.GetValue("phu_thu")));
                        ...
                        ////grdPresDetail.CurrentRow.BeginEdit();
                        //REM từ mục ABCD lấy lại thông tin đơn thuốc cho chuẩn
                        grdList_DoubleClick(grdList, new EventArgs());
                    }
        ...
        private void mnuHuyHoaDon_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (IdHdonLog > 0)
                {
                    ...
                    {
                        HuyThanhtoan(KcbThanhtoan.FetchByID(v_Payment_Id));
        ...
        DataTable dtPhanboQR = new DataTable();
        bool hasCK = false;
        bool isActiveQRPay = false;
        List<string> lstAllowQR = new List<string>();
        void ModifyContextMenu()
        {
            try
            {
                ...
                long id_thanhtoan = Utility.Int64Dbnull(grdPayment.GetValue(KcbThanhtoan.Columns.IdThanhtoan), 0);
                long IdQrcode = Utility.Int64Dbnull(grdPayment.GetValue(QrPhieuThanhtoan.Columns.IdQrCode), 0);
                lstAllowQR = THU_VIEN_CHUNG.Laygiatrithamsohethong("THANHTOAN_PTTT_QR", true).Split(',').ToList<string>();
                isActiveQRPay = THU_VIEN_CHUNG.Laygiatrithamsohethong("QR_ACTIVE", "0", true) == "1";

                dtPhanboQR = Utility.ExecuteSql(string.Format("select * from kcb_thanhtoan_phanbotheoPTTT where id_thanhtoan={0}", id_thanhtoan), CommandType.Text).Tables[0];
                hasCK = (from p in dtPhanboQR.AsEnumerable() where lstAllowQR.Contains(Utility.sDbnull(p[KcbThanhtoanPhanbotheoPTTT.Columns.MaPttt])) select p).Any();
        ...
        private void InPhoiBhyt()
        {
            try
            {
                if (new BHYT_InPhieu().InPhoiBHYT(objLuotkham,  pnlSuangayinphoi.Visible ? dtNgayInPhoi.Value : dtPaymentDate.Value, "BHYT_InPhoi_01"))
                {
                    cbomayinphoiBHYT.Text = PropertyLib._MayInProperties.TenMayInBienlai;
                    //LaydanhsachLichsuthanhtoan_phieuchi();
                    KiemTraDaInPhoiBhyt();
        ...
        void HuychuyenCLS()
        {
            ...
            try
            {
                if (!Utility.AcceptQuestion(string.Format("Bạn có chắc chắn muốn HỦY chuyển Cận lâm sàng cho các dịch vụ Xét nghiệm, CĐHA và thăm dò chức năng của người bệnh {0} hay không?\r\nNhấn Yes đồng nghĩa với việc các dịch vụ ngoại trú được Hủy chuyển cận lâm sàng sẽ cần thanh toán trước khi được thực hiện tại các khoa XN, CĐHA và TDCN.\r\nNhấn No để hủy thao tác", txtPatientName.Text), "Xác nhận HỦY chuyển Cận lâm sàng", true)) return;
        ...
        private void cmdChuyenDT_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (frm.m_blnSuccess)
                {
                    RefreshData(frm.objLuotkhamMoi, frm.txtTenKcbbd.Text);
                    UpdateGroup();
                    GetData();
        ...
        private void cmdInhoadon_Click(object sender, EventArgs e)
        {
            ...
            {
                new INPHIEU_THANHTOAN_NGOAITRU(this.log).InPhieuchi(Utility.Int32Dbnull(grdPayment.CurrentRow.Cells[KcbThanhtoan.Columns.IdThanhtoan].Value, -1));
            }
        ...
        private void cmdTraLaiTien_Click(object sender, EventArgs e)
        {
            try
            {
                    ...
                    "Thông báo", true))
                {
                    ...
                    if (!chotratienngoaitrukhidangnoitru)
                    {
                        if (objLuotkham.TrangthaiNoitru >= Utility.Int32Dbnull(THU_VIEN_CHUNG.Laygiatrithamsohethong("THANHTOAN_CHAN_THANHTOANNGOAITRU", "2", false), 2))
                        {
                    ...
                    KcbThanhtoan objThanhtoan = TaophieuThanhtoanHuy();
                    ...
                    ActionResult actionResult = _THANHTOAN.Tratien(objThanhtoan,
                        objLuotkham,
                    ...
                    switch (actionResult)
                    {
                        case ActionResult.Success:
                            ...
                            Utility.GotoNewRowJanus(grdPhieuChi, KcbThanhtoan.Columns.IdThanhtoan,
                                    Utility.sDbnull(objThanhtoan.IdThanhtoan));
                            if (PropertyLib._MayInProperties.TudonginPhieuchiSaukhitratien)
                            {
                                new INPHIEU_THANHTOAN_NGOAITRU().InPhieuchi(objThanhtoan.IdThanhtoan);
                            }
        ...
        private KcbThanhtoan TaophieuThanhtoanHuy()
        {
            ...
            objThanhtoan.NgayTao = globalVariables.SysDate;
            objThanhtoan.NguoiTao = globalVariables.UserName;
            objThanhtoan.KieuThanhtoan = 1; //0=Thanh toán thường;1= trả lại tiền;2= thanh toán bỏ viện
            objThanhtoan.MaKhoaThuchien = globalVariables.MA_KHOA_THIEN;
            objThanhtoan.NoiTru = v_bytNoitru;
            objThanhtoan.TrangthaiIn = 0;
            objThanhtoan.TrangThai = 0;
            objThanhtoan.NgayIn = null;
            objThanhtoan.NguoiIn = string.Empty;
            objThanhtoan.NgayTonghop = null;
            objThanhtoan.NguoiTonghop = string.Empty;
            ...
            objThanhtoan.MaPttt = THU_VIEN_CHUNG.Laygiatrithamsohethong("THANHTOAN_TRALAITIEN_PTTT_MACDINH", "TM", false);
            ...
            try
            {
                ...
                else//Thực hiện hoàn ứng và in phiếu hoàn ứng
                {
                    ...
                    if (Utility.AcceptQuestion(string.Format("Bạn có chắc chắn muốn hoàn ứng cho người bệnh {0}", txtTenBenhNhan.Text), "Thông báo", true))
                    {
                        ...
                        if (objTT != null)
                        {
                            SPs.NoitruHoanung(-1,id_thanhtoan, maphieu, objLuotkham.MaLuotkham, objLuotkham.IdBenhnhan,id_thanhtoan>0? objTT.NgayThanhtoan:DateTime.Now,
                        globalVariables.gv_intIDNhanvien, globalVariables.UserName,
                        Utility.Int32Dbnull(objLuotkham.IdKhoanoitru, -1), Utility.Int64Dbnull(objLuotkham.IdRavien, -1),
                        Utility.Int32Dbnull(objLuotkham.IdBuong, -1), Utility.Int32Dbnull(objLuotkham.IdGiuong, -1),
                        (byte)0, "TM", "").Execute();
        ...
        private void mnuChuyenCLS_Click(object sender, EventArgs e)
        {

            ChuyenCLS();
        }

        private void mnuHuychuyenCLS_Click(object sender, EventArgs e)
        {
            HuychuyenCLS();
        }

        private void chkDoituongnguoinuocngoai_CheckedChanged(object sender, EventArgs e)
        {
            txtptramtanggia_nguoinuocngoai.Enabled = chkDoituongnguoinuocngoai.Checked;
            if (txtptramtanggia_nguoinuocngoai.Enabled)
            {
                txtptramtanggia_nguoinuocngoai.Focus();
                txtptramtanggia_nguoinuocngoai.SelectAll();
            }
        }

        private void mnuCheckError_Click(object sender, EventArgs e)
        {
        ...
        {
            if (grdDSKCB.Height == 116)
                grdDSKCB.Height = 0;
            else
                grdDSKCB.Height = 116;
        }

        private void mnuHuychonTatca_Click(object sender, EventArgs e)
        {
            try
            {
                grdThongTinChuaThanhToan.UnCheckAllRecords();
                 //Thay hàm TinhToanSoTienPhaithu= hàm SetSumTotalProperties để tính lại tiền BHYT chi trả
                SetSumTotalProperties();
                //TinhToanSoTienPhaithu();
                ModifyCommand();
        ...
        frm_Ghino _ghino = null;
       void Tamthu()
        {
            try
            {
                DataRow[] arrDr = m_dtChiPhiThanhtoan.Select("trangthai_huy<>1 and tthai_tamthu<>1 and trangthai_thanhtoan<>1");
                ...
                if (_ghino == null)
                {
                    _ghino = new frm_Ghino(m_dtChiPhiThanhtoan, dtData, this.log, this.v_bytNoitru, this.lst_IDLoaithanhtoan);
                    _ghino._OnChanged += _ghino__OnChanged;
                    _ghino._On_HuySuDungDvu += _ghino__On_HuySuDungDvu;
                }
                else
                    _ghino.UpdateData(m_dtChiPhiThanhtoan, dtData);
                _ghino.objLuotkham = this.objLuotkham;
                _ghino.XemLichSu = false;
                _ghino.ghi_no = true;
                _ghino.ShowDialog();
                if (!_ghino.isCancel )
                {
        ...
        private void _ghino__On_HuySuDungDvu(byte id_loaithanhtoan, long id_phieu_chitiet,long id_huydvu, byte trangthai_huy)
        {
            foreach(DataRow dr in m_dtChiPhiThanhtoan.Rows)
            {
                if(Utility.ByteDbnull(dr["id_loaithanhtoan"]) == id_loaithanhtoan && Utility.Int64Dbnull(dr["id_phieu_chitiet"] )== id_phieu_chitiet )
                {
                    dr["trangthai_huy"] = trangthai_huy;
                        dr["id_huydvu"] = id_huydvu;
                }    
            }    
        }

        private void _ghino__OnChanged()
        {
            SimpleRefresh_Ghino();
            DataRow[] arrDr = m_dtChiPhiThanhtoan.Select("trangthai_huy<>1 and tthai_tamthu<>1 and trangthai_thanhtoan<>1");
            DataTable dtData = m_dtChiPhiThanhtoan.Clone();
        ...
        /// <summary>
        /// Tự động check các mục để chuẩn bị bấm thanh toán. Không dùng vì thanh toán luôn tại form tạm thu
        /// </summary>
        /// <param name="lstPrivateKey"></param>
        void AutoCheck(List<int> lstPrivateKey)
        {
            grdThongTinChuaThanhToan.UnCheckAllRecords();
            //Tự động chọn các mục đã đánh dấu tạm thu
            try
            {
                foreach (GridEXRow gridExRow in grdThongTinChuaThanhToan.GetDataRows())
                {
                    ...
                    if (gridExRow.RowType == RowType.Record)
                    {
                        if (lstPrivateKey.Contains(Utility.Int32Dbnull(gridExRow.Cells["privatekey"].Value, 0)))
                        {
        ...
        bool IsSimpleRefresh = false;
        void SimpleRefresh_Ghino()
        {
            IsSimpleRefresh = true;
            GetDataChiTiet();
            SetSumTotalProperties();
            IsSimpleRefresh = false;
        }
        ...
        private void cmdKetchuyen_Click(object sender, EventArgs e)
        {
            DataTable dtData = m_dtChiPhiThanhtoan.Clone();
        ...
        List<long> lstIdTamthu = new List<long>();
        void KetchuyenTamthu()
        {
            lstIdTamthu.Clear();
            DataRow[] arrTamthuData = m_dtChiPhiThanhtoan.Select("tthai_tamthu=1");
            if (arrTamthuData.Length > 0)
            {
                frm_ChonTamthu_Ketchuyen _ChonTamthu_Ketchuyen = new frm_ChonTamthu_Ketchuyen(objLuotkham,lst_IDLoaithanhtoan);
                _ChonTamthu_Ketchuyen.uc_tamthu1.ShowData(arrTamthuData.CopyToDataTable(), m_dtTamthu);
                if (_ChonTamthu_Ketchuyen.ShowDialog() == DialogResult.OK)
                {
                    lstIdTamthu = _ChonTamthu_Ketchuyen.lstIdTamthu;
                    ketchuyen_tamthu = true;
                    SetSumTotalProperties();
                }
                else
                    ketchuyen_tamthu = false;
            }
            else
                return;
        }
        ...
        {
            if (Utility.Int32Dbnull(cboChuongtrinhKM.SelectedValue) == -1) return;
            frm_ChitietdichvuTronggoi _ChitietdichvuTronggoi = new frm_ChitietdichvuTronggoi(Utility.Int32Dbnull(cboChuongtrinhKM.SelectedValue) );
            _ChitietdichvuTronggoi._OnSelectionChanged += _ChitietdichvuTronggoi__OnSelectionChanged;
            if (_ChitietdichvuTronggoi.ShowDialog() == DialogResult.OK)
                cboChuongtrinhKM_SelectedIndexChanged(cboChuongtrinhKM, new EventArgs());
        }

        private void cmdTaoQR_Click(object sender, EventArgs e)
        {
            try
            {
                string kieuView = THU_VIEN_CHUNG.Laygiatrithamsohethong("QR_TYPE", "CRC16", true);
                if (dtPhanboQR.Rows.Count<=0)
                {
                    Utility.ShowMsg("Phiếu thu đang chọn không dùng Phương thức thanh toán Chuyển khoản hoặc không phân bổ tiền Chuyển khoản nên không thể tạo QR.\nVui lòng kiểm tra lại");
                    return;
                }    
                decimal sotien_ck =Utility.DecimaltoDbnull( dtPhanboQR.Rows[0]["so_tien"],0);
                string ma_pttt = Utility.sDbnull(dtPhanboQR.Rows[0]["ma_pttt"],"KXD");
                if (sotien_ck <= 0)
                {
                    Utility.ShowMsg("Số tiền thanh toán qua QR phải >0.\nVui lòng kiểm tra lại", "Cảnh báo", MessageBoxIcon.Warning);
                    return;
                }
                data_qrcode objQrData = new data_qrcode();
                Additionaldata additionaldata = new Additionaldata();
                objQrData.merchantId = "";
                objQrData.invoiceId =globalVariables.SysDate.ToString("yyyyMMddHHmmssffffff");
                objQrData.type = "vietqr";
                objQrData.transactionAmount = sotien_ck;
                objQrData.transactionTime = Utility.Int64Dbnull(globalVariables.SysDate.ToString("yyyyMMddHHmmss"));
                additionaldata.serviceCode = objQrData.invoiceId;
                additionaldata.patientCode = objLuotkham.MaLuotkham;
                additionaldata.patientName = Utility.sDbnull(txtPatientName.Text);
                additionaldata.description = Utility.sDbnull(txtObjectType_Code.Text);
                objQrData.additionalData = additionaldata;
                string qrCodeString = new HDBankConnection().CreateQRCode(objQrData);
                if (!string.IsNullOrEmpty(qrCodeString))
                {

                    QrPhieuThanhtoan objDatum = new QrPhieuThanhtoan();
                    objDatum.MaQrcode = objQrData.invoiceId;
                    objDatum.MaLuotkham = objLuotkham.MaLuotkham;
                    objDatum.IdBenhnhan = objLuotkham.IdBenhnhan;
                    objDatum.IdThanhtoan = -1;
                    objDatum.MaPttt = ma_pttt;
                    objDatum.TongTien = sotien_ck;
                    objDatum.SotienQr = Utility.DecimaltoDbnull(objQrData.transactionAmount);
                    objDatum.DataQrcode = qrCodeString;
                    objDatum.TrangThai = 0;
                    objDatum.SoLuot = 0;
                    objDatum.TthaiIn = 0;
                    objDatum.NgayTao = globalVariables.SysDate;
                    objDatum.NguoiTao = globalVariables.UserName;
                    objDatum.IpMacTao = globalVariables.gv_strMacAddress;
                    objDatum.QrSource = KieuThanhToan.NgoaiTru.ToString();
                    List<QrPhieuThanhtoanChitiet> _dataDetails = new List<QrPhieuThanhtoanChitiet>();

                    foreach (GridEXRow gridExRow in grdThongTinChuaThanhToan.GetCheckedRows())
                    {
                        QrPhieuThanhtoanChitiet objDetail = new QrPhieuThanhtoanChitiet();
                        objDetail.IdQrcode = objDatum.MaQrcode;
                        objDetail.IdPhieu = Utility.Int32Dbnull(gridExRow.Cells["id_phieu"].Value, -1);
                        objDetail.IdPhieuChitiet = Utility.Int32Dbnull(gridExRow.Cells["Id_Phieu_Chitiet"].Value, -1);
                        objDetail.IdDichvu = Utility.Int32Dbnull(gridExRow.Cells["Id_dichvu"].Value, -1);
                        objDetail.IdChitietDichvu = Utility.Int32Dbnull(gridExRow.Cells["Id_Chitietdichvu"].Value, -1);
                        objDetail.TenChitietDichvu = Utility.sDbnull(gridExRow.Cells["Ten_Chitietdichvu"].Value, -1);
                        objDetail.IdKham = Utility.Int64Dbnull(gridExRow.Cells["Id_Kham"].Value, -1);
                        objDetail.IdLoaithanhtoan = Utility.ByteDbnull(gridExRow.Cells["id_loaithanhtoan"].Value, -1);
                        objDetail.SoTien = Utility.DecimaltoDbnull(gridExRow.Cells["TT"].Value, -1);
                        _dataDetails.Add(objDetail);
                    }
                    string eMessage = "";
                    ActionResult actionResult = new HDBankConnection().LuuThongTinQRcode(objDatum, _dataDetails, ref eMessage);
                    switch (actionResult)
                    {
                        case ActionResult.Success:
                            switch (kieuView)
                            {
                                case "CRC16":
                                    //ShowQRCodeOnCRC16(qrCodeString, Utility.DecimaltoDbnull(objQrData.transactionAmount), Utility.UnSignedCharacter(txtPatientName.Text));
                                    break;
                                case "BASE64":
                                    //ShowQRcodeOnScreen2(qrCodeString, Utility.DecimaltoDbnull(objQrData.transactionAmount).ToString());
                                    break;
                                case "API":
                                    //ShowQRcodeOnScreen2(qrCodeString, Utility.DecimaltoDbnull(objQrData.transactionAmount).ToString());
                                    break;
        ...
        private void mnuQRData_Click(object sender, EventArgs e)
        {
           long id_thanhtoan= Utility.Int64Dbnull(grdPayment.GetValue("id_thanhtoan"), -1);
        ...
        private void mnu_inbangkechiphikcb_dathanhtoan_Click(object sender, EventArgs e)
        {
            long _Payment_ID = Utility.Int64Dbnull(grdPayment.GetValue(KcbThanhtoan.Columns.IdThanhtoan), -1);
            new INPHIEU_THANHTOAN_NGOAITRU().In_Bangke_CPKCB(_Payment_ID, true, 0);
        }

        private void mnu_chitietcongnophaithu_Click(object sender, EventArgs e)
        {
            frm_ChonTungayDenngay _ChonTungayDenngay = new frm_ChonTungayDenngay();
            if (_ChonTungayDenngay.ShowDialog() == DialogResult.OK)
            {
                DataTable dtData = SPs.CongnoBaocaoChitietcongnoPhaithu(Utility.GetBeginDate(_ChonTungayDenngay.dtp_tungay.Value), Utility.GetEndDate(_ChonTungayDenngay.dtp_denngay.Value), objLuotkham.IdBenhnhan, objLuotkham.MaLuotkham).GetDataSet().Tables[0];
        ...
        private void mnu_capnhatten_dichvu_Click(object sender, EventArgs e)
        {
            try
            {
                int id_chitietdichvu = Utility.Int32Dbnull(grdThongTinChuaThanhToan.GetValue("id_chitietdichvu"));
                int id_dichvu = Utility.Int32Dbnull(grdThongTinChuaThanhToan.GetValue("id_dichvu"));
                byte id_loaithanhtoan = Utility.ByteDbnull(grdThongTinChuaThanhToan.GetValue("id_loaithanhtoan"));
                SPs.ThanhtoanCapnhatTendichvu(id_dichvu, id_chitietdichvu, id_loaithanhtoan).Execute();
            }
            catch (Exception ex)
            {
                Utility.CatchException(ex);
        ...
        private void cmd_Pdf_Click(object sender, EventArgs e)
        {
            try
            {
                VMS.HIS.Danhmuc.frm_PdfViewer _PdfViewer = new VMS.HIS.Danhmuc.frm_PdfViewer(100);
        ...
        private void mnu_Lichsu_Congno_Click(object sender, EventArgs e)
        {
            try
            {
                DataRow[] arrDr = m_dtChiPhiThanhtoan.Select("trangthai_huy<>1 and tthai_tamthu<>1 and trangthai_thanhtoan<>1");
        ...
        private void mnu_kekhai_vtth_theogoi_Click(object sender, EventArgs e)
        {
            try
            {
                if (objLuotkham == null) return;
                DataTable dtData = SPs.ThanhtoanBangkeCongkhaiVtth(objLuotkham.IdBenhnhan, objLuotkham.MaLuotkham,1).GetDataSet().Tables[0];

                THU_VIEN_CHUNG.CreateXML(dtData, "thuoc_phieucongkhai_vtth.xml");
                if (dtData.Rows.Count <= 0)
                {
                    Utility.ShowMsg("Không tìm thấy dữ liệu", "Thông báo", MessageBoxIcon.Warning);
                    return;
                }
                thuoc_baocao.Thuoc_InBaoCao(dtData, "thuoc_phieucongkhai_vtth", DateTime.Now, "");
            }
            catch (Exception ex)
            {
                Utility.ShowMsg("Lỗi:" + ex.Message);
            }
            //frm_InBangKeVTTH_TheoGoiMo _InBangKeVTTH_TheoGoiMo = new frm_InBangKeVTTH_TheoGoiMo(this.objLuotkham);
            //_InBangKeVTTH_TheoGoiMo.ShowDialog();
        }

        private void mnu_inbangkechiphi_kcb_new_Click(object sender, EventArgs e)
        {
            if (objLuotkham == null)
            {
            ...
            short idkhoanoitru = -1;

            frm_InBangKeChiPhiKCB _Xemtonghopchiphi = new frm_InBangKeChiPhiKCB(false, idkhoanoitru.ToString());

            _Xemtonghopchiphi.objLuotkham = objLuotkham;
            _Xemtonghopchiphi.ShowDialog();
        }

        private void mnu_CapNhatNgayThanhToan_Click(object sender, EventArgs e)
        {
            if (Utility.Coquyen("thanhtoan_suangaythanhtoan"))
            {
                v_Payment_ID = Utility.Int32Dbnull(grdPayment.CurrentRow.Cells["Id_thanhtoan"].Value, -1);
                ...
                if (objThanhtoan != null)
                {
                    frm_ChonngayThanhtoan _ChonngayThanhtoan = new frm_ChonngayThanhtoan(objThanhtoan.NgayThanhtoan);
                    if (_ChonngayThanhtoan.ShowDialog() == DialogResult.OK)
                    {
                        CapNhatNgayThanhToan(objThanhtoan, _ChonngayThanhtoan.pdt_InputDate);
                    }
                }
                else
                {
        ...
        private void mnu_ThanhToanCongNo_Click(object sender, EventArgs e)
        {
            if (Utility.Coquyen("thanhtoan_thaydoi_trangthaicongno"))
            {
                v_Payment_ID = Utility.Int32Dbnull(grdPayment.CurrentRow.Cells["Id_thanhtoan"].Value, -1);
                KcbThanhtoan objThanhtoan = KcbThanhtoan.FetchByID(v_Payment_ID);
                if (objThanhtoan != null)
                {
                    new Update(KcbThanhtoan.Schema)
                        .Set(KcbThanhtoan.Columns.ThanhtoanGhino).EqualTo(1)
                        .Where(KcbThanhtoan.Columns.IdThanhtoan).IsEqualTo(v_Payment_ID)
                        .Execute();
                    Utility.Log(this.Name, globalVariables.UserName, string.Format("Thay đổi trạng thái ghi nợ =1 cho thanh toán {0} của idbv {1}, mã khám {2} để lên báo cáo chi tiết công nợ thành công ", v_Payment_ID, objLuotkham.IdBenhnhan, objLuotkham.MaLuotkham), newaction.Update, this.GetType().Assembly.ManifestModule.Name);
                    Utility.ShowMsg("Chứng từ được chọn đã lên báo cáo chi tiết công nợ. Vui lòng kiểm tra");
                }
                else
                {
            ...
            {
                Utility.thongbaokhongcoquyen("thanhtoan_thaydoi_trangthaicongno","thay đổi trạng thái báo cáo công nợ của phiếu thu");
            }
        }

        private void mnu_HuyThanhToanCongNo_Click(object sender, EventArgs e)
        {
            if (Utility.Coquyen("thanhtoan_thaydoi_trangthaicongno"))
            {
                v_Payment_ID = Utility.Int32Dbnull(grdPayment.CurrentRow.Cells["Id_thanhtoan"].Value, -1);
                ...
                {
                    new Update(KcbThanhtoan.Schema)
                        .Set(KcbThanhtoan.Columns.ThanhtoanGhino).EqualTo(0)
                        .Where(KcbThanhtoan.Columns.IdThanhtoan).IsEqualTo(v_Payment_ID)
                        .Execute();
                    Utility.Log(this.Name, globalVariables.UserName, string.Format("Thay đổi trạng thái ghi nợ =0 cho thanh toán {0} của idbv {1}, mã khám {2} để Không lên báo cáo chi tiết công nợ thành công ", v_Payment_ID, objLuotkham.IdBenhnhan, objLuotkham.MaLuotkham), newaction.Update, this.GetType().Assembly.ManifestModule.Name);
                    Utility.ShowMsg("Chứng từ được chọn đã Hủy khỏi báo cáo chi tiết công nợ. Vui lòng kiểm tra");
                }
                else
                {
        ...
        private void mnu_ChonThanhToan_Click(object sender, EventArgs e)
        {
        ...
        void _ChitietdichvuTronggoi__OnSelectionChanged(DataRow dr)
        {
            try
            {
                var p = from q in grdThongTinChuaThanhToan.GetDataRows()
                        where Utility.ByteDbnull(q.Cells["id_loaithanhtoan"].Value, 100) == Utility.ByteDbnull(dr["loai_dvu"], 100)
                        && Utility.Int32Dbnull(q.Cells["id_chitietdichvu"].Value, -1) == Utility.Int32Dbnull(dr["id_chitietdichvu"], -1)
                        && Utility.Int32Dbnull(q.Cells["id_dichvu"].Value, -1) == Utility.Int32Dbnull(dr["id_dichvu"], -1)
                        select q;
                if (p.Any())
                    grdThongTinChuaThanhToan.MoveTo(p.FirstOrDefault());
            }
            catch (Exception ex)
            {
