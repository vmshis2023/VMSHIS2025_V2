...
namespace VNS.HIS.UI.THUOC
{
    public partial class frm_phatthuoc_ngoaitru : Form
    {
        ...
        void grdPhieuChi_UpdatingCell(object sender, UpdatingCellEventArgs e)
        {
            ...
            {
                if (Utility.Coquyen("quyen_suangay_thanhtoan"))
                {
                    //UpdatePaymentDate();
        ...
        {
            if (kieuthuoc_vt == "ALL")
                m_dtKhothuoc = CommonLoadDuoc.LAYTHONGTIN_KHOTHUOC_VATTU_LE();
                ...
                mv_dtDonthuoc =
                    SPs.ThuocTimkiemdonthuocCapphatngoaitru(Utility.Int32Dbnull(txtPres_ID.Text, -1), txtPID.Text,
                                                           ...
                                                           chkByDate.Checked ? dtToDate.Value.ToString("dd/MM/yyyy") : "01/01/1900", Status,
                                                            Utility.Int32Dbnull(cboKho.SelectedValue),kieuthuoc_vt).GetDataSet().Tables[0];

                Utility.SetDataSourceForDataGridEx(grdPres, mv_dtDonthuoc, true, true, "1=1", "ten_benhnhan");
        ...
        private void ModifyCommand()
        {
            try
            {
                ...
                cmdThanhtoan.Enabled = cmdThanhtoan2.Enabled = isValidParent && isValidDetail && grdPresDetail.GetCheckedRows().Length > 0 && _chuathanhtoan;
                ...
                cmdPhatThuoc.Enabled = cmdDuyetcapphat.Enabled = isValidParent && isValidDetail && thanhtoanchuacapphat && !_dacapphathet;// _dathanhtoan && tthai_capphat < 2;
        ...
        private void GetDataPresDetail()
        {
            ...
            m_dtChitietdonthuoc = SPs.KcbThanhtoanLaythongdonthuoctaiquayDethanhtoan(ma_luotkham, v_IdBenhnhan, v_IdDonthuoc).GetDataSet().Tables[0];
            Utility.SetDataSourceForDataGridEx(grdPresDetail, m_dtChitietdonthuoc, false, true, "1=1", KcbDonthuocChitiet.Columns.SttIn);
            ...
            try
            {
                ...
                if (presId > 0 && stockId > 0)
                {
                    ...
                    List<long> lstIdchitietcancaphat    =dtChitietcapphat.AsEnumerable().Select(c=>c.Field<long>(KcbDonthuocChitiet.Columns.IdChitietdonthuoc)).Distinct().ToList<long>();
                        ...
                        ActionResult actionResult = new XuatThuoc().LinhThuocBenhNhan(presId, stockId, lstIdchitietcancaphat, dtNgayPhatThuoc.Value);
                        ...
                        case ActionResult.Success:
                            ...
                            if (IN_HOADON && PropertyLib._MayInProperties.TudonginhoadonSaukhiThanhtoan)
                            {
                                ...
                                if (kcbThanhtoanKieuinhoadon == 2 || kcbThanhtoanKieuinhoadon == 3)
                                    new INPHIEU_THANHTOAN_NGOAITRU().InBienlai(false, v_Payment_ID, objLuotkham, 0);
                            }
            ...
            try
            {
                ...
                DataTable dtData =
                       _THANHTOAN.LaythongtinCacLanthanhtoan(objLuotkham.MaLuotkham,
                           ...
                           globalVariables.MA_KHOA_THIEN, lstIdLoaiTtoan);
        ...
        private void cmdXoadonthuoc_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (Utility.AcceptQuestion(string.Format("Bạn có chắc chắn muốn xóa đơn thuốc của người bệnh {0}?", grdPres.GetValue("ten_benhnhan").ToString()), "xác nhận xóa", true))
                {
                    ...
                    DataRow[] arrdr = m_dtChitietdonthuoc.Select(string.Format("{0}={1}", KcbDonthuoc.Columns.IdDonthuoc, Pres_ID));
                    if (arrdr.Length > 0)
                        m_dtChitietdonthuoc.Rows.Remove(arrdr[0]);
                    m_dtChitietdonthuoc.AcceptChanges();
                    grdPres_SelectionChanged(grdPres, e);
