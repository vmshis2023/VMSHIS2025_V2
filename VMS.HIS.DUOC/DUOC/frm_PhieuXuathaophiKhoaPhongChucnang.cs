...
{
    public partial class frm_PhieuXuathaophiKhoaPhongChucnang : Form
    {
        ...
        public string KHOA_KHO = "KHOA";//Cấp theo khoa=KHOA; Cấp theo kho của khoa=KHO
        public string KIEU_THUOC_VT = "THUOC";
        public frm_PhieuXuathaophiKhoaPhongChucnang(string KIEU_THUOC_VT)
        {
            InitializeComponent();
            Utility.SetVisualStyle(this);
            this.KIEU_THUOC_VT = KIEU_THUOC_VT.Split('-')[0];
            KHOA_KHO = KIEU_THUOC_VT.Split('-')[1];
            dtFromdate.Value = globalVariables.SysDate.AddMonths(-1);
            dtToDate.Value = globalVariables.SysDate;
            cboKhonhan.Visible = lblKhonhan.Visible = KHOA_KHO == "KHO";
        ...
        void setquyen()
        {
            try
            {
                cmdThemPhieuNhap.Visible = Utility.Coquyen("thuoc_phieuxuat_khoaphongchucnang_themmoi");
                cmdUpdatePhieuNhap.Visible = Utility.Coquyen("thuoc_phieuxuat_khoaphongchucnang_sua");
                cmdXoaPhieuNhap.Visible = Utility.Coquyen("thuoc_phieuxuat_khoaphongchucnang_xoa");
                cmdNhapKho.Visible = Utility.Coquyen("thuoc_phieuxuat_khoaphongchucnang_xacnhan");
                cmdHuychuyenkho.Visible = Utility.Coquyen("thuoc_phieuxuat_khoaphongchucnang_huyxacnhan");
                cmdExIm.Visible = Utility.Coquyen("thuoc_phieuxuat_khoaphongchucnang_ExportImport");
            }
            catch (Exception ex)
            {
        ...
        private void TIMKIEM_THONGTIN()
        {
            ...
            m_dtDataNhapKho =
                 new THUOC_NHAPKHO().Laydanhsachphieunhapkho(chkByDate.Checked ? dtFromdate.Value.ToString("dd/MM/yyyy") : "01/01/1900",
                                             ...
                                             cboKhonhan.Visible ? Utility.Int32Dbnull(cboKhonhan.SelectedValue, -1) : -1,
                                             Utility.Int32Dbnull(cboKhoxuat.SelectedValue, -1), Utility.Int32Dbnull(cboNhanVien.SelectedValue, -1),
                                             -1, "ALL", Utility.sDbnull(txtSoPhieu.Text), TRANG_THAI, (int)LoaiPhieu.PhieuXuatKhoaPhongchucnang, MaKho, 2, KIEU_THUOC_VT);
        ...
        {
            m_KhoaLinh = THU_VIEN_CHUNG.Laydanhmuckhoa("ALL", "ALL", -1);
            if (KIEU_THUOC_VT == "THUOC")
            {
                m_dtKhoXuat = CommonLoadDuoc.LAYDANHMUCKHO(-1, "ALL", "THUOC,THUOCVT", "CHANLE,LE", 0, 100, 1);// CommonLoadDuoc.LAYTHONGTIN_KHOTHUOC_LE_NOITRU();
                m_dtKhonhan = CommonLoadDuoc.LAYDANHMUCKHO(-1, "ALL", "THUOC,THUOCVT", "CHANLE,LE", 100, 100, 1);// CommonLoadDuoc.LAYTHONGTIN_TUTHUOC();
            }
            else
            {
                m_dtKhoXuat = CommonLoadDuoc.LAYDANHMUCKHO(-1, "ALL", "VT,THUOCVT", "CHANLE,LE", 0, 100, 1);// CommonLoadDuoc.LAYTHONGTIN_KHOVATTU_CHAN();
                m_dtKhonhan = CommonLoadDuoc.LAYDANHMUCKHO(-1, "ALL", "VT,THUOCVT", "CHANLE,LE", 100, 100, 1);//  CommonLoadDuoc.LAYTHONGTIN_KHOVATTU_LE(new List<string> { "TATCA",  "NOITRU" });
            }
            ...
            if (objPhieuNhap != null)
            {
                ...
                else
                {
                    if (THU_VIEN_CHUNG.Laygiatrithamsohethong("THUOC_INPHIEUXUATKHO _2LIEN", "0", false) == "1")
                        VNS.HIS.UI.Baocao.thuoc_phieuin_nhapxuat.InphieuXuatkho_2lien(IDPhieuNhap, "PHIẾU XUẤT THUỐC KHOA PHÒNG CHỨC NĂNG", globalVariables.SysDate);
                    else
                        VNS.HIS.UI.Baocao.thuoc_phieuin_nhapxuat.InphieuXuatkho(IDPhieuNhap, "PHIẾU XUẤT THUỐC KHOA PHÒNG CHỨC NĂNG", globalVariables.SysDate);
                }
            }
        ...
        private void cmdNhapKho_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cmdNhapKho.Enabled = false;
                if (Utility.AcceptQuestion("Bạn có chắc chắn muốn xác nhận Phiếu xuất hao phí khoa phòng chức năng đang chọn hay không?", "Thông báo", true))
                {
                    string errMsg = "";
                    int IdPhieu = Utility.Int32Dbnull(grdList.GetValue(TPhieuNhapxuatthuoc.Columns.IdPhieu), -1);
                    ...
                    if (objTPhieuNhapxuatthuoc != null)
                    {
                        ...
                        {
                            case ActionResult.Success:
                                Utility.SetMsg(uiStatusBar2.Panels["MSG"], "Đã xác nhận Phiếu xuất hao phí khoa phòng chức năng thành công", false);
                            ...
                            case ActionResult.Exceed:
                                Utility.ShowMsg("Không có thuốc trong kho xuất nên không thể xác nhận Phiếu xuất hao phí khoa phòng chức năng\n" + errMsg);
                                break;
                            case ActionResult.NotEnoughDrugInStock:
                                Utility.ShowMsg("Thuốc trong kho xuất không còn đủ số lượng nên không thể xác nhận Phiếu xuất hao phí khoa phòng chức năng\n" + errMsg);
                                break;
                            case ActionResult.Error:
        ...
        private void cmdThemPhieuNhap_Click(object sender, EventArgs e)
        {
            try
            {
                frm_themmoi_phieuxuatHaophi frm = new frm_themmoi_phieuxuatHaophi(KHOA_KHO);
        ...
        private void cmdHuychuyenkho_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Utility.AcceptQuestion("Bạn có chắc chắn muốn hủy xác nhận Phiếu xuất hao phí khoa phòng chức năng đang chọn hay không?", "Thông báo", true))
                {
                    string errMsg = "";
                    int IdPhieu = Utility.Int32Dbnull(grdList.GetValue(TPhieuNhapxuatthuoc.Columns.IdPhieu), -1);
                    TPhieuNhapxuatthuoc objTPhieuNhapxuatthuoc = TPhieuNhapxuatthuoc.FetchByID(IdPhieu);
                    if (objTPhieuNhapxuatthuoc != null)
                    {
                        ...
                        {
                            case ActionResult.Success:
                                Utility.SetMsg(uiStatusBar2.Panels["MSG"], "Bạn thực hiện hủy xác nhận Phiếu xuất hao phí khoa phòng chức năng thành công", false);
                                Utility.ShowMsg(uiStatusBar2.Panels["MSG"].Text);
                                grdList.CurrentRow.BeginEdit();
                                ...
                                break;
                            case ActionResult.Exceed:
                                Utility.ShowMsg("Thuốc trong khoa đã được sử dụng hết nên bạn không thể hủy Phiếu xuất hao phí khoa phòng chức năng", "Thông báo lỗi", MessageBoxIcon.Error);
                                break;
                            case ActionResult.NotEnoughDrugInStock:
                                Utility.ShowMsg(string.Format("Thuốc trong Phiếu xuất hao phí khoa phòng chức năng không đủ lượng tồn để hủy xác nhận(trừ khỏi kho thuốc khoa phòng chức năng). Vui lòng kiểm tra lại\n{0}", errMsg), "Thông báo lỗi", MessageBoxIcon.Error);
                                break;
                            case ActionResult.Error:
                                break;
                        }
        ...
        private void exportToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Utility.ShowMsg("Bạn cần chọn ít nhất một phiếu chuyển kho");
                    return;
                }
                ...
                if (_folder.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    {
                        string id_ma = string.Format("{0}-{1}", _row.Cells["id_phieu"].Value.ToString(), _row.Cells["ma_phieu"].Value.ToString());
                        long id_phieu = Utility.Int64Dbnull(_row.Cells["id_phieu"].Value, -1);
                        DataSet dsData = SPs.ThuocPhieunhapkhoExport(id_phieu).GetDataSet();
                        dsData.WriteXml(string.Format(@"{0}\{1}.xml", _folder.SelectedPath, id_ma));
                    }
                }
            }
            catch (Exception ex)
        ...
        private void importToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                foreach (string sFile in _FileDialog.FileNames)
                {
                    try
                    {
                        if (Utility.AcceptQuestion("Bạn có chắc chắn muốn import dữ liệu phiếu nhập kho từ file: " + sFile, "Xác nhận", true))
                        {
                            ...
                            if (dsData.Tables.Count == 2)
                            {
                                ...
                                int IdPhieu = Utility.Int32Dbnull(dsData.Tables[0].Rows[0]["id_phieu"], -1);
                                frm_themmoi_phieuxuatHaophi frm = new frm_themmoi_phieuxuatHaophi(KHOA_KHO);
                                frm.m_enAction = action.Update;
                                frm.KIEU_THUOC_VT = KIEU_THUOC_VT;
                                frm.grdList = grdList;
