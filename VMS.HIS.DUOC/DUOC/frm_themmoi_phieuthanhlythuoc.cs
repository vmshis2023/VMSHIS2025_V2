...
{
    public partial class frm_themmoi_phieuthanhlythuoc : Form
    {
        ...
        string SplitterPath = "";
        public frm_themmoi_phieuthanhlythuoc()
        {
            InitializeComponent();
            this.FormClosing += frm_themmoi_phieuthanhlythuoc_FormClosing;
            this.Shown += frm_themmoi_phieuthanhlythuoc_Shown;
        ...
        void frm_themmoi_phieuthanhlythuoc_Shown(object sender, EventArgs e)
        {
            Try2Splitter();
        }

        void frm_themmoi_phieuthanhlythuoc_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        void InitEvents()
        {
            cmdThemnguoinhuy.Click += new EventHandler(cmdThemnguoinhuy_Click);
            ...
            this.Load += new System.EventHandler(this.frm_themmoi_phieuthanhlythuoc_Load);
            ...
            this.KeyDown += new KeyEventHandler(frm_themmoi_phieuthanhlythuoc_KeyDown);
            ...
            txtDiadiem._OnShowData += new UCs.AutoCompleteTextbox_Danhmucchung.OnShowData(txtDiadiem__OnShowData);
        }

        void txtDiadiem__OnShowData()
        {
        ...
        void cmdAddDetail_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Utility.isValidGrid(grdKhoXuat))
                {
                    ...
                    {
                        Utility.SetMsg(lblMsg, string.Format("Số lượng thanh lý {0} phải <= Số lượng có {1}", txtSoluongdutru.Text, slkhachuyen.ToString()), true);
                    ...
                    grdKhoXuat.CurrentRow.Cells["DON_GIA"].Value = Utility.DecimaltoDbnull(txtDongia.Text, -1);
        ...
        bool isValidDetailData()
        {
            ...
            {
                Utility.SetMsg(lblMsg, "Bạn phải chọn thuốc thanh lý",true);
                txtthuoc.Focus();
                return false;
            }
            if (Utility.DecimaltoDbnull(txtSoluongdutru.Text,-1)<=0)
            {
                Utility.SetMsg(lblMsg, "Bạn phải nhập số lượng thanh lý",true);
                txtSoluongdutru.Focus();
                return false;
            }
        ...
        private void cmdInPhieuNhap_Click(object sender, EventArgs e)
        {
            ...
            {
                VNS.HIS.UI.Baocao.thuoc_phieuin_nhapxuat.InphieuThanhly(IDPhieuNhap, "BIÊN BẢN THANH LÝ", globalVariables.SysDate);
            }
        ...
        private void InitalData()
        {
            ...
            {
                m_dtKhohuy = CommonLoadDuoc.LAYDANHMUCKHO(-1, "ALL", "THUOC,THUOCVT", "ALL", 0, 100, 1); //CommonLoadDuoc.LAYTHONGTIN_KHOTHUOC_TATCA();
            }
            else
            {
                m_dtKhohuy = CommonLoadDuoc.LAYDANHMUCKHO(-1, "ALL", "VT,THUOCVT", "ALL", 0, 100, 1); //CommonLoadDuoc.LAYTHONGTIN_KHOVATTU_TATCA();
            }
        ...
        private void getData()
        {
            if (m_enAction == action.Update || m_enAction == action.View)
            {
                ...
                if (objPhieuNhap != null)
                {
                    ...
                    txtMotathem.Text = objPhieuNhap.YkienDexuat;
                    txtDiadiem._Text = objPhieuNhap.DiadiemThanhly;
                    txtHoidong.Rtf = objPhieuNhap.HoidongThanhly;
                    dtpFrom.Value = objPhieuNhap.ThoigianThanhlyTu.Value;
                    dtpTo.Value = objPhieuNhap.ThoigianThanhlyDen.Value;
        ...
        private void frm_themmoi_phieuthanhlythuoc_Load(object sender, EventArgs e)
        {
            txtLydohuy.Init();
            txtDiadiem.Init();
            InitalData();
        ...
        private bool isValidData()
        {
            ...
            {
                Utility.SetMsg(lblMsg, "Bạn phải chọn kho thanh lý", true);
                cboKhohuy.Focus();
                return false;
            }
            if (Utility.DoTrim(txtHoidong.Text)=="")
            {
                Utility.SetMsg(lblMsg, "Bạn phải nhập thông tin hội đồng thanh lý", true);
                txtHoidong.Focus();
                return false;
            }
            if (txtDiadiem.myCode == txtDiadiem.DefaultCode)
            {
                Utility.SetMsg(lblMsg, "Bạn phải chọn nơi thanh lý",true);
                txtDiadiem.Focus();
                return false;
            }
            if (Utility.DoTrim(txtMotathem.Text) == "")
            {
                Utility.SetMsg(lblMsg, "Bạn phải nhập ý kiến đề xuất",true);
                txtMotathem.Focus();
                return false;
            }
            return true;
        ...
        private void RemoveDetails()
        {
            ...
            {
                Utility.ShowMsg("Lỗi khi thanh lý chuyển thuốc:\n"+ex.Message);
            }
        }
