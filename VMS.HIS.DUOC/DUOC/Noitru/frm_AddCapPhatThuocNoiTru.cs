...
namespace VNS.HIS.UI.THUOC
{
    public partial class frm_AddCapPhatThuocNoiTru : Form
    {
        ...
        private void LoadDepartments()
        {
            try
            {
                DataBinding.BindDataCombobox(cboDepartment,THU_VIEN_CHUNG.LaydanhsachKhoatheoUser(globalVariables.UserName, Utility.Bool2byte(globalVariables.IsAdmin),1),
        ...
        private void LaythongtinTongHopDonthuoc(int DepartmentID,int Status)
        {
            try
            {
                int kieudonthuoc = optNoitru.Checked ? 0 : 3;
                ...
                dsData = SPs.ThuocNoitruLaydanhsachdonthuocTonghoplinhthuocnoitru(id_khoa, id_kho, loaiphieu, kieudonthuoc, KIEUTHUOC_VT, (Int16)Status, _IDCAPPHAT, chkCheck.Checked ? dtTuNgay.Value.ToString("dd/MM/yyyy") : "01/01/1900", chkCheck.Checked ? dtDenNgay.Value.ToString("dd/MM/yyyy") : "01/01/1900").GetDataSet();
               ...
               TPhieuCapphatNoitru objTPhieuCapphatNoitru = TaoPhieuCapphatNoitru();
                ...
                switch (actionResult)
                {
                    case ActionResult.Success:
                        Utility.Log(this.Name, globalVariables.UserName, string.Format("Cập nhật thành công phiếu cấp phát thuốc nội trú với Id={0} ", objTPhieuCapphatNoitru.IdCapphat), newaction.Update, this.GetType().Assembly.ManifestModule.Name);
                        cmdPrint.Enabled = true;
                        cmdPrintDetail.Enabled = true;
                        break;
               ...
               ActionResult actionResult = new CapphatThuocKhoa().ThemPhieuCapPhatNoiTru(objTPhieuCapphatNoitru, TaoPhieuCapphatChitiet(), lstTThuocCapphatChitiet);
                ...
                switch (actionResult)
                {
                    case ActionResult.Success:
                        ...
                        _IDCAPPHAT = objTPhieuCapphatNoitru.IdCapphat;
                        Utility.Log(this.Name, globalVariables.UserName, string.Format("Tạo thành công phiếu cấp phát thuốc nội trú với Id={0} ", _IDCAPPHAT), newaction.Insert, this.GetType().Assembly.ManifestModule.Name);
                        cmdPrint.Enabled = true;
                        cmdPrintDetail.Enabled = true;
       ...
       List< TPhieuCapphatChitiet> TaoPhieuCapphatChitiet()
        ...
        private void cboDepartment_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable _newTable = CommonLoadDuoc.LAYDANHMUCKHO_THEOKHOA(IDKhoa, -1, "TATCA,NOITRU", KIEUTHUOC_VT, 0, 100); 
