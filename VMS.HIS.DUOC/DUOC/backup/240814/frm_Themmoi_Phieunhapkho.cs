       ...
       // private DataTable m_dtDataNhaCungCap=new DataTable();
        ...
        void cboSoQDthau_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DataTable dtThuoc = SPs.ThuocThauLaydanhmucthuoc(Utility.sDbnull(cboSoQDthau.SelectedValue), txtNhacungcap.myCode, this.KIEU_THUOC_VT).GetDataSet().Tables[0];
                    if (chkTrongthau.Checked)
                    {
        ...
        void txtNhacungcap__OnEnterMe()
        {
            try
            {
                DataTable dtDataSoQD = Utility.ExecuteSql(string.Format("select distinct maso_qdinh from t_thau_qdinh where ma_nha_cc='{0}'", txtNhacungcap.myCode),CommandType.Text).Tables[0];
        ...
        void cmdQuickCreate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable dtThuockho = new Select().From(TThuockho.Schema).Where(TThuockho.Columns.IdKho).IsEqualTo(txtKhonhap.MyID).ExecuteDataSet().Tables[0];
                ...
                if (Utility.Int32Dbnull(txtDrug_ID.Text,-1)>0)
                {
                   ...
                   if (objThuoc != null)
                   {
                       ...
                       else
                       {
                           ...
                           txtsoQDthau_Dmuc.Text = string.Format("{0};{1};{2}", Utility.sDbnull(objThuoc.QD31, ""), Utility.sDbnull(objThuoc.LoaiThau), Utility.sDbnull(objThuoc.NhomThau));
        ...
        private List< TPhieuNhapxuatthuocChitiet> TaoChitietNhapkho()
        {
            ...
            try
            {
                ...
                foreach (GridEXRow gridExRow in grdPhieuNhapChiTiet.GetDataRows())
                {
                    if (gridExRow.RowType == RowType.Record)
                    {
                        TPhieuNhapxuatthuocChitiet newItem = new TPhieuNhapxuatthuocChitiet();
                        newItem.MaNhacungcap = txtNhacungcap.myCode;
                        newItem.IdThuoc =
        ...
        private void grdPhieuNhapChiTiet_UpdatingCell(object sender, UpdatingCellEventArgs e)
        {
            ...
            if (e.Column.Key.ToUpper() == TPhieuNhapxuatthuocChitiet.Columns.SoLuong.ToUpper())
            {
                ...
                long id_thau_ct = Utility.Int64Dbnull(grdPhieuNhapChiTiet.GetValue("id_thau_ct"));
                DataTable dtData = SPs.ThuocThauLaythongtinSoluong(id_thau_ct).GetDataSet().Tables[0];
                int slkhachuyen = Utility.Int32Dbnull(dtData.Rows[0]["sl_khachuyen"]);
                //Lấy số lượng chờ trong chính phiếu đang có
                int slcho_trongphieu = Utility.Int32Dbnull(m_dtDataPhieuChiTiet.Compute("sum(so_luong)", TThauDieutietCt.Columns.IdThauCt + "=" + id_thau_ct.ToString()), 0);
            ...
            {
                decimal soluong = Utility.DecimaltoDbnull(grdPhieuNhapChiTiet.GetValue(TPhieuNhapxuatthuocChitiet.Columns.SoLuong));
