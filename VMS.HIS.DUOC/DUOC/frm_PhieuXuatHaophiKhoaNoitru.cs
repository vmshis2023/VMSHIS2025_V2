...
{
    public partial class frm_PhieuXuatHaophiKhoaNoitru : Form
    {
        ...
        public frm_PhieuXuatHaophiKhoaNoitru(string KIEU_THUOC_VT)
        {
            InitializeComponent();
        ...
        void setquyen()
        {
            try
            {
                cmdThemPhieuNhap.Visible = Utility.Coquyen("thuoc_phieuhuy_themmoi");
                cmdUpdatePhieuNhap.Visible = Utility.Coquyen("thuoc_phieuhuy_sua");
                cmdXoaPhieuNhap.Visible = Utility.Coquyen("thuoc_phieuhuy_xoa");
                cmdNhapKho.Visible = Utility.Coquyen("thuoc_phieuhuy_xacnhan");
                cmdHuychuyenkho.Visible = Utility.Coquyen("thuoc_phieuhuy_huyxacnhan");
            }
            catch (Exception ex)
            {
        ...
        void InitEvents()
        {
            ...
            this.Load += new System.EventHandler(this.frm_PhieuXuatHaophiKhoaNoitru_Load);
            this.KeyDown += new KeyEventHandler(frm_PhieuXuatHaophiKhoaNoitru_KeyDown);
        ...
        /// <param name="e"></param>
        private void frm_PhieuXuatHaophiKhoaNoitru_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            m_dtDataNhapKho =
                new THUOC_NHAPKHO().Laydanhsachphieunhapkho(chkByDate.Checked ? dtFromdate.Value.ToString("dd/MM/yyyy") :"01/01/1900",
                                            ...
                                            -1, Utility.Int32Dbnull(cboKhohuy.SelectedValue, -1),
                                             ...
                                             -1, "-1", Utility.sDbnull(txtSoPhieu.Text), TRANG_THAI, (int)LoaiPhieu.PhieuXuatHaophi, MaKho, 2, KIEU_THUOC_VT);
        ...
        private bool IsValid4UpdateDelete()
        {
            ...
            {
                Utility.SetMsg(uiStatusBar2.Panels["MSG"], "Phiếu hao phí nội trú thuốc đang chọn đã xác nhận, Bạn không thể sửa hoặc xóa thông tin", false);
                return false;
            }
        ...
        private void cmdXoaPhieuNhap_Click(object sender, EventArgs e)
        {
            ...
            if (Utility.AcceptQuestion(string.Format("Bạn có muốn xóa thông tin phiếu hao phí nội trú thuốc với mã phiếu {0}\n hay không?", IdPhieu), "Thông báo", true))
            {
                ...
                switch (actionResult)
                {
                    case ActionResult.Success:
                        ...
                        Utility.SetMsg(uiStatusBar2.Panels["MSG"], "Bạn xóa thông tin phiếu hao phí nội trú thuốc thành công", false);
                        break;
        ...
        private void frm_PhieuXuatHaophiKhoaNoitru_Load(object sender, EventArgs e)
        {
            InitData();
            AutocompleteThuoc();
            TIMKIEM_THONGTIN();
        ...
        private void InitData()
        {
            ...
            {
                m_dtKhoXuat = CommonLoadDuoc.LAYDANHMUCKHO(-1,"ALL", "THUOC,THUOCVT", "ALL", 100, 100, 1); // CommonLoadDuoc.LAYTHONGTIN_KHOTHUOC_TATCA();
            }
            else
            {
                m_dtKhoXuat = CommonLoadDuoc.LAYDANHMUCKHO(-1,"ALL", "VT,THUOCVT", "ALL", 100, 100, 1); // CommonLoadDuoc.LAYTHONGTIN_KHOVATTU_TATCA();
            }
        ...
        private void cmdInPhieuNhapKho_Click(object sender, EventArgs e)
        {
            ...
            {
                VNS.HIS.UI.Baocao.thuoc_phieuin_nhapxuat.InphieuHuythuoc(id_PhieuNhap, "BIÊN BẢN hao phí nội trú THUỐC", globalVariables.SysDate);
            }
        ...
        private void cmdNhapKho_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (objTPhieuNhapxuatthuoc != null)
                {
                    ...
                    {
                        case ActionResult.Success:
                            Utility.SetMsg(uiStatusBar2.Panels["MSG"], "Xác nhận phiếu hao phí nội trú thuốc thành công", false);
                        ...
                        case ActionResult.Exceed:
                            Utility.ShowMsg("Không có thuốc trong kho xuất nên không thể xác nhận phiếu hao phí nội trú\n" + errMsg, "Thông báo lỗi", MessageBoxIcon.Warning);
                            break;
                        case ActionResult.NotEnoughDrugInStock:
                            Utility.ShowMsg("Thuốc trong kho xuất không còn đủ số lượng nên không thể xác nhận phiếu hao phí nội trú\n" + errMsg, "Thông báo lỗi", MessageBoxIcon.Warning);
                            break;
                        case ActionResult.Error:
                            break;
                    }
        ...
        private void cmdThemPhieuNhap_Click(object sender, EventArgs e)
        {
            try
            {
                frm_themmoi_phieuxuathaophi_khoanoitru frm = new frm_themmoi_phieuxuathaophi_khoanoitru();
        ...
        private void cmdHuychuyenkho_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Utility.SetMsg(uiStatusBar2.Panels["MSG"], "", false);
                if (Utility.AcceptQuestion("Bạn có muốn hao phí nội trú xác nhận phiếu hao phí nội trú thuốc đang chọn này không?", "Thông báo", true))
                {
                    int IdPhieu = Utility.Int32Dbnull(grdList.GetValue(TPhieuNhapxuatthuoc.Columns.IdPhieu), -1);
                    TPhieuNhapxuatthuoc objTPhieuNhapxuatthuoc = TPhieuNhapxuatthuoc.FetchByID(IdPhieu);
                    if (objTPhieuNhapxuatthuoc != null)
                    {
                        ...
                        {
                            case ActionResult.Success:
                                Utility.SetMsg(uiStatusBar2.Panels["MSG"], "Bạn thực hiện hao phí nội trú xác nhận phiếu hao phí nội trú thuốc thành công", false);
                                Utility.ShowMsg(uiStatusBar2.Panels["MSG"].Text);
                                grdList.CurrentRow.BeginEdit();
        ...
        private void mnuUpdateIdThuockho_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Utility.ShowMsg("Phiếu đã chọn đã được xác nhận nên bạn không được phép cập nhật lại id thuốc kho của bất kỳ chi tiết nào thuộc phiếu. Muốn cập nhật id thuốc kho thì cần hao phí nội trú xác nhận phiếu");
                    return;
                }
