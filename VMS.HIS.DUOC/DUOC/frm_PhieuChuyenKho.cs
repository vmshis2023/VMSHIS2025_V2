...
namespace VNS.HIS.UI.THUOC
{
    public partial class frm_PhieuChuyenKho : Form
    {
        ...
        void setquyen()
        {
            try
            {
                cmdThemPhieuNhap.Visible = Utility.Coquyen("thuoc_phieuchuyenkho_themmoi");
                cmdUpdatePhieuNhap.Visible = Utility.Coquyen("thuoc_phieuchuyenkho_sua");
                cmdXoaPhieuNhap.Visible = Utility.Coquyen("thuoc_phieuchuyenkho_xoa");
                cmdNhapKho.Visible = Utility.Coquyen("thuoc_phieuchuyenkho_xacnhan");
                cmdHuychuyenkho.Visible = Utility.Coquyen("thuoc_phieuchuyenkho_huyxacnhan");
                cmdExIm.Visible = Utility.Coquyen("thuoc_phieuchuyenkho_ExportImport");
            }
            catch (Exception ex)
            {
                    ...
                    case ActionResult.Success:
                        ...
                        Utility.Log(this.Name, globalVariables.UserName,
                                        string.Format(
                                            ...
                                            id_PhieuNhap, khoxuat, khonhap), newaction.Delete, this.GetType().Assembly.ManifestModule.Name);
                        break;
        ...
        private void InitData()
        {
            ...
            if(m_dtKhoXuat.Rows.Count>1)
                DataBinding.BindDataCombobox(cboKhoXuat, m_dtKhoXuat,                                       TDmucKho.Columns.IdKho, TDmucKho.Columns.TenKho,                                       "---Kho xuất---",false);
            else
                DataBinding.BindDataCombobox(cboKhoXuat, m_dtKhoXuat,                                       TDmucKho.Columns.IdKho, TDmucKho.Columns.TenKho, "---Chọn---", true);
            DataBinding.BindDataCombobox(cboKhoNhap, m_dtKhoNhap,                                      TDmucKho.Columns.IdKho, TDmucKho.Columns.TenKho,                                      "---Kho nhập---", false);
        ...
        private void cmdNhapKho_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (objTPhieuNhapxuatthuoc != null)
                {
                    ...
                    {
                        Utility.ShowMsg(string.Format("Phiếu bạn chọn đã được xác nhận bởi người dùng có tài khoản {0} vào lúc {1}. Ngày xác nhận được người dùng lựa chọn là {2}.\nVui lòng kiểm tra lại", objTPhieuNhapxuatthuoc.NguoiXacnhan, objTPhieuNhapxuatthuoc.NgaySua.Value.ToString("dd/MM/yyyy HH:mm:ss"), objTPhieuNhapxuatthuoc.NgayXacnhan.Value.ToString("dd/MM/yyyy HH:mm:ss")));
                        return;
                    }
                    if (Utility.sDbnull(objTPhieuNhapxuatthuoc.NguoiTao) != globalVariables.UserName && THU_VIEN_CHUNG.Laygiatrithamsohethong_off("THUOC_PHANQUYEN_USER", "1", false) == "1")
                    {
                    ...
                    ActionResult actionResult = new XuatThuoc().XacNhanPhieuXuatKho(objTPhieuNhapxuatthuoc, _ngayxacnhan, ref errMsg);
                    switch (actionResult)
                    {
                        case ActionResult.Success:
            ...
            try
            {
                ...
                if (Utility.AcceptQuestion("Bạn có chắc chắn muốn hủy xác nhận phiếu chuyển kho đang chọn hay không?", "Thông báo", true))
                {
                    ...
                    if (objTPhieuNhapxuatthuoc != null)
                    {
                        ...
                        {
                            Utility.ShowMsg(string.Format("Phiếu bạn chọn đang ở trạng thái chưa xác nhận. Thông tin ghi nhận hành động lần cuối là {0} ",objTPhieuNhapxuatthuoc.LastActionName));
                            return;
                        }
                        ...
                        switch (actionResult)
                        {
                            case ActionResult.Success:
                                ...
                                Utility.Log(this.Name, globalVariables.UserName,
                                        string.Format(
                                            "Xóa phiếu chuyển kho với số phiếu là :{0} - Từ kho {1} đến kho {2}",
                                            id_PhieuNhap, khoxuat, khonhap), newaction.Update, this.GetType().Assembly.ManifestModule.Name);
                                break;
                            case ActionResult.notExists:
                                Utility.ShowMsg("Không tìm thấy id thuốc kho xuất để cộng số lượng hủy. Vui lòng kiểm tra lại\n" + errMsg, "Thông báo lỗi", MessageBoxIcon.Error);
                                break;
                            case ActionResult.Exceed:
                            ...
                            case ActionResult.DataUsed:
                                Utility.ShowMsg("" + ten_kieuthuoc_vt + " nhập từ phiếu này đã được sử dụng nên bạn không thể hủy xác nhận phiếu\n" + errMsg, "Thông báo lỗi", MessageBoxIcon.Error);
                                break;
                            case ActionResult.NotEnoughDrugInStock:
                                Utility.ShowMsg("" + ten_kieuthuoc_vt + " trong phiếu đã được sử dụng và không còn đủ số lượng để hủy nên bạn không thể hủy xác nhận phiếu\n" + errMsg, "Thông báo lỗi", MessageBoxIcon.Error);
                                break;
                            case ActionResult.Error:
                                break;
                        }
