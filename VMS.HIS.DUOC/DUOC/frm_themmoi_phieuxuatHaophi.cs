...
{
    public partial class frm_themmoi_phieuxuatHaophi : Form
    {
        ...
        string KHOA_KHO = "KHOA";
        private DataTable m_PhieuDuTru = new DataTable();
        bool b_Hasloaded=false;
        string SplitterPath = "";
        public frm_themmoi_phieuxuatHaophi(string KHOA_KHO)
        {
            InitializeComponent();
            SplitterPath = string.Format(@"{0}\{1}.splitter", Application.StartupPath, this.Name);
            this.Shown += frm_themmoi_phieuxuatHaophi_Shown;
            this.FormClosing += frm_themmoi_phieuxuatHaophi_FormClosing;
          
            this.KHOA_KHO = KHOA_KHO;
        ...
        void frm_themmoi_phieuxuatHaophi_FormClosing(object sender, FormClosingEventArgs e)
        {
            Utility.SaveValue2Lines(SplitterPath, new List<string>() { splitContainer1.SplitterDistance.ToString() });
        }

        void frm_themmoi_phieuxuatHaophi_Shown(object sender, EventArgs e)
        {
            Try2Splitter();
        ...
        void Initevents()
        {
            ...
            this.KeyDown += new KeyEventHandler(frm_themmoi_phieuxuatHaophi_KeyDown);
        ...
        void cboKhoalinh_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (!b_Hasloaded || KHOA_KHO=="KHOA") return;
        ...
        bool isValidDetailData()
        {
            ...
            if (txtthuoc.MyID == txtthuoc.DefaultID)
            {
                ...
                errorProvider1.SetError(txtthuoc, lblMsg.Text);
                txtthuoc.Focus();
                return false;
            }
            if (Utility.DecimaltoDbnull(txtSoluongdutru.Text, -1) <= 0)
            {
                ...
                errorProvider1.SetError(txtSoluongdutru, lblMsg.Text);
                txtSoluongdutru.Focus();
                return false;
            }
        ...
        void frm_themmoi_phieuxuatHaophi_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void ModifyCommand()
        {
            ...
            finally
            {
                ...
                {
                    cmdPrevius.Enabled = cmdNext.Enabled =  cmdTaoNhanh.Enabled = cmdSave.Enabled = cmdAddDetail.Enabled = false;
                }
            }
            //TinhSumThanhTien();
        }
        ...
        private void LoadKho()
        {
            if (KIEU_THUOC_VT == "THUOC")
            {
                ...
                m_dtKhoNhap = CommonLoadDuoc.LAYDANHMUCKHO(-1, "ALL", "THUOC,THUOCVT", "CHANLE,LE", 100, 100, 1);// CommonLoadDuoc.LAYTHONGTIN_TUTHUOC();
            }
            else
            {
                m_dtKhoXuat = CommonLoadDuoc.LAYDANHMUCKHO(-1, "ALL", "VT,THUOCVT", "CHANLE,LE", 0, 100, 1);// CommonLoadDuoc.LAYTHONGTIN_KHOVTTH_LE_NOITRU();
                m_dtKhoNhap = CommonLoadDuoc.LAYDANHMUCKHO(-1, "ALL", "VT,THUOCVT", "CHANLE,LE", 100, 100, 1);//  CommonLoadDuoc.LAYTHONGTIN_TUVTTH();
            }
            DataBinding.BindDataCombobox(cboKhonhan, m_dtKhoNhap,
                                          TDmucKho.Columns.IdKho, TDmucKho.Columns.TenKho,
                                          "---Chọn kho nhận---", true);
        ...
        void AutoLoadMainData(DataTable dtMainData)
        {
            ...
            dtNgayNhap.Value = Convert.ToDateTime(dtMainData.Rows[0][ TPhieuNhapxuatthuoc.Columns.NgayHoadon]);
            cboKhoalinh.SelectedValue = Utility.sDbnull(dtMainData.Rows[0][ TPhieuNhapxuatthuoc.Columns.IdKhoalinh]);
            cboKhoXuat.SelectedValue = Utility.sDbnull(dtMainData.Rows[0][ TPhieuNhapxuatthuoc.Columns.IdKhoxuat]);
            cboKhonhan.SelectedValue = Utility.sDbnull(dtMainData.Rows[0][ TPhieuNhapxuatthuoc.Columns.IdKhonhap]);
            cboNhanVien.SelectedValue = Utility.sDbnull(dtMainData.Rows[0][ TPhieuNhapxuatthuoc.Columns.IdNhanvien]);
            ...
            if (m_enAction == action.Update || m_enAction == action.View)
            {
                ...
                if (objPhieuNhap != null)
                {
                    ...
                    cboKhoalinh.SelectedValue = Utility.sDbnull(objPhieuNhap.IdKhoalinh);
                        ...
                        cboKhonhan.SelectedValue = Utility.sDbnull(objPhieuNhap.IdKhonhap);
        ...
        private void frm_themmoi_phieuxuatHaophi_Load(object sender, EventArgs e)
        {
        ...
        private TPhieuNhapxuatthuoc CreatePhieuNhapKho()
        {
            ...
            objTPhieuNhapxuatthuoc.IdKhonhap = KHOA_KHO == "KHO" ? Utility.Int16Dbnull(cboKhonhan.SelectedValue, -1) : (short)-1;
            ...
            objTPhieuNhapxuatthuoc.MotaThem = "Xuất thuốc khoa phòng chức năng";
            ...
            objTPhieuNhapxuatthuoc.LoaiPhieu = (byte)LoaiPhieu.PhieuXuatKhoaPhongchucnang;
            objTPhieuNhapxuatthuoc.TenLoaiphieu = Utility.TenLoaiPhieu(LoaiPhieu.PhieuXuatKhoaPhongchucnang);
        ...
        private bool InValiNhapKho()
        {
            ...
            if (cboKhoalinh.SelectedValue.ToString() == "-1")
            {
                ...
                errorProvider1.SetError(cboKhoalinh, "Bạn phải chọn khoa lĩnh");
                cboKhoalinh.Focus();
                return false;
            }
            if (KHOA_KHO == "KHO")
            ...
            if (m_enAction == action.Insert)
            {
                ...
                {
                    Utility.SetMsg(lblMsg, "Chưa có thuốc-VT xuất khoa phòng chức năng. Vui lòng chọn ít nhất 1 thuốc-VT để xuất", true);
                    errorProvider1.SetError(cmdNext, lblMsg.Text);
                    cmdNext.Focus();
                    return false;
                }
            ...
            foreach (Janus.Windows.GridEX.GridEXRow gridExRow in grdKhoXuat.GetDataRows())
            {
                if (gridExRow.RowType == RowType.Record)
                {
                    ...
                    if (query.Any())
                    {
                        ...
                        gridExRow.BeginEdit();
                        gridExRow.Cells["SO_LUONG"].Value = soluongthat  - soluongao;
                        gridExRow.Cells["SO_LUONG_CHUYEN"].Value = 0;
                        gridExRow.EndEdit();
                        grdKhoXuat.UpdateData();
                        m_dtDataThuocKho.AcceptChanges();
