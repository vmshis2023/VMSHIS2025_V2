...
namespace VMS.Helpers
{
    public class Utility
    {
        ...
        public static void AddOrUpdateSetting(string fileName, IConfiguration configuration, string env, string sectionPathKey, string value)
        {
            try
            {
                var filePath = string.Format(@"{0}\{1}.json", env.ToLower() == "development" ? Directory.GetCurrentDirectory() : AppContext.BaseDirectory, fileName);
        ...
        public static void AddSetting(string fileName, IConfiguration configuration, string env, string sectionPathKey, string value)
        {
            try
            {
                var filePath = string.Format(@"{0}\{1}.json", env.ToLower() == "development" ? Directory.GetCurrentDirectory() : AppContext.BaseDirectory, fileName);
