using System; 
using System.Text; 
using System.Data;
using System.Data.SqlClient;
using System.Data.Common;
using System.Collections;
using System.Collections.Generic;
using System.ComponentModel;
using System.Configuration; 
using System.Xml; 
using System.Xml.Serialization;
using SubSonic; 
using SubSonic.Utilities;
// <auto-generated />
namespace VMS.HIS.DAL
{
	/// <summary>
	/// Strongly-typed collection for the KcbDonthuocChitiet class.
	/// </summary>
    [Serializable]
	public partial class KcbDonthuocChitietCollection : ActiveList<KcbDonthuocChitiet, KcbDonthuocChitietCollection>
	{	   
		public KcbDonthuocChitietCollection() {}
        
        /// <summary>
		/// Filters an existing collection based on the set criteria. This is an in-memory filter
		/// Thanks to developingchris for this!
        /// </summary>
        /// <returns>KcbDonthuocChitietCollection</returns>
		public KcbDonthuocChitietCollection Filter()
        {
            for (int i = this.Count - 1; i > -1; i--)
            {
                KcbDonthuocChitiet o = this[i];
                foreach (SubSonic.Where w in this.wheres)
                {
                    bool remove = false;
                    System.Reflection.PropertyInfo pi = o.GetType().GetProperty(w.ColumnName);
                    if (pi.CanRead)
                    {
                        object val = pi.GetValue(o, null);
                        switch (w.Comparison)
                        {
                            case SubSonic.Comparison.Equals:
                                if (!val.Equals(w.ParameterValue))
                                {
                                    remove = true;
                                }
                                break;
                        }
                    }
                    if (remove)
                    {
                        this.Remove(o);
                        break;
                    }
                }
            }
            return this;
        }
		
		
	}
	/// <summary>
	/// This is an ActiveRecord class which wraps the kcb_donthuoc_chitiet table.
	/// </summary>
	[Serializable]
	public partial class KcbDonthuocChitiet : ActiveRecord<KcbDonthuocChitiet>, IActiveRecord
	{
		#region .ctors and Default Settings
		
		public KcbDonthuocChitiet()
		{
		  SetSQLProps();
		  InitSetDefaults();
		  MarkNew();
		}
		
		private void InitSetDefaults() { SetDefaults(); }
		
		public KcbDonthuocChitiet(bool useDatabaseDefaults)
		{
			SetSQLProps();
			if(useDatabaseDefaults)
				ForceDefaults();
			MarkNew();
		}
        
		public KcbDonthuocChitiet(object keyID)
		{
			SetSQLProps();
			InitSetDefaults();
			LoadByKey(keyID);
		}
		 
		public KcbDonthuocChitiet(string columnName, object columnValue)
		{
			SetSQLProps();
			InitSetDefaults();
			LoadByParam(columnName,columnValue);
		}
		
		protected static void SetSQLProps() { GetTableSchema(); }
		
		#endregion
		
		#region Schema and Query Accessor	
		public static Query CreateQuery() { return new Query(Schema); }
		public static TableSchema.Table Schema
		{
			get
			{
				if (BaseSchema == null)
					SetSQLProps();
				return BaseSchema;
			}
		}
		
		private static void GetTableSchema() 
		{
			if(!IsSchemaInitialized)
			{
				//Schema declaration
				TableSchema.Table schema = new TableSchema.Table("kcb_donthuoc_chitiet", TableType.Table, DataService.GetInstance("ORM"));
				schema.Columns = new TableSchema.TableColumnCollection();
				schema.SchemaName = @"dbo";
				//columns
				
				TableSchema.TableColumn colvarIdChitietdonthuoc = new TableSchema.TableColumn(schema);
				colvarIdChitietdonthuoc.ColumnName = "id_chitietdonthuoc";
				colvarIdChitietdonthuoc.DataType = DbType.Int64;
				colvarIdChitietdonthuoc.MaxLength = 0;
				colvarIdChitietdonthuoc.AutoIncrement = true;
				colvarIdChitietdonthuoc.IsNullable = false;
				colvarIdChitietdonthuoc.IsPrimaryKey = true;
				colvarIdChitietdonthuoc.IsForeignKey = false;
				colvarIdChitietdonthuoc.IsReadOnly = false;
				colvarIdChitietdonthuoc.DefaultSetting = @"";
				colvarIdChitietdonthuoc.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdChitietdonthuoc);
				
				TableSchema.TableColumn colvarIdDonthuoc = new TableSchema.TableColumn(schema);
				colvarIdDonthuoc.ColumnName = "id_donthuoc";
				colvarIdDonthuoc.DataType = DbType.Int64;
				colvarIdDonthuoc.MaxLength = 0;
				colvarIdDonthuoc.AutoIncrement = false;
				colvarIdDonthuoc.IsNullable = false;
				colvarIdDonthuoc.IsPrimaryKey = false;
				colvarIdDonthuoc.IsForeignKey = false;
				colvarIdDonthuoc.IsReadOnly = false;
				colvarIdDonthuoc.DefaultSetting = @"";
				colvarIdDonthuoc.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdDonthuoc);
				
				TableSchema.TableColumn colvarIdDonthuocChuyengoi = new TableSchema.TableColumn(schema);
				colvarIdDonthuocChuyengoi.ColumnName = "id_donthuoc_chuyengoi";
				colvarIdDonthuocChuyengoi.DataType = DbType.Int64;
				colvarIdDonthuocChuyengoi.MaxLength = 0;
				colvarIdDonthuocChuyengoi.AutoIncrement = false;
				colvarIdDonthuocChuyengoi.IsNullable = true;
				colvarIdDonthuocChuyengoi.IsPrimaryKey = false;
				colvarIdDonthuocChuyengoi.IsForeignKey = false;
				colvarIdDonthuocChuyengoi.IsReadOnly = false;
				colvarIdDonthuocChuyengoi.DefaultSetting = @"";
				colvarIdDonthuocChuyengoi.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdDonthuocChuyengoi);
				
				TableSchema.TableColumn colvarIdBenhnhan = new TableSchema.TableColumn(schema);
				colvarIdBenhnhan.ColumnName = "id_benhnhan";
				colvarIdBenhnhan.DataType = DbType.Int64;
				colvarIdBenhnhan.MaxLength = 0;
				colvarIdBenhnhan.AutoIncrement = false;
				colvarIdBenhnhan.IsNullable = true;
				colvarIdBenhnhan.IsPrimaryKey = false;
				colvarIdBenhnhan.IsForeignKey = false;
				colvarIdBenhnhan.IsReadOnly = false;
				colvarIdBenhnhan.DefaultSetting = @"";
				colvarIdBenhnhan.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdBenhnhan);
				
				TableSchema.TableColumn colvarMaLuotkham = new TableSchema.TableColumn(schema);
				colvarMaLuotkham.ColumnName = "ma_luotkham";
				colvarMaLuotkham.DataType = DbType.String;
				colvarMaLuotkham.MaxLength = 10;
				colvarMaLuotkham.AutoIncrement = false;
				colvarMaLuotkham.IsNullable = true;
				colvarMaLuotkham.IsPrimaryKey = false;
				colvarMaLuotkham.IsForeignKey = false;
				colvarMaLuotkham.IsReadOnly = false;
				colvarMaLuotkham.DefaultSetting = @"";
				colvarMaLuotkham.ForeignKeyTableName = "";
				schema.Columns.Add(colvarMaLuotkham);
				
				TableSchema.TableColumn colvarIdKham = new TableSchema.TableColumn(schema);
				colvarIdKham.ColumnName = "id_kham";
				colvarIdKham.DataType = DbType.Int64;
				colvarIdKham.MaxLength = 0;
				colvarIdKham.AutoIncrement = false;
				colvarIdKham.IsNullable = true;
				colvarIdKham.IsPrimaryKey = false;
				colvarIdKham.IsForeignKey = false;
				colvarIdKham.IsReadOnly = false;
				colvarIdKham.DefaultSetting = @"";
				colvarIdKham.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdKham);
				
				TableSchema.TableColumn colvarIdKho = new TableSchema.TableColumn(schema);
				colvarIdKho.ColumnName = "id_kho";
				colvarIdKho.DataType = DbType.Int32;
				colvarIdKho.MaxLength = 0;
				colvarIdKho.AutoIncrement = false;
				colvarIdKho.IsNullable = true;
				colvarIdKho.IsPrimaryKey = false;
				colvarIdKho.IsForeignKey = false;
				colvarIdKho.IsReadOnly = false;
				colvarIdKho.DefaultSetting = @"";
				colvarIdKho.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdKho);
				
				TableSchema.TableColumn colvarIdThuoc = new TableSchema.TableColumn(schema);
				colvarIdThuoc.ColumnName = "id_thuoc";
				colvarIdThuoc.DataType = DbType.Int32;
				colvarIdThuoc.MaxLength = 0;
				colvarIdThuoc.AutoIncrement = false;
				colvarIdThuoc.IsNullable = false;
				colvarIdThuoc.IsPrimaryKey = false;
				colvarIdThuoc.IsForeignKey = false;
				colvarIdThuoc.IsReadOnly = false;
				colvarIdThuoc.DefaultSetting = @"";
				colvarIdThuoc.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdThuoc);
				
				TableSchema.TableColumn colvarNgayHethan = new TableSchema.TableColumn(schema);
				colvarNgayHethan.ColumnName = "ngay_hethan";
				colvarNgayHethan.DataType = DbType.DateTime;
				colvarNgayHethan.MaxLength = 0;
				colvarNgayHethan.AutoIncrement = false;
				colvarNgayHethan.IsNullable = false;
				colvarNgayHethan.IsPrimaryKey = false;
				colvarNgayHethan.IsForeignKey = false;
				colvarNgayHethan.IsReadOnly = false;
				colvarNgayHethan.DefaultSetting = @"";
				colvarNgayHethan.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNgayHethan);
				
				TableSchema.TableColumn colvarSoLuong = new TableSchema.TableColumn(schema);
				colvarSoLuong.ColumnName = "so_luong";
				colvarSoLuong.DataType = DbType.Decimal;
				colvarSoLuong.MaxLength = 0;
				colvarSoLuong.AutoIncrement = false;
				colvarSoLuong.IsNullable = false;
				colvarSoLuong.IsPrimaryKey = false;
				colvarSoLuong.IsForeignKey = false;
				colvarSoLuong.IsReadOnly = false;
				colvarSoLuong.DefaultSetting = @"";
				colvarSoLuong.ForeignKeyTableName = "";
				schema.Columns.Add(colvarSoLuong);
				
				TableSchema.TableColumn colvarSluongSua = new TableSchema.TableColumn(schema);
				colvarSluongSua.ColumnName = "sluong_sua";
				colvarSluongSua.DataType = DbType.Decimal;
				colvarSluongSua.MaxLength = 0;
				colvarSluongSua.AutoIncrement = false;
				colvarSluongSua.IsNullable = true;
				colvarSluongSua.IsPrimaryKey = false;
				colvarSluongSua.IsForeignKey = false;
				colvarSluongSua.IsReadOnly = false;
				
						colvarSluongSua.DefaultSetting = @"((0))";
				colvarSluongSua.ForeignKeyTableName = "";
				schema.Columns.Add(colvarSluongSua);
				
				TableSchema.TableColumn colvarSluongLinh = new TableSchema.TableColumn(schema);
				colvarSluongLinh.ColumnName = "sluong_linh";
				colvarSluongLinh.DataType = DbType.Decimal;
				colvarSluongLinh.MaxLength = 0;
				colvarSluongLinh.AutoIncrement = false;
				colvarSluongLinh.IsNullable = true;
				colvarSluongLinh.IsPrimaryKey = false;
				colvarSluongLinh.IsForeignKey = false;
				colvarSluongLinh.IsReadOnly = false;
				
						colvarSluongLinh.DefaultSetting = @"((0))";
				colvarSluongLinh.ForeignKeyTableName = "";
				schema.Columns.Add(colvarSluongLinh);
				
				TableSchema.TableColumn colvarDonGia = new TableSchema.TableColumn(schema);
				colvarDonGia.ColumnName = "don_gia";
				colvarDonGia.DataType = DbType.Decimal;
				colvarDonGia.MaxLength = 0;
				colvarDonGia.AutoIncrement = false;
				colvarDonGia.IsNullable = false;
				colvarDonGia.IsPrimaryKey = false;
				colvarDonGia.IsForeignKey = false;
				colvarDonGia.IsReadOnly = false;
				
						colvarDonGia.DefaultSetting = @"((0))";
				colvarDonGia.ForeignKeyTableName = "";
				schema.Columns.Add(colvarDonGia);
				
				TableSchema.TableColumn colvarIdThuockho = new TableSchema.TableColumn(schema);
				colvarIdThuockho.ColumnName = "id_thuockho";
				colvarIdThuockho.DataType = DbType.Int64;
				colvarIdThuockho.MaxLength = 0;
				colvarIdThuockho.AutoIncrement = false;
				colvarIdThuockho.IsNullable = true;
				colvarIdThuockho.IsPrimaryKey = false;
				colvarIdThuockho.IsForeignKey = false;
				colvarIdThuockho.IsReadOnly = false;
				colvarIdThuockho.DefaultSetting = @"";
				colvarIdThuockho.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdThuockho);
				
				TableSchema.TableColumn colvarNgayNhap = new TableSchema.TableColumn(schema);
				colvarNgayNhap.ColumnName = "ngay_nhap";
				colvarNgayNhap.DataType = DbType.DateTime;
				colvarNgayNhap.MaxLength = 0;
				colvarNgayNhap.AutoIncrement = false;
				colvarNgayNhap.IsNullable = true;
				colvarNgayNhap.IsPrimaryKey = false;
				colvarNgayNhap.IsForeignKey = false;
				colvarNgayNhap.IsReadOnly = false;
				colvarNgayNhap.DefaultSetting = @"";
				colvarNgayNhap.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNgayNhap);
				
				TableSchema.TableColumn colvarGiaNhap = new TableSchema.TableColumn(schema);
				colvarGiaNhap.ColumnName = "gia_nhap";
				colvarGiaNhap.DataType = DbType.Decimal;
				colvarGiaNhap.MaxLength = 0;
				colvarGiaNhap.AutoIncrement = false;
				colvarGiaNhap.IsNullable = true;
				colvarGiaNhap.IsPrimaryKey = false;
				colvarGiaNhap.IsForeignKey = false;
				colvarGiaNhap.IsReadOnly = false;
				colvarGiaNhap.DefaultSetting = @"";
				colvarGiaNhap.ForeignKeyTableName = "";
				schema.Columns.Add(colvarGiaNhap);
				
				TableSchema.TableColumn colvarGiaBan = new TableSchema.TableColumn(schema);
				colvarGiaBan.ColumnName = "gia_ban";
				colvarGiaBan.DataType = DbType.Decimal;
				colvarGiaBan.MaxLength = 0;
				colvarGiaBan.AutoIncrement = false;
				colvarGiaBan.IsNullable = true;
				colvarGiaBan.IsPrimaryKey = false;
				colvarGiaBan.IsForeignKey = false;
				colvarGiaBan.IsReadOnly = false;
				colvarGiaBan.DefaultSetting = @"";
				colvarGiaBan.ForeignKeyTableName = "";
				schema.Columns.Add(colvarGiaBan);
				
				TableSchema.TableColumn colvarGiaBhyt = new TableSchema.TableColumn(schema);
				colvarGiaBhyt.ColumnName = "gia_bhyt";
				colvarGiaBhyt.DataType = DbType.Decimal;
				colvarGiaBhyt.MaxLength = 0;
				colvarGiaBhyt.AutoIncrement = false;
				colvarGiaBhyt.IsNullable = true;
				colvarGiaBhyt.IsPrimaryKey = false;
				colvarGiaBhyt.IsForeignKey = false;
				colvarGiaBhyt.IsReadOnly = false;
				colvarGiaBhyt.DefaultSetting = @"";
				colvarGiaBhyt.ForeignKeyTableName = "";
				schema.Columns.Add(colvarGiaBhyt);
				
				TableSchema.TableColumn colvarSoLo = new TableSchema.TableColumn(schema);
				colvarSoLo.ColumnName = "so_lo";
				colvarSoLo.DataType = DbType.String;
				colvarSoLo.MaxLength = 20;
				colvarSoLo.AutoIncrement = false;
				colvarSoLo.IsNullable = true;
				colvarSoLo.IsPrimaryKey = false;
				colvarSoLo.IsForeignKey = false;
				colvarSoLo.IsReadOnly = false;
				colvarSoLo.DefaultSetting = @"";
				colvarSoLo.ForeignKeyTableName = "";
				schema.Columns.Add(colvarSoLo);
				
				TableSchema.TableColumn colvarVat = new TableSchema.TableColumn(schema);
				colvarVat.ColumnName = "vat";
				colvarVat.DataType = DbType.Decimal;
				colvarVat.MaxLength = 0;
				colvarVat.AutoIncrement = false;
				colvarVat.IsNullable = true;
				colvarVat.IsPrimaryKey = false;
				colvarVat.IsForeignKey = false;
				colvarVat.IsReadOnly = false;
				colvarVat.DefaultSetting = @"";
				colvarVat.ForeignKeyTableName = "";
				schema.Columns.Add(colvarVat);
				
				TableSchema.TableColumn colvarMaNhacungcap = new TableSchema.TableColumn(schema);
				colvarMaNhacungcap.ColumnName = "ma_nhacungcap";
				colvarMaNhacungcap.DataType = DbType.String;
				colvarMaNhacungcap.MaxLength = 20;
				colvarMaNhacungcap.AutoIncrement = false;
				colvarMaNhacungcap.IsNullable = true;
				colvarMaNhacungcap.IsPrimaryKey = false;
				colvarMaNhacungcap.IsForeignKey = false;
				colvarMaNhacungcap.IsReadOnly = false;
				colvarMaNhacungcap.DefaultSetting = @"";
				colvarMaNhacungcap.ForeignKeyTableName = "";
				schema.Columns.Add(colvarMaNhacungcap);
				
				TableSchema.TableColumn colvarPhuThu = new TableSchema.TableColumn(schema);
				colvarPhuThu.ColumnName = "phu_thu";
				colvarPhuThu.DataType = DbType.Decimal;
				colvarPhuThu.MaxLength = 0;
				colvarPhuThu.AutoIncrement = false;
				colvarPhuThu.IsNullable = true;
				colvarPhuThu.IsPrimaryKey = false;
				colvarPhuThu.IsForeignKey = false;
				colvarPhuThu.IsReadOnly = false;
				
						colvarPhuThu.DefaultSetting = @"((0))";
				colvarPhuThu.ForeignKeyTableName = "";
				schema.Columns.Add(colvarPhuThu);
				
				TableSchema.TableColumn colvarPhuthuDungtuyen = new TableSchema.TableColumn(schema);
				colvarPhuthuDungtuyen.ColumnName = "phuthu_dungtuyen";
				colvarPhuthuDungtuyen.DataType = DbType.Decimal;
				colvarPhuthuDungtuyen.MaxLength = 0;
				colvarPhuthuDungtuyen.AutoIncrement = false;
				colvarPhuthuDungtuyen.IsNullable = true;
				colvarPhuthuDungtuyen.IsPrimaryKey = false;
				colvarPhuthuDungtuyen.IsForeignKey = false;
				colvarPhuthuDungtuyen.IsReadOnly = false;
				colvarPhuthuDungtuyen.DefaultSetting = @"";
				colvarPhuthuDungtuyen.ForeignKeyTableName = "";
				schema.Columns.Add(colvarPhuthuDungtuyen);
				
				TableSchema.TableColumn colvarPhuthuTraituyen = new TableSchema.TableColumn(schema);
				colvarPhuthuTraituyen.ColumnName = "phuthu_traituyen";
				colvarPhuthuTraituyen.DataType = DbType.Decimal;
				colvarPhuthuTraituyen.MaxLength = 0;
				colvarPhuthuTraituyen.AutoIncrement = false;
				colvarPhuthuTraituyen.IsNullable = true;
				colvarPhuthuTraituyen.IsPrimaryKey = false;
				colvarPhuthuTraituyen.IsForeignKey = false;
				colvarPhuthuTraituyen.IsReadOnly = false;
				colvarPhuthuTraituyen.DefaultSetting = @"";
				colvarPhuthuTraituyen.ForeignKeyTableName = "";
				schema.Columns.Add(colvarPhuthuTraituyen);
				
				TableSchema.TableColumn colvarMotaThem = new TableSchema.TableColumn(schema);
				colvarMotaThem.ColumnName = "mota_them";
				colvarMotaThem.DataType = DbType.String;
				colvarMotaThem.MaxLength = 500;
				colvarMotaThem.AutoIncrement = false;
				colvarMotaThem.IsNullable = true;
				colvarMotaThem.IsPrimaryKey = false;
				colvarMotaThem.IsForeignKey = false;
				colvarMotaThem.IsReadOnly = false;
				colvarMotaThem.DefaultSetting = @"";
				colvarMotaThem.ForeignKeyTableName = "";
				schema.Columns.Add(colvarMotaThem);
				
				TableSchema.TableColumn colvarSoluongHuy = new TableSchema.TableColumn(schema);
				colvarSoluongHuy.ColumnName = "soluong_huy";
				colvarSoluongHuy.DataType = DbType.Int32;
				colvarSoluongHuy.MaxLength = 0;
				colvarSoluongHuy.AutoIncrement = false;
				colvarSoluongHuy.IsNullable = true;
				colvarSoluongHuy.IsPrimaryKey = false;
				colvarSoluongHuy.IsForeignKey = false;
				colvarSoluongHuy.IsReadOnly = false;
				
						colvarSoluongHuy.DefaultSetting = @"((0))";
				colvarSoluongHuy.ForeignKeyTableName = "";
				schema.Columns.Add(colvarSoluongHuy);
				
				TableSchema.TableColumn colvarTrangthaiHuy = new TableSchema.TableColumn(schema);
				colvarTrangthaiHuy.ColumnName = "trangthai_huy";
				colvarTrangthaiHuy.DataType = DbType.Byte;
				colvarTrangthaiHuy.MaxLength = 0;
				colvarTrangthaiHuy.AutoIncrement = false;
				colvarTrangthaiHuy.IsNullable = true;
				colvarTrangthaiHuy.IsPrimaryKey = false;
				colvarTrangthaiHuy.IsForeignKey = false;
				colvarTrangthaiHuy.IsReadOnly = false;
				
						colvarTrangthaiHuy.DefaultSetting = @"((0))";
				colvarTrangthaiHuy.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTrangthaiHuy);
				
				TableSchema.TableColumn colvarNguoiHuy = new TableSchema.TableColumn(schema);
				colvarNguoiHuy.ColumnName = "nguoi_huy";
				colvarNguoiHuy.DataType = DbType.String;
				colvarNguoiHuy.MaxLength = 20;
				colvarNguoiHuy.AutoIncrement = false;
				colvarNguoiHuy.IsNullable = true;
				colvarNguoiHuy.IsPrimaryKey = false;
				colvarNguoiHuy.IsForeignKey = false;
				colvarNguoiHuy.IsReadOnly = false;
				colvarNguoiHuy.DefaultSetting = @"";
				colvarNguoiHuy.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNguoiHuy);
				
				TableSchema.TableColumn colvarNgayHuy = new TableSchema.TableColumn(schema);
				colvarNgayHuy.ColumnName = "ngay_huy";
				colvarNgayHuy.DataType = DbType.DateTime;
				colvarNgayHuy.MaxLength = 0;
				colvarNgayHuy.AutoIncrement = false;
				colvarNgayHuy.IsNullable = true;
				colvarNgayHuy.IsPrimaryKey = false;
				colvarNgayHuy.IsForeignKey = false;
				colvarNgayHuy.IsReadOnly = false;
				colvarNgayHuy.DefaultSetting = @"";
				colvarNgayHuy.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNgayHuy);
				
				TableSchema.TableColumn colvarTuTuc = new TableSchema.TableColumn(schema);
				colvarTuTuc.ColumnName = "tu_tuc";
				colvarTuTuc.DataType = DbType.Byte;
				colvarTuTuc.MaxLength = 0;
				colvarTuTuc.AutoIncrement = false;
				colvarTuTuc.IsNullable = true;
				colvarTuTuc.IsPrimaryKey = false;
				colvarTuTuc.IsForeignKey = false;
				colvarTuTuc.IsReadOnly = false;
				
						colvarTuTuc.DefaultSetting = @"((0))";
				colvarTuTuc.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTuTuc);
				
				TableSchema.TableColumn colvarTrangThai = new TableSchema.TableColumn(schema);
				colvarTrangThai.ColumnName = "trang_thai";
				colvarTrangThai.DataType = DbType.Byte;
				colvarTrangThai.MaxLength = 0;
				colvarTrangThai.AutoIncrement = false;
				colvarTrangThai.IsNullable = true;
				colvarTrangThai.IsPrimaryKey = false;
				colvarTrangThai.IsForeignKey = false;
				colvarTrangThai.IsReadOnly = false;
				
						colvarTrangThai.DefaultSetting = @"((0))";
				colvarTrangThai.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTrangThai);
				
				TableSchema.TableColumn colvarTrangthaiTonghop = new TableSchema.TableColumn(schema);
				colvarTrangthaiTonghop.ColumnName = "trangthai_tonghop";
				colvarTrangthaiTonghop.DataType = DbType.Byte;
				colvarTrangthaiTonghop.MaxLength = 0;
				colvarTrangthaiTonghop.AutoIncrement = false;
				colvarTrangthaiTonghop.IsNullable = true;
				colvarTrangthaiTonghop.IsPrimaryKey = false;
				colvarTrangthaiTonghop.IsForeignKey = false;
				colvarTrangthaiTonghop.IsReadOnly = false;
				colvarTrangthaiTonghop.DefaultSetting = @"";
				colvarTrangthaiTonghop.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTrangthaiTonghop);
				
				TableSchema.TableColumn colvarNgayXacnhan = new TableSchema.TableColumn(schema);
				colvarNgayXacnhan.ColumnName = "ngay_xacnhan";
				colvarNgayXacnhan.DataType = DbType.DateTime;
				colvarNgayXacnhan.MaxLength = 0;
				colvarNgayXacnhan.AutoIncrement = false;
				colvarNgayXacnhan.IsNullable = true;
				colvarNgayXacnhan.IsPrimaryKey = false;
				colvarNgayXacnhan.IsForeignKey = false;
				colvarNgayXacnhan.IsReadOnly = false;
				colvarNgayXacnhan.DefaultSetting = @"";
				colvarNgayXacnhan.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNgayXacnhan);
				
				TableSchema.TableColumn colvarTrangthaiBhyt = new TableSchema.TableColumn(schema);
				colvarTrangthaiBhyt.ColumnName = "trangthai_bhyt";
				colvarTrangthaiBhyt.DataType = DbType.Byte;
				colvarTrangthaiBhyt.MaxLength = 0;
				colvarTrangthaiBhyt.AutoIncrement = false;
				colvarTrangthaiBhyt.IsNullable = true;
				colvarTrangthaiBhyt.IsPrimaryKey = false;
				colvarTrangthaiBhyt.IsForeignKey = false;
				colvarTrangthaiBhyt.IsReadOnly = false;
				
						colvarTrangthaiBhyt.DefaultSetting = @"((0))";
				colvarTrangthaiBhyt.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTrangthaiBhyt);
				
				TableSchema.TableColumn colvarSttIn = new TableSchema.TableColumn(schema);
				colvarSttIn.ColumnName = "stt_in";
				colvarSttIn.DataType = DbType.Int16;
				colvarSttIn.MaxLength = 0;
				colvarSttIn.AutoIncrement = false;
				colvarSttIn.IsNullable = true;
				colvarSttIn.IsPrimaryKey = false;
				colvarSttIn.IsForeignKey = false;
				colvarSttIn.IsReadOnly = false;
				colvarSttIn.DefaultSetting = @"";
				colvarSttIn.ForeignKeyTableName = "";
				schema.Columns.Add(colvarSttIn);
				
				TableSchema.TableColumn colvarMadoituongGia = new TableSchema.TableColumn(schema);
				colvarMadoituongGia.ColumnName = "madoituong_gia";
				colvarMadoituongGia.DataType = DbType.String;
				colvarMadoituongGia.MaxLength = 5;
				colvarMadoituongGia.AutoIncrement = false;
				colvarMadoituongGia.IsNullable = true;
				colvarMadoituongGia.IsPrimaryKey = false;
				colvarMadoituongGia.IsForeignKey = false;
				colvarMadoituongGia.IsReadOnly = false;
				colvarMadoituongGia.DefaultSetting = @"";
				colvarMadoituongGia.ForeignKeyTableName = "";
				schema.Columns.Add(colvarMadoituongGia);
				
				TableSchema.TableColumn colvarPtramBhytGoc = new TableSchema.TableColumn(schema);
				colvarPtramBhytGoc.ColumnName = "ptram_bhyt_goc";
				colvarPtramBhytGoc.DataType = DbType.Decimal;
				colvarPtramBhytGoc.MaxLength = 0;
				colvarPtramBhytGoc.AutoIncrement = false;
				colvarPtramBhytGoc.IsNullable = true;
				colvarPtramBhytGoc.IsPrimaryKey = false;
				colvarPtramBhytGoc.IsForeignKey = false;
				colvarPtramBhytGoc.IsReadOnly = false;
				
						colvarPtramBhytGoc.DefaultSetting = @"((0))";
				colvarPtramBhytGoc.ForeignKeyTableName = "";
				schema.Columns.Add(colvarPtramBhytGoc);
				
				TableSchema.TableColumn colvarPtramBhyt = new TableSchema.TableColumn(schema);
				colvarPtramBhyt.ColumnName = "ptram_bhyt";
				colvarPtramBhyt.DataType = DbType.Decimal;
				colvarPtramBhyt.MaxLength = 0;
				colvarPtramBhyt.AutoIncrement = false;
				colvarPtramBhyt.IsNullable = true;
				colvarPtramBhyt.IsPrimaryKey = false;
				colvarPtramBhyt.IsForeignKey = false;
				colvarPtramBhyt.IsReadOnly = false;
				
						colvarPtramBhyt.DefaultSetting = @"((0))";
				colvarPtramBhyt.ForeignKeyTableName = "";
				schema.Columns.Add(colvarPtramBhyt);
				
				TableSchema.TableColumn colvarBhytChitra = new TableSchema.TableColumn(schema);
				colvarBhytChitra.ColumnName = "bhyt_chitra";
				colvarBhytChitra.DataType = DbType.Decimal;
				colvarBhytChitra.MaxLength = 0;
				colvarBhytChitra.AutoIncrement = false;
				colvarBhytChitra.IsNullable = true;
				colvarBhytChitra.IsPrimaryKey = false;
				colvarBhytChitra.IsForeignKey = false;
				colvarBhytChitra.IsReadOnly = false;
				
						colvarBhytChitra.DefaultSetting = @"((0))";
				colvarBhytChitra.ForeignKeyTableName = "";
				schema.Columns.Add(colvarBhytChitra);
				
				TableSchema.TableColumn colvarBnhanChitra = new TableSchema.TableColumn(schema);
				colvarBnhanChitra.ColumnName = "bnhan_chitra";
				colvarBnhanChitra.DataType = DbType.Decimal;
				colvarBnhanChitra.MaxLength = 0;
				colvarBnhanChitra.AutoIncrement = false;
				colvarBnhanChitra.IsNullable = true;
				colvarBnhanChitra.IsPrimaryKey = false;
				colvarBnhanChitra.IsForeignKey = false;
				colvarBnhanChitra.IsReadOnly = false;
				
						colvarBnhanChitra.DefaultSetting = @"((0))";
				colvarBnhanChitra.ForeignKeyTableName = "";
				schema.Columns.Add(colvarBnhanChitra);
				
				TableSchema.TableColumn colvarMaDoituongKcb = new TableSchema.TableColumn(schema);
				colvarMaDoituongKcb.ColumnName = "ma_doituong_kcb";
				colvarMaDoituongKcb.DataType = DbType.AnsiString;
				colvarMaDoituongKcb.MaxLength = 50;
				colvarMaDoituongKcb.AutoIncrement = false;
				colvarMaDoituongKcb.IsNullable = true;
				colvarMaDoituongKcb.IsPrimaryKey = false;
				colvarMaDoituongKcb.IsForeignKey = false;
				colvarMaDoituongKcb.IsReadOnly = false;
				colvarMaDoituongKcb.DefaultSetting = @"";
				colvarMaDoituongKcb.ForeignKeyTableName = "";
				schema.Columns.Add(colvarMaDoituongKcb);
				
				TableSchema.TableColumn colvarIdThanhtoan = new TableSchema.TableColumn(schema);
				colvarIdThanhtoan.ColumnName = "id_thanhtoan";
				colvarIdThanhtoan.DataType = DbType.Int32;
				colvarIdThanhtoan.MaxLength = 0;
				colvarIdThanhtoan.AutoIncrement = false;
				colvarIdThanhtoan.IsNullable = true;
				colvarIdThanhtoan.IsPrimaryKey = false;
				colvarIdThanhtoan.IsForeignKey = false;
				colvarIdThanhtoan.IsReadOnly = false;
				colvarIdThanhtoan.DefaultSetting = @"";
				colvarIdThanhtoan.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdThanhtoan);
				
				TableSchema.TableColumn colvarTrangthaiThanhtoan = new TableSchema.TableColumn(schema);
				colvarTrangthaiThanhtoan.ColumnName = "trangthai_thanhtoan";
				colvarTrangthaiThanhtoan.DataType = DbType.Byte;
				colvarTrangthaiThanhtoan.MaxLength = 0;
				colvarTrangthaiThanhtoan.AutoIncrement = false;
				colvarTrangthaiThanhtoan.IsNullable = true;
				colvarTrangthaiThanhtoan.IsPrimaryKey = false;
				colvarTrangthaiThanhtoan.IsForeignKey = false;
				colvarTrangthaiThanhtoan.IsReadOnly = false;
				
						colvarTrangthaiThanhtoan.DefaultSetting = @"((0))";
				colvarTrangthaiThanhtoan.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTrangthaiThanhtoan);
				
				TableSchema.TableColumn colvarNgayThanhtoan = new TableSchema.TableColumn(schema);
				colvarNgayThanhtoan.ColumnName = "ngay_thanhtoan";
				colvarNgayThanhtoan.DataType = DbType.DateTime;
				colvarNgayThanhtoan.MaxLength = 0;
				colvarNgayThanhtoan.AutoIncrement = false;
				colvarNgayThanhtoan.IsNullable = true;
				colvarNgayThanhtoan.IsPrimaryKey = false;
				colvarNgayThanhtoan.IsForeignKey = false;
				colvarNgayThanhtoan.IsReadOnly = false;
				colvarNgayThanhtoan.DefaultSetting = @"";
				colvarNgayThanhtoan.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNgayThanhtoan);
				
				TableSchema.TableColumn colvarCachDung = new TableSchema.TableColumn(schema);
				colvarCachDung.ColumnName = "cach_dung";
				colvarCachDung.DataType = DbType.String;
				colvarCachDung.MaxLength = 255;
				colvarCachDung.AutoIncrement = false;
				colvarCachDung.IsNullable = true;
				colvarCachDung.IsPrimaryKey = false;
				colvarCachDung.IsForeignKey = false;
				colvarCachDung.IsReadOnly = false;
				colvarCachDung.DefaultSetting = @"";
				colvarCachDung.ForeignKeyTableName = "";
				schema.Columns.Add(colvarCachDung);
				
				TableSchema.TableColumn colvarChidanThem = new TableSchema.TableColumn(schema);
				colvarChidanThem.ColumnName = "chidan_them";
				colvarChidanThem.DataType = DbType.String;
				colvarChidanThem.MaxLength = 255;
				colvarChidanThem.AutoIncrement = false;
				colvarChidanThem.IsNullable = true;
				colvarChidanThem.IsPrimaryKey = false;
				colvarChidanThem.IsForeignKey = false;
				colvarChidanThem.IsReadOnly = false;
				colvarChidanThem.DefaultSetting = @"";
				colvarChidanThem.ForeignKeyTableName = "";
				schema.Columns.Add(colvarChidanThem);
				
				TableSchema.TableColumn colvarDonviTinh = new TableSchema.TableColumn(schema);
				colvarDonviTinh.ColumnName = "donvi_tinh";
				colvarDonviTinh.DataType = DbType.String;
				colvarDonviTinh.MaxLength = 20;
				colvarDonviTinh.AutoIncrement = false;
				colvarDonviTinh.IsNullable = true;
				colvarDonviTinh.IsPrimaryKey = false;
				colvarDonviTinh.IsForeignKey = false;
				colvarDonviTinh.IsReadOnly = false;
				colvarDonviTinh.DefaultSetting = @"";
				colvarDonviTinh.ForeignKeyTableName = "";
				schema.Columns.Add(colvarDonviTinh);
				
				TableSchema.TableColumn colvarSolanDung = new TableSchema.TableColumn(schema);
				colvarSolanDung.ColumnName = "solan_dung";
				colvarSolanDung.DataType = DbType.String;
				colvarSolanDung.MaxLength = 10;
				colvarSolanDung.AutoIncrement = false;
				colvarSolanDung.IsNullable = true;
				colvarSolanDung.IsPrimaryKey = false;
				colvarSolanDung.IsForeignKey = false;
				colvarSolanDung.IsReadOnly = false;
				colvarSolanDung.DefaultSetting = @"";
				colvarSolanDung.ForeignKeyTableName = "";
				schema.Columns.Add(colvarSolanDung);
				
				TableSchema.TableColumn colvarSoluongDung = new TableSchema.TableColumn(schema);
				colvarSoluongDung.ColumnName = "soluong_dung";
				colvarSoluongDung.DataType = DbType.String;
				colvarSoluongDung.MaxLength = 10;
				colvarSoluongDung.AutoIncrement = false;
				colvarSoluongDung.IsNullable = true;
				colvarSoluongDung.IsPrimaryKey = false;
				colvarSoluongDung.IsForeignKey = false;
				colvarSoluongDung.IsReadOnly = false;
				colvarSoluongDung.DefaultSetting = @"";
				colvarSoluongDung.ForeignKeyTableName = "";
				schema.Columns.Add(colvarSoluongDung);
				
				TableSchema.TableColumn colvarTrangthaiChuyen = new TableSchema.TableColumn(schema);
				colvarTrangthaiChuyen.ColumnName = "trangthai_chuyen";
				colvarTrangthaiChuyen.DataType = DbType.Byte;
				colvarTrangthaiChuyen.MaxLength = 0;
				colvarTrangthaiChuyen.AutoIncrement = false;
				colvarTrangthaiChuyen.IsNullable = true;
				colvarTrangthaiChuyen.IsPrimaryKey = false;
				colvarTrangthaiChuyen.IsForeignKey = false;
				colvarTrangthaiChuyen.IsReadOnly = false;
				
						colvarTrangthaiChuyen.DefaultSetting = @"((0))";
				colvarTrangthaiChuyen.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTrangthaiChuyen);
				
				TableSchema.TableColumn colvarNgayTao = new TableSchema.TableColumn(schema);
				colvarNgayTao.ColumnName = "ngay_tao";
				colvarNgayTao.DataType = DbType.DateTime;
				colvarNgayTao.MaxLength = 0;
				colvarNgayTao.AutoIncrement = false;
				colvarNgayTao.IsNullable = true;
				colvarNgayTao.IsPrimaryKey = false;
				colvarNgayTao.IsForeignKey = false;
				colvarNgayTao.IsReadOnly = false;
				colvarNgayTao.DefaultSetting = @"";
				colvarNgayTao.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNgayTao);
				
				TableSchema.TableColumn colvarNguoiTao = new TableSchema.TableColumn(schema);
				colvarNguoiTao.ColumnName = "nguoi_tao";
				colvarNguoiTao.DataType = DbType.String;
				colvarNguoiTao.MaxLength = 30;
				colvarNguoiTao.AutoIncrement = false;
				colvarNguoiTao.IsNullable = true;
				colvarNguoiTao.IsPrimaryKey = false;
				colvarNguoiTao.IsForeignKey = false;
				colvarNguoiTao.IsReadOnly = false;
				colvarNguoiTao.DefaultSetting = @"";
				colvarNguoiTao.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNguoiTao);
				
				TableSchema.TableColumn colvarNgaySua = new TableSchema.TableColumn(schema);
				colvarNgaySua.ColumnName = "ngay_sua";
				colvarNgaySua.DataType = DbType.DateTime;
				colvarNgaySua.MaxLength = 0;
				colvarNgaySua.AutoIncrement = false;
				colvarNgaySua.IsNullable = true;
				colvarNgaySua.IsPrimaryKey = false;
				colvarNgaySua.IsForeignKey = false;
				colvarNgaySua.IsReadOnly = false;
				colvarNgaySua.DefaultSetting = @"";
				colvarNgaySua.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNgaySua);
				
				TableSchema.TableColumn colvarNguoiSua = new TableSchema.TableColumn(schema);
				colvarNguoiSua.ColumnName = "nguoi_sua";
				colvarNguoiSua.DataType = DbType.String;
				colvarNguoiSua.MaxLength = 30;
				colvarNguoiSua.AutoIncrement = false;
				colvarNguoiSua.IsNullable = true;
				colvarNguoiSua.IsPrimaryKey = false;
				colvarNguoiSua.IsForeignKey = false;
				colvarNguoiSua.IsReadOnly = false;
				colvarNguoiSua.DefaultSetting = @"";
				colvarNguoiSua.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNguoiSua);
				
				TableSchema.TableColumn colvarTileChietkhau = new TableSchema.TableColumn(schema);
				colvarTileChietkhau.ColumnName = "tile_chietkhau";
				colvarTileChietkhau.DataType = DbType.Decimal;
				colvarTileChietkhau.MaxLength = 0;
				colvarTileChietkhau.AutoIncrement = false;
				colvarTileChietkhau.IsNullable = true;
				colvarTileChietkhau.IsPrimaryKey = false;
				colvarTileChietkhau.IsForeignKey = false;
				colvarTileChietkhau.IsReadOnly = false;
				
						colvarTileChietkhau.DefaultSetting = @"((0))";
				colvarTileChietkhau.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTileChietkhau);
				
				TableSchema.TableColumn colvarTienChietkhau = new TableSchema.TableColumn(schema);
				colvarTienChietkhau.ColumnName = "tien_chietkhau";
				colvarTienChietkhau.DataType = DbType.Decimal;
				colvarTienChietkhau.MaxLength = 0;
				colvarTienChietkhau.AutoIncrement = false;
				colvarTienChietkhau.IsNullable = true;
				colvarTienChietkhau.IsPrimaryKey = false;
				colvarTienChietkhau.IsForeignKey = false;
				colvarTienChietkhau.IsReadOnly = false;
				
						colvarTienChietkhau.DefaultSetting = @"((0))";
				colvarTienChietkhau.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTienChietkhau);
				
				TableSchema.TableColumn colvarKieuChietkhau = new TableSchema.TableColumn(schema);
				colvarKieuChietkhau.ColumnName = "kieu_chietkhau";
				colvarKieuChietkhau.DataType = DbType.String;
				colvarKieuChietkhau.MaxLength = 5;
				colvarKieuChietkhau.AutoIncrement = false;
				colvarKieuChietkhau.IsNullable = true;
				colvarKieuChietkhau.IsPrimaryKey = false;
				colvarKieuChietkhau.IsForeignKey = false;
				colvarKieuChietkhau.IsReadOnly = false;
				colvarKieuChietkhau.DefaultSetting = @"";
				colvarKieuChietkhau.ForeignKeyTableName = "";
				schema.Columns.Add(colvarKieuChietkhau);
				
				TableSchema.TableColumn colvarIdGoi = new TableSchema.TableColumn(schema);
				colvarIdGoi.ColumnName = "id_goi";
				colvarIdGoi.DataType = DbType.Int32;
				colvarIdGoi.MaxLength = 0;
				colvarIdGoi.AutoIncrement = false;
				colvarIdGoi.IsNullable = true;
				colvarIdGoi.IsPrimaryKey = false;
				colvarIdGoi.IsForeignKey = false;
				colvarIdGoi.IsReadOnly = false;
				colvarIdGoi.DefaultSetting = @"";
				colvarIdGoi.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdGoi);
				
				TableSchema.TableColumn colvarTrongGoi = new TableSchema.TableColumn(schema);
				colvarTrongGoi.ColumnName = "trong_goi";
				colvarTrongGoi.DataType = DbType.Byte;
				colvarTrongGoi.MaxLength = 0;
				colvarTrongGoi.AutoIncrement = false;
				colvarTrongGoi.IsNullable = true;
				colvarTrongGoi.IsPrimaryKey = false;
				colvarTrongGoi.IsForeignKey = false;
				colvarTrongGoi.IsReadOnly = false;
				colvarTrongGoi.DefaultSetting = @"";
				colvarTrongGoi.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTrongGoi);
				
				TableSchema.TableColumn colvarKieuBiendong = new TableSchema.TableColumn(schema);
				colvarKieuBiendong.ColumnName = "kieu_biendong";
				colvarKieuBiendong.DataType = DbType.String;
				colvarKieuBiendong.MaxLength = 30;
				colvarKieuBiendong.AutoIncrement = false;
				colvarKieuBiendong.IsNullable = true;
				colvarKieuBiendong.IsPrimaryKey = false;
				colvarKieuBiendong.IsForeignKey = false;
				colvarKieuBiendong.IsReadOnly = false;
				colvarKieuBiendong.DefaultSetting = @"";
				colvarKieuBiendong.ForeignKeyTableName = "";
				schema.Columns.Add(colvarKieuBiendong);
				
				TableSchema.TableColumn colvarNguonThanhtoan = new TableSchema.TableColumn(schema);
				colvarNguonThanhtoan.ColumnName = "nguon_thanhtoan";
				colvarNguonThanhtoan.DataType = DbType.Byte;
				colvarNguonThanhtoan.MaxLength = 0;
				colvarNguonThanhtoan.AutoIncrement = false;
				colvarNguonThanhtoan.IsNullable = true;
				colvarNguonThanhtoan.IsPrimaryKey = false;
				colvarNguonThanhtoan.IsForeignKey = false;
				colvarNguonThanhtoan.IsReadOnly = false;
				colvarNguonThanhtoan.DefaultSetting = @"";
				colvarNguonThanhtoan.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNguonThanhtoan);
				
				TableSchema.TableColumn colvarIpMaytao = new TableSchema.TableColumn(schema);
				colvarIpMaytao.ColumnName = "ip_maytao";
				colvarIpMaytao.DataType = DbType.String;
				colvarIpMaytao.MaxLength = 30;
				colvarIpMaytao.AutoIncrement = false;
				colvarIpMaytao.IsNullable = true;
				colvarIpMaytao.IsPrimaryKey = false;
				colvarIpMaytao.IsForeignKey = false;
				colvarIpMaytao.IsReadOnly = false;
				colvarIpMaytao.DefaultSetting = @"";
				colvarIpMaytao.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIpMaytao);
				
				TableSchema.TableColumn colvarIpMaysua = new TableSchema.TableColumn(schema);
				colvarIpMaysua.ColumnName = "ip_maysua";
				colvarIpMaysua.DataType = DbType.String;
				colvarIpMaysua.MaxLength = 30;
				colvarIpMaysua.AutoIncrement = false;
				colvarIpMaysua.IsNullable = true;
				colvarIpMaysua.IsPrimaryKey = false;
				colvarIpMaysua.IsForeignKey = false;
				colvarIpMaysua.IsReadOnly = false;
				colvarIpMaysua.DefaultSetting = @"";
				colvarIpMaysua.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIpMaysua);
				
				TableSchema.TableColumn colvarTenMaytao = new TableSchema.TableColumn(schema);
				colvarTenMaytao.ColumnName = "ten_maytao";
				colvarTenMaytao.DataType = DbType.String;
				colvarTenMaytao.MaxLength = 100;
				colvarTenMaytao.AutoIncrement = false;
				colvarTenMaytao.IsNullable = true;
				colvarTenMaytao.IsPrimaryKey = false;
				colvarTenMaytao.IsForeignKey = false;
				colvarTenMaytao.IsReadOnly = false;
				colvarTenMaytao.DefaultSetting = @"";
				colvarTenMaytao.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTenMaytao);
				
				TableSchema.TableColumn colvarTenMaysua = new TableSchema.TableColumn(schema);
				colvarTenMaysua.ColumnName = "ten_maysua";
				colvarTenMaysua.DataType = DbType.String;
				colvarTenMaysua.MaxLength = 100;
				colvarTenMaysua.AutoIncrement = false;
				colvarTenMaysua.IsNullable = true;
				colvarTenMaysua.IsPrimaryKey = false;
				colvarTenMaysua.IsForeignKey = false;
				colvarTenMaysua.IsReadOnly = false;
				colvarTenMaysua.DefaultSetting = @"";
				colvarTenMaysua.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTenMaysua);
				
				TableSchema.TableColumn colvarDaDung = new TableSchema.TableColumn(schema);
				colvarDaDung.ColumnName = "da_dung";
				colvarDaDung.DataType = DbType.Byte;
				colvarDaDung.MaxLength = 0;
				colvarDaDung.AutoIncrement = false;
				colvarDaDung.IsNullable = true;
				colvarDaDung.IsPrimaryKey = false;
				colvarDaDung.IsForeignKey = false;
				colvarDaDung.IsReadOnly = false;
				colvarDaDung.DefaultSetting = @"";
				colvarDaDung.ForeignKeyTableName = "";
				schema.Columns.Add(colvarDaDung);
				
				TableSchema.TableColumn colvarLydoTiemchung = new TableSchema.TableColumn(schema);
				colvarLydoTiemchung.ColumnName = "lydo_tiemchung";
				colvarLydoTiemchung.DataType = DbType.String;
				colvarLydoTiemchung.MaxLength = 20;
				colvarLydoTiemchung.AutoIncrement = false;
				colvarLydoTiemchung.IsNullable = true;
				colvarLydoTiemchung.IsPrimaryKey = false;
				colvarLydoTiemchung.IsForeignKey = false;
				colvarLydoTiemchung.IsReadOnly = false;
				colvarLydoTiemchung.DefaultSetting = @"";
				colvarLydoTiemchung.ForeignKeyTableName = "";
				schema.Columns.Add(colvarLydoTiemchung);
				
				TableSchema.TableColumn colvarNguoiTiem = new TableSchema.TableColumn(schema);
				colvarNguoiTiem.ColumnName = "nguoi_tiem";
				colvarNguoiTiem.DataType = DbType.Int16;
				colvarNguoiTiem.MaxLength = 0;
				colvarNguoiTiem.AutoIncrement = false;
				colvarNguoiTiem.IsNullable = true;
				colvarNguoiTiem.IsPrimaryKey = false;
				colvarNguoiTiem.IsForeignKey = false;
				colvarNguoiTiem.IsReadOnly = false;
				colvarNguoiTiem.DefaultSetting = @"";
				colvarNguoiTiem.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNguoiTiem);
				
				TableSchema.TableColumn colvarVitriTiem = new TableSchema.TableColumn(schema);
				colvarVitriTiem.ColumnName = "vitri_tiem";
				colvarVitriTiem.DataType = DbType.String;
				colvarVitriTiem.MaxLength = 20;
				colvarVitriTiem.AutoIncrement = false;
				colvarVitriTiem.IsNullable = true;
				colvarVitriTiem.IsPrimaryKey = false;
				colvarVitriTiem.IsForeignKey = false;
				colvarVitriTiem.IsReadOnly = false;
				colvarVitriTiem.DefaultSetting = @"";
				colvarVitriTiem.ForeignKeyTableName = "";
				schema.Columns.Add(colvarVitriTiem);
				
				TableSchema.TableColumn colvarMuiThu = new TableSchema.TableColumn(schema);
				colvarMuiThu.ColumnName = "mui_thu";
				colvarMuiThu.DataType = DbType.Byte;
				colvarMuiThu.MaxLength = 0;
				colvarMuiThu.AutoIncrement = false;
				colvarMuiThu.IsNullable = true;
				colvarMuiThu.IsPrimaryKey = false;
				colvarMuiThu.IsForeignKey = false;
				colvarMuiThu.IsReadOnly = false;
				colvarMuiThu.DefaultSetting = @"";
				colvarMuiThu.ForeignKeyTableName = "";
				schema.Columns.Add(colvarMuiThu);
				
				TableSchema.TableColumn colvarNgayhenMuiketiep = new TableSchema.TableColumn(schema);
				colvarNgayhenMuiketiep.ColumnName = "ngayhen_muiketiep";
				colvarNgayhenMuiketiep.DataType = DbType.DateTime;
				colvarNgayhenMuiketiep.MaxLength = 0;
				colvarNgayhenMuiketiep.AutoIncrement = false;
				colvarNgayhenMuiketiep.IsNullable = true;
				colvarNgayhenMuiketiep.IsPrimaryKey = false;
				colvarNgayhenMuiketiep.IsForeignKey = false;
				colvarNgayhenMuiketiep.IsReadOnly = false;
				colvarNgayhenMuiketiep.DefaultSetting = @"";
				colvarNgayhenMuiketiep.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNgayhenMuiketiep);
				
				TableSchema.TableColumn colvarPhanungSautiem = new TableSchema.TableColumn(schema);
				colvarPhanungSautiem.ColumnName = "phanung_sautiem";
				colvarPhanungSautiem.DataType = DbType.String;
				colvarPhanungSautiem.MaxLength = 20;
				colvarPhanungSautiem.AutoIncrement = false;
				colvarPhanungSautiem.IsNullable = true;
				colvarPhanungSautiem.IsPrimaryKey = false;
				colvarPhanungSautiem.IsForeignKey = false;
				colvarPhanungSautiem.IsReadOnly = false;
				colvarPhanungSautiem.DefaultSetting = @"";
				colvarPhanungSautiem.ForeignKeyTableName = "";
				schema.Columns.Add(colvarPhanungSautiem);
				
				TableSchema.TableColumn colvarXutri = new TableSchema.TableColumn(schema);
				colvarXutri.ColumnName = "xutri";
				colvarXutri.DataType = DbType.String;
				colvarXutri.MaxLength = 20;
				colvarXutri.AutoIncrement = false;
				colvarXutri.IsNullable = true;
				colvarXutri.IsPrimaryKey = false;
				colvarXutri.IsForeignKey = false;
				colvarXutri.IsReadOnly = false;
				colvarXutri.DefaultSetting = @"";
				colvarXutri.ForeignKeyTableName = "";
				schema.Columns.Add(colvarXutri);
				
				TableSchema.TableColumn colvarKetluanNguyennhan = new TableSchema.TableColumn(schema);
				colvarKetluanNguyennhan.ColumnName = "ketluan_nguyennhan";
				colvarKetluanNguyennhan.DataType = DbType.String;
				colvarKetluanNguyennhan.MaxLength = 20;
				colvarKetluanNguyennhan.AutoIncrement = false;
				colvarKetluanNguyennhan.IsNullable = true;
				colvarKetluanNguyennhan.IsPrimaryKey = false;
				colvarKetluanNguyennhan.IsForeignKey = false;
				colvarKetluanNguyennhan.IsReadOnly = false;
				colvarKetluanNguyennhan.DefaultSetting = @"";
				colvarKetluanNguyennhan.ForeignKeyTableName = "";
				schema.Columns.Add(colvarKetluanNguyennhan);
				
				TableSchema.TableColumn colvarKetQua = new TableSchema.TableColumn(schema);
				colvarKetQua.ColumnName = "ket_qua";
				colvarKetQua.DataType = DbType.String;
				colvarKetQua.MaxLength = 20;
				colvarKetQua.AutoIncrement = false;
				colvarKetQua.IsNullable = true;
				colvarKetQua.IsPrimaryKey = false;
				colvarKetQua.IsForeignKey = false;
				colvarKetQua.IsReadOnly = false;
				colvarKetQua.DefaultSetting = @"";
				colvarKetQua.ForeignKeyTableName = "";
				schema.Columns.Add(colvarKetQua);
				
				TableSchema.TableColumn colvarNgaySudung = new TableSchema.TableColumn(schema);
				colvarNgaySudung.ColumnName = "ngay_sudung";
				colvarNgaySudung.DataType = DbType.DateTime;
				colvarNgaySudung.MaxLength = 0;
				colvarNgaySudung.AutoIncrement = false;
				colvarNgaySudung.IsNullable = true;
				colvarNgaySudung.IsPrimaryKey = false;
				colvarNgaySudung.IsForeignKey = false;
				colvarNgaySudung.IsReadOnly = false;
				colvarNgaySudung.DefaultSetting = @"";
				colvarNgaySudung.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNgaySudung);
				
				TableSchema.TableColumn colvarSoDky = new TableSchema.TableColumn(schema);
				colvarSoDky.ColumnName = "so_dky";
				colvarSoDky.DataType = DbType.String;
				colvarSoDky.MaxLength = 30;
				colvarSoDky.AutoIncrement = false;
				colvarSoDky.IsNullable = true;
				colvarSoDky.IsPrimaryKey = false;
				colvarSoDky.IsForeignKey = false;
				colvarSoDky.IsReadOnly = false;
				colvarSoDky.DefaultSetting = @"";
				colvarSoDky.ForeignKeyTableName = "";
				schema.Columns.Add(colvarSoDky);
				
				TableSchema.TableColumn colvarSoQdinhthau = new TableSchema.TableColumn(schema);
				colvarSoQdinhthau.ColumnName = "so_qdinhthau";
				colvarSoQdinhthau.DataType = DbType.String;
				colvarSoQdinhthau.MaxLength = 30;
				colvarSoQdinhthau.AutoIncrement = false;
				colvarSoQdinhthau.IsNullable = true;
				colvarSoQdinhthau.IsPrimaryKey = false;
				colvarSoQdinhthau.IsForeignKey = false;
				colvarSoQdinhthau.IsReadOnly = false;
				colvarSoQdinhthau.DefaultSetting = @"";
				colvarSoQdinhthau.ForeignKeyTableName = "";
				schema.Columns.Add(colvarSoQdinhthau);
				
				TableSchema.TableColumn colvarLydoHuy = new TableSchema.TableColumn(schema);
				colvarLydoHuy.ColumnName = "lydo_huy";
				colvarLydoHuy.DataType = DbType.String;
				colvarLydoHuy.MaxLength = 20;
				colvarLydoHuy.AutoIncrement = false;
				colvarLydoHuy.IsNullable = true;
				colvarLydoHuy.IsPrimaryKey = false;
				colvarLydoHuy.IsForeignKey = false;
				colvarLydoHuy.IsReadOnly = false;
				
						colvarLydoHuy.DefaultSetting = @"((0))";
				colvarLydoHuy.ForeignKeyTableName = "";
				schema.Columns.Add(colvarLydoHuy);
				
				TableSchema.TableColumn colvarIdPhieuTXuatthuocBenhnhan = new TableSchema.TableColumn(schema);
				colvarIdPhieuTXuatthuocBenhnhan.ColumnName = "id_phieu_t_xuatthuoc_benhnhan";
				colvarIdPhieuTXuatthuocBenhnhan.DataType = DbType.Int64;
				colvarIdPhieuTXuatthuocBenhnhan.MaxLength = 0;
				colvarIdPhieuTXuatthuocBenhnhan.AutoIncrement = false;
				colvarIdPhieuTXuatthuocBenhnhan.IsNullable = true;
				colvarIdPhieuTXuatthuocBenhnhan.IsPrimaryKey = false;
				colvarIdPhieuTXuatthuocBenhnhan.IsForeignKey = false;
				colvarIdPhieuTXuatthuocBenhnhan.IsReadOnly = false;
				colvarIdPhieuTXuatthuocBenhnhan.DefaultSetting = @"";
				colvarIdPhieuTXuatthuocBenhnhan.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdPhieuTXuatthuocBenhnhan);
				
				TableSchema.TableColumn colvarIdCapphat = new TableSchema.TableColumn(schema);
				colvarIdCapphat.ColumnName = "id_capphat";
				colvarIdCapphat.DataType = DbType.Int64;
				colvarIdCapphat.MaxLength = 0;
				colvarIdCapphat.AutoIncrement = false;
				colvarIdCapphat.IsNullable = true;
				colvarIdCapphat.IsPrimaryKey = false;
				colvarIdCapphat.IsForeignKey = false;
				colvarIdCapphat.IsReadOnly = false;
				
						colvarIdCapphat.DefaultSetting = @"((-1))";
				colvarIdCapphat.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdCapphat);
				
				TableSchema.TableColumn colvarSang = new TableSchema.TableColumn(schema);
				colvarSang.ColumnName = "sang";
				colvarSang.DataType = DbType.String;
				colvarSang.MaxLength = 5;
				colvarSang.AutoIncrement = false;
				colvarSang.IsNullable = true;
				colvarSang.IsPrimaryKey = false;
				colvarSang.IsForeignKey = false;
				colvarSang.IsReadOnly = false;
				colvarSang.DefaultSetting = @"";
				colvarSang.ForeignKeyTableName = "";
				schema.Columns.Add(colvarSang);
				
				TableSchema.TableColumn colvarTrua = new TableSchema.TableColumn(schema);
				colvarTrua.ColumnName = "trua";
				colvarTrua.DataType = DbType.String;
				colvarTrua.MaxLength = 5;
				colvarTrua.AutoIncrement = false;
				colvarTrua.IsNullable = true;
				colvarTrua.IsPrimaryKey = false;
				colvarTrua.IsForeignKey = false;
				colvarTrua.IsReadOnly = false;
				colvarTrua.DefaultSetting = @"";
				colvarTrua.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTrua);
				
				TableSchema.TableColumn colvarChieu = new TableSchema.TableColumn(schema);
				colvarChieu.ColumnName = "chieu";
				colvarChieu.DataType = DbType.String;
				colvarChieu.MaxLength = 5;
				colvarChieu.AutoIncrement = false;
				colvarChieu.IsNullable = true;
				colvarChieu.IsPrimaryKey = false;
				colvarChieu.IsForeignKey = false;
				colvarChieu.IsReadOnly = false;
				colvarChieu.DefaultSetting = @"";
				colvarChieu.ForeignKeyTableName = "";
				schema.Columns.Add(colvarChieu);
				
				TableSchema.TableColumn colvarToi = new TableSchema.TableColumn(schema);
				colvarToi.ColumnName = "toi";
				colvarToi.DataType = DbType.String;
				colvarToi.MaxLength = 5;
				colvarToi.AutoIncrement = false;
				colvarToi.IsNullable = true;
				colvarToi.IsPrimaryKey = false;
				colvarToi.IsForeignKey = false;
				colvarToi.IsReadOnly = false;
				colvarToi.DefaultSetting = @"";
				colvarToi.ForeignKeyTableName = "";
				schema.Columns.Add(colvarToi);
				
				TableSchema.TableColumn colvarTyleTt = new TableSchema.TableColumn(schema);
				colvarTyleTt.ColumnName = "tyle_tt";
				colvarTyleTt.DataType = DbType.Decimal;
				colvarTyleTt.MaxLength = 0;
				colvarTyleTt.AutoIncrement = false;
				colvarTyleTt.IsNullable = true;
				colvarTyleTt.IsPrimaryKey = false;
				colvarTyleTt.IsForeignKey = false;
				colvarTyleTt.IsReadOnly = false;
				
						colvarTyleTt.DefaultSetting = @"((100))";
				colvarTyleTt.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTyleTt);
				
				TableSchema.TableColumn colvarIdThe = new TableSchema.TableColumn(schema);
				colvarIdThe.ColumnName = "id_the";
				colvarIdThe.DataType = DbType.Int64;
				colvarIdThe.MaxLength = 0;
				colvarIdThe.AutoIncrement = false;
				colvarIdThe.IsNullable = true;
				colvarIdThe.IsPrimaryKey = false;
				colvarIdThe.IsForeignKey = false;
				colvarIdThe.IsReadOnly = false;
				colvarIdThe.DefaultSetting = @"";
				colvarIdThe.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdThe);
				
				TableSchema.TableColumn colvarIdDangky = new TableSchema.TableColumn(schema);
				colvarIdDangky.ColumnName = "id_dangky";
				colvarIdDangky.DataType = DbType.Int32;
				colvarIdDangky.MaxLength = 0;
				colvarIdDangky.AutoIncrement = false;
				colvarIdDangky.IsNullable = true;
				colvarIdDangky.IsPrimaryKey = false;
				colvarIdDangky.IsForeignKey = false;
				colvarIdDangky.IsReadOnly = false;
				colvarIdDangky.DefaultSetting = @"";
				colvarIdDangky.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdDangky);
				
				TableSchema.TableColumn colvarTthaiTamthu = new TableSchema.TableColumn(schema);
				colvarTthaiTamthu.ColumnName = "tthai_tamthu";
				colvarTthaiTamthu.DataType = DbType.Byte;
				colvarTthaiTamthu.MaxLength = 0;
				colvarTthaiTamthu.AutoIncrement = false;
				colvarTthaiTamthu.IsNullable = true;
				colvarTthaiTamthu.IsPrimaryKey = false;
				colvarTthaiTamthu.IsForeignKey = false;
				colvarTthaiTamthu.IsReadOnly = false;
				colvarTthaiTamthu.DefaultSetting = @"";
				colvarTthaiTamthu.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTthaiTamthu);
				
				TableSchema.TableColumn colvarIdTamthu = new TableSchema.TableColumn(schema);
				colvarIdTamthu.ColumnName = "id_tamthu";
				colvarIdTamthu.DataType = DbType.Int64;
				colvarIdTamthu.MaxLength = 0;
				colvarIdTamthu.AutoIncrement = false;
				colvarIdTamthu.IsNullable = true;
				colvarIdTamthu.IsPrimaryKey = false;
				colvarIdTamthu.IsForeignKey = false;
				colvarIdTamthu.IsReadOnly = false;
				colvarIdTamthu.DefaultSetting = @"";
				colvarIdTamthu.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdTamthu);
				
				TableSchema.TableColumn colvarNgayTamthu = new TableSchema.TableColumn(schema);
				colvarNgayTamthu.ColumnName = "ngay_tamthu";
				colvarNgayTamthu.DataType = DbType.DateTime;
				colvarNgayTamthu.MaxLength = 0;
				colvarNgayTamthu.AutoIncrement = false;
				colvarNgayTamthu.IsNullable = true;
				colvarNgayTamthu.IsPrimaryKey = false;
				colvarNgayTamthu.IsForeignKey = false;
				colvarNgayTamthu.IsReadOnly = false;
				colvarNgayTamthu.DefaultSetting = @"";
				colvarNgayTamthu.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNgayTamthu);
				
				TableSchema.TableColumn colvarBhytNguonKhac = new TableSchema.TableColumn(schema);
				colvarBhytNguonKhac.ColumnName = "bhyt_nguon_khac";
				colvarBhytNguonKhac.DataType = DbType.Byte;
				colvarBhytNguonKhac.MaxLength = 0;
				colvarBhytNguonKhac.AutoIncrement = false;
				colvarBhytNguonKhac.IsNullable = true;
				colvarBhytNguonKhac.IsPrimaryKey = false;
				colvarBhytNguonKhac.IsForeignKey = false;
				colvarBhytNguonKhac.IsReadOnly = false;
				colvarBhytNguonKhac.DefaultSetting = @"";
				colvarBhytNguonKhac.ForeignKeyTableName = "";
				schema.Columns.Add(colvarBhytNguonKhac);
				
				TableSchema.TableColumn colvarBhytGiaTyle = new TableSchema.TableColumn(schema);
				colvarBhytGiaTyle.ColumnName = "bhyt_gia_tyle";
				colvarBhytGiaTyle.DataType = DbType.Decimal;
				colvarBhytGiaTyle.MaxLength = 0;
				colvarBhytGiaTyle.AutoIncrement = false;
				colvarBhytGiaTyle.IsNullable = true;
				colvarBhytGiaTyle.IsPrimaryKey = false;
				colvarBhytGiaTyle.IsForeignKey = false;
				colvarBhytGiaTyle.IsReadOnly = false;
				
						colvarBhytGiaTyle.DefaultSetting = @"((0))";
				colvarBhytGiaTyle.ForeignKeyTableName = "";
				schema.Columns.Add(colvarBhytGiaTyle);
				
				TableSchema.TableColumn colvarBnTtt = new TableSchema.TableColumn(schema);
				colvarBnTtt.ColumnName = "bn_ttt";
				colvarBnTtt.DataType = DbType.Decimal;
				colvarBnTtt.MaxLength = 0;
				colvarBnTtt.AutoIncrement = false;
				colvarBnTtt.IsNullable = true;
				colvarBnTtt.IsPrimaryKey = false;
				colvarBnTtt.IsForeignKey = false;
				colvarBnTtt.IsReadOnly = false;
				
						colvarBnTtt.DefaultSetting = @"((0))";
				colvarBnTtt.ForeignKeyTableName = "";
				schema.Columns.Add(colvarBnTtt);
				
				TableSchema.TableColumn colvarBnCct = new TableSchema.TableColumn(schema);
				colvarBnCct.ColumnName = "bn_cct";
				colvarBnCct.DataType = DbType.Decimal;
				colvarBnCct.MaxLength = 0;
				colvarBnCct.AutoIncrement = false;
				colvarBnCct.IsNullable = true;
				colvarBnCct.IsPrimaryKey = false;
				colvarBnCct.IsForeignKey = false;
				colvarBnCct.IsReadOnly = false;
				
						colvarBnCct.DefaultSetting = @"((0))";
				colvarBnCct.ForeignKeyTableName = "";
				schema.Columns.Add(colvarBnCct);
				
				TableSchema.TableColumn colvarGpp = new TableSchema.TableColumn(schema);
				colvarGpp.ColumnName = "GPP";
				colvarGpp.DataType = DbType.Boolean;
				colvarGpp.MaxLength = 0;
				colvarGpp.AutoIncrement = false;
				colvarGpp.IsNullable = true;
				colvarGpp.IsPrimaryKey = false;
				colvarGpp.IsForeignKey = false;
				colvarGpp.IsReadOnly = false;
				
						colvarGpp.DefaultSetting = @"((0))";
				colvarGpp.ForeignKeyTableName = "";
				schema.Columns.Add(colvarGpp);
				
				TableSchema.TableColumn colvarTinhChiphi = new TableSchema.TableColumn(schema);
				colvarTinhChiphi.ColumnName = "tinh_chiphi";
				colvarTinhChiphi.DataType = DbType.Byte;
				colvarTinhChiphi.MaxLength = 0;
				colvarTinhChiphi.AutoIncrement = false;
				colvarTinhChiphi.IsNullable = true;
				colvarTinhChiphi.IsPrimaryKey = false;
				colvarTinhChiphi.IsForeignKey = false;
				colvarTinhChiphi.IsReadOnly = false;
				
						colvarTinhChiphi.DefaultSetting = @"((1))";
				colvarTinhChiphi.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTinhChiphi);
				
				TableSchema.TableColumn colvarGiaGoc = new TableSchema.TableColumn(schema);
				colvarGiaGoc.ColumnName = "gia_goc";
				colvarGiaGoc.DataType = DbType.Decimal;
				colvarGiaGoc.MaxLength = 0;
				colvarGiaGoc.AutoIncrement = false;
				colvarGiaGoc.IsNullable = true;
				colvarGiaGoc.IsPrimaryKey = false;
				colvarGiaGoc.IsForeignKey = false;
				colvarGiaGoc.IsReadOnly = false;
				colvarGiaGoc.DefaultSetting = @"";
				colvarGiaGoc.ForeignKeyTableName = "";
				schema.Columns.Add(colvarGiaGoc);
				
				TableSchema.TableColumn colvarNoidungthuchienSang = new TableSchema.TableColumn(schema);
				colvarNoidungthuchienSang.ColumnName = "noidungthuchien_sang";
				colvarNoidungthuchienSang.DataType = DbType.String;
				colvarNoidungthuchienSang.MaxLength = 255;
				colvarNoidungthuchienSang.AutoIncrement = false;
				colvarNoidungthuchienSang.IsNullable = true;
				colvarNoidungthuchienSang.IsPrimaryKey = false;
				colvarNoidungthuchienSang.IsForeignKey = false;
				colvarNoidungthuchienSang.IsReadOnly = false;
				colvarNoidungthuchienSang.DefaultSetting = @"";
				colvarNoidungthuchienSang.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNoidungthuchienSang);
				
				TableSchema.TableColumn colvarIdNguoithuchienSang = new TableSchema.TableColumn(schema);
				colvarIdNguoithuchienSang.ColumnName = "id_nguoithuchien_sang";
				colvarIdNguoithuchienSang.DataType = DbType.Int16;
				colvarIdNguoithuchienSang.MaxLength = 0;
				colvarIdNguoithuchienSang.AutoIncrement = false;
				colvarIdNguoithuchienSang.IsNullable = true;
				colvarIdNguoithuchienSang.IsPrimaryKey = false;
				colvarIdNguoithuchienSang.IsForeignKey = false;
				colvarIdNguoithuchienSang.IsReadOnly = false;
				colvarIdNguoithuchienSang.DefaultSetting = @"";
				colvarIdNguoithuchienSang.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdNguoithuchienSang);
				
				TableSchema.TableColumn colvarTenNguoithuchienSang = new TableSchema.TableColumn(schema);
				colvarTenNguoithuchienSang.ColumnName = "ten_nguoithuchien_sang";
				colvarTenNguoithuchienSang.DataType = DbType.String;
				colvarTenNguoithuchienSang.MaxLength = 255;
				colvarTenNguoithuchienSang.AutoIncrement = false;
				colvarTenNguoithuchienSang.IsNullable = true;
				colvarTenNguoithuchienSang.IsPrimaryKey = false;
				colvarTenNguoithuchienSang.IsForeignKey = false;
				colvarTenNguoithuchienSang.IsReadOnly = false;
				colvarTenNguoithuchienSang.DefaultSetting = @"";
				colvarTenNguoithuchienSang.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTenNguoithuchienSang);
				
				TableSchema.TableColumn colvarNoidungthuchienTrua = new TableSchema.TableColumn(schema);
				colvarNoidungthuchienTrua.ColumnName = "noidungthuchien_trua";
				colvarNoidungthuchienTrua.DataType = DbType.String;
				colvarNoidungthuchienTrua.MaxLength = 255;
				colvarNoidungthuchienTrua.AutoIncrement = false;
				colvarNoidungthuchienTrua.IsNullable = true;
				colvarNoidungthuchienTrua.IsPrimaryKey = false;
				colvarNoidungthuchienTrua.IsForeignKey = false;
				colvarNoidungthuchienTrua.IsReadOnly = false;
				colvarNoidungthuchienTrua.DefaultSetting = @"";
				colvarNoidungthuchienTrua.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNoidungthuchienTrua);
				
				TableSchema.TableColumn colvarIdNguoithuchienTrua = new TableSchema.TableColumn(schema);
				colvarIdNguoithuchienTrua.ColumnName = "id_nguoithuchien_trua";
				colvarIdNguoithuchienTrua.DataType = DbType.Int16;
				colvarIdNguoithuchienTrua.MaxLength = 0;
				colvarIdNguoithuchienTrua.AutoIncrement = false;
				colvarIdNguoithuchienTrua.IsNullable = true;
				colvarIdNguoithuchienTrua.IsPrimaryKey = false;
				colvarIdNguoithuchienTrua.IsForeignKey = false;
				colvarIdNguoithuchienTrua.IsReadOnly = false;
				colvarIdNguoithuchienTrua.DefaultSetting = @"";
				colvarIdNguoithuchienTrua.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdNguoithuchienTrua);
				
				TableSchema.TableColumn colvarTenNguoithuchienTrua = new TableSchema.TableColumn(schema);
				colvarTenNguoithuchienTrua.ColumnName = "ten_nguoithuchien_trua";
				colvarTenNguoithuchienTrua.DataType = DbType.String;
				colvarTenNguoithuchienTrua.MaxLength = 255;
				colvarTenNguoithuchienTrua.AutoIncrement = false;
				colvarTenNguoithuchienTrua.IsNullable = true;
				colvarTenNguoithuchienTrua.IsPrimaryKey = false;
				colvarTenNguoithuchienTrua.IsForeignKey = false;
				colvarTenNguoithuchienTrua.IsReadOnly = false;
				colvarTenNguoithuchienTrua.DefaultSetting = @"";
				colvarTenNguoithuchienTrua.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTenNguoithuchienTrua);
				
				TableSchema.TableColumn colvarNoidungthuchienChieu = new TableSchema.TableColumn(schema);
				colvarNoidungthuchienChieu.ColumnName = "noidungthuchien_chieu";
				colvarNoidungthuchienChieu.DataType = DbType.String;
				colvarNoidungthuchienChieu.MaxLength = 255;
				colvarNoidungthuchienChieu.AutoIncrement = false;
				colvarNoidungthuchienChieu.IsNullable = true;
				colvarNoidungthuchienChieu.IsPrimaryKey = false;
				colvarNoidungthuchienChieu.IsForeignKey = false;
				colvarNoidungthuchienChieu.IsReadOnly = false;
				colvarNoidungthuchienChieu.DefaultSetting = @"";
				colvarNoidungthuchienChieu.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNoidungthuchienChieu);
				
				TableSchema.TableColumn colvarIdNguoithuchienChieu = new TableSchema.TableColumn(schema);
				colvarIdNguoithuchienChieu.ColumnName = "id_nguoithuchien_chieu";
				colvarIdNguoithuchienChieu.DataType = DbType.Int16;
				colvarIdNguoithuchienChieu.MaxLength = 0;
				colvarIdNguoithuchienChieu.AutoIncrement = false;
				colvarIdNguoithuchienChieu.IsNullable = true;
				colvarIdNguoithuchienChieu.IsPrimaryKey = false;
				colvarIdNguoithuchienChieu.IsForeignKey = false;
				colvarIdNguoithuchienChieu.IsReadOnly = false;
				colvarIdNguoithuchienChieu.DefaultSetting = @"";
				colvarIdNguoithuchienChieu.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdNguoithuchienChieu);
				
				TableSchema.TableColumn colvarTenNguoithuchienChieu = new TableSchema.TableColumn(schema);
				colvarTenNguoithuchienChieu.ColumnName = "ten_nguoithuchien_chieu";
				colvarTenNguoithuchienChieu.DataType = DbType.String;
				colvarTenNguoithuchienChieu.MaxLength = 255;
				colvarTenNguoithuchienChieu.AutoIncrement = false;
				colvarTenNguoithuchienChieu.IsNullable = true;
				colvarTenNguoithuchienChieu.IsPrimaryKey = false;
				colvarTenNguoithuchienChieu.IsForeignKey = false;
				colvarTenNguoithuchienChieu.IsReadOnly = false;
				colvarTenNguoithuchienChieu.DefaultSetting = @"";
				colvarTenNguoithuchienChieu.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTenNguoithuchienChieu);
				
				TableSchema.TableColumn colvarNoidungthuchienToi = new TableSchema.TableColumn(schema);
				colvarNoidungthuchienToi.ColumnName = "noidungthuchien_toi";
				colvarNoidungthuchienToi.DataType = DbType.String;
				colvarNoidungthuchienToi.MaxLength = 255;
				colvarNoidungthuchienToi.AutoIncrement = false;
				colvarNoidungthuchienToi.IsNullable = true;
				colvarNoidungthuchienToi.IsPrimaryKey = false;
				colvarNoidungthuchienToi.IsForeignKey = false;
				colvarNoidungthuchienToi.IsReadOnly = false;
				colvarNoidungthuchienToi.DefaultSetting = @"";
				colvarNoidungthuchienToi.ForeignKeyTableName = "";
				schema.Columns.Add(colvarNoidungthuchienToi);
				
				TableSchema.TableColumn colvarIdNguoithuchienToi = new TableSchema.TableColumn(schema);
				colvarIdNguoithuchienToi.ColumnName = "id_nguoithuchien_toi";
				colvarIdNguoithuchienToi.DataType = DbType.Int16;
				colvarIdNguoithuchienToi.MaxLength = 0;
				colvarIdNguoithuchienToi.AutoIncrement = false;
				colvarIdNguoithuchienToi.IsNullable = true;
				colvarIdNguoithuchienToi.IsPrimaryKey = false;
				colvarIdNguoithuchienToi.IsForeignKey = false;
				colvarIdNguoithuchienToi.IsReadOnly = false;
				colvarIdNguoithuchienToi.DefaultSetting = @"";
				colvarIdNguoithuchienToi.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdNguoithuchienToi);
				
				TableSchema.TableColumn colvarTenNguoithuchienToi = new TableSchema.TableColumn(schema);
				colvarTenNguoithuchienToi.ColumnName = "ten_nguoithuchien_toi";
				colvarTenNguoithuchienToi.DataType = DbType.String;
				colvarTenNguoithuchienToi.MaxLength = 255;
				colvarTenNguoithuchienToi.AutoIncrement = false;
				colvarTenNguoithuchienToi.IsNullable = true;
				colvarTenNguoithuchienToi.IsPrimaryKey = false;
				colvarTenNguoithuchienToi.IsForeignKey = false;
				colvarTenNguoithuchienToi.IsReadOnly = false;
				colvarTenNguoithuchienToi.DefaultSetting = @"";
				colvarTenNguoithuchienToi.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTenNguoithuchienToi);
				
				TableSchema.TableColumn colvarTthaiGui = new TableSchema.TableColumn(schema);
				colvarTthaiGui.ColumnName = "tthai_gui";
				colvarTthaiGui.DataType = DbType.Byte;
				colvarTthaiGui.MaxLength = 0;
				colvarTthaiGui.AutoIncrement = false;
				colvarTthaiGui.IsNullable = true;
				colvarTthaiGui.IsPrimaryKey = false;
				colvarTthaiGui.IsForeignKey = false;
				colvarTthaiGui.IsReadOnly = false;
				
						colvarTthaiGui.DefaultSetting = @"((0))";
				colvarTthaiGui.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTthaiGui);
				
				TableSchema.TableColumn colvarTransactionId = new TableSchema.TableColumn(schema);
				colvarTransactionId.ColumnName = "transaction_id";
				colvarTransactionId.DataType = DbType.String;
				colvarTransactionId.MaxLength = 255;
				colvarTransactionId.AutoIncrement = false;
				colvarTransactionId.IsNullable = true;
				colvarTransactionId.IsPrimaryKey = false;
				colvarTransactionId.IsForeignKey = false;
				colvarTransactionId.IsReadOnly = false;
				colvarTransactionId.DefaultSetting = @"";
				colvarTransactionId.ForeignKeyTableName = "";
				schema.Columns.Add(colvarTransactionId);
				
				TableSchema.TableColumn colvarIdGui = new TableSchema.TableColumn(schema);
				colvarIdGui.ColumnName = "id_gui";
				colvarIdGui.DataType = DbType.Int64;
				colvarIdGui.MaxLength = 0;
				colvarIdGui.AutoIncrement = false;
				colvarIdGui.IsNullable = true;
				colvarIdGui.IsPrimaryKey = false;
				colvarIdGui.IsForeignKey = false;
				colvarIdGui.IsReadOnly = false;
				colvarIdGui.DefaultSetting = @"";
				colvarIdGui.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdGui);
				
				TableSchema.TableColumn colvarIdHuydvu = new TableSchema.TableColumn(schema);
				colvarIdHuydvu.ColumnName = "id_huydvu";
				colvarIdHuydvu.DataType = DbType.Int64;
				colvarIdHuydvu.MaxLength = 0;
				colvarIdHuydvu.AutoIncrement = false;
				colvarIdHuydvu.IsNullable = true;
				colvarIdHuydvu.IsPrimaryKey = false;
				colvarIdHuydvu.IsForeignKey = false;
				colvarIdHuydvu.IsReadOnly = false;
				colvarIdHuydvu.DefaultSetting = @"";
				colvarIdHuydvu.ForeignKeyTableName = "";
				schema.Columns.Add(colvarIdHuydvu);
				
				TableSchema.TableColumn colvarHienthiBienlai = new TableSchema.TableColumn(schema);
				colvarHienthiBienlai.ColumnName = "hienthi_bienlai";
				colvarHienthiBienlai.DataType = DbType.Byte;
				colvarHienthiBienlai.MaxLength = 0;
				colvarHienthiBienlai.AutoIncrement = false;
				colvarHienthiBienlai.IsNullable = true;
				colvarHienthiBienlai.IsPrimaryKey = false;
				colvarHienthiBienlai.IsForeignKey = false;
				colvarHienthiBienlai.IsReadOnly = false;
				
						colvarHienthiBienlai.DefaultSetting = @"((1))";
				colvarHienthiBienlai.ForeignKeyTableName = "";
				schema.Columns.Add(colvarHienthiBienlai);
				
				BaseSchema = schema;
				//add this schema to the provider
				//so we can query it later
				DataService.Providers["ORM"].AddSchema("kcb_donthuoc_chitiet",schema);
			}
		}
		#endregion
		
		#region Props
		  
		[XmlAttribute("IdChitietdonthuoc")]
		[Bindable(true)]
		public long IdChitietdonthuoc 
		{
			get { return GetColumnValue<long>(Columns.IdChitietdonthuoc); }
			set { SetColumnValue(Columns.IdChitietdonthuoc, value); }
		}
		  
		[XmlAttribute("IdDonthuoc")]
		[Bindable(true)]
		public long IdDonthuoc 
		{
			get { return GetColumnValue<long>(Columns.IdDonthuoc); }
			set { SetColumnValue(Columns.IdDonthuoc, value); }
		}
		  
		[XmlAttribute("IdDonthuocChuyengoi")]
		[Bindable(true)]
		public long? IdDonthuocChuyengoi 
		{
			get { return GetColumnValue<long?>(Columns.IdDonthuocChuyengoi); }
			set { SetColumnValue(Columns.IdDonthuocChuyengoi, value); }
		}
		  
		[XmlAttribute("IdBenhnhan")]
		[Bindable(true)]
		public long? IdBenhnhan 
		{
			get { return GetColumnValue<long?>(Columns.IdBenhnhan); }
			set { SetColumnValue(Columns.IdBenhnhan, value); }
		}
		  
		[XmlAttribute("MaLuotkham")]
		[Bindable(true)]
		public string MaLuotkham 
		{
			get { return GetColumnValue<string>(Columns.MaLuotkham); }
			set { SetColumnValue(Columns.MaLuotkham, value); }
		}
		  
		[XmlAttribute("IdKham")]
		[Bindable(true)]
		public long? IdKham 
		{
			get { return GetColumnValue<long?>(Columns.IdKham); }
			set { SetColumnValue(Columns.IdKham, value); }
		}
		  
		[XmlAttribute("IdKho")]
		[Bindable(true)]
		public int? IdKho 
		{
			get { return GetColumnValue<int?>(Columns.IdKho); }
			set { SetColumnValue(Columns.IdKho, value); }
		}
		  
		[XmlAttribute("IdThuoc")]
		[Bindable(true)]
		public int IdThuoc 
		{
			get { return GetColumnValue<int>(Columns.IdThuoc); }
			set { SetColumnValue(Columns.IdThuoc, value); }
		}
		  
		[XmlAttribute("NgayHethan")]
		[Bindable(true)]
		public DateTime NgayHethan 
		{
			get { return GetColumnValue<DateTime>(Columns.NgayHethan); }
			set { SetColumnValue(Columns.NgayHethan, value); }
		}
		  
		[XmlAttribute("SoLuong")]
		[Bindable(true)]
		public decimal SoLuong 
		{
			get { return GetColumnValue<decimal>(Columns.SoLuong); }
			set { SetColumnValue(Columns.SoLuong, value); }
		}
		  
		[XmlAttribute("SluongSua")]
		[Bindable(true)]
		public decimal? SluongSua 
		{
			get { return GetColumnValue<decimal?>(Columns.SluongSua); }
			set { SetColumnValue(Columns.SluongSua, value); }
		}
		  
		[XmlAttribute("SluongLinh")]
		[Bindable(true)]
		public decimal? SluongLinh 
		{
			get { return GetColumnValue<decimal?>(Columns.SluongLinh); }
			set { SetColumnValue(Columns.SluongLinh, value); }
		}
		  
		[XmlAttribute("DonGia")]
		[Bindable(true)]
		public decimal DonGia 
		{
			get { return GetColumnValue<decimal>(Columns.DonGia); }
			set { SetColumnValue(Columns.DonGia, value); }
		}
		  
		[XmlAttribute("IdThuockho")]
		[Bindable(true)]
		public long? IdThuockho 
		{
			get { return GetColumnValue<long?>(Columns.IdThuockho); }
			set { SetColumnValue(Columns.IdThuockho, value); }
		}
		  
		[XmlAttribute("NgayNhap")]
		[Bindable(true)]
		public DateTime? NgayNhap 
		{
			get { return GetColumnValue<DateTime?>(Columns.NgayNhap); }
			set { SetColumnValue(Columns.NgayNhap, value); }
		}
		  
		[XmlAttribute("GiaNhap")]
		[Bindable(true)]
		public decimal? GiaNhap 
		{
			get { return GetColumnValue<decimal?>(Columns.GiaNhap); }
			set { SetColumnValue(Columns.GiaNhap, value); }
		}
		  
		[XmlAttribute("GiaBan")]
		[Bindable(true)]
		public decimal? GiaBan 
		{
			get { return GetColumnValue<decimal?>(Columns.GiaBan); }
			set { SetColumnValue(Columns.GiaBan, value); }
		}
		  
		[XmlAttribute("GiaBhyt")]
		[Bindable(true)]
		public decimal? GiaBhyt 
		{
			get { return GetColumnValue<decimal?>(Columns.GiaBhyt); }
			set { SetColumnValue(Columns.GiaBhyt, value); }
		}
		  
		[XmlAttribute("SoLo")]
		[Bindable(true)]
		public string SoLo 
		{
			get { return GetColumnValue<string>(Columns.SoLo); }
			set { SetColumnValue(Columns.SoLo, value); }
		}
		  
		[XmlAttribute("Vat")]
		[Bindable(true)]
		public decimal? Vat 
		{
			get { return GetColumnValue<decimal?>(Columns.Vat); }
			set { SetColumnValue(Columns.Vat, value); }
		}
		  
		[XmlAttribute("MaNhacungcap")]
		[Bindable(true)]
		public string MaNhacungcap 
		{
			get { return GetColumnValue<string>(Columns.MaNhacungcap); }
			set { SetColumnValue(Columns.MaNhacungcap, value); }
		}
		  
		[XmlAttribute("PhuThu")]
		[Bindable(true)]
		public decimal? PhuThu 
		{
			get { return GetColumnValue<decimal?>(Columns.PhuThu); }
			set { SetColumnValue(Columns.PhuThu, value); }
		}
		  
		[XmlAttribute("PhuthuDungtuyen")]
		[Bindable(true)]
		public decimal? PhuthuDungtuyen 
		{
			get { return GetColumnValue<decimal?>(Columns.PhuthuDungtuyen); }
			set { SetColumnValue(Columns.PhuthuDungtuyen, value); }
		}
		  
		[XmlAttribute("PhuthuTraituyen")]
		[Bindable(true)]
		public decimal? PhuthuTraituyen 
		{
			get { return GetColumnValue<decimal?>(Columns.PhuthuTraituyen); }
			set { SetColumnValue(Columns.PhuthuTraituyen, value); }
		}
		  
		[XmlAttribute("MotaThem")]
		[Bindable(true)]
		public string MotaThem 
		{
			get { return GetColumnValue<string>(Columns.MotaThem); }
			set { SetColumnValue(Columns.MotaThem, value); }
		}
		  
		[XmlAttribute("SoluongHuy")]
		[Bindable(true)]
		public int? SoluongHuy 
		{
			get { return GetColumnValue<int?>(Columns.SoluongHuy); }
			set { SetColumnValue(Columns.SoluongHuy, value); }
		}
		  
		[XmlAttribute("TrangthaiHuy")]
		[Bindable(true)]
		public byte? TrangthaiHuy 
		{
			get { return GetColumnValue<byte?>(Columns.TrangthaiHuy); }
			set { SetColumnValue(Columns.TrangthaiHuy, value); }
		}
		  
		[XmlAttribute("NguoiHuy")]
		[Bindable(true)]
		public string NguoiHuy 
		{
			get { return GetColumnValue<string>(Columns.NguoiHuy); }
			set { SetColumnValue(Columns.NguoiHuy, value); }
		}
		  
		[XmlAttribute("NgayHuy")]
		[Bindable(true)]
		public DateTime? NgayHuy 
		{
			get { return GetColumnValue<DateTime?>(Columns.NgayHuy); }
			set { SetColumnValue(Columns.NgayHuy, value); }
		}
		  
		[XmlAttribute("TuTuc")]
		[Bindable(true)]
		public byte? TuTuc 
		{
			get { return GetColumnValue<byte?>(Columns.TuTuc); }
			set { SetColumnValue(Columns.TuTuc, value); }
		}
		  
		[XmlAttribute("TrangThai")]
		[Bindable(true)]
		public byte? TrangThai 
		{
			get { return GetColumnValue<byte?>(Columns.TrangThai); }
			set { SetColumnValue(Columns.TrangThai, value); }
		}
		  
		[XmlAttribute("TrangthaiTonghop")]
		[Bindable(true)]
		public byte? TrangthaiTonghop 
		{
			get { return GetColumnValue<byte?>(Columns.TrangthaiTonghop); }
			set { SetColumnValue(Columns.TrangthaiTonghop, value); }
		}
		  
		[XmlAttribute("NgayXacnhan")]
		[Bindable(true)]
		public DateTime? NgayXacnhan 
		{
			get { return GetColumnValue<DateTime?>(Columns.NgayXacnhan); }
			set { SetColumnValue(Columns.NgayXacnhan, value); }
		}
		  
		[XmlAttribute("TrangthaiBhyt")]
		[Bindable(true)]
		public byte? TrangthaiBhyt 
		{
			get { return GetColumnValue<byte?>(Columns.TrangthaiBhyt); }
			set { SetColumnValue(Columns.TrangthaiBhyt, value); }
		}
		  
		[XmlAttribute("SttIn")]
		[Bindable(true)]
		public short? SttIn 
		{
			get { return GetColumnValue<short?>(Columns.SttIn); }
			set { SetColumnValue(Columns.SttIn, value); }
		}
		  
		[XmlAttribute("MadoituongGia")]
		[Bindable(true)]
		public string MadoituongGia 
		{
			get { return GetColumnValue<string>(Columns.MadoituongGia); }
			set { SetColumnValue(Columns.MadoituongGia, value); }
		}
		  
		[XmlAttribute("PtramBhytGoc")]
		[Bindable(true)]
		public decimal? PtramBhytGoc 
		{
			get { return GetColumnValue<decimal?>(Columns.PtramBhytGoc); }
			set { SetColumnValue(Columns.PtramBhytGoc, value); }
		}
		  
		[XmlAttribute("PtramBhyt")]
		[Bindable(true)]
		public decimal? PtramBhyt 
		{
			get { return GetColumnValue<decimal?>(Columns.PtramBhyt); }
			set { SetColumnValue(Columns.PtramBhyt, value); }
		}
		  
		[XmlAttribute("BhytChitra")]
		[Bindable(true)]
		public decimal? BhytChitra 
		{
			get { return GetColumnValue<decimal?>(Columns.BhytChitra); }
			set { SetColumnValue(Columns.BhytChitra, value); }
		}
		  
		[XmlAttribute("BnhanChitra")]
		[Bindable(true)]
		public decimal? BnhanChitra 
		{
			get { return GetColumnValue<decimal?>(Columns.BnhanChitra); }
			set { SetColumnValue(Columns.BnhanChitra, value); }
		}
		  
		[XmlAttribute("MaDoituongKcb")]
		[Bindable(true)]
		public string MaDoituongKcb 
		{
			get { return GetColumnValue<string>(Columns.MaDoituongKcb); }
			set { SetColumnValue(Columns.MaDoituongKcb, value); }
		}
		  
		[XmlAttribute("IdThanhtoan")]
		[Bindable(true)]
		public int? IdThanhtoan 
		{
			get { return GetColumnValue<int?>(Columns.IdThanhtoan); }
			set { SetColumnValue(Columns.IdThanhtoan, value); }
		}
		  
		[XmlAttribute("TrangthaiThanhtoan")]
		[Bindable(true)]
		public byte? TrangthaiThanhtoan 
		{
			get { return GetColumnValue<byte?>(Columns.TrangthaiThanhtoan); }
			set { SetColumnValue(Columns.TrangthaiThanhtoan, value); }
		}
		  
		[XmlAttribute("NgayThanhtoan")]
		[Bindable(true)]
		public DateTime? NgayThanhtoan 
		{
			get { return GetColumnValue<DateTime?>(Columns.NgayThanhtoan); }
			set { SetColumnValue(Columns.NgayThanhtoan, value); }
		}
		  
		[XmlAttribute("CachDung")]
		[Bindable(true)]
		public string CachDung 
		{
			get { return GetColumnValue<string>(Columns.CachDung); }
			set { SetColumnValue(Columns.CachDung, value); }
		}
		  
		[XmlAttribute("ChidanThem")]
		[Bindable(true)]
		public string ChidanThem 
		{
			get { return GetColumnValue<string>(Columns.ChidanThem); }
			set { SetColumnValue(Columns.ChidanThem, value); }
		}
		  
		[XmlAttribute("DonviTinh")]
		[Bindable(true)]
		public string DonviTinh 
		{
			get { return GetColumnValue<string>(Columns.DonviTinh); }
			set { SetColumnValue(Columns.DonviTinh, value); }
		}
		  
		[XmlAttribute("SolanDung")]
		[Bindable(true)]
		public string SolanDung 
		{
			get { return GetColumnValue<string>(Columns.SolanDung); }
			set { SetColumnValue(Columns.SolanDung, value); }
		}
		  
		[XmlAttribute("SoluongDung")]
		[Bindable(true)]
		public string SoluongDung 
		{
			get { return GetColumnValue<string>(Columns.SoluongDung); }
			set { SetColumnValue(Columns.SoluongDung, value); }
		}
		  
		[XmlAttribute("TrangthaiChuyen")]
		[Bindable(true)]
		public byte? TrangthaiChuyen 
		{
			get { return GetColumnValue<byte?>(Columns.TrangthaiChuyen); }
			set { SetColumnValue(Columns.TrangthaiChuyen, value); }
		}
		  
		[XmlAttribute("NgayTao")]
		[Bindable(true)]
		public DateTime? NgayTao 
		{
			get { return GetColumnValue<DateTime?>(Columns.NgayTao); }
			set { SetColumnValue(Columns.NgayTao, value); }
		}
		  
		[XmlAttribute("NguoiTao")]
		[Bindable(true)]
		public string NguoiTao 
		{
			get { return GetColumnValue<string>(Columns.NguoiTao); }
			set { SetColumnValue(Columns.NguoiTao, value); }
		}
		  
		[XmlAttribute("NgaySua")]
		[Bindable(true)]
		public DateTime? NgaySua 
		{
			get { return GetColumnValue<DateTime?>(Columns.NgaySua); }
			set { SetColumnValue(Columns.NgaySua, value); }
		}
		  
		[XmlAttribute("NguoiSua")]
		[Bindable(true)]
		public string NguoiSua 
		{
			get { return GetColumnValue<string>(Columns.NguoiSua); }
			set { SetColumnValue(Columns.NguoiSua, value); }
		}
		  
		[XmlAttribute("TileChietkhau")]
		[Bindable(true)]
		public decimal? TileChietkhau 
		{
			get { return GetColumnValue<decimal?>(Columns.TileChietkhau); }
			set { SetColumnValue(Columns.TileChietkhau, value); }
		}
		  
		[XmlAttribute("TienChietkhau")]
		[Bindable(true)]
		public decimal? TienChietkhau 
		{
			get { return GetColumnValue<decimal?>(Columns.TienChietkhau); }
			set { SetColumnValue(Columns.TienChietkhau, value); }
		}
		  
		[XmlAttribute("KieuChietkhau")]
		[Bindable(true)]
		public string KieuChietkhau 
		{
			get { return GetColumnValue<string>(Columns.KieuChietkhau); }
			set { SetColumnValue(Columns.KieuChietkhau, value); }
		}
		  
		[XmlAttribute("IdGoi")]
		[Bindable(true)]
		public int? IdGoi 
		{
			get { return GetColumnValue<int?>(Columns.IdGoi); }
			set { SetColumnValue(Columns.IdGoi, value); }
		}
		  
		[XmlAttribute("TrongGoi")]
		[Bindable(true)]
		public byte? TrongGoi 
		{
			get { return GetColumnValue<byte?>(Columns.TrongGoi); }
			set { SetColumnValue(Columns.TrongGoi, value); }
		}
		  
		[XmlAttribute("KieuBiendong")]
		[Bindable(true)]
		public string KieuBiendong 
		{
			get { return GetColumnValue<string>(Columns.KieuBiendong); }
			set { SetColumnValue(Columns.KieuBiendong, value); }
		}
		  
		[XmlAttribute("NguonThanhtoan")]
		[Bindable(true)]
		public byte? NguonThanhtoan 
		{
			get { return GetColumnValue<byte?>(Columns.NguonThanhtoan); }
			set { SetColumnValue(Columns.NguonThanhtoan, value); }
		}
		  
		[XmlAttribute("IpMaytao")]
		[Bindable(true)]
		public string IpMaytao 
		{
			get { return GetColumnValue<string>(Columns.IpMaytao); }
			set { SetColumnValue(Columns.IpMaytao, value); }
		}
		  
		[XmlAttribute("IpMaysua")]
		[Bindable(true)]
		public string IpMaysua 
		{
			get { return GetColumnValue<string>(Columns.IpMaysua); }
			set { SetColumnValue(Columns.IpMaysua, value); }
		}
		  
		[XmlAttribute("TenMaytao")]
		[Bindable(true)]
		public string TenMaytao 
		{
			get { return GetColumnValue<string>(Columns.TenMaytao); }
			set { SetColumnValue(Columns.TenMaytao, value); }
		}
		  
		[XmlAttribute("TenMaysua")]
		[Bindable(true)]
		public string TenMaysua 
		{
			get { return GetColumnValue<string>(Columns.TenMaysua); }
			set { SetColumnValue(Columns.TenMaysua, value); }
		}
		  
		[XmlAttribute("DaDung")]
		[Bindable(true)]
		public byte? DaDung 
		{
			get { return GetColumnValue<byte?>(Columns.DaDung); }
			set { SetColumnValue(Columns.DaDung, value); }
		}
		  
		[XmlAttribute("LydoTiemchung")]
		[Bindable(true)]
		public string LydoTiemchung 
		{
			get { return GetColumnValue<string>(Columns.LydoTiemchung); }
			set { SetColumnValue(Columns.LydoTiemchung, value); }
		}
		  
		[XmlAttribute("NguoiTiem")]
		[Bindable(true)]
		public short? NguoiTiem 
		{
			get { return GetColumnValue<short?>(Columns.NguoiTiem); }
			set { SetColumnValue(Columns.NguoiTiem, value); }
		}
		  
		[XmlAttribute("VitriTiem")]
		[Bindable(true)]
		public string VitriTiem 
		{
			get { return GetColumnValue<string>(Columns.VitriTiem); }
			set { SetColumnValue(Columns.VitriTiem, value); }
		}
		  
		[XmlAttribute("MuiThu")]
		[Bindable(true)]
		public byte? MuiThu 
		{
			get { return GetColumnValue<byte?>(Columns.MuiThu); }
			set { SetColumnValue(Columns.MuiThu, value); }
		}
		  
		[XmlAttribute("NgayhenMuiketiep")]
		[Bindable(true)]
		public DateTime? NgayhenMuiketiep 
		{
			get { return GetColumnValue<DateTime?>(Columns.NgayhenMuiketiep); }
			set { SetColumnValue(Columns.NgayhenMuiketiep, value); }
		}
		  
		[XmlAttribute("PhanungSautiem")]
		[Bindable(true)]
		public string PhanungSautiem 
		{
			get { return GetColumnValue<string>(Columns.PhanungSautiem); }
			set { SetColumnValue(Columns.PhanungSautiem, value); }
		}
		  
		[XmlAttribute("Xutri")]
		[Bindable(true)]
		public string Xutri 
		{
			get { return GetColumnValue<string>(Columns.Xutri); }
			set { SetColumnValue(Columns.Xutri, value); }
		}
		  
		[XmlAttribute("KetluanNguyennhan")]
		[Bindable(true)]
		public string KetluanNguyennhan 
		{
			get { return GetColumnValue<string>(Columns.KetluanNguyennhan); }
			set { SetColumnValue(Columns.KetluanNguyennhan, value); }
		}
		  
		[XmlAttribute("KetQua")]
		[Bindable(true)]
		public string KetQua 
		{
			get { return GetColumnValue<string>(Columns.KetQua); }
			set { SetColumnValue(Columns.KetQua, value); }
		}
		  
		[XmlAttribute("NgaySudung")]
		[Bindable(true)]
		public DateTime? NgaySudung 
		{
			get { return GetColumnValue<DateTime?>(Columns.NgaySudung); }
			set { SetColumnValue(Columns.NgaySudung, value); }
		}
		  
		[XmlAttribute("SoDky")]
		[Bindable(true)]
		public string SoDky 
		{
			get { return GetColumnValue<string>(Columns.SoDky); }
			set { SetColumnValue(Columns.SoDky, value); }
		}
		  
		[XmlAttribute("SoQdinhthau")]
		[Bindable(true)]
		public string SoQdinhthau 
		{
			get { return GetColumnValue<string>(Columns.SoQdinhthau); }
			set { SetColumnValue(Columns.SoQdinhthau, value); }
		}
		  
		[XmlAttribute("LydoHuy")]
		[Bindable(true)]
		public string LydoHuy 
		{
			get { return GetColumnValue<string>(Columns.LydoHuy); }
			set { SetColumnValue(Columns.LydoHuy, value); }
		}
		  
		[XmlAttribute("IdPhieuTXuatthuocBenhnhan")]
		[Bindable(true)]
		public long? IdPhieuTXuatthuocBenhnhan 
		{
			get { return GetColumnValue<long?>(Columns.IdPhieuTXuatthuocBenhnhan); }
			set { SetColumnValue(Columns.IdPhieuTXuatthuocBenhnhan, value); }
		}
		  
		[XmlAttribute("IdCapphat")]
		[Bindable(true)]
		public long? IdCapphat 
		{
			get { return GetColumnValue<long?>(Columns.IdCapphat); }
			set { SetColumnValue(Columns.IdCapphat, value); }
		}
		  
		[XmlAttribute("Sang")]
		[Bindable(true)]
		public string Sang 
		{
			get { return GetColumnValue<string>(Columns.Sang); }
			set { SetColumnValue(Columns.Sang, value); }
		}
		  
		[XmlAttribute("Trua")]
		[Bindable(true)]
		public string Trua 
		{
			get { return GetColumnValue<string>(Columns.Trua); }
			set { SetColumnValue(Columns.Trua, value); }
		}
		  
		[XmlAttribute("Chieu")]
		[Bindable(true)]
		public string Chieu 
		{
			get { return GetColumnValue<string>(Columns.Chieu); }
			set { SetColumnValue(Columns.Chieu, value); }
		}
		  
		[XmlAttribute("Toi")]
		[Bindable(true)]
		public string Toi 
		{
			get { return GetColumnValue<string>(Columns.Toi); }
			set { SetColumnValue(Columns.Toi, value); }
		}
		  
		[XmlAttribute("TyleTt")]
		[Bindable(true)]
		public decimal? TyleTt 
		{
			get { return GetColumnValue<decimal?>(Columns.TyleTt); }
			set { SetColumnValue(Columns.TyleTt, value); }
		}
		  
		[XmlAttribute("IdThe")]
		[Bindable(true)]
		public long? IdThe 
		{
			get { return GetColumnValue<long?>(Columns.IdThe); }
			set { SetColumnValue(Columns.IdThe, value); }
		}
		  
		[XmlAttribute("IdDangky")]
		[Bindable(true)]
		public int? IdDangky 
		{
			get { return GetColumnValue<int?>(Columns.IdDangky); }
			set { SetColumnValue(Columns.IdDangky, value); }
		}
		  
		[XmlAttribute("TthaiTamthu")]
		[Bindable(true)]
		public byte? TthaiTamthu 
		{
			get { return GetColumnValue<byte?>(Columns.TthaiTamthu); }
			set { SetColumnValue(Columns.TthaiTamthu, value); }
		}
		  
		[XmlAttribute("IdTamthu")]
		[Bindable(true)]
		public long? IdTamthu 
		{
			get { return GetColumnValue<long?>(Columns.IdTamthu); }
			set { SetColumnValue(Columns.IdTamthu, value); }
		}
		  
		[XmlAttribute("NgayTamthu")]
		[Bindable(true)]
		public DateTime? NgayTamthu 
		{
			get { return GetColumnValue<DateTime?>(Columns.NgayTamthu); }
			set { SetColumnValue(Columns.NgayTamthu, value); }
		}
		  
		[XmlAttribute("BhytNguonKhac")]
		[Bindable(true)]
		public byte? BhytNguonKhac 
		{
			get { return GetColumnValue<byte?>(Columns.BhytNguonKhac); }
			set { SetColumnValue(Columns.BhytNguonKhac, value); }
		}
		  
		[XmlAttribute("BhytGiaTyle")]
		[Bindable(true)]
		public decimal? BhytGiaTyle 
		{
			get { return GetColumnValue<decimal?>(Columns.BhytGiaTyle); }
			set { SetColumnValue(Columns.BhytGiaTyle, value); }
		}
		  
		[XmlAttribute("BnTtt")]
		[Bindable(true)]
		public decimal? BnTtt 
		{
			get { return GetColumnValue<decimal?>(Columns.BnTtt); }
			set { SetColumnValue(Columns.BnTtt, value); }
		}
		  
		[XmlAttribute("BnCct")]
		[Bindable(true)]
		public decimal? BnCct 
		{
			get { return GetColumnValue<decimal?>(Columns.BnCct); }
			set { SetColumnValue(Columns.BnCct, value); }
		}
		  
		[XmlAttribute("Gpp")]
		[Bindable(true)]
		public bool? Gpp 
		{
			get { return GetColumnValue<bool?>(Columns.Gpp); }
			set { SetColumnValue(Columns.Gpp, value); }
		}
		  
		[XmlAttribute("TinhChiphi")]
		[Bindable(true)]
		public byte? TinhChiphi 
		{
			get { return GetColumnValue<byte?>(Columns.TinhChiphi); }
			set { SetColumnValue(Columns.TinhChiphi, value); }
		}
		  
		[XmlAttribute("GiaGoc")]
		[Bindable(true)]
		public decimal? GiaGoc 
		{
			get { return GetColumnValue<decimal?>(Columns.GiaGoc); }
			set { SetColumnValue(Columns.GiaGoc, value); }
		}
		  
		[XmlAttribute("NoidungthuchienSang")]
		[Bindable(true)]
		public string NoidungthuchienSang 
		{
			get { return GetColumnValue<string>(Columns.NoidungthuchienSang); }
			set { SetColumnValue(Columns.NoidungthuchienSang, value); }
		}
		  
		[XmlAttribute("IdNguoithuchienSang")]
		[Bindable(true)]
		public short? IdNguoithuchienSang 
		{
			get { return GetColumnValue<short?>(Columns.IdNguoithuchienSang); }
			set { SetColumnValue(Columns.IdNguoithuchienSang, value); }
		}
		  
		[XmlAttribute("TenNguoithuchienSang")]
		[Bindable(true)]
		public string TenNguoithuchienSang 
		{
			get { return GetColumnValue<string>(Columns.TenNguoithuchienSang); }
			set { SetColumnValue(Columns.TenNguoithuchienSang, value); }
		}
		  
		[XmlAttribute("NoidungthuchienTrua")]
		[Bindable(true)]
		public string NoidungthuchienTrua 
		{
			get { return GetColumnValue<string>(Columns.NoidungthuchienTrua); }
			set { SetColumnValue(Columns.NoidungthuchienTrua, value); }
		}
		  
		[XmlAttribute("IdNguoithuchienTrua")]
		[Bindable(true)]
		public short? IdNguoithuchienTrua 
		{
			get { return GetColumnValue<short?>(Columns.IdNguoithuchienTrua); }
			set { SetColumnValue(Columns.IdNguoithuchienTrua, value); }
		}
		  
		[XmlAttribute("TenNguoithuchienTrua")]
		[Bindable(true)]
		public string TenNguoithuchienTrua 
		{
			get { return GetColumnValue<string>(Columns.TenNguoithuchienTrua); }
			set { SetColumnValue(Columns.TenNguoithuchienTrua, value); }
		}
		  
		[XmlAttribute("NoidungthuchienChieu")]
		[Bindable(true)]
		public string NoidungthuchienChieu 
		{
			get { return GetColumnValue<string>(Columns.NoidungthuchienChieu); }
			set { SetColumnValue(Columns.NoidungthuchienChieu, value); }
		}
		  
		[XmlAttribute("IdNguoithuchienChieu")]
		[Bindable(true)]
		public short? IdNguoithuchienChieu 
		{
			get { return GetColumnValue<short?>(Columns.IdNguoithuchienChieu); }
			set { SetColumnValue(Columns.IdNguoithuchienChieu, value); }
		}
		  
		[XmlAttribute("TenNguoithuchienChieu")]
		[Bindable(true)]
		public string TenNguoithuchienChieu 
		{
			get { return GetColumnValue<string>(Columns.TenNguoithuchienChieu); }
			set { SetColumnValue(Columns.TenNguoithuchienChieu, value); }
		}
		  
		[XmlAttribute("NoidungthuchienToi")]
		[Bindable(true)]
		public string NoidungthuchienToi 
		{
			get { return GetColumnValue<string>(Columns.NoidungthuchienToi); }
			set { SetColumnValue(Columns.NoidungthuchienToi, value); }
		}
		  
		[XmlAttribute("IdNguoithuchienToi")]
		[Bindable(true)]
		public short? IdNguoithuchienToi 
		{
			get { return GetColumnValue<short?>(Columns.IdNguoithuchienToi); }
			set { SetColumnValue(Columns.IdNguoithuchienToi, value); }
		}
		  
		[XmlAttribute("TenNguoithuchienToi")]
		[Bindable(true)]
		public string TenNguoithuchienToi 
		{
			get { return GetColumnValue<string>(Columns.TenNguoithuchienToi); }
			set { SetColumnValue(Columns.TenNguoithuchienToi, value); }
		}
		  
		[XmlAttribute("TthaiGui")]
		[Bindable(true)]
		public byte? TthaiGui 
		{
			get { return GetColumnValue<byte?>(Columns.TthaiGui); }
			set { SetColumnValue(Columns.TthaiGui, value); }
		}
		  
		[XmlAttribute("TransactionId")]
		[Bindable(true)]
		public string TransactionId 
		{
			get { return GetColumnValue<string>(Columns.TransactionId); }
			set { SetColumnValue(Columns.TransactionId, value); }
		}
		  
		[XmlAttribute("IdGui")]
		[Bindable(true)]
		public long? IdGui 
		{
			get { return GetColumnValue<long?>(Columns.IdGui); }
			set { SetColumnValue(Columns.IdGui, value); }
		}
		  
		[XmlAttribute("IdHuydvu")]
		[Bindable(true)]
		public long? IdHuydvu 
		{
			get { return GetColumnValue<long?>(Columns.IdHuydvu); }
			set { SetColumnValue(Columns.IdHuydvu, value); }
		}
		  
		[XmlAttribute("HienthiBienlai")]
		[Bindable(true)]
		public byte? HienthiBienlai 
		{
			get { return GetColumnValue<byte?>(Columns.HienthiBienlai); }
			set { SetColumnValue(Columns.HienthiBienlai, value); }
		}
		
		#endregion
		
		
			
		
		//no foreign key tables defined (0)
		
		
		
		//no ManyToMany tables defined (0)
		
        
        
		#region ObjectDataSource support
		
		
		/// <summary>
		/// Inserts a record, can be used with the Object Data Source
		/// </summary>
		public static void Insert(long varIdDonthuoc,long? varIdDonthuocChuyengoi,long? varIdBenhnhan,string varMaLuotkham,long? varIdKham,int? varIdKho,int varIdThuoc,DateTime varNgayHethan,decimal varSoLuong,decimal? varSluongSua,decimal? varSluongLinh,decimal varDonGia,long? varIdThuockho,DateTime? varNgayNhap,decimal? varGiaNhap,decimal? varGiaBan,decimal? varGiaBhyt,string varSoLo,decimal? varVat,string varMaNhacungcap,decimal? varPhuThu,decimal? varPhuthuDungtuyen,decimal? varPhuthuTraituyen,string varMotaThem,int? varSoluongHuy,byte? varTrangthaiHuy,string varNguoiHuy,DateTime? varNgayHuy,byte? varTuTuc,byte? varTrangThai,byte? varTrangthaiTonghop,DateTime? varNgayXacnhan,byte? varTrangthaiBhyt,short? varSttIn,string varMadoituongGia,decimal? varPtramBhytGoc,decimal? varPtramBhyt,decimal? varBhytChitra,decimal? varBnhanChitra,string varMaDoituongKcb,int? varIdThanhtoan,byte? varTrangthaiThanhtoan,DateTime? varNgayThanhtoan,string varCachDung,string varChidanThem,string varDonviTinh,string varSolanDung,string varSoluongDung,byte? varTrangthaiChuyen,DateTime? varNgayTao,string varNguoiTao,DateTime? varNgaySua,string varNguoiSua,decimal? varTileChietkhau,decimal? varTienChietkhau,string varKieuChietkhau,int? varIdGoi,byte? varTrongGoi,string varKieuBiendong,byte? varNguonThanhtoan,string varIpMaytao,string varIpMaysua,string varTenMaytao,string varTenMaysua,byte? varDaDung,string varLydoTiemchung,short? varNguoiTiem,string varVitriTiem,byte? varMuiThu,DateTime? varNgayhenMuiketiep,string varPhanungSautiem,string varXutri,string varKetluanNguyennhan,string varKetQua,DateTime? varNgaySudung,string varSoDky,string varSoQdinhthau,string varLydoHuy,long? varIdPhieuTXuatthuocBenhnhan,long? varIdCapphat,string varSang,string varTrua,string varChieu,string varToi,decimal? varTyleTt,long? varIdThe,int? varIdDangky,byte? varTthaiTamthu,long? varIdTamthu,DateTime? varNgayTamthu,byte? varBhytNguonKhac,decimal? varBhytGiaTyle,decimal? varBnTtt,decimal? varBnCct,bool? varGpp,byte? varTinhChiphi,decimal? varGiaGoc,string varNoidungthuchienSang,short? varIdNguoithuchienSang,string varTenNguoithuchienSang,string varNoidungthuchienTrua,short? varIdNguoithuchienTrua,string varTenNguoithuchienTrua,string varNoidungthuchienChieu,short? varIdNguoithuchienChieu,string varTenNguoithuchienChieu,string varNoidungthuchienToi,short? varIdNguoithuchienToi,string varTenNguoithuchienToi,byte? varTthaiGui,string varTransactionId,long? varIdGui,long? varIdHuydvu,byte? varHienthiBienlai)
		{
			KcbDonthuocChitiet item = new KcbDonthuocChitiet();
			
			item.IdDonthuoc = varIdDonthuoc;
			
			item.IdDonthuocChuyengoi = varIdDonthuocChuyengoi;
			
			item.IdBenhnhan = varIdBenhnhan;
			
			item.MaLuotkham = varMaLuotkham;
			
			item.IdKham = varIdKham;
			
			item.IdKho = varIdKho;
			
			item.IdThuoc = varIdThuoc;
			
			item.NgayHethan = varNgayHethan;
			
			item.SoLuong = varSoLuong;
			
			item.SluongSua = varSluongSua;
			
			item.SluongLinh = varSluongLinh;
			
			item.DonGia = varDonGia;
			
			item.IdThuockho = varIdThuockho;
			
			item.NgayNhap = varNgayNhap;
			
			item.GiaNhap = varGiaNhap;
			
			item.GiaBan = varGiaBan;
			
			item.GiaBhyt = varGiaBhyt;
			
			item.SoLo = varSoLo;
			
			item.Vat = varVat;
			
			item.MaNhacungcap = varMaNhacungcap;
			
			item.PhuThu = varPhuThu;
			
			item.PhuthuDungtuyen = varPhuthuDungtuyen;
			
			item.PhuthuTraituyen = varPhuthuTraituyen;
			
			item.MotaThem = varMotaThem;
			
			item.SoluongHuy = varSoluongHuy;
			
			item.TrangthaiHuy = varTrangthaiHuy;
			
			item.NguoiHuy = varNguoiHuy;
			
			item.NgayHuy = varNgayHuy;
			
			item.TuTuc = varTuTuc;
			
			item.TrangThai = varTrangThai;
			
			item.TrangthaiTonghop = varTrangthaiTonghop;
			
			item.NgayXacnhan = varNgayXacnhan;
			
			item.TrangthaiBhyt = varTrangthaiBhyt;
			
			item.SttIn = varSttIn;
			
			item.MadoituongGia = varMadoituongGia;
			
			item.PtramBhytGoc = varPtramBhytGoc;
			
			item.PtramBhyt = varPtramBhyt;
			
			item.BhytChitra = varBhytChitra;
			
			item.BnhanChitra = varBnhanChitra;
			
			item.MaDoituongKcb = varMaDoituongKcb;
			
			item.IdThanhtoan = varIdThanhtoan;
			
			item.TrangthaiThanhtoan = varTrangthaiThanhtoan;
			
			item.NgayThanhtoan = varNgayThanhtoan;
			
			item.CachDung = varCachDung;
			
			item.ChidanThem = varChidanThem;
			
			item.DonviTinh = varDonviTinh;
			
			item.SolanDung = varSolanDung;
			
			item.SoluongDung = varSoluongDung;
			
			item.TrangthaiChuyen = varTrangthaiChuyen;
			
			item.NgayTao = varNgayTao;
			
			item.NguoiTao = varNguoiTao;
			
			item.NgaySua = varNgaySua;
			
			item.NguoiSua = varNguoiSua;
			
			item.TileChietkhau = varTileChietkhau;
			
			item.TienChietkhau = varTienChietkhau;
			
			item.KieuChietkhau = varKieuChietkhau;
			
			item.IdGoi = varIdGoi;
			
			item.TrongGoi = varTrongGoi;
			
			item.KieuBiendong = varKieuBiendong;
			
			item.NguonThanhtoan = varNguonThanhtoan;
			
			item.IpMaytao = varIpMaytao;
			
			item.IpMaysua = varIpMaysua;
			
			item.TenMaytao = varTenMaytao;
			
			item.TenMaysua = varTenMaysua;
			
			item.DaDung = varDaDung;
			
			item.LydoTiemchung = varLydoTiemchung;
			
			item.NguoiTiem = varNguoiTiem;
			
			item.VitriTiem = varVitriTiem;
			
			item.MuiThu = varMuiThu;
			
			item.NgayhenMuiketiep = varNgayhenMuiketiep;
			
			item.PhanungSautiem = varPhanungSautiem;
			
			item.Xutri = varXutri;
			
			item.KetluanNguyennhan = varKetluanNguyennhan;
			
			item.KetQua = varKetQua;
			
			item.NgaySudung = varNgaySudung;
			
			item.SoDky = varSoDky;
			
			item.SoQdinhthau = varSoQdinhthau;
			
			item.LydoHuy = varLydoHuy;
			
			item.IdPhieuTXuatthuocBenhnhan = varIdPhieuTXuatthuocBenhnhan;
			
			item.IdCapphat = varIdCapphat;
			
			item.Sang = varSang;
			
			item.Trua = varTrua;
			
			item.Chieu = varChieu;
			
			item.Toi = varToi;
			
			item.TyleTt = varTyleTt;
			
			item.IdThe = varIdThe;
			
			item.IdDangky = varIdDangky;
			
			item.TthaiTamthu = varTthaiTamthu;
			
			item.IdTamthu = varIdTamthu;
			
			item.NgayTamthu = varNgayTamthu;
			
			item.BhytNguonKhac = varBhytNguonKhac;
			
			item.BhytGiaTyle = varBhytGiaTyle;
			
			item.BnTtt = varBnTtt;
			
			item.BnCct = varBnCct;
			
			item.Gpp = varGpp;
			
			item.TinhChiphi = varTinhChiphi;
			
			item.GiaGoc = varGiaGoc;
			
			item.NoidungthuchienSang = varNoidungthuchienSang;
			
			item.IdNguoithuchienSang = varIdNguoithuchienSang;
			
			item.TenNguoithuchienSang = varTenNguoithuchienSang;
			
			item.NoidungthuchienTrua = varNoidungthuchienTrua;
			
			item.IdNguoithuchienTrua = varIdNguoithuchienTrua;
			
			item.TenNguoithuchienTrua = varTenNguoithuchienTrua;
			
			item.NoidungthuchienChieu = varNoidungthuchienChieu;
			
			item.IdNguoithuchienChieu = varIdNguoithuchienChieu;
			
			item.TenNguoithuchienChieu = varTenNguoithuchienChieu;
			
			item.NoidungthuchienToi = varNoidungthuchienToi;
			
			item.IdNguoithuchienToi = varIdNguoithuchienToi;
			
			item.TenNguoithuchienToi = varTenNguoithuchienToi;
			
			item.TthaiGui = varTthaiGui;
			
			item.TransactionId = varTransactionId;
			
			item.IdGui = varIdGui;
			
			item.IdHuydvu = varIdHuydvu;
			
			item.HienthiBienlai = varHienthiBienlai;
			
		
			if (System.Web.HttpContext.Current != null)
				item.Save(System.Web.HttpContext.Current.User.Identity.Name);
			else
				item.Save(System.Threading.Thread.CurrentPrincipal.Identity.Name);
		}
		
		/// <summary>
		/// Updates a record, can be used with the Object Data Source
		/// </summary>
		public static void Update(long varIdChitietdonthuoc,long varIdDonthuoc,long? varIdDonthuocChuyengoi,long? varIdBenhnhan,string varMaLuotkham,long? varIdKham,int? varIdKho,int varIdThuoc,DateTime varNgayHethan,decimal varSoLuong,decimal? varSluongSua,decimal? varSluongLinh,decimal varDonGia,long? varIdThuockho,DateTime? varNgayNhap,decimal? varGiaNhap,decimal? varGiaBan,decimal? varGiaBhyt,string varSoLo,decimal? varVat,string varMaNhacungcap,decimal? varPhuThu,decimal? varPhuthuDungtuyen,decimal? varPhuthuTraituyen,string varMotaThem,int? varSoluongHuy,byte? varTrangthaiHuy,string varNguoiHuy,DateTime? varNgayHuy,byte? varTuTuc,byte? varTrangThai,byte? varTrangthaiTonghop,DateTime? varNgayXacnhan,byte? varTrangthaiBhyt,short? varSttIn,string varMadoituongGia,decimal? varPtramBhytGoc,decimal? varPtramBhyt,decimal? varBhytChitra,decimal? varBnhanChitra,string varMaDoituongKcb,int? varIdThanhtoan,byte? varTrangthaiThanhtoan,DateTime? varNgayThanhtoan,string varCachDung,string varChidanThem,string varDonviTinh,string varSolanDung,string varSoluongDung,byte? varTrangthaiChuyen,DateTime? varNgayTao,string varNguoiTao,DateTime? varNgaySua,string varNguoiSua,decimal? varTileChietkhau,decimal? varTienChietkhau,string varKieuChietkhau,int? varIdGoi,byte? varTrongGoi,string varKieuBiendong,byte? varNguonThanhtoan,string varIpMaytao,string varIpMaysua,string varTenMaytao,string varTenMaysua,byte? varDaDung,string varLydoTiemchung,short? varNguoiTiem,string varVitriTiem,byte? varMuiThu,DateTime? varNgayhenMuiketiep,string varPhanungSautiem,string varXutri,string varKetluanNguyennhan,string varKetQua,DateTime? varNgaySudung,string varSoDky,string varSoQdinhthau,string varLydoHuy,long? varIdPhieuTXuatthuocBenhnhan,long? varIdCapphat,string varSang,string varTrua,string varChieu,string varToi,decimal? varTyleTt,long? varIdThe,int? varIdDangky,byte? varTthaiTamthu,long? varIdTamthu,DateTime? varNgayTamthu,byte? varBhytNguonKhac,decimal? varBhytGiaTyle,decimal? varBnTtt,decimal? varBnCct,bool? varGpp,byte? varTinhChiphi,decimal? varGiaGoc,string varNoidungthuchienSang,short? varIdNguoithuchienSang,string varTenNguoithuchienSang,string varNoidungthuchienTrua,short? varIdNguoithuchienTrua,string varTenNguoithuchienTrua,string varNoidungthuchienChieu,short? varIdNguoithuchienChieu,string varTenNguoithuchienChieu,string varNoidungthuchienToi,short? varIdNguoithuchienToi,string varTenNguoithuchienToi,byte? varTthaiGui,string varTransactionId,long? varIdGui,long? varIdHuydvu,byte? varHienthiBienlai)
		{
			KcbDonthuocChitiet item = new KcbDonthuocChitiet();
			
				item.IdChitietdonthuoc = varIdChitietdonthuoc;
			
				item.IdDonthuoc = varIdDonthuoc;
			
				item.IdDonthuocChuyengoi = varIdDonthuocChuyengoi;
			
				item.IdBenhnhan = varIdBenhnhan;
			
				item.MaLuotkham = varMaLuotkham;
			
				item.IdKham = varIdKham;
			
				item.IdKho = varIdKho;
			
				item.IdThuoc = varIdThuoc;
			
				item.NgayHethan = varNgayHethan;
			
				item.SoLuong = varSoLuong;
			
				item.SluongSua = varSluongSua;
			
				item.SluongLinh = varSluongLinh;
			
				item.DonGia = varDonGia;
			
				item.IdThuockho = varIdThuockho;
			
				item.NgayNhap = varNgayNhap;
			
				item.GiaNhap = varGiaNhap;
			
				item.GiaBan = varGiaBan;
			
				item.GiaBhyt = varGiaBhyt;
			
				item.SoLo = varSoLo;
			
				item.Vat = varVat;
			
				item.MaNhacungcap = varMaNhacungcap;
			
				item.PhuThu = varPhuThu;
			
				item.PhuthuDungtuyen = varPhuthuDungtuyen;
			
				item.PhuthuTraituyen = varPhuthuTraituyen;
			
				item.MotaThem = varMotaThem;
			
				item.SoluongHuy = varSoluongHuy;
			
				item.TrangthaiHuy = varTrangthaiHuy;
			
				item.NguoiHuy = varNguoiHuy;
			
				item.NgayHuy = varNgayHuy;
			
				item.TuTuc = varTuTuc;
			
				item.TrangThai = varTrangThai;
			
				item.TrangthaiTonghop = varTrangthaiTonghop;
			
				item.NgayXacnhan = varNgayXacnhan;
			
				item.TrangthaiBhyt = varTrangthaiBhyt;
			
				item.SttIn = varSttIn;
			
				item.MadoituongGia = varMadoituongGia;
			
				item.PtramBhytGoc = varPtramBhytGoc;
			
				item.PtramBhyt = varPtramBhyt;
			
				item.BhytChitra = varBhytChitra;
			
				item.BnhanChitra = varBnhanChitra;
			
				item.MaDoituongKcb = varMaDoituongKcb;
			
				item.IdThanhtoan = varIdThanhtoan;
			
				item.TrangthaiThanhtoan = varTrangthaiThanhtoan;
			
				item.NgayThanhtoan = varNgayThanhtoan;
			
				item.CachDung = varCachDung;
			
				item.ChidanThem = varChidanThem;
			
				item.DonviTinh = varDonviTinh;
			
				item.SolanDung = varSolanDung;
			
				item.SoluongDung = varSoluongDung;
			
				item.TrangthaiChuyen = varTrangthaiChuyen;
			
				item.NgayTao = varNgayTao;
			
				item.NguoiTao = varNguoiTao;
			
				item.NgaySua = varNgaySua;
			
				item.NguoiSua = varNguoiSua;
			
				item.TileChietkhau = varTileChietkhau;
			
				item.TienChietkhau = varTienChietkhau;
			
				item.KieuChietkhau = varKieuChietkhau;
			
				item.IdGoi = varIdGoi;
			
				item.TrongGoi = varTrongGoi;
			
				item.KieuBiendong = varKieuBiendong;
			
				item.NguonThanhtoan = varNguonThanhtoan;
			
				item.IpMaytao = varIpMaytao;
			
				item.IpMaysua = varIpMaysua;
			
				item.TenMaytao = varTenMaytao;
			
				item.TenMaysua = varTenMaysua;
			
				item.DaDung = varDaDung;
			
				item.LydoTiemchung = varLydoTiemchung;
			
				item.NguoiTiem = varNguoiTiem;
			
				item.VitriTiem = varVitriTiem;
			
				item.MuiThu = varMuiThu;
			
				item.NgayhenMuiketiep = varNgayhenMuiketiep;
			
				item.PhanungSautiem = varPhanungSautiem;
			
				item.Xutri = varXutri;
			
				item.KetluanNguyennhan = varKetluanNguyennhan;
			
				item.KetQua = varKetQua;
			
				item.NgaySudung = varNgaySudung;
			
				item.SoDky = varSoDky;
			
				item.SoQdinhthau = varSoQdinhthau;
			
				item.LydoHuy = varLydoHuy;
			
				item.IdPhieuTXuatthuocBenhnhan = varIdPhieuTXuatthuocBenhnhan;
			
				item.IdCapphat = varIdCapphat;
			
				item.Sang = varSang;
			
				item.Trua = varTrua;
			
				item.Chieu = varChieu;
			
				item.Toi = varToi;
			
				item.TyleTt = varTyleTt;
			
				item.IdThe = varIdThe;
			
				item.IdDangky = varIdDangky;
			
				item.TthaiTamthu = varTthaiTamthu;
			
				item.IdTamthu = varIdTamthu;
			
				item.NgayTamthu = varNgayTamthu;
			
				item.BhytNguonKhac = varBhytNguonKhac;
			
				item.BhytGiaTyle = varBhytGiaTyle;
			
				item.BnTtt = varBnTtt;
			
				item.BnCct = varBnCct;
			
				item.Gpp = varGpp;
			
				item.TinhChiphi = varTinhChiphi;
			
				item.GiaGoc = varGiaGoc;
			
				item.NoidungthuchienSang = varNoidungthuchienSang;
			
				item.IdNguoithuchienSang = varIdNguoithuchienSang;
			
				item.TenNguoithuchienSang = varTenNguoithuchienSang;
			
				item.NoidungthuchienTrua = varNoidungthuchienTrua;
			
				item.IdNguoithuchienTrua = varIdNguoithuchienTrua;
			
				item.TenNguoithuchienTrua = varTenNguoithuchienTrua;
			
				item.NoidungthuchienChieu = varNoidungthuchienChieu;
			
				item.IdNguoithuchienChieu = varIdNguoithuchienChieu;
			
				item.TenNguoithuchienChieu = varTenNguoithuchienChieu;
			
				item.NoidungthuchienToi = varNoidungthuchienToi;
			
				item.IdNguoithuchienToi = varIdNguoithuchienToi;
			
				item.TenNguoithuchienToi = varTenNguoithuchienToi;
			
				item.TthaiGui = varTthaiGui;
			
				item.TransactionId = varTransactionId;
			
				item.IdGui = varIdGui;
			
				item.IdHuydvu = varIdHuydvu;
			
				item.HienthiBienlai = varHienthiBienlai;
			
			item.IsNew = false;
			if (System.Web.HttpContext.Current != null)
				item.Save(System.Web.HttpContext.Current.User.Identity.Name);
			else
				item.Save(System.Threading.Thread.CurrentPrincipal.Identity.Name);
		}
		#endregion
        
        
        
        #region Typed Columns
        
        
        public static TableSchema.TableColumn IdChitietdonthuocColumn
        {
            get { return Schema.Columns[0]; }
        }
        
        
        
        public static TableSchema.TableColumn IdDonthuocColumn
        {
            get { return Schema.Columns[1]; }
        }
        
        
        
        public static TableSchema.TableColumn IdDonthuocChuyengoiColumn
        {
            get { return Schema.Columns[2]; }
        }
        
        
        
        public static TableSchema.TableColumn IdBenhnhanColumn
        {
            get { return Schema.Columns[3]; }
        }
        
        
        
        public static TableSchema.TableColumn MaLuotkhamColumn
        {
            get { return Schema.Columns[4]; }
        }
        
        
        
        public static TableSchema.TableColumn IdKhamColumn
        {
            get { return Schema.Columns[5]; }
        }
        
        
        
        public static TableSchema.TableColumn IdKhoColumn
        {
            get { return Schema.Columns[6]; }
        }
        
        
        
        public static TableSchema.TableColumn IdThuocColumn
        {
            get { return Schema.Columns[7]; }
        }
        
        
        
        public static TableSchema.TableColumn NgayHethanColumn
        {
            get { return Schema.Columns[8]; }
        }
        
        
        
        public static TableSchema.TableColumn SoLuongColumn
        {
            get { return Schema.Columns[9]; }
        }
        
        
        
        public static TableSchema.TableColumn SluongSuaColumn
        {
            get { return Schema.Columns[10]; }
        }
        
        
        
        public static TableSchema.TableColumn SluongLinhColumn
        {
            get { return Schema.Columns[11]; }
        }
        
        
        
        public static TableSchema.TableColumn DonGiaColumn
        {
            get { return Schema.Columns[12]; }
        }
        
        
        
        public static TableSchema.TableColumn IdThuockhoColumn
        {
            get { return Schema.Columns[13]; }
        }
        
        
        
        public static TableSchema.TableColumn NgayNhapColumn
        {
            get { return Schema.Columns[14]; }
        }
        
        
        
        public static TableSchema.TableColumn GiaNhapColumn
        {
            get { return Schema.Columns[15]; }
        }
        
        
        
        public static TableSchema.TableColumn GiaBanColumn
        {
            get { return Schema.Columns[16]; }
        }
        
        
        
        public static TableSchema.TableColumn GiaBhytColumn
        {
            get { return Schema.Columns[17]; }
        }
        
        
        
        public static TableSchema.TableColumn SoLoColumn
        {
            get { return Schema.Columns[18]; }
        }
        
        
        
        public static TableSchema.TableColumn VatColumn
        {
            get { return Schema.Columns[19]; }
        }
        
        
        
        public static TableSchema.TableColumn MaNhacungcapColumn
        {
            get { return Schema.Columns[20]; }
        }
        
        
        
        public static TableSchema.TableColumn PhuThuColumn
        {
            get { return Schema.Columns[21]; }
        }
        
        
        
        public static TableSchema.TableColumn PhuthuDungtuyenColumn
        {
            get { return Schema.Columns[22]; }
        }
        
        
        
        public static TableSchema.TableColumn PhuthuTraituyenColumn
        {
            get { return Schema.Columns[23]; }
        }
        
        
        
        public static TableSchema.TableColumn MotaThemColumn
        {
            get { return Schema.Columns[24]; }
        }
        
        
        
        public static TableSchema.TableColumn SoluongHuyColumn
        {
            get { return Schema.Columns[25]; }
        }
        
        
        
        public static TableSchema.TableColumn TrangthaiHuyColumn
        {
            get { return Schema.Columns[26]; }
        }
        
        
        
        public static TableSchema.TableColumn NguoiHuyColumn
        {
            get { return Schema.Columns[27]; }
        }
        
        
        
        public static TableSchema.TableColumn NgayHuyColumn
        {
            get { return Schema.Columns[28]; }
        }
        
        
        
        public static TableSchema.TableColumn TuTucColumn
        {
            get { return Schema.Columns[29]; }
        }
        
        
        
        public static TableSchema.TableColumn TrangThaiColumn
        {
            get { return Schema.Columns[30]; }
        }
        
        
        
        public static TableSchema.TableColumn TrangthaiTonghopColumn
        {
            get { return Schema.Columns[31]; }
        }
        
        
        
        public static TableSchema.TableColumn NgayXacnhanColumn
        {
            get { return Schema.Columns[32]; }
        }
        
        
        
        public static TableSchema.TableColumn TrangthaiBhytColumn
        {
            get { return Schema.Columns[33]; }
        }
        
        
        
        public static TableSchema.TableColumn SttInColumn
        {
            get { return Schema.Columns[34]; }
        }
        
        
        
        public static TableSchema.TableColumn MadoituongGiaColumn
        {
            get { return Schema.Columns[35]; }
        }
        
        
        
        public static TableSchema.TableColumn PtramBhytGocColumn
        {
            get { return Schema.Columns[36]; }
        }
        
        
        
        public static TableSchema.TableColumn PtramBhytColumn
        {
            get { return Schema.Columns[37]; }
        }
        
        
        
        public static TableSchema.TableColumn BhytChitraColumn
        {
            get { return Schema.Columns[38]; }
        }
        
        
        
        public static TableSchema.TableColumn BnhanChitraColumn
        {
            get { return Schema.Columns[39]; }
        }
        
        
        
        public static TableSchema.TableColumn MaDoituongKcbColumn
        {
            get { return Schema.Columns[40]; }
        }
        
        
        
        public static TableSchema.TableColumn IdThanhtoanColumn
        {
            get { return Schema.Columns[41]; }
        }
        
        
        
        public static TableSchema.TableColumn TrangthaiThanhtoanColumn
        {
            get { return Schema.Columns[42]; }
        }
        
        
        
        public static TableSchema.TableColumn NgayThanhtoanColumn
        {
            get { return Schema.Columns[43]; }
        }
        
        
        
        public static TableSchema.TableColumn CachDungColumn
        {
            get { return Schema.Columns[44]; }
        }
        
        
        
        public static TableSchema.TableColumn ChidanThemColumn
        {
            get { return Schema.Columns[45]; }
        }
        
        
        
        public static TableSchema.TableColumn DonviTinhColumn
        {
            get { return Schema.Columns[46]; }
        }
        
        
        
        public static TableSchema.TableColumn SolanDungColumn
        {
            get { return Schema.Columns[47]; }
        }
        
        
        
        public static TableSchema.TableColumn SoluongDungColumn
        {
            get { return Schema.Columns[48]; }
        }
        
        
        
        public static TableSchema.TableColumn TrangthaiChuyenColumn
        {
            get { return Schema.Columns[49]; }
        }
        
        
        
        public static TableSchema.TableColumn NgayTaoColumn
        {
            get { return Schema.Columns[50]; }
        }
        
        
        
        public static TableSchema.TableColumn NguoiTaoColumn
        {
            get { return Schema.Columns[51]; }
        }
        
        
        
        public static TableSchema.TableColumn NgaySuaColumn
        {
            get { return Schema.Columns[52]; }
        }
        
        
        
        public static TableSchema.TableColumn NguoiSuaColumn
        {
            get { return Schema.Columns[53]; }
        }
        
        
        
        public static TableSchema.TableColumn TileChietkhauColumn
        {
            get { return Schema.Columns[54]; }
        }
        
        
        
        public static TableSchema.TableColumn TienChietkhauColumn
        {
            get { return Schema.Columns[55]; }
        }
        
        
        
        public static TableSchema.TableColumn KieuChietkhauColumn
        {
            get { return Schema.Columns[56]; }
        }
        
        
        
        public static TableSchema.TableColumn IdGoiColumn
        {
            get { return Schema.Columns[57]; }
        }
        
        
        
        public static TableSchema.TableColumn TrongGoiColumn
        {
            get { return Schema.Columns[58]; }
        }
        
        
        
        public static TableSchema.TableColumn KieuBiendongColumn
        {
            get { return Schema.Columns[59]; }
        }
        
        
        
        public static TableSchema.TableColumn NguonThanhtoanColumn
        {
            get { return Schema.Columns[60]; }
        }
        
        
        
        public static TableSchema.TableColumn IpMaytaoColumn
        {
            get { return Schema.Columns[61]; }
        }
        
        
        
        public static TableSchema.TableColumn IpMaysuaColumn
        {
            get { return Schema.Columns[62]; }
        }
        
        
        
        public static TableSchema.TableColumn TenMaytaoColumn
        {
            get { return Schema.Columns[63]; }
        }
        
        
        
        public static TableSchema.TableColumn TenMaysuaColumn
        {
            get { return Schema.Columns[64]; }
        }
        
        
        
        public static TableSchema.TableColumn DaDungColumn
        {
            get { return Schema.Columns[65]; }
        }
        
        
        
        public static TableSchema.TableColumn LydoTiemchungColumn
        {
            get { return Schema.Columns[66]; }
        }
        
        
        
        public static TableSchema.TableColumn NguoiTiemColumn
        {
            get { return Schema.Columns[67]; }
        }
        
        
        
        public static TableSchema.TableColumn VitriTiemColumn
        {
            get { return Schema.Columns[68]; }
        }
        
        
        
        public static TableSchema.TableColumn MuiThuColumn
        {
            get { return Schema.Columns[69]; }
        }
        
        
        
        public static TableSchema.TableColumn NgayhenMuiketiepColumn
        {
            get { return Schema.Columns[70]; }
        }
        
        
        
        public static TableSchema.TableColumn PhanungSautiemColumn
        {
            get { return Schema.Columns[71]; }
        }
        
        
        
        public static TableSchema.TableColumn XutriColumn
        {
            get { return Schema.Columns[72]; }
        }
        
        
        
        public static TableSchema.TableColumn KetluanNguyennhanColumn
        {
            get { return Schema.Columns[73]; }
        }
        
        
        
        public static TableSchema.TableColumn KetQuaColumn
        {
            get { return Schema.Columns[74]; }
        }
        
        
        
        public static TableSchema.TableColumn NgaySudungColumn
        {
            get { return Schema.Columns[75]; }
        }
        
        
        
        public static TableSchema.TableColumn SoDkyColumn
        {
            get { return Schema.Columns[76]; }
        }
        
        
        
        public static TableSchema.TableColumn SoQdinhthauColumn
        {
            get { return Schema.Columns[77]; }
        }
        
        
        
        public static TableSchema.TableColumn LydoHuyColumn
        {
            get { return Schema.Columns[78]; }
        }
        
        
        
        public static TableSchema.TableColumn IdPhieuTXuatthuocBenhnhanColumn
        {
            get { return Schema.Columns[79]; }
        }
        
        
        
        public static TableSchema.TableColumn IdCapphatColumn
        {
            get { return Schema.Columns[80]; }
        }
        
        
        
        public static TableSchema.TableColumn SangColumn
        {
            get { return Schema.Columns[81]; }
        }
        
        
        
        public static TableSchema.TableColumn TruaColumn
        {
            get { return Schema.Columns[82]; }
        }
        
        
        
        public static TableSchema.TableColumn ChieuColumn
        {
            get { return Schema.Columns[83]; }
        }
        
        
        
        public static TableSchema.TableColumn ToiColumn
        {
            get { return Schema.Columns[84]; }
        }
        
        
        
        public static TableSchema.TableColumn TyleTtColumn
        {
            get { return Schema.Columns[85]; }
        }
        
        
        
        public static TableSchema.TableColumn IdTheColumn
        {
            get { return Schema.Columns[86]; }
        }
        
        
        
        public static TableSchema.TableColumn IdDangkyColumn
        {
            get { return Schema.Columns[87]; }
        }
        
        
        
        public static TableSchema.TableColumn TthaiTamthuColumn
        {
            get { return Schema.Columns[88]; }
        }
        
        
        
        public static TableSchema.TableColumn IdTamthuColumn
        {
            get { return Schema.Columns[89]; }
        }
        
        
        
        public static TableSchema.TableColumn NgayTamthuColumn
        {
            get { return Schema.Columns[90]; }
        }
        
        
        
        public static TableSchema.TableColumn BhytNguonKhacColumn
        {
            get { return Schema.Columns[91]; }
        }
        
        
        
        public static TableSchema.TableColumn BhytGiaTyleColumn
        {
            get { return Schema.Columns[92]; }
        }
        
        
        
        public static TableSchema.TableColumn BnTttColumn
        {
            get { return Schema.Columns[93]; }
        }
        
        
        
        public static TableSchema.TableColumn BnCctColumn
        {
            get { return Schema.Columns[94]; }
        }
        
        
        
        public static TableSchema.TableColumn GppColumn
        {
            get { return Schema.Columns[95]; }
        }
        
        
        
        public static TableSchema.TableColumn TinhChiphiColumn
        {
            get { return Schema.Columns[96]; }
        }
        
        
        
        public static TableSchema.TableColumn GiaGocColumn
        {
            get { return Schema.Columns[97]; }
        }
        
        
        
        public static TableSchema.TableColumn NoidungthuchienSangColumn
        {
            get { return Schema.Columns[98]; }
        }
        
        
        
        public static TableSchema.TableColumn IdNguoithuchienSangColumn
        {
            get { return Schema.Columns[99]; }
        }
        
        
        
        public static TableSchema.TableColumn TenNguoithuchienSangColumn
        {
            get { return Schema.Columns[100]; }
        }
        
        
        
        public static TableSchema.TableColumn NoidungthuchienTruaColumn
        {
            get { return Schema.Columns[101]; }
        }
        
        
        
        public static TableSchema.TableColumn IdNguoithuchienTruaColumn
        {
            get { return Schema.Columns[102]; }
        }
        
        
        
        public static TableSchema.TableColumn TenNguoithuchienTruaColumn
        {
            get { return Schema.Columns[103]; }
        }
        
        
        
        public static TableSchema.TableColumn NoidungthuchienChieuColumn
        {
            get { return Schema.Columns[104]; }
        }
        
        
        
        public static TableSchema.TableColumn IdNguoithuchienChieuColumn
        {
            get { return Schema.Columns[105]; }
        }
        
        
        
        public static TableSchema.TableColumn TenNguoithuchienChieuColumn
        {
            get { return Schema.Columns[106]; }
        }
        
        
        
        public static TableSchema.TableColumn NoidungthuchienToiColumn
        {
            get { return Schema.Columns[107]; }
        }
        
        
        
        public static TableSchema.TableColumn IdNguoithuchienToiColumn
        {
            get { return Schema.Columns[108]; }
        }
        
        
        
        public static TableSchema.TableColumn TenNguoithuchienToiColumn
        {
            get { return Schema.Columns[109]; }
        }
        
        
        
        public static TableSchema.TableColumn TthaiGuiColumn
        {
            get { return Schema.Columns[110]; }
        }
        
        
        
        public static TableSchema.TableColumn TransactionIdColumn
        {
            get { return Schema.Columns[111]; }
        }
        
        
        
        public static TableSchema.TableColumn IdGuiColumn
        {
            get { return Schema.Columns[112]; }
        }
        
        
        
        public static TableSchema.TableColumn IdHuydvuColumn
        {
            get { return Schema.Columns[113]; }
        }
        
        
        
        public static TableSchema.TableColumn HienthiBienlaiColumn
        {
            get { return Schema.Columns[114]; }
        }
        
        
        
        #endregion
		#region Columns Struct
		public struct Columns
		{
			 public static string IdChitietdonthuoc = @"id_chitietdonthuoc";
			 public static string IdDonthuoc = @"id_donthuoc";
			 public static string IdDonthuocChuyengoi = @"id_donthuoc_chuyengoi";
			 public static string IdBenhnhan = @"id_benhnhan";
			 public static string MaLuotkham = @"ma_luotkham";
			 public static string IdKham = @"id_kham";
			 public static string IdKho = @"id_kho";
			 public static string IdThuoc = @"id_thuoc";
			 public static string NgayHethan = @"ngay_hethan";
			 public static string SoLuong = @"so_luong";
			 public static string SluongSua = @"sluong_sua";
			 public static string SluongLinh = @"sluong_linh";
			 public static string DonGia = @"don_gia";
			 public static string IdThuockho = @"id_thuockho";
			 public static string NgayNhap = @"ngay_nhap";
			 public static string GiaNhap = @"gia_nhap";
			 public static string GiaBan = @"gia_ban";
			 public static string GiaBhyt = @"gia_bhyt";
			 public static string SoLo = @"so_lo";
			 public static string Vat = @"vat";
			 public static string MaNhacungcap = @"ma_nhacungcap";
			 public static string PhuThu = @"phu_thu";
			 public static string PhuthuDungtuyen = @"phuthu_dungtuyen";
			 public static string PhuthuTraituyen = @"phuthu_traituyen";
			 public static string MotaThem = @"mota_them";
			 public static string SoluongHuy = @"soluong_huy";
			 public static string TrangthaiHuy = @"trangthai_huy";
			 public static string NguoiHuy = @"nguoi_huy";
			 public static string NgayHuy = @"ngay_huy";
			 public static string TuTuc = @"tu_tuc";
			 public static string TrangThai = @"trang_thai";
			 public static string TrangthaiTonghop = @"trangthai_tonghop";
			 public static string NgayXacnhan = @"ngay_xacnhan";
			 public static string TrangthaiBhyt = @"trangthai_bhyt";
			 public static string SttIn = @"stt_in";
			 public static string MadoituongGia = @"madoituong_gia";
			 public static string PtramBhytGoc = @"ptram_bhyt_goc";
			 public static string PtramBhyt = @"ptram_bhyt";
			 public static string BhytChitra = @"bhyt_chitra";
			 public static string BnhanChitra = @"bnhan_chitra";
			 public static string MaDoituongKcb = @"ma_doituong_kcb";
			 public static string IdThanhtoan = @"id_thanhtoan";
			 public static string TrangthaiThanhtoan = @"trangthai_thanhtoan";
			 public static string NgayThanhtoan = @"ngay_thanhtoan";
			 public static string CachDung = @"cach_dung";
			 public static string ChidanThem = @"chidan_them";
			 public static string DonviTinh = @"donvi_tinh";
			 public static string SolanDung = @"solan_dung";
			 public static string SoluongDung = @"soluong_dung";
			 public static string TrangthaiChuyen = @"trangthai_chuyen";
			 public static string NgayTao = @"ngay_tao";
			 public static string NguoiTao = @"nguoi_tao";
			 public static string NgaySua = @"ngay_sua";
			 public static string NguoiSua = @"nguoi_sua";
			 public static string TileChietkhau = @"tile_chietkhau";
			 public static string TienChietkhau = @"tien_chietkhau";
			 public static string KieuChietkhau = @"kieu_chietkhau";
			 public static string IdGoi = @"id_goi";
			 public static string TrongGoi = @"trong_goi";
			 public static string KieuBiendong = @"kieu_biendong";
			 public static string NguonThanhtoan = @"nguon_thanhtoan";
			 public static string IpMaytao = @"ip_maytao";
			 public static string IpMaysua = @"ip_maysua";
			 public static string TenMaytao = @"ten_maytao";
			 public static string TenMaysua = @"ten_maysua";
			 public static string DaDung = @"da_dung";
			 public static string LydoTiemchung = @"lydo_tiemchung";
			 public static string NguoiTiem = @"nguoi_tiem";
			 public static string VitriTiem = @"vitri_tiem";
			 public static string MuiThu = @"mui_thu";
			 public static string NgayhenMuiketiep = @"ngayhen_muiketiep";
			 public static string PhanungSautiem = @"phanung_sautiem";
			 public static string Xutri = @"xutri";
			 public static string KetluanNguyennhan = @"ketluan_nguyennhan";
			 public static string KetQua = @"ket_qua";
			 public static string NgaySudung = @"ngay_sudung";
			 public static string SoDky = @"so_dky";
			 public static string SoQdinhthau = @"so_qdinhthau";
			 public static string LydoHuy = @"lydo_huy";
			 public static string IdPhieuTXuatthuocBenhnhan = @"id_phieu_t_xuatthuoc_benhnhan";
			 public static string IdCapphat = @"id_capphat";
			 public static string Sang = @"sang";
			 public static string Trua = @"trua";
			 public static string Chieu = @"chieu";
			 public static string Toi = @"toi";
			 public static string TyleTt = @"tyle_tt";
			 public static string IdThe = @"id_the";
			 public static string IdDangky = @"id_dangky";
			 public static string TthaiTamthu = @"tthai_tamthu";
			 public static string IdTamthu = @"id_tamthu";
			 public static string NgayTamthu = @"ngay_tamthu";
			 public static string BhytNguonKhac = @"bhyt_nguon_khac";
			 public static string BhytGiaTyle = @"bhyt_gia_tyle";
			 public static string BnTtt = @"bn_ttt";
			 public static string BnCct = @"bn_cct";
			 public static string Gpp = @"GPP";
			 public static string TinhChiphi = @"tinh_chiphi";
			 public static string GiaGoc = @"gia_goc";
			 public static string NoidungthuchienSang = @"noidungthuchien_sang";
			 public static string IdNguoithuchienSang = @"id_nguoithuchien_sang";
			 public static string TenNguoithuchienSang = @"ten_nguoithuchien_sang";
			 public static string NoidungthuchienTrua = @"noidungthuchien_trua";
			 public static string IdNguoithuchienTrua = @"id_nguoithuchien_trua";
			 public static string TenNguoithuchienTrua = @"ten_nguoithuchien_trua";
			 public static string NoidungthuchienChieu = @"noidungthuchien_chieu";
			 public static string IdNguoithuchienChieu = @"id_nguoithuchien_chieu";
			 public static string TenNguoithuchienChieu = @"ten_nguoithuchien_chieu";
			 public static string NoidungthuchienToi = @"noidungthuchien_toi";
			 public static string IdNguoithuchienToi = @"id_nguoithuchien_toi";
			 public static string TenNguoithuchienToi = @"ten_nguoithuchien_toi";
			 public static string TthaiGui = @"tthai_gui";
			 public static string TransactionId = @"transaction_id";
			 public static string IdGui = @"id_gui";
			 public static string IdHuydvu = @"id_huydvu";
			 public static string HienthiBienlai = @"hienthi_bienlai";
						
		}
		#endregion
		
		#region Update PK Collections
		
        #endregion
    
        #region Deep Save
		
        #endregion
	}
}
