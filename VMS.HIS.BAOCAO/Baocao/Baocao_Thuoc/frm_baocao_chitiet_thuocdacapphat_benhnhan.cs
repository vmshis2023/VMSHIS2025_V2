...
namespace VNS.HIS.UI.BaoCao.Form_BaoCao
{
    public partial class frm_baocao_chitiet_thuocdacapphat_benhnhan : Form
    {
        ...
        private void cmdInPhieuXN_Click(object sender, EventArgs e)
        {
            ...
            Utility.AddColumToDataTable(ref m_dtReport, "DieukienTimkiem", typeof(string));
            
            Utility.UpdateLogotoDatatable(ref m_dtReport);
            Condition = string.Format("Từ ngày {0} đến {1}- Đối tượng {2} - Thuộc kho :{3}", dtFromDate.Text,
                                             dtToDate.Text,
        ...
        {
            if (Utility.DoTrim(baocaO_TIEUDE1.lblExcelFile.Text) == "")
            {
            ...
            else//Xuất ExcelTemplate trong thư mục ExcelTemplates
            {
                var dialog = new SaveFileDialog();
                dialog.Filter = "Excel Files|*.xlsx;*.xlsm";
                if (dialog.ShowDialog() != DialogResult.OK) return;
                string excelTemplateName = baocaO_TIEUDE1.lblExcelFile.Text;
                int excelConfigLine = Utility.Int32Dbnull(baocaO_TIEUDE1.nmrLine.Value);
                bool excelUseGridHeader = false;
                bool excelGroupOnePerSheet = false;
                int excelGroupConfigLine = 5;
                // Dùng grid header làm header cho excel luôn
                if (excelUseGridHeader)
                {
                    ExcelExporter.ExportWithTemplate(m_dtReport, excelTemplateName, dialog.FileName, false, excelConfigLine);
                }
                else
                {
                    // Xử lý group
                    ...
                    {
                        var groupByColumn = grdList.RootTable.Groups[0].Column.Key;
                        var ds = new DataSet();
                        if (excelGroupOnePerSheet)
                        {

                            var lstValueByColumn = (from row in m_dtReport.AsEnumerable()
                                                    select Utility.sDbnull(row[groupByColumn])).Distinct().ToList();
                            foreach (string valueByColumn in lstValueByColumn)
                            {
                                var dtResultByColumn = (from row in m_dtReport.AsEnumerable()
                                                        where Utility.sDbnull(row[groupByColumn]) == valueByColumn
                                                        select row).CopyToDataTable();
                                dtResultByColumn.TableName = valueByColumn;

                                ds.Tables.Add(dtResultByColumn);
                            }
                            ExcelExporter.ExportWithTemplate(ds, excelTemplateName, dialog.FileName, true, excelConfigLine);
                        }
                        else
                        {
                            ExcelExporter.ExportWithTemplate(m_dtReport, excelTemplateName, dialog.FileName, true, excelConfigLine, false, 
                                excelGroupConfigLine, groupByColumn);
                        }
                    }
                    else
                    {
                        // Dùng header của excel và điền dữ liệu vào dòng thứ excelConfigLine
                        ExcelExporter.ExportWithTemplate(m_dtReport, excelTemplateName, dialog.FileName, true, excelConfigLine);
                    }
                }
            }
        }
