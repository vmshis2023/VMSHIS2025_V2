...
namespace VNS.HIS.UI.Baocao
{
    public partial class fm_baocaothuchiendichvu : Form
    {
        ...
        public DataTable GetAllDoctors()
        {
            ...
            lstKhoaphong = VMS.HIS.Danhmuc.Util.GetKhoaPhong();
        ...
        private void cmdInPhieuXN_Click(object sender, EventArgs e)
        {
            ...
            _dtData =
               BAOCAO_NGOAITRU.BaocaoThuchiendichvu(
                chkByDate.Checked ? fromdate : "01/01/1900",
                chkByDate.Checked ? todate : globalVariables.SysDate.ToString("dd/MM/yyyy"),
                Utility.Int32Dbnull(txtLoaiDV.MyID, -1), Utility.Int32Dbnull(txtDvu.MyID, -1),txtBSthuchien.MyCode,0);
            THU_VIEN_CHUNG.CreateXML(_dtData, "baocao_thuchiendichvu.xml");
            ...
            string Condition = string.Format("{0} Từ ngày {1} đến {2} - Loại dịch vụ : {3} - Dịch vụ chỉ định :{4}", FromDateToDate, fromdate, todate,
                                         ...
                                         txtDvu.MyID!="-1"
                                              ? txtDvu.Text
                                              : "Tất cả");
            string reportcode = "baocao_thuchiendichvu";
