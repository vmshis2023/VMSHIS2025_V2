    ...
    public partial class frm_MainForm_new : Form
    {
        ...
        public frm_MainForm_new()
        {
            try
            {
                ...
                ucError1.AskYesNo = true;
                ucError1._OnAccept += ucError1__OnAccept;
               ...
               globalVariables.logReportCode=LogManager.GetLogger("ma_baocao");
                ...
                globalVariables.AppLog = LogManager.GetLogger("AppLog");
                ...
                Resize += frm_MainForm_new_Resize;
                this.Shown += frm_MainForm_new_Shown;
                cmdrelogin.Click+=cmdrelogin_Click;
                cmdChangePWD.Click+=cmdChangePWD_Click;
                txtFunction.LostFocus += txtFunction_LostFocus;
                txtFunction.GotFocus += txtFunction_GotFocus;
                txtFunction._OnEnterMe += txtFunction__OnEnterMe;
             //   lblCopyright.Click += lblCopyright_Click;
                //InitLogs();
                try
                {
                    if (Utility.DoTrim(PropertyLib._ConfigProperties.WSURL) == string.Empty)
                        WS._AdminWS.Url = "http://localhost:9003/AdminWS.asmx";
                    else
                ...
                string sqlCnn=new ConnectionSQL().KhoiTaoKetNoi();
                log.Trace(sqlCnn);

                Utility.InitSubSonic(sqlCnn, "ORM");
                Try2ReadApp();
        ...
        void txtFunction__OnEnterMe()
        {
            try
            {
                var q = (from p in dtSysRoles.AsEnumerable() where Utility.Int32Dbnull(p["iRoleID"], -1) == Utility.Int32Dbnull(txtFunction.MyID) select p);
                if (q.Any())
                    DisplaySingleFunction(q.FirstOrDefault());
        ...
        {
            txtFunction.ForeColor = Color.Black;
            txtFunction.Clear(); 
        }

        void txtFunction_LostFocus(object sender, EventArgs e)
        {
            txtFunction.ForeColor = Color.Gray;
            txtFunction.Text = "Nhấn vào đây để tìm kiếm nhanh chức năng";
        }

        void ucError1__OnAccept()
        {
            Utility.OpenProcess(AppName); //Khi gọi phần này thì tự động sẽ kill chính bản thân chương trình này
        }

        void frm_MainForm_new_Resize(object sender, EventArgs e)
        {
            //BackgroundImage = global::VNSCore.Properties.Resources.Logo2;
        }
        ...
        void SetlogoAndBackGround()
        {
            try
            {
                //if (File.Exists(Application.StartupPath + @"\logosize_top.txt"))
                //{
                //    List<string> lstsize = File.ReadAllLines(Application.StartupPath + @"\logosize_top.txt")[0].Split('x').ToList<string>();
                //    if (lstsize.Count == 2)
                //    {
        ...
        private void mnureLoad_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Utility.ShowMsg("Đã nạp tham số hệ thống,quyền người dùng và thông tin đơn vị thành công. Nhấn OK để kết thúc");
            }
            catch (Exception ex)
            {
                Utility.CatchException(ex);
        ...
        public void LoadList()
        {
            try
            {
                globalVariables.hasLoadedCommonData = false;
                Utility.SetMsg(statusStrip1.Panels["lblStatus"], "Nạp dữ liệu các danh mục dùng chung...", false);
                ...
                Utility.SetMsg(statusStrip1.Panels["lblStatus"], "Nạp dữ liệu danh mục bệnh...", false);
                ...
                Utility.SetMsg(statusStrip1.Panels["lblStatus"], "Nạp dữ liệu hệ thống khác...", false);
                ...
                globalVariables.hoten_canbo= THU_VIEN_CHUNG.Laygiatrithamsohethong("bhyt_hoten_canbo", "", false);
                globalVariables.cccd_canbo = THU_VIEN_CHUNG.Laygiatrithamsohethong("bhyt_cccd_canbo", "", false);
                //globalVariables.UCL =globalVariables.UCL.Substring(globalVariables.UCL.IndexOf(globalVariables.UserName),
                Utility.SetMsg(statusStrip1.Panels["lblStatus"], "",false);
            ...
            {
                globalVariables.hasLoadedCommonData = true;
        ...
        private void CheckFilesVersion()
        {
            try
            {
                DataTable dtData = SPs.SysGetversionsCheck("dll").GetDataSet().Tables[0];
                ...
                var q = from p in dtData.AsEnumerable()
                        where !Utility.sDbnull(p["sFileName"], "").ToLower().Contains(".dll") && !Utility.sDbnull(p["sFileName"], "").ToLower().Contains("Core.exe".ToLower())
                        select p;
                if (q.Any())
                    dtData = q.CopyToDataTable();
                else
                    if (Click2Update)
                    {
                        Utility.ShowMsg(
                ...
                foreach (DataRow dr in dtData.Rows)
                {
                    ...
                    else
                    {
                        ...
                        {
                            log.Trace(string.Format("file {0} has tick in db ={1} > LastWriteTime.Ticks ={2}", fullfilePath, Utility.Int64Dbnull(dr["tick"], -1), ticks));
                            InsertNewRow(dr, dtData, ref dtLastVersion);
                        }
                        else if (sVersion != Utility.sDbnull(dr["sVersion"], sVersion))
                        {
                            log.Trace(string.Format("file {0} has version in db ={1} <> version of file ={2}", fullfilePath, Utility.sDbnull(dr["sVersion"], "xxx"), sVersion));
                            InsertNewRow(dr, dtData, ref dtLastVersion);
                        }

                        //if (Utility.Int64Dbnull(dr["tick"], -1) > ticks || sVersion != Utility.sDbnull(dr["sVersion"], sVersion))
                        //{
                        //    InsertNewRow(dr, dtData, ref dtLastVersion);
                        //}
                    }
                ...
                if (dtLastVersion != null && dtLastVersion.Rows.Count > 0)
                {
                    ...
                    //Thực hiện download file và không phải đóng phần mềm
                    UpdateFiles(dtLastVersion);
                }
        ...
        void UpdateFiles(DataTable dtLastVersion)
        {
            List<string> lstID = (from p in dtLastVersion.AsEnumerable()
                                  select Utility.sDbnull(p["PK_intID"], "-1")).ToList<string>();
            DataTable dtData = SPs.SysGetversionsData(string.Join(",", lstID.ToArray<string>())).GetDataSet().Tables[0];
            if (dtData.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in dtData.Rows)
                {
                    try
                    {
                        ...
                        string trueFile = "";
                        if (Utility.sDbnull(dr["sFolder"], "") != "" && Utility.sDbnull(dr["sFolder"], "") != "YES")
                        {
                            fullfilePath = Application.StartupPath + "\\" + Utility.sDbnull(dr["sFolder"], "") + @"\" + Utility.sDbnull(dr["sFileName"], "");
                            fullfilePath_rar = Application.StartupPath + "\\" + Utility.sDbnull(dr["sFolder"], "") + @"\" + Utility.sDbnull(dr["sRarFileName"], "");
                        }
                        else
                        {
                            fullfilePath = Application.StartupPath + "\\" + Utility.sDbnull(dr["sFileName"], "");
                            fullfilePath_rar = Application.StartupPath + "\\" + Utility.sDbnull(dr["sRarFileName"], "");
                        }
                        trueFile = Utility.Int32Dbnull(dr["intRar"], 0) == 0 ? fullfilePath : fullfilePath_rar;
                        ...
                        //if (Utility.Int32Dbnull(dr["intRar"], 0) == 1)
                        //{
                        //    continue;
                        ...
                        //    if (Utility.sDbnull(dr["sFolder"], "") == "YES")
                        //    {
                        //        //Kiểm tra nếu là filerar thì thực hiện giải nén không mật khẩu
                        //        ProcessStartInfo info = new ProcessStartInfo();
                        //        //lblStatus.Text = "Giải nén:" + Utility.sDbnull(dr["sFileName"]).ToString() + "...";
                ...
                if (Click2Update) Utility.ShowMsg("Đã cập nhật xong. Nhấn OK để kết thúc");
               
            }
        }
        private void ThongbaoVersion()
        {
            try
            {
                ...
                DataTable dtData = SPs.SysGetversionsCheck("ALL").GetDataSet().Tables[0];
                if (dtData == null || dtData.Rows.Count <= 0)
                {
                    log.Trace("No versions found");
                    return;
                }
                DataTable dtLastVersion = dtData.Clone();
                foreach (DataRow dr in dtData.Rows)
                {
                    ...
                    if (!File.Exists(fullfilePath))
                    {
                        log.Trace(string.Format("File will be download due to not exists: {0}", Utility.sDbnull(dr["sFileName"], "")));
                        InsertNewRow(dr, dtData, ref dtLastVersion);
                        //Nếu tồn tại thì kiểm tra xem Version có khác nhau không?
                    }
                    else
                    {
                        ...
                        if (Utility.Int64Dbnull(dr["tick"], -1) > ticks || sVersion != Utility.sDbnull(dr["sVersion"], sVersion))
                        {
                            log.Trace(string.Format("File will be download due to lastest version: {0}", Utility.sDbnull(dr["sFileName"], "")));
                            InsertNewRow(dr, dtData, ref dtLastVersion);
                        }
                    }
                }
                ...
                if (dtLastVersion != null && dtLastVersion.Rows.Count > 0)
                {
                    log.Trace(string.Format("new Versions found. Click to update"));
                    ShowTop();
                    string msg = "Hệ thống phát hiện có phiên bản mới được cập nhật lên hệ thống. Vui lòng Click vào đây hoặc chọn menu ngay bên cạnh <Chức năng khác-->Kiểm tra phiên bản> để được cập nhật phiên bản mới nhất";
                    string AskMsg = "Hệ thống phát hiện có phiên bản mới được cập nhật lên hệ thống. Bạn có chắc chắn muốn cập nhật phiên bản hay không?\nChú ý: Nếu đồng ý cập nhật, phần mềm sẽ tự động đóng lại và bật lên ngay sau khi cập nhật xong";
                    if (msg.Trim() != "")
                        ShowErrorStatus(msg, AskMsg);
                    return;
                }
                else
                    log.Trace(string.Format("Phien ban tren may ban dang la moi nhat"));
            ...
            try
            {
                ...
                if (dtLastVersion != null && dtLastVersion.Rows.Count > 0)
                {
                    if (Click2Update)
                        if (
                            !Utility.AcceptQuestion(
                                "Hệ thống phát hiện có phiên bản mới được cập nhật lên hệ thống. Bạn có chắc chắn muốn cập nhật phiên bản hay không?\nChú ý: Nếu đồng ý cập nhật, phần mềm sẽ tự động đóng lại và bật lên ngay sau khi cập nhật xong",
                                "Thông báo cập nhật phiên bản mới", true))
                            return;
                        else //Đồng ý-->Đánh dấu tự động đăng nhập
                        {
        ...
        private void SetsyncDateTime()
        {
            ...
            log.Trace("gio tren may chu dang la {0}", dateTime.ToString("dd/MM/yyyy HH:mm:ss"));
            ...
            log.Trace("SetLocalTime OK");
        }
        private void SetsyncDateTime_v1()
        {
            var updatedTime = new SYSTEMTIME();
            DateTime dateTime = new LoginService().GetSysDateTime();
            log.Trace("gio tren may chu dang la {0}", dateTime.ToString("dd/MM/yyyy HH:mm:ss"));
            updatedTime.wDayOfWeek = (ushort)dateTime.DayOfWeek;
            updatedTime.wYear = (ushort)dateTime.Year;
            updatedTime.wMonth = (ushort)dateTime.Month;
            updatedTime.wDay = (ushort)dateTime.Day;
            updatedTime.wHour = (ushort)dateTime.Hour;
            updatedTime.wMinute = (ushort)dateTime.Minute;
            updatedTime.wSecond = (ushort)dateTime.Second;
        ...
        /// </summary>
        private void LoadBackground(string imgname,ImageLayout imglayout)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(filename))
                {
                    ...
                    pnlMDI.BackgroundImage = img;
                    pnlMDI.BackgroundImageLayout = imglayout;
                }
            }
            catch (Exception)
            {
        ...
        private void LoadFormMain()
        {
            try
            {
                ...
                {
                    AddClient();
                    globalVariables.AppSessionID = THU_VIEN_CHUNG.GetGUID();
                    ...
                    LoadMenu();
                    LoadQuickSearchData();
                    ucError1.EllapsedTime = Utility.Int32Dbnull(THU_VIEN_CHUNG.Laygiatrithamsohethong("CORE_CANHBAOPHIENBANMOI_THOIGIANNHAPNHAY", "1000", false), 1000);
                    ucError1.NumberofBrlink = Utility.Int32Dbnull(THU_VIEN_CHUNG.Laygiatrithamsohethong("CORE_CANHBAOPHIENBANMOI_SOLANNHAPNHAY", "10", false), 60);
                }
            }
        ...
        void AddClient()
        {
            try
            {
                DataTable dtDataCheck = new Select().From(SysClient.Schema).Where(SysClient.Columns.Ipaddress).IsEqualTo(globalVariables.gv_strIPAddress).ExecuteDataSet().Tables[0];
                if (dtDataCheck.Rows.Count <= 0)
                {
                    SysClient newClient = new SysClient();
                    newClient.Ipaddress = globalVariables.gv_strIPAddress;
                    newClient.Computername = globalVariables.gv_strComputerName;
                    newClient.MacAddress = globalVariables.gv_strMacAddress;
                    newClient.NgayKichhoat = DateTime.Now;
                    newClient.Save();
                }
                else
                {
                    SysClient oldClient = new Select().From(SysClient.Schema).Where(SysClient.Columns.Ipaddress).IsEqualTo(globalVariables.gv_strIPAddress).ExecuteSingle<SysClient>();
                    oldClient.Computername = globalVariables.gv_strComputerName;
                    oldClient.MacAddress = globalVariables.gv_strMacAddress;
                    oldClient.NgayTruycapGannhat= DateTime.Now;
                    oldClient.IsNew = false;
                    oldClient.Save();
                }
        ...
        void LoadQuickSearchData()
        {
            try
            {
                if(dtSysRoles.Rows.Count>0)
                {
                    DataTable dtFunctionRoles = (from p in dtSysRoles.AsEnumerable() where Utility.Int32Dbnull(p["PK_iID"], -1) > 0 select p).CopyToDataTable();
                    txtFunction.Init(dtFunctionRoles, new List<string>() { "iRoleID", "iRoleID", "sRoleName" });
                }    
        ...
        void ShowErrorStatus(string msg,string AskMsg)
        {
            try
            {
                UIAction._Visible(ucError1, true);
                ucError1.Reset();
                ucError1.Start(msg, AskMsg);
        ...
        private void CallLogin()
        {
            ...
            frm._OnloadData += frm__OnloadData;
            frm.ShowDialog();
            Utility.DefaultNow(this);
        }

        void frm__OnloadData()
        {
            //Application.DoEvents();
            //var objThread = new Thread(LoadList);
            //objThread.Start();
            //Application.DoEvents();
             LoadList();
        }
        ...
        private void SetInfor()
        {
            try
            {
                ...
                Utility.SetMsg(statusStrip1.Panels["DB"],string.Format("{0}",  globalVariables.sDbName));
               ...
               Utility.SetMsg(statusStrip1.Panels["lblIP"] , globalVariables.gv_strIPAddress);
                ...
                string ten_cty = THU_VIEN_CHUNG.Laygiatrithamsohethong("TEN_CONGTYPHANMEM", "COPYRIGHT © Công ty cổ phần CNTT VMS Việt Nam", false);
                string ten_phanmem = THU_VIEN_CHUNG.Laygiatrithamsohethong("TEN_PHANMEM", "HỆ THỐNG QUẢN LÝ THÔNG TIN BỆNH VIỆN - VMS HIS", false);
                string ho_tro = THU_VIEN_CHUNG.Laygiatrithamsohethong("HOTRO_DONVI", "Đào Văn Cường: 0915 150 148", false);
                ten_cty = System.Text.Encoding.UTF8.GetString(System.Convert.FromBase64String(objEncrypt.GiaiMa( ten_cty)));
                ho_tro = System.Text.Encoding.UTF8.GetString(System.Convert.FromBase64String(objEncrypt.GiaiMa(ho_tro)));
                ten_phanmem = System.Text.Encoding.UTF8.GetString(System.Convert.FromBase64String(objEncrypt.GiaiMa(ten_phanmem)));

                globalVariables.ten_congty = ten_cty;
                globalVariables.ten_phanmem = ten_phanmem;
                globalVariables.ten_nguoihotro = ho_tro;
                ...
                //      "HỆ THỐNG QUẢN LÝ THÔNG TIN BỆNH VIỆN - VMS HIS", false) ;
                //Utility.SetMsg(statusStrip1.Panels["lblUpdateVersion"], THU_VIEN_CHUNG.Laygiatrithamsohethong("HOTRO_DONVI",
                //       "Thông tin trực hỗ trợ", false));

                Utility.SetMsg(statusStrip1.Panels["lblCopyright"], ten_cty);
              label1.Text = ten_phanmem;
              this.Text = ten_phanmem;
              Utility.SetMsg(statusStrip1.Panels["lblUpdateVersion"], ho_tro);
        ...
        private void LoadMenuStyleNormal()
        {
            try
            {
                ...
                menuStrip.LayoutStyle = (ToolStripLayoutStyle)Utility.Int32Dbnull(THU_VIEN_CHUNG.Laygiatrithamsohethong("CORE_MENUSTRIP_LAYOUTSTYLE", "0", true));
                FindMenuChild(-2);
            }
            catch (Exception ex)
            {
        ...
        void setImage4MenuItem(ToolStripMenuItem mymenu, string imgFileName)
        {
            try
            {
                if (Utility.sDbnull( imgFileName ).Length<=0) return ;
                string filePath = Application.StartupPath + @"\MenuIcons\" + Path.GetFileName( imgFileName);
                if (File.Exists(filePath))
                {
                    mymenu.Image = Image.FromFile(filePath);
                }
            }
            catch (Exception ex)
            {
        ...
        private void FindMenuChild(int Parent_ID)
        {
            ...
            foreach (DataRow drCha in arrDrPhanhe) //Phân hệ
            {
                ...
                setImage4MenuItem(objMenuphanhe, Utility.sDbnull(drCha["sIconPath"], ""));
        ...
        private void AddChildMenu(ToolStripMenuItem ParentMenuItem, int iRoleID)
        {
            ...
            foreach (DataRow drMenu in arrDrMenu) //Top node loop
            {
                ...
                setImage4MenuItem(objMenuChild, Utility.sDbnull(drMenu["sIconPath"], ""));
        ...
        private void LoadRibbonMenu()
        {
            try
            {
                ...
                foreach (DataRow drCha in arrDrPhanhe) //Phân hệ
                {
                    ...
                    group.Panels.Add(objPanel);
                    pMain.Panels.Add(group);
                }
                //foreach (UIPanel _panel in lstPanel)
                //    pMain.Panels.Add(_panel);
                if (arrDrActive.Length > 1)
        ...
        private void DisplaySingleFunction(DataRow drSingle)
        {
            ...
            try
            {
                ...
                while (!globalVariables.hasLoadedCommonData)
                {
        ...
        void SetBackgroundImages()
        {
            try
            {
                ...
                {
                    pnlMDI.Dock = DockStyle.Fill;
                    pnlMDI.Visible = true;
                }
                else
                {
                    pnlMDI.Dock = DockStyle.None;
                    pnlMDI.Visible = false;
                }
            }
            catch (Exception)
            {
        ...
        bool isExisted(Form f)
        {
            foreach(var child in MdiChildren)
                if (child.Tag.Equals(f.Tag))
                {
                    child.Activate();
                    return true;
                }
        ...
        private void objTreeView_MouseDown(object sender, MouseEventArgs e)
        {
           ...
           var objTreeView = (TreeView) sender;
            if (objTreeView.SelectedNode == null) return;
        ...
        private void PanelManager_MdiTabClosed(object sender, MdiTabClosedEventArgs e)
        {
            ...
            try
            {
                ...
                if (e.Tab.Form != null && e.Tab.Form.Tag != null)
                {
                    if (e.Tab.Form != null && e.Tab.Form.Tag != null && lstTab.Contains(e.Tab.Form.Tag.ToString()))
                    {
                        lstTab.Remove(e.Tab.Form.Tag.ToString());
                        if (e.Tab.Form != null)
                        {

                           
                            e.Tab.Form.Dispose();
                            e.Tab.Form.MdiParent = null;
                            GC.Collect();
                        }

                    }
                    if (e.Tab.Form != null && e.Tab.Form.Tag != null && dic.ContainsKey(e.Tab.Form.Tag.ToString()))
                    {
                        dic.Remove(e.Tab.Form.Tag.ToString());
                        if (e.Tab.Form != null)
                        {
                    ...
                    foreach (var child in MdiChildren)
                        if (e.Tab.Form != null && e.Tab.Form.Tag != null && child.Tag.Equals(e.Tab.Form.Tag))
                        {
                ...
                if (lstTab.Count <= 0) SetBackgroundImages();
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            try
            {
                log.Trace("Kiem tra phien ban");

                if (THU_VIEN_CHUNG.Laygiatrithamsohethong("CORE_CANHBAOPHIENBANMOI_THONGBAO", "1", true) == "1")
                {
                    log.Trace(string.Format( "CORE_CANHBAOPHIENBANMOI_THONGBAO =1"));
                    ThongbaoVersion();
                }
        ...
        private void cmdOpenMulti_Click(object sender, EventArgs e)
        {
             string CoreAppName = Application.StartupPath + @"\Core.exe";
             OpenProcess(CoreAppName);
        }
        ...
        private void statusStrip1_PanelClick(object sender, StatusBarEventArgs e)
        {
            if (e.Panel.Key == "lblUpdateVersion")
            {
                Click2Update = true;
                CheckVersion();
                Click2Update = false;
        ...
        private void mnuOpenMulti_Click(object sender, EventArgs e)
        {
            string CoreAppName = Application.StartupPath + @"\Core.exe";
            OpenProcess(CoreAppName);
        }

        private void cmdLoadSysparams_Click_1(object sender, EventArgs e)
        {
        ...
        {
            contextMenuStrip1.Show(cmdChucnangkhac, new Point(0, cmdChucnangkhac.Height));
        }

        private void mnuUpdateVersion_Click(object sender, EventArgs e)
        {
            Click2Update = true;
            CheckVersion();
            Click2Update = false;
        ...
        private void mnureLoad_Click_1(object sender, EventArgs e)
        {
            
        }

        private void mnuUpdateFiles_Click(object sender, EventArgs e)
        {
            Click2Update = true;
            CheckFilesVersion();
            Click2Update = false;
        ...
        {
            CIS2008.frm_Versions _Versions = new CIS2008.frm_Versions();
            _Versions.ShowDialog();
        }

        private void mnuSuperAdmin_Click(object sender, EventArgs e)
        {
            ...
            if (_frm.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                //Utility.ShowMsg("Xác nhận quyền super admin thành công. Một số form có thể phải bật lại để quyền Super Admin có hiệu lực");
            }
        }
