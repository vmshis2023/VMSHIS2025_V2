...
{
    public partial class BHYT_19A : UserControl
    {
        ...
        void InitEvents()
        {
            ...
            this.KeyDown += new KeyEventHandler(BHYT_19A_KeyDown);
            this.Load += new EventHandler(BHYT_19A_Load);
            ...
            txtTinhthanh._OnSelectionChanged += new VNS.HIS.UI.DANHMUC.UCs.AutoCompleteTextbox.OnSelectionChanged(txtTinhthanh__OnSelectionChanged);
            txtLoaithuoc._OnSelectionChanged += new VNS.HIS.UI.DANHMUC.UCs.AutoCompleteTextbox.OnSelectionChanged(txtLoaithuoc__OnSelectionChanged);
            txtLoaithuoc._OnEnterMe += new VNS.HIS.UI.DANHMUC.UCs.AutoCompleteTextbox.OnEnterMe(txtLoaithuoc__OnEnterMe);
        ...
        void BHYT_19A_Load(object sender, EventArgs e)
        {
     ...
     new Select().From(DmucLoaithuoc.Schema).Where(DmucLoaithuoc.KieuThuocvattuColumn).IsEqualTo("THUOC").ExecuteDataSet().Tables[0];
        ...
        void BHYT_19A_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        void cmdExcel_Click(object sender, EventArgs e)
        {
            try
            {
                if (radTheo19AHD.Checked)
                {
                    ...
                    saveFileDialog1.FileName = "BaoCaoBHYT-19a.xls";
                ...
                {
                    string reportcode = "BHYT_19A_EXCEL";
                    ...
                    if (chktuyen.Checked)
                    {
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sheet[idxRow, idxColSh].SetValue(i + 1, HamDungChung.styleNumber_UnStyle(book));
                            sheet[idxRow, idxColSh + 1].SetValue(Convert.ToString(dt.Rows[i]["ma_QD40"]), HamDungChung.styleStringCenter(book));
                            ...
                            sheet[idxRow, idxColSh + 5].SetValue(Convert.ToString(dt.Rows[i]["ten_donvitinh"]), HamDungChung.styleStringCenter(book));
                            sheet[idxRow, idxColSh + 6].SetValue(Convert.ToDecimal(dt.Rows[i]["gia_muavao"]), HamDungChung.styleDecimalBoldAllBorder_Money(book));
                            sheet[idxRow, idxColSh + 7].SetValue(Convert.ToDecimal(dt.Rows[i]["so_luong"]), HamDungChung.styleNumber(book));
                            sheet[idxRow, idxColSh + 8].SetValue(Convert.ToDecimal(dt.Rows[i]["so_luongnt"]), HamDungChung.styleNumber(book));
                            sheet[idxRow, idxColSh + 9].SetValue(Convert.ToDecimal(dt.Rows[i]["don_gia"]), HamDungChung.styleDecimalBoldAllBorder_Money(book));
                            sheet[idxRow, idxColSh + 10].SetValue(Convert.ToDecimal(dt.Rows[i]["thanh_tien"]), HamDungChung.styleDecimalBoldAllBorder_Money(book));
                            idxRow = idxRow + 1;
                        }
                    }
                    else
                    ...
                    sheet[idxRow, idxColSh + 7].SetValue(Convert.ToDecimal(dt.Compute("Sum(so_luong)", "1=1")), HamDungChung.styleNumber(book));
                    sheet[idxRow, idxColSh + 8].SetValue(Convert.ToDecimal(dt.Compute("Sum(so_luongnt)", "1=1")), HamDungChung.styleNumber(book));
                    sheet[idxRow, idxColSh + 10].SetValue(Convert.ToDecimal(dt.Compute("Sum(thanh_tien)", "1=1")), HamDungChung.styleDecimalBoldAllBorder_Money(book));
                    string getdate = string.Format("Ngày {0} tháng {1} năm {2}", dtpNgayIn.Value.Day,
                        dtpNgayIn.Value.Month, dtpNgayIn.Value.Year);
                    sheet[idxRow + 2, 9].SetValue(getdate, HamDungChung.styleStringCenter_UnBorder(book));
                    ...
                    sheet[idxRow + 3, 9].SetValue("GIÁM ĐỐC BỆNH VIỆN", HamDungChung.styleDecimalBoldAllBorder_Money(book));
