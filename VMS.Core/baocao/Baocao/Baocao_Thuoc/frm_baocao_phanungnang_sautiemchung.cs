...
{
    public partial class frm_baocao_phanungnang_sautiemchung : Form
    {
        ...
        public frm_baocao_phanungnang_sautiemchung(string args)
        {
            InitializeComponent();
            ...
            this.Load+=new EventHandler(frm_baocao_phanungnang_sautiemchung_Load);
            cmdBaoCao.Click+=new EventHandler(cmdBaoCao_Click);
            this.KeyDown+=new KeyEventHandler(frm_baocao_phanungnang_sautiemchung_KeyDown);
            txtLoaithuoc._OnSelectionChanged +=new VNS.HIS.UI.DANHMUC.UCs.AutoCompleteTextbox_Thuoc.OnSelectionChanged(txtLoaithuoc__OnSelectionChanged);
            txtLoaithuoc._OnEnterMe +=new VNS.HIS.UI.DANHMUC.UCs.AutoCompleteTextbox_Thuoc.OnEnterMe(txtLoaithuoc__OnEnterMe);
            ...
            txtGioitinh._OnShowData += txtGioitinh__OnShowData;
            gridEXExporter1.GridEX = grdList;
            CauHinh();
        }

        void txtGioitinh__OnShowData()
        {
            DMUC_DCHUNG _DMUC_DCHUNG = new DMUC_DCHUNG(txtGioitinh.LOAI_DANHMUC);
            _DMUC_DCHUNG.ShowDialog();
            if (!_DMUC_DCHUNG.m_blnCancel)
            {
                string oldCode = txtGioitinh.myCode;
                txtGioitinh.Init();
                txtGioitinh.SetCode(oldCode);
                txtGioitinh.Focus();
            }   
        }
        ...
        void modifyTieude()
        {
            ...
            if (chkTheoNhomThuoc.Checked)
            {
                ...
                baocaO_TIEUDE1.Init("vacxin_baocao_phanungnang_sautiemchung_theonhom");
     ...
     new Select().From(DmucLoaithuoc.Schema).Where(DmucLoaithuoc.KieuThuocvattuColumn).IsEqualTo(KIEU_THUOC_VT).ExecuteDataSet().Tables[0];
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frm_baocao_phanungnang_sautiemchung_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void frm_baocao_phanungnang_sautiemchung_KeyDown_1(object sender, KeyEventArgs e)
        {
