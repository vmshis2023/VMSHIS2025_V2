...
namespace CIS.CoreApp
{
    ...
    public partial class frm_MainForm_new : Form
    {
        ...
        void SetlogoAndBackGround()
        {
            ...
            {
                //LoadBackground("default1",ImageLayout.Zoom);
                //SetLogo("logo_top");
            }
        }
        ...
        private void CheckFilesVersion()
        {
            try
            {
                ...
                var q = from p in dtData.AsEnumerable()
                        where !Utility.sDbnull(p["sFileName"], "").ToLower().Contains(".dll") && !Utility.sDbnull(p["sFileName"], "").ToLower().Contains(".exe")
                        select p;
                if (q.Any())
        ...
        void UpdateFiles(DataTable dtLastVersion)
        {
            if (dtLastVersion.Rows.Count > 0)
            {
                ...
                Utility.ShowMsg("Đã cập nhật xong. Nhấn OK để kết thúc");
