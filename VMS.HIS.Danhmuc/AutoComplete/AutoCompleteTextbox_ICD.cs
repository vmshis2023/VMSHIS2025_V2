...
namespace VNS.HIS.UCs
{
    ...
    // and is therfore called TextBox, but ment is this class (AutoCompleteTextbox_ICD)
    public class AutoCompleteTextbox_ICD : TextBox
    {
        #region Fields
        ...
        public delegate void OnGridSelectionChanged(Int16 id_benh, string ma_benh,string ten_benh);
        ...
        // the constructor
        public AutoCompleteTextbox_ICD()
            : base()
        {
            ...
            _grid._OnCellValueChanged += new uc_ICD.OnCellValueChanged(_grid__OnCellValueChanged);
            _grid._OnKeyDown += new uc_ICD.OnKeyDown(_grid__OnKeyDown);
            _grid._OnPreviewKeyDown += new uc_ICD.OnPreviewKeyDown(_grid__OnPreviewKeyDown);
            _grid._OnSelectionChanged += new uc_ICD.OnSelectionChanged(_grid__OnSelectionChanged);
        ...
        void _grid__OnSelectionChanged(Int16 id_benh, string ma_benh, string ten_benh)
        {
            if (!AllowTextChanged) return;
            if (txtMyID != null)
                txtMyID.Text = id_benh.ToString();
            if (txtMyID_Edit != null)
                txtMyID_Edit.Text = id_benh.ToString();
            MyID = id_benh;
            if (txtMyName != null)
                txtMyName.Text = ten_benh;
            if (txtMyName_Edit != null)
                txtMyName_Edit.Text = ten_benh;
         if(_OnGridSelectionChanged!=null)   _OnGridSelectionChanged(id_benh,ma_benh, ten_benh);
        }

        void _grid__OnKeyDown(Int16 id_benh, string ma_benh, string ten_benh)
        {
            _Text = ten_benh;
        ...
        void _grid__OnCellValueChanged(Int16 id_benh, string ma_benh, string ten_benh)
        {
        ...
        public int SelectedIndex
        {
            get
            {
                if (GridView)
                {
                    if (_grid.grdICD.CurrentRow == null) return -1;
                    return _grid.grdICD.CurrentRow.Position;
                }
                else
                {
...
//    _grid.grdICD.se
        ...
        public void ChangeDataSource()
        {
            try
            {
                ...
                if(GridView)
                {
                    Utility.SetDataSourceForDataGridEx_BasicNofilter(_grid.grdICD, dtData, true, true, "1=1", DmucBenh.Columns.TenBenh);
                    if (_grid.grdICD.GetDataRows().Length > 0) _grid.grdICD.MoveFirst();
                }
            }
            catch
            {
        ...
        private void CreateAutocomplete()
        {
            ...
            finally
            {
                ...
                var query = from p in dtData.Select("1=1", DmucBenh.Columns.TenBenh).AsEnumerable()
                            select p[DmucBenh.Columns.IdBenh].ToString() + "#" + p[DmucBenh.Columns.MaBenh].ToString() + string.Format("@Id: {0};Code: {1}; ", p[DmucBenh.Columns.IdBenh].ToString(), p[DmucBenh.Columns.MaBenh].ToString()) + p[DmucBenh.Columns.TenBenh].ToString() + "@" + p["shortcut"].ToString() + "$" +  "-1";
                source = query.ToList();
        ...
        private void LoadFilterDrugCode(string text)
        {
            ...
            try
            {
                ...
                if (text.Trim() != "")
                {
                    rowFilter = DmucBenh.Columns.TenBenh + " like '%" + text + "%' OR " + DmucBenh.Columns.MaBenh + " LIKE '%" + text + "%'";
                }
                dtData.DefaultView.RowFilter = rowFilter;
               _grid.grdICD.MoveFirst();
            }
            catch (Exception)
            {
            }
        ...
        {
            if ((_grid.grdICD.RowCount + 1) * 15 >= 210) return 210;
            else
                return (_grid.grdICD.RowCount + 1) * 20;
        }
        // hides the listbox
        public void HideSuggestionListBox()
        {
            if ((TopLevelControl != null))
            {
                ...
                {
                    if (_grid.grdICD != null && _grid.grdICD.CurrentRow != null)
                        GridRowPosition = _grid.grdICD.CurrentRow.Position;
        ...
        protected override void OnKeyDown(KeyEventArgs args)
        {
            ...
            else if ((args.KeyCode == Keys.Enter))
            {
                ...
                if (GridView)
                {
                    try
                    {
                        if (GridRowPosition > 0 && _grid.grdICD.CurrentRow.Position <= 0 )
                            _grid.grdICD.MoveTo(GridRowPosition);
                    }
                    catch (Exception)
                    {
                    ...
                    if (Utility.isValidGrid(_grid.grdICD) && _grid.grdICD.CurrentRow.Position >= 0 )
                        SelectItem();
                    _grid.RowIndex = -1;
                    _grid.AllowedChanged = true;
        ...
        private void MoveSelectionInGrid(bool foreward)
        {
            ...
            if (!Utility.isValidGrid(_grid.grdICD)) return;
            // beginning of list
            if (foreward)
            {
                if (_grid.grdICD.CurrentRow.Position < _grid.grdICD.RowCount - 1)
                    _grid.grdICD.MoveNext();
                else
                    _grid.grdICD.MoveFirst();
            }
            else
            {
                if (_grid.grdICD.CurrentRow.Position > 0)
                    _grid.grdICD.MovePrevious();
                 else
                    _grid.grdICD.MoveLast();
            }
        }
        ...
        private bool SelectItem()
        {
            try
            {
                if (GridView)
                {
                    if (Utility.isValidGrid(_grid.grdICD))
                    {
                        ...
                        //else
                        _Text = _grid.grdICD.GetValue(DmucBenh.Columns.TenBenh).ToString();
                        MyID = Utility.Int32Dbnull(_grid.grdICD.GetValue("id_benh"), "-1");
                        MyCode = Utility.sDbnull(_grid.grdICD.GetValue("ma_benh"), "");
        ...
        private void ShowSuggests()
        {
            ...
            if (this.Text.Length >= MinTypedCharacters)
            {
                ...
                if ((GridView && ((DataView)_grid.grdICD.DataSource).Count>0) || ((CurrentAutoCompleteList != null) && CurrentAutoCompleteList.Count > 0))
                {
                    ...
                    if (Utility.sDbnull(MyID) == "")
                        ...
                        Utility.GotoNewRowJanus(_grid.grdICD, "id_benh", Utility.sDbnull(MyID));
                    if (!GridView)
                ...
                if (Utility.DoTrim(this.Text) == "" && Utility.sDbnull(MyID)=="")
                {
                    setDefaultValue();
                }
            }
