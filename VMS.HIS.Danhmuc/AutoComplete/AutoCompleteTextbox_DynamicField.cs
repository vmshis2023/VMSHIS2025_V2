...
namespace VNS.HIS.UCs
{
    ...
    // and is therfore called TextBox, but ment is this class (AutoCompleteTextbox_DynamicField)
    public class AutoCompleteTextbox_DynamicField : TextBox
    {
        #region Fields
        ...
        public delegate void DOnKeydown(KeyEventArgs args);
        public event DOnKeydown _OnKeydown;

        public delegate void DOnLostFocus(System.EventArgs e);
        public event DOnLostFocus _OnLostFocus;

        public delegate void DOnTextChanged(EventArgs args);
        public event DOnTextChanged _OnTextChanged;

        public delegate void OnEnterMe();
        public event OnEnterMe _OnEnterMe;

        public delegate void OnShowData(AutoCompleteTextbox_DynamicField txt);
        ...
        // the constructor
        public AutoCompleteTextbox_DynamicField()
            : base()
        {
        ...
        protected override void OnKeyDown(KeyEventArgs args)
        {
            ...
            if (_OnKeydown != null) _OnKeydown(args);
        }
        ...
        private void ShowSuggests(string this_text)
        {
            if (!AllowTextChanged) return;
            
            // show only if MinTypedCharacters have been typed
            if (this_text.Length >= MinTypedCharacters)
            {
                ...
                if ((this_text.Length > 0) && (this.oldText == this_text.Substring(0, this_text.Length - 1)))
                {
                    //handle forward typing with refresh
                    UpdateCurrentAutoCompleteList();
                }
                // user is typing backward - char bas been removed
                else if ((this.oldText.Length > 0) && (this_text == this.oldText.Substring(0, this.oldText.Length - 1)))
                {
            ...
            if (Utility.DoTrim(this_text) == "" && RaiseEventEnterWhenEmpty && _OnEnterMe != null) _OnEnterMe();
        }
        ...
        private void UpdateListBoxItems()
        {
            try
            {
                ...
                if ((TopLevelControl != null))
                {
                    ...
                    if (panel.Height <=50) panel.Height = 100;
