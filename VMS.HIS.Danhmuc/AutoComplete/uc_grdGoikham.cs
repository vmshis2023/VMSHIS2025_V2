...
{
    public partial class uc_grdGoikham : UserControl
    {
        public delegate void OnKeyDown(int id_goi,   string ma_goi, string ten_goi);
        public event OnKeyDown _OnKeyDown;

        public delegate void OnSelectionChanged( int id_goi,  string ma_goi, string ten_goi);
        public event OnSelectionChanged _OnSelectionChanged;

        public delegate void OnCellValueChanged(int id_goi, string ma_goi, string ten_goi);
        ...
        private void grdICD_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter && Utility.isValidGrid(grdList) && grdList.Visible)
                {
                    ...
                    RowIndex = 1;
                    _OnKeyDown(Utility.Int32Dbnull(grdList.CurrentRow.Cells["id_goi"].Value, -1), Utility.sDbnull( grdList.CurrentRow.Cells["ma_goi"].Value.ToString()),  Utility.sDbnull(grdList.CurrentRow.Cells["ten_goi"].Value, ""));
                }
        ...
        private void grdICD_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    RowIndex = 1;
                    _OnSelectionChanged(Utility.Int32Dbnull(grdList.CurrentRow.Cells["id_goi"].Value, ""), Utility.sDbnull(grdList.CurrentRow.Cells["ma_goi"].Value.ToString()),  Utility.sDbnull(grdList.CurrentRow.Cells["ten_goi"].Value, ""));
                }
            }
            catch (Exception ex)
            {
