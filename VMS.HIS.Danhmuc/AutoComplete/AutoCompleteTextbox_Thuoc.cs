...
//    _grid.grdListDrug.se
        ...
        #region Methods
        public void Updatesoluongton(int id_thuoc,int soluongvuake)
        {
            try
            {
                _grid.AllowedChanged = false;
                DataRow[] arrDr = dtData.Select("id_thuoc=" + id_thuoc);
                if (arrDr.Length > 0)
                    arrDr[0]["so_luong"] = Utility.Int32Dbnull(arrDr[0]["oldValue"]) - soluongvuake;
        ...
        private void CreateAutocomplete()
        {
            ...
            try
            {
                ...
                foreach (DataRow dr in dtData.Rows)
                {
                    ...
                    string realName = dr[DmucThuoc.Columns.TenThuoc].ToString().Trim() + " " +
                        (dtData.Columns.Contains(DmucHoatchat.Columns.TenHoatchat) ? Utility.Bodau(dr[DmucHoatchat.Columns.TenHoatchat].ToString().Trim()) : Utility.Bodau(dr[DmucHoatchat.Columns.TenHoatchat].ToString().Trim()));
            ...
            finally
            {
                ...
                var query = from p in dtData.Select("1=1", DmucThuoc.Columns.TenThuoc).AsEnumerable()
                            select p[DmucThuoc.Columns.IdThuoc].ToString() + "#" + p[DmucThuoc.Columns.MaThuoc].ToString() + string.Format("@Id: {0};Code: {1}; ", p[DmucThuoc.Columns.IdThuoc].ToString(), p[DmucThuoc.Columns.MaThuoc].ToString()) + p[DmucThuoc.Columns.TenThuoc].ToString() + "(" + (dtData.Columns.Contains(DmucHoatchat.Columns.TenHoatchat) ? p[DmucHoatchat.Columns.TenHoatchat].ToString().Trim() : p[DmucHoatchat.Columns.TenHoatchat].ToString().Trim()) + ")" + "@" + p["shortcut"].ToString() + "$" + (existed ? p["id_thuockho"].ToString() : "-1");
                source = query.ToList();
        ...
        private void LoadFilterDrugCode(string text)
        {
            ...
            try
            {
                ...
                if (text.Trim() != "")
                {
                    rowFilter = DmucHoatchat.Columns.TenHoatchat + " like '%" + text + "%' OR " + DmucThuoc.Columns.MaThuoc + " LIKE '%" + text + "%' OR " + DmucThuoc.Columns.TenThuoc + " LIKE '%" + text + "%' OR " + DmucHoatchat.Columns.TenHoatchat + " LIKE '%" + text + "%'";
                }
                //dtData.DefaultView.RowFilter = "1=1";
        ...
        public void HideSuggestionListBox()
        {
            if ((TopLevelControl != null))
            {
                ...
                {
                    if (_grid.grdListDrug != null && _grid.grdListDrug.CurrentRow != null)
                        GridRowPosition = _grid.grdListDrug.CurrentRow.Position;
        ...
        protected override void OnKeyDown(KeyEventArgs args)
        {
            ...
            else if ((args.KeyCode == Keys.Enter))
            {
                ...
                if (GridView)
                {
                    try
                    {
                        if (GridRowPosition > 0 && _grid.grdListDrug.CurrentRow.Position <= 0)
                            _grid.grdListDrug.MoveTo(GridRowPosition);
                    }
                    catch (Exception)
                    {
        ...
        {
            if (!AllowTextChanged || (GridView && _grid.grdListDrug.DataSource==null)) return;
