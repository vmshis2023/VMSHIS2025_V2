...
{
    public partial class frm_themnhanh_donthuocmau : Form
    {
        ...
        public frm_themnhanh_donthuocmau(string KIEU_THUOC_VT)
        {
            InitializeComponent();
        ...
        void InitEvents()
        {
            Load += new EventHandler(frm_themnhanh_donthuocmau_Load);
            KeyDown += new KeyEventHandler(frm_themnhanh_donthuocmau_KeyDown);
            FormClosing += new FormClosingEventHandler(frm_themnhanh_donthuocmau_FormClosing);
            ...
            grdChitietthuoc.CellUpdated += new ColumnActionEventHandler(grdChitietthuoc_CellUpdated);
            grdChitietthuoc.CellValueChanged += new ColumnActionEventHandler(grdChitietthuoc_CellValueChanged);
            grdChitietthuoc.ColumnHeaderClick += new ColumnActionEventHandler(grdChitietthuoc_ColumnHeaderClick);
            grdChitietthuoc.FormattingRow += new RowLoadEventHandler(grdChitietthuoc_FormattingRow);
            grdChitietthuoc.UpdatingCell += new UpdatingCellEventHandler(grdChitietthuoc_UpdatingCell);
            grdChitietthuoc.KeyDown += new KeyEventHandler(grdChitietthuoc_KeyDown);
        ...
        void grdChitietthuoc_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void frm_themnhanh_donthuocmau_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (grdChitietthuoc.RowCount > 0)
            {
                if (!isSaved)
                {
        ...
        private void FormatUserNhapChiDinh()
        {
            try
            {
                GridEXColumn gridExColumn =
                    grdChitietthuoc.RootTable.Columns[DmucDonthuocmauChitiet.Columns.NguoiTao];
                GridEXColumn gridExColumnTarget =
                    grdChitietthuoc.RootTable.Columns[DmucThuoc.Columns.TenThuoc];
                ...
                grdChitietthuoc.RootTable.FormatConditions.Add(gridExFormatCondition);
            }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frm_themnhanh_donthuocmau_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (GridEXRow gridExRow in grdChitietthuoc.GetCheckedRows())
                {
                    ...
                    gridExRow.Delete();
                    grdChitietthuoc.UpdateData();
                    grdChitietthuoc.Refresh();
        ...
        {
            if (grdChitietthuoc.RowCount <= 0 || grdChitietthuoc.CurrentRow.RowType != RowType.Record)
            {
                Utility.ShowMsg("Bạn phải thực hiện chọn một thuốc cần xóa khỏi nhóm",
                                "Thông báo", MessageBoxIcon.Warning);
                grdChitietthuoc.Focus();
            ...
            if (!globalVariables.IsAdmin)
            {
                ...
                IdChitiet_ID =
                    Utility.Int32Dbnull(
                        grdChitietthuoc.CurrentRow.Cells[DmucDonthuocmauChitiet.Columns.IdChitiet].Value, -1);
        ...
        private bool IsValidData()
        {
            ...
            {
                Utility.ShowMsg("Bạn cần nhập mã đơn thuốc mẫu");
                txtManhom.Focus();
                return false;
            }
            if (Utility.DoTrim(txtTennhom.Text) == "")
            {
                Utility.ShowMsg("Bạn cần nhập tên đơn thuốc mẫu");
                txtTennhom.Focus();
                return false;
            }
            if (Utility.DoTrim(txtLoainhom.myCode) == "-1")
            {
                Utility.ShowMsg("Bạn cần chọn loại đơn thuốc mẫu");
                txtLoainhom.Focus();
                return false;
            }
            if (grdChitietthuoc.GetCheckedRows().Count() <= 1)
            {
                Utility.ShowMsg("Bạn cần chọn ít nhất hai thuốc khác nhau đưa vào đơn thuốc mẫu");
                cmdSave.Focus();
                return false;
            }
        ...
        private void ModifyCommand()
        {
            try
            {
                cmdSave.Enabled = grdChitietthuoc.RowCount > 0;
                cmdDelete.Enabled = grdChitietthuoc.RowCount > 0;
                cmdInPhieuCLS.Enabled = grdChitietthuoc.RowCount > 0;
                ModifyButtonCommand();
            }
            catch (Exception ex)
            {
        ...
        private List< DmucDonthuocmauChitiet> TaoDmucDonThuocMauChitiet()
        {
            ...
            foreach (GridEXRow gridExRow in grdChitietthuoc.GetDataRows())
            {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void grdChitietthuoc_CellValueChanged(object sender, ColumnActionEventArgs e)
        {
            ModifyButtonCommand();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frm_themnhanh_donthuocmau_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void AddDetail()
        {
            ...
            //            if (!selectnew)
            //            {
            //                Utility.GonewRowJanus(grdChitietthuoc, DmucDonthuocmauChitiet.Columns.IdThuoc, Utility.sDbnull(newDr[DmucDonthuocmauChitiet.Columns.IdThuoc], "0"));
            //                selectnew = true;
            //            }
            //        }
            //    }
        ...
        {
            cmdDelete.Enabled = grdChitietthuoc.GetCheckedRows().Length > 0;
            cmdSave.Enabled = grdChitietthuoc.RowCount > 0;
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void grdChitietthuoc_UpdatingCell(object sender, UpdatingCellEventArgs e)
        {
            try
            {
                if (e.Column.Key == DmucNhomcanlamsangChitiet.Columns.SoLuong)
                {
                    ...
                    GridEXRow _row = grdChitietthuoc.CurrentRow;
        ...
        private void grdChitietthuoc_FormattingRow(object sender, RowLoadEventArgs e)
        {
        ...
        private void grdChitietthuoc_ColumnHeaderClick(object sender, ColumnActionEventArgs e)
        {
            ModifyButtonCommand();
        }

        private void grdChitietthuoc_CellUpdated(object sender, ColumnActionEventArgs e)
        {
            ModifyButtonCommand();
        }
        ...
        private void mnuDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                long IdChitiet =
                    Utility.Int64Dbnull(grdChitietthuoc.CurrentRow.Cells[DmucDonthuocmauChitiet.Columns.IdChitiet].Value, -1);

                //CHIDINH_CANLAMSANG.XoaCLSChitietKhoinhom(IdChitiet);
                new Delete().From(DmucDonthuocmauChitiet.Schema).Where(DmucDonthuocmauChitiet.Columns.IdChitiet).IsEqualTo(IdChitiet).Execute();
                if (_OnActionSuccess != null) _OnActionSuccess();
                grdChitietthuoc.CurrentRow.Delete();
                grdChitietthuoc.UpdateData();
                grdChitietthuoc.Refresh();
                m_dtChitietNhom.AcceptChanges();
                m_blnCancel = false;
        ...
        private void LayThongTin_Chitiet_donthuocmau()
        {
            try
            {
                ...
                Utility.SetDataSourceForDataGridEx(grdChitietthuoc, m_dtChitietNhom, false, true, "1=1",  "" + DmucThuoc.Columns.TenThuoc);
                grdChitietthuoc.MoveFirst();
                ModifyCommand();
            }
            catch (Exception ex)
            {
