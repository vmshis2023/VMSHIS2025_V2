...
{
    public partial class frm_themmoi_dinhmuc_vtth : Form
    {
        ...
        public DmucDichvuclsChitiet objDvuChitiet;
        ...
        public frm_themmoi_dinhmuc_vtth(string KIEU_THUOC_VT)
        {
            InitializeComponent();
        ...
        void InitEvents()
        {
            Load += new EventHandler(frm_themmoi_dinhmuc_vtth_Load);
            KeyDown += new KeyEventHandler(frm_themmoi_dinhmuc_vtth_KeyDown);
            FormClosing += new FormClosingEventHandler(frm_themmoi_dinhmuc_vtth_FormClosing);
        ...
        private void frm_themmoi_dinhmuc_vtth_FormClosing(object sender, FormClosingEventArgs e)
        {
                    ...
                    if (m_dtChitietNhom.Select("NoSave=1").Length > 0)
                    {
                        if (
                            Utility.AcceptQuestion(
                                "Bạn đã thêm mới một số Vật tư mà chưa nhấn Ghi.\nBạn nhấn yes để hệ thống tự động lưu thông tin.\nNhấn No để hủy bỏ các Vật tư vừa thêm mới.", "Thông báo",
                                true))
                        {
        ...
        private void FormatUserNhapChiDinh()
        {
            try
            {
                GridEXColumn gridExColumn =
                    grdAssignDetail.RootTable.Columns[TDinhmucVtth.Columns.NguoiTao];
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frm_themmoi_dinhmuc_vtth_Load(object sender, EventArgs e)
        {
            try
            {
                if (objDvuChitiet != null)
                    this.Text = string.Format("Kê định mức VTTH cho dịch vụ: {0}",objDvuChitiet.TenChitietdichvu);
        ...
        private bool IsValidDataXoaCLS()
        {
            ...
            {
                Utility.ShowMsg("Bạn phải thực hiện chọn một Vật tư cần xóa",
            ...
            if (!globalVariables.IsAdmin)
            {
                ...
                {
                    int IdChitiet_ID =
                        Utility.Int32Dbnull(gridExRow.Cells[TDinhmucVtth.Columns.Id].Value, -1);
                    SqlQuery sqlQuery = new Select().From(TDinhmucVtth.Schema)
                        .Where(TDinhmucVtth.Columns.Id).IsEqualTo(IdChitiet_ID)
                        .And(TDinhmucVtth.Columns.NguoiTao).IsNotEqualTo(globalVariables.UserName);
                    if (sqlQuery.GetRecordCount() > 0)
                    {
                        Utility.ShowMsg("Trong các Vật tư bạn chọn xóa, có một số dịch vụ được thêm bởi Bác sĩ khác nên bạn không được phép xóa." +
                                        " Mời bạn chọn lại Vật tư do chính bạn thêm vào nhóm để thực hiện xóa");
                        return false;
        ...
        private bool IsValidDataXoaCLS_Selected()
        {
            ...
            if (!globalVariables.IsAdmin)
            {
                ...
                IdChitiet_ID =
                    Utility.Int32Dbnull(
                        grdAssignDetail.CurrentRow.Cells[TDinhmucVtth.Columns.Id].Value, -1);
                sqlQuery = new Select().From(TDinhmucVtth.Schema)
                    .Where(TDinhmucVtth.Columns.Id).IsEqualTo(IdChitiet_ID)
                   .And(TDinhmucVtth.Columns.NguoiTao).IsNotEqualTo(globalVariables.UserName);
                if (sqlQuery.GetRecordCount() > 0)
                {
        ...
        private bool IsValidData()
        {
            ...
            if (grdAssignDetail.RowCount <= 0)
            {
                Utility.ShowMsg( "Chưa chọn vật tư nào để làm định mức. Vui lòng kiểm tra lại", "Thông báo",
        ...
        {
            actionResult =
               kedonthuoc.ThemDinhmucVTTH(TaoTDinhmucVtth());
            switch (actionResult)
            {
        ...
        private void Themmoinhom(TDinhmucVtth objDonthuocmau)
        {
            DataRow dr = m_dtNhom.NewRow();
            dr[TDinhmucVtth.Columns.Id] = objDonthuocmau.Id;
            dr["ten_loainhom"] = txtLoainhom.Text;
            dr[TDinhmucVtth.Columns.NguoiTao] = objDonthuocmau.NguoiTao;
            dr[TDinhmucVtth.Columns.NgayTao] = objDonthuocmau.NgayTao;

            m_dtNhom.Rows.Add(dr);
        }
        private void CapnhatNhom(TDinhmucVtth objDonthuocmau)
        {
            DataRow[] arrDr = m_dtNhom.Select(TDinhmucVtth.Columns.Id + "=" + objDonthuocmau.Id);
            if (arrDr.Length > 0)
            {
                arrDr[0]["ten_loainhom"] = txtLoainhom.Text;
                arrDr[0][TDinhmucVtth.Columns.NguoiSua] = objDonthuocmau.NguoiSua;
                arrDr[0][TDinhmucVtth.Columns.NgaySua] = objDonthuocmau.NgaySua;
        ...
        /// </summary>
        /// <returns></returns>
        private List< TDinhmucVtth> TaoTDinhmucVtth()
        {
            List<TDinhmucVtth> lstChitiet = new List<TDinhmucVtth>();
            foreach (GridEXRow gridExRow in grdAssignDetail.GetDataRows())
            {
                if (gridExRow.RowType == RowType.Record)
                {
                    TDinhmucVtth newItem = new TDinhmucVtth();
                    int newID = Utility.Int32Dbnull(gridExRow.Cells[TDinhmucVtth.Columns.Id].Value, -1);
                    if (newID > 0)
                    {
                        newItem = TDinhmucVtth.FetchByID(newID);
                    ...
                    newItem.IdChitietdichvu = objDvuChitiet.IdChitietdichvu;
                    newItem.IdThuoc = Utility.Int32Dbnull(  gridExRow.Cells[TDinhmucVtth.Columns.IdThuoc].Value, -1);
                    newItem.SoLuong = Utility.Int32Dbnull(gridExRow.Cells[TDinhmucVtth.Columns.SoLuong].Value, 1);
                    ...
                    else
                    {
                        ...
                        newItem.NgaySua = globalVariables.SysDate;
                    }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frm_themmoi_dinhmuc_vtth_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void mnuDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                long IdChitiet =
                    Utility.Int64Dbnull(grdAssignDetail.CurrentRow.Cells[TDinhmucVtth.Columns.Id].Value, -1);
        ...
        private void grdServiceDetail_UpdatingCell(object sender, UpdatingCellEventArgs e)
        {
            try
            {
                if (e.Column.Key == TDinhmucVtth.Columns.SoLuong)
                {
                    if (!Numbers.IsNumber(e.Value.ToString()))
                    {
                        Utility.ShowMsg("Bạn phải số lượng phải là số", "Thông báo", MessageBoxIcon.Warning);
        ...
        private void LayThongTin_Chitiet_donthuocmau()
        {
            try
            {
                m_dtChitietNhom = kedonthuoc.DmucLaychitietDinhmucVtth(objDvuChitiet.IdChitietdichvu);
        ...
        private bool IsvalidChitiet()
        {
            ...
            if (sqlQuery.GetRecordCount() <= 0)
            {
                Utility.ShowMsg("Bạn cần chọn Vật tư để nhập");
                txtDrugName.Focus();
                return false;
            }
            return true;
        ...
        private void ThemMoiChiTiet()
        {
            try
            {
                DataRow[] arrDr = m_dtChitietNhom.Select(TDinhmucVtth.Columns.IdThuoc + "=" + txtDrug_ID.Text);
                if (arrDr.Length > 0)
                {
                    int newquantity = (int)(Utility.DecimaltoDbnull(arrDr[0][TDinhmucVtth.Columns.SoLuong], 0) + Utility.DecimaltoDbnull(txtSoluong.Text));
                    arrDr[0][TDinhmucVtth.Columns.SoLuong] = newquantity;
                    m_dtChitietNhom.AcceptChanges();
                }
                else
                {
                    DataRow drv = m_dtChitietNhom.NewRow();
                    drv["id_chitietdichvu"] =objDvuChitiet.IdChitietdichvu;
                    drv[TDinhmucVtth.Columns.IdThuoc] = txtDrug_ID.Text;
                    drv[DmucThuoc.Columns.TenThuoc] = objThuoc.TenThuoc;
                    drv[TDinhmucVtth.Columns.SoLuong] = Utility.DecimaltoDbnull(txtSoluong.Text);
                    drv["ten_donvitinh"] = txtDonViTinh.Text;
                    drv[TDinhmucVtth.Columns.NguoiTao] = globalVariables.UserName;
                    drv[TDinhmucVtth.Columns.NgayTao] = globalVariables.SysDate;
