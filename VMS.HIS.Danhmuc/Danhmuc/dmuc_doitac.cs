...
using VNS.Libs;
using VMS.HIS.DAL;
using VMS.HIS.NGHIEPVU;
using SubSonic;
namespace VNS.HIS.UI.DANHMUC
{
    public partial class dmuc_doitac : Form
    {
        #region Khai báo các biến cấp Module
        dmuc_doitac_busrule m_BusRules = new dmuc_doitac_busrule();
        ...
        void InitEvents()
        {
            ...
            {
                //Form load
                this.Load += new EventHandler(dmuc_doitac_Load);
                //Bắt sự kiện KeyDown trên form
                this.KeyDown += new KeyEventHandler(dmuc_doitac_KeyDown);
        ...
        void grdList_UpdatingCell(object sender, Janus.Windows.GridEX.UpdatingCellEventArgs e)
        {
            try
            {
                if (e.Column.Key == DmucDoitac.Columns.TenDoitac)
                {
                    new Update(DmucDoitac.Schema).Set(DmucDoitac.Columns.TenDoitac).EqualTo(e.Value).Where(DmucDoitac.Columns.Id).IsEqualTo(Utility.Int32Dbnull(Utility.getValueOfGridCell(grdList, DmucDoitac.Columns.Id))).Execute();
                }
            }
            catch (Exception ex)
            {
        ...
        private bool IsValidInputData()
        {
            try
            {
                ...
                if (txtNGT.myCode=="-1")
                {
                    Utility.SetMsg(lblMsg, "Bạn cần nhập nguồn giới thiệu", true);
                    txtNGT.Focus();
                    return false;
                }
        ...
        private void InsertDataTable()
        {
            ...
            try
            {
                ...
                drNewRow[DmucDoitac.Columns.Id] = _item.Id; 
                drNewRow[DmucDoitac.Columns.MaDoitac] =Utility.DoTrim( txtMa.Text);
                drNewRow[DmucDoitac.Columns.TenDoitac] = Utility.DoTrim(txtTen.Text);
                drNewRow[DmucDoitac.Columns.TrangThai] = chkTrangthai.Checked ? 1 : 0;
                drNewRow[DmucDoitac.Columns.TrangthaiMacdinh] = chkTrangthaiMacdinh.Checked ? 1 : 0;
                drNewRow[DmucDoitac.Columns.MaNguongioithieu] = Utility.DoTrim(txtNGT.myCode);
                drNewRow[DmucDoitac.Columns.Stt] = Convert.ToInt16(txtSTT.Text);
                drNewRow["ten_nguongioithieu"] = txtNGT.Text;
                drNewRow[DmucDoitac.Columns.MotaThem] = Utility.DoTrim(txtNGT.myCode);
                drNewRow[DmucDoitac.Columns.NguoiTao] = globalVariables.UserName;
                drNewRow[DmucDoitac.Columns.NgayTao] = globalVariables.SysDate;
        ...
        private void UpdateDataTable()
        {
            try
            {
                ...
                drRow[DmucDoitac.Columns.MaDoitac] = Utility.DoTrim(txtMa.Text);
                drRow[DmucDoitac.Columns.TenDoitac] = Utility.DoTrim(txtTen.Text);
                drRow[DmucDoitac.Columns.TrangThai] = chkTrangthai.Checked ? 1 : 0;
                drRow[DmucDoitac.Columns.MaNguongioithieu] = Utility.DoTrim(txtNGT.myCode);
                drRow["ten_nguongioithieu"] = txtNGT.Text;
                drRow[DmucDoitac.Columns.TrangthaiMacdinh] = chkTrangthaiMacdinh.Checked ? 1 : 0;
                drRow[DmucDoitac.Columns.Stt] = Convert.ToInt16(txtSTT.Text);
                drRow[DmucDoitac.Columns.MotaThem] = Utility.DoTrim(txtNGT.myCode);
                drRow[DmucDoitac.Columns.NguoiSua] = globalVariables.UserName;
                drRow[DmucDoitac.Columns.NgaySua] = globalVariables.SysDate;
                m_dtData.AcceptChanges();
            }
            catch (Exception)
            {
        ...
        /// </summary>
        /// <returns></returns>
        private DmucDoitac GetObject()
        {
            try
            {

                DmucDoitac obj = new DmucDoitac();
                if (m_enAct == action.Update) obj.Id =Utility.Int32Dbnull( txtID.Text,0);
                obj.MaDoitac = Utility.DoTrim(txtMa.Text);
                obj.TenDoitac = Utility.DoTrim(txtTen.Text);
                obj.MaNguongioithieu = Utility.DoTrim(txtNGT.myCode);
                obj.TrangThai = Convert.ToByte(chkTrangthai.Checked ? 1 : 0);
                obj.TrangthaiMacdinh = Convert.ToByte(chkTrangthaiMacdinh.Checked ? 1 : 0);
                obj.Stt = Convert.ToInt16(txtSTT.Text);
                obj.MotaThem = Utility.DoTrim(txtNGT.myCode);
        ...
        int m_intOldOrder = 0;
        DmucDoitac _item = new DmucDoitac();
        /// <summary>
        ...
        private bool PerformInserAct()
        {
            try
            {
                ...
                if (ActResult == ActionResult.Success.ToString())
                {
                    ...
                    Utility.GonewRowJanus(grdList, DmucDoitac.Columns.Id, txtID.Text.Trim());
                    ...
                    Utility.SetMsg(lblMsg, "Thêm mới đối tác thành công", false);
                ...
                {
                    Utility.SetMsg(lblMsg, "Mã đối tác đã tồn tại. Đề nghị bạn nhập mã khác!", true);
                    txtMa.Focus();
                    return false;
                }
                else if (ActResult == ActionResult.Exception.ToString())
                {
                    ...
                    Utility.ShowMsg("Lỗi khi thực hiện thêm mới đối tác\n" + ActResult, "Thông báo");
                    return false;
                }
                return true;
            }
        ...
        private bool PerformUpdateAct()
        {
            ...
            try
            {
                ...
                if (ActResult == ActionResult.Success.ToString())
                {
                    //Cho phép chọn trên lưới để fill dữ liệu xuống Vùng nhập liệu
                    m_blnAllowCurrentCellChanged = true;
                    Utility.SetMsg(lblMsg, "Cập nhật đối tác thành công!", false);
                ...
                {
                    Utility.ShowMsg("Lỗi khi Update danh mục. Liên hệ với VMS để được hỗ trợ");
                    txtTen.Focus();
                    return false;
        ...
        private bool PerformDeleteAct()
        {
            try
            {
                ...
                string DeleteContent = "đối tác (" + txtMa.Text.Trim() + "-" + txtTen.Text.Trim() + ")";
                int _ID = Utility.Int32Dbnull(Utility.getValueOfGridCell(grdList, DmucDoitac.IdColumn.ColumnName), 0);
                if (!Utility.AcceptQuestion("Bạn có muốn xóa " + DeleteContent + " hay không?", "Xác nhận trước khi xóa danh mục", true)) return false;
                //if (SPs.DmucKieudanhmucCheckDelete(txtMa.Text.Trim()).GetDataSet().Tables[0].Rows.Count > 0)
                //{
                //    Utility.ShowMsg("Đã có dữ liệu của danh mục này trong bảng Danh mục dùng chung nên bạn không thể xóa.\n Chú ý: Muốn xóa bạn cần vào xóa hết dữ liệu danh mục liên quan đến đối tác này trước.");
                //    return false;
                //}
               ...
               _ActResult= m_BusRules.DeleteList(_ID,txtMa.Text.Trim());
                ...
                {
                    Utility.ShowMsg("Lỗi khi xóa đối tác");
                    return false;
                }
                return true;
        ...
        public void SearchData()
        {
            try
            {
                ...
                m_dtData = SPs.TiepdonDmucdoitacLaytheonguongioithieu("ALL").GetDataSet().Tables[0];
                if (m_dtData == null)
                {
                ...
                Utility.SetDataSourceForDataGridEx(grdList, m_dtData, true, true, "1=1", "stt,ten_doitac");
                txtMa.Init(m_dtData, new List<string>() { DmucDoitac.Columns.Id, DmucDoitac.Columns.MaDoitac, DmucDoitac.Columns.TenDoitac });
        ...
        void ResetDataRegion()
        {
            ...
            txtMa.Text = "";
            txtTen.Text = "";
            chkTrangthaiMacdinh.Checked = false;
            txtNGT.SetCode("-1");
        ...
        /// <param name="e"></param>
        void dmuc_doitac_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        /// <param name="e"></param>
         ...
         void dmuc_doitac_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            try
            {
               ...
               else  if (e.Control && e.KeyCode == Keys.N) cmdNew_Click(cmdNew, new EventArgs());
               else  if (e.Control && e.KeyCode == Keys.U) cmdUpdate_Click(cmdUpdate, new EventArgs());
                else if (e.Control && e.KeyCode == Keys.D) cmdDelete_Click(cmdDelete, new EventArgs());
                else if (e.Control && e.KeyCode == Keys.P) cmdPrint_Click(cmdPrint, new EventArgs());
         ...
         void Refresh()
         {
             try
             {
                 DataTable dt = new Select(DmucChung.Columns.Loai).From(DmucChung.Schema).Distinct().ExecuteDataSet().Tables[0];
                 bool haschanged = false;
                 foreach (DataRow dr in dt.Rows)
                     if (m_dtData.Select("MA_LOAI='" + Utility.sDbnull(dr["LOAI"]) + "'").Length <= 0)//Auto insert
                     {
                         DmucDoitac item = new DmucDoitac();
                         item.MaDoitac = Utility.sDbnull(dr["LOAI"]);
                         item.TenDoitac = item.MaDoitac;
                         item.TrangThai = 1;
                         item.MotaThem = "";
                         item.NgayTao = DateTime.Now;
                         item.NguoiTao = globalVariables.UserName;
                         item.IsNew = true;
                         item.Save();
                         haschanged = true;
                     }
        ...
        void grdList_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dr != null)
                {
                    txtID.Text = dr[DmucDoitac.Columns.Id].ToString();
                    txtMa._Text = dr[DmucDoitac.Columns.MaDoitac].ToString();
                    txtTen.Text = dr[DmucDoitac.Columns.TenDoitac].ToString();

                    chkTrangthaiMacdinh.Checked = dr[DmucDoitac.Columns.TrangthaiMacdinh].ToString() == "1" ? true : false;
                    chkTrangthai.Checked = dr[DmucDoitac.Columns.TrangThai].ToString() == "1" ? true : false;
                    txtNGT.SetCode( dr[DmucDoitac.Columns.MotaThem].ToString());
                }
        ...
        private void cmdPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DMUC_DCHUNG _DMUC_DCHUNG = new DMUC_DCHUNG(Utility.sDbnull(Utility.getValueOfGridCell(grdList, DmucDoitac.Columns.MaDoitac), ""));
