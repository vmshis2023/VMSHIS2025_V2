...
{
    public partial class frm_InBangKeChiPhiKCB : Form
    {
        ...
        string SplitterPath = "";
        public frm_InBangKeChiPhiKCB(bool Khoanoitrutonghop,string idkhoanoitru)
        {
            InitializeComponent();
        ...
        void InitEvents()
        {
            this.Load += frm_InBangKeChiPhiKCB_Load;
            this.KeyDown += frm_InBangKeChiPhiKCB_KeyDown;
            Shown+=frm_InBangKeChiPhiKCB_Shown;
            FormClosing += frm_InBangKeChiPhiKCB_FormClosing;
        ...
        void frm_InBangKeChiPhiKCB_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        void frm_InBangKeChiPhiKCB_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        void frm_InBangKeChiPhiKCB_Shown(object sender, EventArgs e)
        {
            Try2Splitter();
        }
        ...
        void cmdPrint_Click(object sender, EventArgs e)
        {
            ...
            if (chk_internal.Checked)
            {
                ...
                new INPHIEU_THANHTOAN_NGOAITRU().Inbienlai_DichvuChuathanhtoan(m_dtChiPhiThanhtoan, true, 100);
        }
        void Intonghop()
        {
            //try
        ...
        void Inchiphi()
        {
            try
            {
                ...
                THU_VIEN_CHUNG.Sapxepthutuin(ref m_dtChiPhiThanhtoan, false);
                m_dtChiPhiThanhtoan.DefaultView.Sort = "stt_in,stt_hthi_loaidichvu ,stt_hthi_dichvu,stt_hthi_chitiet,ten_chitietdichvu";

                THU_VIEN_CHUNG.CreateXML(m_dtChiPhiThanhtoan, Application.StartupPath + @"\Xml4Reports\Thanhtoan_InBienLai_DV_chuathanhtoan.XML");
                if (m_dtChiPhiThanhtoan.Rows.Count <= 0)
                {
                    Utility.ShowMsg("Không tìm thấy dữ liệu in phiếu (KCB_THANHTOAN_LAYTHONGTIN_INPHIEU_DICHVU)", "Thông báo");
                    return;
                }
                DataTable dtDataPrint = m_dtChiPhiThanhtoan.Clone();
                DataRow[] arrDr = null;
                if (optAll1.Checked)
                {
                    dtDataPrint = m_dtChiPhiThanhtoan.Copy();
                }
                else if (optNoitru1.Checked)
                {
                    arrDr = m_dtChiPhiThanhtoan.Select("Noi_tru=1");
                    if (arrDr.Length > 0) dtDataPrint = arrDr.CopyToDataTable();
                }
                else if (optNgoaitru1.Checked)
                {
                    arrDr = m_dtChiPhiThanhtoan.Select("Noi_tru=0");
                    if (arrDr.Length > 0) dtDataPrint = arrDr.CopyToDataTable();
                }
        ...
        void frm_InBangKeChiPhiKCB_Load(object sender, EventArgs e)
        {
            ShowHideMienGiam();
        ...
        void LoadData()
        {
            try
            {
                byte tthai_noitru = 100;
                byte tthai_tamthu = 100;
                if (optNoitru.Checked)
                    tthai_noitru = 1;
                else if (optNgoaitru.Checked)
                    tthai_noitru = 0;

                if (opt_loaichiphi_tamthu.Checked)
                    tthai_tamthu = 1;
                else if (opt_loaichiphi_dathanhtoan.Checked)
                    tthai_tamthu = 0;

                ucThongtinnguoibenh_doc_v51.Refresh(objLuotkham.MaLuotkham);
                m_dtChiPhiThanhtoan =
                   SPs.ThanhtoanBangkechiphiKcbIn(objLuotkham.MaLuotkham, (int)objLuotkham.IdBenhnhan,Utility.Bool2byte( opt_loaichiphi_tamthu.Checked), idkhoanoitru, getidphieudieutri(),Utility.Bool2byte(chk_Donthuocravien.Checked), tthai_noitru,tthai_tamthu).GetDataSet().Tables[0];
