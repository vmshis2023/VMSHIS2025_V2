...
{
    public partial class frm_KCB_CHIDINH_CLS_Auto : Form
    {
        ...
        public frm_KCB_CHIDINH_CLS_Auto(string nhomchidinh, byte kieu_chidinh)
        {
            ...
            txtFilterName.Focus();
            dtp_thoigianchidinh.Value = globalVariables.SysDate;
            dtp_thoigianchidinh.ReadOnly = !globalVariables.IsAdmin;
        ...
        void InitEvents()
        {
            Load += new EventHandler(frm_KCB_CHIDINH_CLS_Auto_Load);
            KeyDown += new KeyEventHandler(frm_KCB_CHIDINH_CLS_Auto_KeyDown);
            FormClosing += new FormClosingEventHandler(frm_KCB_CHIDINH_CLS_Auto_FormClosing);
        ...
        private void CboDoituongKcb_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                m_dtDanhsachDichvuCLS = CHIDINH_CANLAMSANG.LaydanhsachCLS_chidinh(ma_doituongkcb, batbuoc_tutuc,
                                                                                   ...
                                                                                   globalVariables.MA_KHOA_THIEN, nhomchidinh, tnv_chidinh, dtp_thoigianchidinh.Value);
        ...
        private void frm_KCB_CHIDINH_CLS_Auto_FormClosing(object sender, FormClosingEventArgs e)
        {
            SaveUserConfigs();
        ...
        int goi_id_dangky = -1;
        int goi_id_kham = -1;
        public void SetInfor(KcbLuotkham objLuotkham,DateTime ngay_chidinh, int goi_id_kham, int goi_id_goi, int goi_id_dangky, int id_bacsi_chidinh)
        {
            this.objLuotkham = objLuotkham;
            dtp_thoigianchidinh.Value = ngay_chidinh;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frm_KCB_CHIDINH_CLS_Auto_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private bool IsValidData()
        {
            ...
            {
                if (objPhieudieutriNoitru.NgayDieutri != null && dtp_thoigianchidinh.Value.Date > objPhieudieutriNoitru.NgayDieutri.Value.Date)
                {
                    Utility.ShowMsg("Ngày chỉ định phải <= " + objPhieudieutriNoitru.NgayDieutri.Value.ToString("dd/MM/yyyy"));
                    dtp_thoigianchidinh.Focus();
                    return false;
                }
            }
        ...
        private void Auto_InsertDataCls()
        {
        ...
        private KcbChidinhcl TaoPhieuchidinh()
        {
            ...
            objKcbChidinhcls.NgayChidinh = dtp_thoigianchidinh.Value;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frm_KCB_CHIDINH_CLS_Auto_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        bool kiemtratrungchidinhcls_trongngay_tieptuc(GridEXRow gridExRow, bool canhbao)
        {
            try
            {
                ...
                DataTable dtCheck = SPs.ChidinhclsKiemtrachidinhtrungTrongNgay(id_chitietdichvu, objLuotkham.IdBenhnhan, objLuotkham.MaLuotkham, dtp_thoigianchidinh.Value.ToString("dd/MM/yyyy")).GetDataSet().Tables[0];
                if (dtCheck.Rows.Count > 0)//Kiểm tra
                {

                    if (canhbao)//Cảnh báo và hỏi
                    {
                        if (Utility.AcceptQuestion(string.Format("Dịch vụ {0} đã được chỉ định trong ngày {1} tại phiếu có Id={2}, mã chỉ định ={3}. Bạn có muốn tiếp tục thêm chỉ định này hay không?", ten_dichvu, dtp_thoigianchidinh.Value.ToString("dd/MM/yyyy"), Utility.sDbnull(dtCheck.Rows[0]["id_chidinh"], ""), Utility.sDbnull(dtCheck.Rows[0]["ma_chidinh"], "")), "Cảnh báo", true))
                        {
                            return true;
                        }
                        else
        ...
        private void GetData()
        {
            ...
            if (objKcbChidinhcls != null)
            {
                ...
                dtp_thoigianchidinh.Value = objKcbChidinhcls.NgayChidinh;
            ...
            else
            {
                ...
                if (objPhieudieutriNoitru != null)
                {
                    if (objPhieudieutriNoitru.NgayDieutri != null)
                        dtp_thoigianchidinh.Value = objPhieudieutriNoitru.NgayDieutri.Value.AddHours(Utility.Int32Dbnull(objPhieudieutriNoitru.GioDieutri.Split(':')[0], 0)).AddMinutes(Utility.Int32Dbnull(objPhieudieutriNoitru.GioDieutri.Split(':')[1], 0));
                }
                else
                dtp_thoigianchidinh.Value = globalVariables.SysDate;
                txtAssignCode.Text = THU_VIEN_CHUNG.SinhMaChidinhCLS();
        ...
        private KcbChandoanKetluan TaoDulieuChandoanKetluan()
        {
            try
            {
                ...
                _KcbChandoanKetluan.NgayChandoan = dtp_thoigianchidinh.Value;
                _KcbChandoanKetluan.Ketluan ="";
