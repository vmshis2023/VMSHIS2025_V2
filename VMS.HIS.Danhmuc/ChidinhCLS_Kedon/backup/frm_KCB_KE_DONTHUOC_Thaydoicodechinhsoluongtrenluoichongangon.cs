...
namespace VNS.HIS.UI.NGOAITRU
{
    public partial class frm_KCB_KE_DONTHUOC : Form
    {
        ...
        private void AddPreDetail()
        {
            try
            {
                ...
                if (noitru == 0 || objPhieudieutriNoitru == null)//Hiện chỉ cảnh báo cho kê đơn ngoại trú
                {
                    ...
                    if (dtRows.Any())
                    {
                        if (!Utility.AcceptQuestion(string.Format("Thuốc {0} đã được kê, bạn có muốn tiếp tục kê thêm?", txtDrug_Name.Text), "Thông báo kê trùng thuốc", true))
                        {
                            return;
                        }
                    }
                ...
                foreach (DataRow thuockho in listdata.Rows)
                {
                    ...
                    if (soluong > 0)
                    {
                        ...
                        if (IdThuockho > 0)//100%
                        {
                            ...
                            THU_VIEN_CHUNG.UpdateKeTam(-1, -1, GUID,privateguid, IdThuockho, objThuoc.IdThuoc, Utility.Int16Dbnull(cboStock.SelectedValue), Utility.DecimaltoDbnull(soluong), (byte)LoaiTamKe.KEDONTHUOC,
                                   txtPatientCode.Text, Utility.Int32Dbnull(txtPatientID.Text), noitru, THU_VIEN_CHUNG.GetSysDateTime(),"Thêm mới thuốc");
                        }
        ...
        private bool AddQuantity(int id_thuoc, int id_thuockho, decimal SlMoi,decimal SlCu)
        {
            try
            {
        ...
        private void frm_KCB_KE_DONTHUOC_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (!isSaved)
            {
                ...
                {
                    DataTable dtPaymentData = SPs.DonthuocKiemtratrangthaiChitiet(donthuoc.IdDonthuoc).GetDataSet().Tables[0];
                    if (dtPaymentData.Rows.Count <= 0)//Chưa có thành phần nào trong đơn được thanh toán hoặc tổng hợp cấp phát
                    {
                        getdata4Save();
                        // List<KcbDonthuocChitiet> changedData = GetChangedData();
            ...
            try
            {
                ...
                else//Đơn thường
                {
                    if (noitru == 0)// Kê đơn ngoại trú tại phòng khám
                    {
                        ...
                        if (KIEU_THUOC_VT == "THUOC")
                        {
                            dtStockList = CommonLoadDuoc.LAYDANHMUCKHO_THEOKHOA(Utility.Int16Dbnull(globalVariables.idKhoatheoMay, -1), -1, "THUOC,THUOCVT", 100, Utility.ByteDbnull(objLuotkham.TrangthaiCapcuu, 0));// CommonLoadDuoc.LAYTHONGTIN_KHOTHUOC_LE_NGOAI_TRU_KEDON(objLuotkham.MaDoituongKcb);
                        }
                        else
                        {
                            dtStockList = CommonLoadDuoc.LAYDANHMUCKHO_THEOKHOA(Utility.Int16Dbnull(globalVariables.idKhoatheoMay, -1), -1, "VT,THUOCVT", 100, Utility.ByteDbnull(objLuotkham.TrangthaiCapcuu, 0));// CommonLoadDuoc.LAYTHONGTIN_KHOTHUOC_LE_NGOAI_TRU_KEDON(objLuotkham.MaDoituongKcb);
                        }
                        //}
                    }
                    else //Nội trú
        ...
        private void grdPresDetail_UpdatingCell(object sender, UpdatingCellEventArgs e)
        {
            try
            {
                ...
                else if ((e.Column.Key != KcbDonthuocChitiet.Columns.TuTuc) && (e.Column.Key == KcbDonthuocChitiet.Columns.SoLuong))
                {
                    ...
                    objThuoc = DmucThuoc.FetchByID(idThuoc);
                    int _IdThuockho = Utility.Int32Dbnull(currentRow.Cells[KcbDonthuocChitiet.Columns.IdThuockho].Value, 0);
                    int _IdDonthuoc = Utility.Int32Dbnull(currentRow.Cells[KcbDonthuocChitiet.Columns.IdDonthuoc].Value, 0);
                    int _IdChitietdonthuoc = Utility.Int32Dbnull(currentRow.Cells[KcbDonthuocChitiet.Columns.IdChitietdonthuoc].Value, 0);
                    ...
                    //decimal sluong_cu = Utility.DecimaltoDbnull(e.InitialValue, 0);
                    decimal sluong_moi = Utility.DecimaltoDbnull(e.Value, 0);
                    if (sluong_cu != sluong_moi)
                    {
                        hasChanged = true;
                    ...
                    decimal chenhlech_tang = sluong_moi - sluong_cu;
                    DataRow[] arrDrChange;
                    if (sluong_moi != sluong_cu)
                    {
                        if (sluong_moi > sluong_cu)
                        {
                            if (!AddQuantity(idThuoc, _IdThuockho, sluong_moi, sluong_cu))
                        ...
                        else//Giảm thuốc thì thực hiện bước bớt 
                        {
                            //if (_IdChitietdonthuoc > 0)
                            //    //Xử lý trong dòng tạm kê
                            //    THU_VIEN_CHUNG.UpdateQtyKeTam(_IdChitietdonthuoc, _IdDonthuoc, GUID, privateguid,_IdThuockho, objThuoc.IdThuoc, Utility.Int16Dbnull(cboStock.SelectedValue), Utility.DecimaltoDbnull(sluong_moi), (byte)LoaiTamKe.KEDONTHUOC, 1);
                            //else
                                THU_VIEN_CHUNG.UpdateKeTam(_IdChitietdonthuoc, _IdDonthuoc, GUID, privateguid, _IdThuockho, objThuoc.IdThuoc, Utility.Int16Dbnull(cboStock.SelectedValue), Utility.DecimaltoDbnull(sluong_moi), (byte)LoaiTamKe.KEDONTHUOC,
                            ...
                            //    predicate =
                            //        q =>
                            //            (Utility.Int32Dbnull(q[KcbDonthuocChitiet.Columns.IdThuoc], 0) == idThuoc) &&
                            //            (Utility.DecimaltoDbnull(q[KcbDonthuocChitiet.Columns.DonGia], 0) == donGia);
                            //}
                            //DataRow[] rowArray =
                            ...
                            //decimal chenhlech_giam = sluong_cu - sluong_moi;
                            //var dictionary = new Dictionary<int, decimal>();
                            //var lstDeleteId = new List<int>();
                            //int iddetail = -1;
                            //string lstIdChitietDonthuoc = "";
                            //for (int i = 0; i <= (rowArray.Length - 1); i++)//duyệt qua các thuốc cùng giá để bắt đầu trừ
                            //{
                            //    if (chenhlech_giam > 0)
                            //    {
                            //        decimal soluongthuoc = Utility.DecimaltoDbnull(rowArray[i][KcbDonthuocChitiet.Columns.SoLuong],
                            //            0);
                            //        if (soluongthuoc >= chenhlech_giam)//Bỏ bớt thuốc này là xong. Nếu không xét tiếp thuốc kế tiếp
                            //        {
                            //            rowArray[i][KcbDonthuocChitiet.Columns.SoLuong] = soluongthuoc - chenhlech_giam;
                            //            chenhlech_giam = soluongthuoc - chenhlech_giam;
                            //            dictionary.Add(Utility.Int32Dbnull(rowArray[i][KcbDonthuocChitiet.Columns.IdChitietdonthuoc]), chenhlech_giam);
                            //            iddetail =
                            //                Utility.Int32Dbnull(
                            ...
                            //            break;
                            //        }
                            //        //Cập nhật và tiếp tục xét dòng kế tiếp
                            ...
                            //            Utility.Int32Dbnull(
                            //                rowArray[i][KcbDonthuocChitiet.Columns.IdChitietdonthuoc]));
                            //        chenhlech_giam -= soluongthuoc;
                            //    }
                            //}
                            //Dùng cách tạm kê
                            //Rem đoạn này để nghĩ cách xử lý dùng tạm kê 230522
                            //_kedonthuoc.XoaChitietDonthuoc(lstIdChitietDonthuoc, iddetail, chenhlech_giam);
                            //grdPresDetail.UpdateData();
                            //deletefromDatatable(lstDeleteId, iddetail, chenhlech_giam);
                        }
                        //decimal sluonghientai =
                        //    Utility.DecimaltoDbnull(
                        //        m_dtDonthuocChitiet_View.Select(KcbDonthuocChitiet.Columns.IdThuoc + "=" + idThuoc +
                        //                                        " AND " + KcbDonthuocChitiet.Columns.DonGia + "=" +
                        //                                        donGia)[0][KcbDonthuocChitiet.Columns.SoLuong], 0);
                        //if (chenhlech_tang > 0)
                        //{
                        //    e.Value = sluonghientai;
                        //}
                        //else
                        //{
                        //    sluonghientai = Utility.DecimaltoDbnull(e.Value, 0);
                        //    e.Value = e.Value;
                        //}
                        DataRow[] rowArray2 =
                            m_dtDonthuocChitiet_View.Select(KcbDonthuocChitiet.Columns.IdThuoc + "=" + idThuoc);
        ...
        private void PerformAction(KcbDonthuocChitiet[] arrPresDetail)
        {
            ...
            if (em_Action == action.Update)
            {
                ...
                if (Utility.Byte2Bool(objdt.TthaiTonghop))
                {
