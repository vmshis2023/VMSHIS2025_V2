...
namespace VNS.HIS.UI.NGOAITRU
{
    public partial class frm_KCB_CHIDINH_CLS : Form
    {
        ...
        void mnuSaveAs_Click(object sender, EventArgs e)
        {
            try
            {
                if (grdAssignDetail.GetCheckedRows().Length <= 1)
                {
                ...
                _themnhanh_nhomcls.grdAssignDetail = this.grdAssignDetail;
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (GridEXRow gridExRow in grdAssignDetail.GetCheckedRows())
                {
                    ...
                    {
                        CHIDINH_CANLAMSANG.XoaChiDinhCLSChitiet(idChidinhchitiet);
        ...
        private bool IsValidDataXoaCls()
        {
            ...
            if (!globalVariables.IsAdmin)
            {
                ...
                foreach (GridEXRow gridExRow in grdAssignDetail.GetCheckedRows())
                {
                    int assignDetailId =
                        Utility.Int32Dbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.IdChitietchidinh].Value, -1);
                  int i =   m_dtChitietPhieuCLS.Select(KcbChidinhclsChitiet.Columns.IdChitietchidinh + " = " + assignDetailId +
                    ...
                    //    .Where(KcbChidinhclsChitiet.Columns.IdChitietchidinh).IsEqualTo(assignDetailId)
        ...
        private bool IsValidDataXoaCLS_Selected()
        {
            ...
            int AssignDetail_ID = Utility.Int32Dbnull(grdAssignDetail.CurrentRow.Cells[KcbChidinhclsChitiet.Columns.IdChitietchidinh].Value, -1);
        ...
        private bool KiemTraTamUng()
        {
            ...
            foreach (GridEXRow  row  in  grdAssignDetail.GetDataRows())
            {
        ...
        private void ModifyCommand()
        {
            try
            {
                ...
                cmdDelete.Enabled = Utility.isValidGrid(grdAssignDetail) || grdAssignDetail.GetCheckedRows().Count() > 0;
                cmdInPhieuCLS.Enabled =cmdIntachPhieu.Enabled= grdAssignDetail.RowCount > 0;
                ModifyButtonCommand();
            }
            catch (Exception ex)
            {
        ...
        private void InsertDataCls()
        {
            ...
            actionResult =
               CHIDINH_CANLAMSANG.InsertDataChiDinhCls(
                    objKcbChidinhcls, objLuotkham, TaoChitietchidinh(), GetIdDangkyGoi(), getDichvutronggoi(), ref ErrMsg);
            switch (actionResult)
            {
        ...
        private KcbChidinhclsChitiet[] TaoChitietchidinh()
        {
            ...
            foreach (GridEXRow gridExRow in grdAssignDetail.GetDataRows())
            {
                if (gridExRow.RowType == RowType.Record)
                {
                    ...
                    arrAssignDetail[idx].GiaDanhmuc = Utility.DecimaltoDbnull(
                       gridExRow.Cells["gia_goc"].Value, 0);
                    ...
                    if (arrAssignDetail[idx].IdGoi > 0) arrAssignDetail[idx].TrongGoi = 1;
            ...
            try
            {
                ...
                foreach (GridEXRow gridExRow in arrCheckList)
                {
                    ...
                    if (!query.Any())
                    {
                        ...
                        decimal don_gia =Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.DonGia], 0);
                        decimal phu_thu = Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.PhuThu], 0); 
                        decimal PtramBhyt=Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.PtramBhyt], 0); 
                        decimal tyle_tt=!THU_VIEN_CHUNG.IsBaoHiem(objLuotkham.IdLoaidoituongKcb)?100:Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.TyleTt], 100);
                        decimal bhyt_gia_tyle =0m; 
                        decimal bhyt_cct = 0m;
                        decimal bn_cct = 0m;
                        decimal bn_ttt = 0m;
                        decimal bnhan_chitra = 0m;
                        decimal so_luong = Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.SoLuong], 1);
                        //Tính lại mức hưởng
                        if (objLuotkham.DungTuyen == 1)
                        {
                        }
                        ...
                        {
                            if (objLuotkham.TrangthaiNoitru <= 0)//Ngoại trú trái tuyến
                            {
                                PtramBhyt = 0;
                            }
                            else
                       ...
                       if(THU_VIEN_CHUNG.IsBaoHiem(objLuotkham.IdLoaidoituongKcb))
                       {
                           ...
                           bhyt_cct=bhyt_gia_tyle*PtramBhyt;
                           bn_cct=bhyt_gia_tyle-bhyt_cct;
                           bn_ttt = don_gia - bhyt_gia_tyle;
                       }
                       bnhan_chitra = don_gia - bhyt_cct;// hoặc =bn_cct+bn_ttt
                       newDr[KcbChidinhclsChitiet.Columns.BhytChitra] = bhyt_cct;
                       newDr[KcbChidinhclsChitiet.Columns.BnhanChitra] = bnhan_chitra;
                       newDr[KcbChidinhclsChitiet.Columns.BhytGiaTyle] = bhyt_gia_tyle;
                       newDr[KcbChidinhclsChitiet.Columns.BnCct] = bn_cct;
                       newDr[KcbChidinhclsChitiet.Columns.BnTtt] = bn_ttt;
                        //Tính thành tiền
                       newDr["tt_bv"] = (don_gia + phu_thu) * so_luong;
                       newDr["tt_bhyt"] = don_gia*so_luong;

                        //if (Utility.Int32Dbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.TuTuc].Value, 0) == 0 && chkTutuc.Checked == false)
                        //{
                          
                        //    if (THU_VIEN_CHUNG.IsBaoHiem(objLuotkham.IdLoaidoituongKcb))//Đối tượng BHYT thì tính lại giá BHYT và giá BN_CCT
                        //    {
                        //        if (objLuotkham.DungTuyen == 1)
                        //        {
                        //            BHCT = Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.DonGia].Value, 0) * (Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.TyleTt].Value, 0) / 100) * (Utility.DecimaltoDbnull(objLuotkham.PtramBhyt, 0) / 100);
                        //        }
                        //        else//Trái tuyến
                        //        {
                        //            if (objLuotkham.TrangthaiNoitru <= 0)
                        //                BHCT = Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.DonGia].Value, 0) * (Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.TyleTt].Value, 0) / 100) * (Utility.DecimaltoDbnull(objLuotkham.PtramBhyt, 0) / 100);
                        //            else//Nội trú cần tính=đơn giá * % đầu thẻ * % tuyến
                        //                BHCT = Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.DonGia].Value, 0) * (Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.TyleTt].Value, 0) / 100) * (Utility.DecimaltoDbnull(objLuotkham.PtramBhytGoc, 0) / 100) * (BHYT_PTRAM_TRAITUYENNOITRU / 100);
                        //        }
                        //        // BNCT = Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.DonGia].Value, 0) * (Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.TyleTt].Value, 0) / 100) - BHCT;
                        //        BNCT = Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.DonGia].Value, 0) - BHCT;

                        //    }
                        //    newDr[KcbChidinhclsChitiet.Columns.BhytChitra] = BHCT;
                        //    newDr[KcbChidinhclsChitiet.Columns.BnhanChitra] = BNCT;
                        //    newDr[KcbChidinhclsChitiet.Columns.TuTuc] = Utility.Int32Dbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.TuTuc].Value);
                        //    //newDr[KcbChidinhclsChitiet.Columns.TuTuc] = Utility.Int32Dbnull(false);
                        //}
                        //else//Tự túc
                        //{
                        ...
                        //    newDr[KcbChidinhclsChitiet.Columns.GiaDanhmuc] = Utility.DecimaltoDbnull(gridExRow.Cells["gia_goc"].Value, 0);// Utility.DecimaltoDbnull(objQheDtuongGia.DonGia, 0);
                        ...
                        //}
                        //newDr["TT_BHYT"] = (Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.BhytChitra], 0)) *
                        //            (Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.TyleTt], 0) / 100) *
                        //            Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.SoLuong], 0);

                        //newDr["TT_BN"] = (Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.BnhanChitra], 0) *
                        //                  (Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.TyleTt], 0) / 100) +
                        //                  Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.PhuThu], 0)) *
                        //                 Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.SoLuong], 0);

                        //newDr["TT"] = (Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.DonGia], 0) *
                        //               (Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.TyleTt], 0) / 100) +
                        //               Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.PhuThu], 0)) *
                        //              Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.SoLuong], 0);

                        //newDr["TT_PHUTHU"] = (Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.PhuThu], 0)) *
                        //                     Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.SoLuong], 0);

                        //newDr["TT_KHONG_PHUTHU"] = Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.DonGia], 0) *
                        //                           (Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.TyleTt], 0) / 100) *
                        //                           Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.SoLuong], 0);

                        //newDr["TT_BN_KHONG_PHUTHU"] =
                        //    Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.BnhanChitra], 0) *
                        //    (Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.TyleTt], 0) / 100) *
                        //    Utility.DecimaltoDbnull(newDr[KcbChidinhclsChitiet.Columns.SoLuong], 0);
        ...
        private void grdAssignDetail_UpdatingCell(object sender, UpdatingCellEventArgs e)
        {
            try
            {
                ...
                if (e.Column.Key == KcbChidinhclsChitiet.Columns.SoLuong)
                {
                    ...
                    int id_dangky = Utility.Int32Dbnull(grdAssignDetail.GetValue("id_dangky"), -1);
                    if (id_dangky > 0)
                    {
                        //GoiTinhtrangsudung goittsd = GoiTinhtrangsudung.FetchByID(id_dangky);
                        ...
                        //}
                        e.Value = e.InitialValue;
                        string msg = string.Format("Dịch vụ trong gói : {0}  không được phép chỉnh số lượng trên lưới. Muốn kê thêm thì lập phiếu chỉ định mới", grdAssignDetail.GetValue("ten_chitietdichvu").ToString());
                        Utility.ShowMsg(msg);
                        return;
                    }
                ...
                if (query.Count() <= 0)
                {
                   ...
                   Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.TyleTt].Value, 0);
                    ...
                    if (Utility.Int32Dbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.TuTuc].Value, 0) == 0 && chkTutuc.Checked == false)
                    {
                        ...
                        if (objLuotkham.DungTuyen == 1)
                        {
                            BHCT = Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.DonGia].Value, 0)
                                * (Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.TyleTt].Value, 0)/100) * (Utility.DecimaltoDbnull(objLuotkham.PtramBhyt, 0) / 100);
                        }
                        else
                        {
                            if (objLuotkham.TrangthaiNoitru <= 0)
                                BHCT = Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.DonGia].Value, 0) 
                                    *(Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.TyleTt].Value, 0) / 100) 
                                    * (Utility.DecimaltoDbnull(objLuotkham.PtramBhyt, 0) / 100);
                            else//Nội trú cần tính=đơn giá * % đầu thẻ * % tuyến
                                BHCT = Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.DonGia].Value, 0)
                                    * (Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.TyleTt].Value, 0) / 100) 
                                    * (Utility.DecimaltoDbnull(objLuotkham.PtramBhytGoc, 0) / 100) * (BHYT_PTRAM_TRAITUYENNOITRU / 100);
                        }
                        decimal BNCT = Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.DonGia].Value, 0)
                            *(Utility.DecimaltoDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.TyleTt].Value, 0) / 100) - BHCT;
                        newDr[KcbChidinhclsChitiet.Columns.BhytChitra] = BHCT;
                        newDr[KcbChidinhclsChitiet.Columns.BnhanChitra] = BNCT;
                        newDr[KcbChidinhclsChitiet.Columns.TuTuc] = false;
                    }
                    else//Tự túc
                    {

                        newDr[KcbChidinhclsChitiet.Columns.TyleTt] = 100;
                            newDr[KcbChidinhclsChitiet.Columns.BhytChitra] = 0;
                            QheDoituongDichvucl objQheDtuongGia = TinhCLS.LayGiaTheoDoiTuong(Utility.sDbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.MadoituongGia].Value, "DV"),
                                                                                             Utility.Int32Dbnull(
                                                                                                 gridExRow.Cells[
                                                                                                     DmucDichvuclsChitiet
                                                                                                         .Columns.
        ...
        private void mnuTuTuc_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                byte tutuc = (byte)grdAssignDetail.CurrentRow.Cells["tu_tuc"].Value;
                //if (idChidinhChitiet > 0)
                //{
        ...
        private void cmdluuchandoan_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                ActionResult act = _KCB_THAMKHAM.LuuHoibenhvaChandoan(TaoDulieuChandoanKetluan(),null, null, false);
                ...
                if (cboGoi.SelectedValue.ToString() == "-1")
                {
                    ...
                    else
                    {
                        m_dtDanhsachDichvuCLS = CHIDINH_CANLAMSANG.LaydanhsachCLS_chidinh(objLuotkham.MaDoituongKcb,
                                                                                    ...
                                                                                    Utility.Int32Dbnull(
                                                                                        objLuotkham.DungTuyen.Value, 0),
                                                                                    globalVariables.MA_KHOA_THIEN, nhomchidinh, tnv_chidinh);
                        if (!m_dtDanhsachDichvuCLS.Columns.Contains(KcbChidinhclsChitiet.Columns.SoLuong))
                            m_dtDanhsachDichvuCLS.Columns.Add(KcbChidinhclsChitiet.Columns.SoLuong, typeof(int));
                        if (!m_dtDanhsachDichvuCLS.Columns.Contains("ten_donvitinh"))
                            m_dtDanhsachDichvuCLS.Columns.Add("ten_donvitinh", typeof(string));
