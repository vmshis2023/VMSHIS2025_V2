...
namespace VNS.HIS.UI.NGOAITRU
{
    public partial class frm_KCB_KE_DONTHUOC_TAIQUAY : Form
    {
        ...
        Decimal LaygiaQhe(int id_thuoc)
        {
            try
            {
                ...
                if (arrDr.Length > 0)
                {
                    ...
                    //Lấy giá thuốc theo đối tượng KCB
                    ...
                    if (_data.Any())
                    {
                        phuthu_dungtuyen = Utility.DecimaltoDbnull(_data.FirstOrDefault()[QheDoituongThuoc.Columns.PhuthuDungtuyen]);
                        phuthu_traituyen = Utility.DecimaltoDbnull(_data.FirstOrDefault()[QheDoituongThuoc.Columns.PhuthuTraituyen]);
                        return Utility.DecimaltoDbnull(_data.FirstOrDefault()[QheDoituongThuoc.Columns.DonGia]);
                    }
                    else//Lấy giá thuốc dịch vụ
                    {
                        ...
                        {
                            Utility.ShowMsg(string.Format("Thuốc {0} chưa được thiết lập giá trong bảng Quan hệ giá Thuốc- Đối tượng. Đề nghị liên hệ bộ phận tạo giá trước khi thực hiện kê đơn đối với thuốc này", txtdrug.Text));
                            return 0;
                        }
                    }
                }
        ...
        private void AddPreDetail()
        {
            try
            {
                bool tachthuoc_theocachdung = THU_VIEN_CHUNG.Laygiatrithamsohethong("THUOC_TACHTHUOC_THEOCACHDUNG", "0", false) == "1";
                ...
                if (objDKho == null && !chk_ketay.Checked)
                {
                ...
                //else if (THU_VIEN_CHUNG.IsBaoHiem(objLuotkham.IdLoaidoituongKcb) && donthuoctaiquay == 0)//REM lại luôn bắt kê
                //{
                if (KIEU_THUOC_VT == "THUOC")
                {
                    if (string.IsNullOrEmpty(txtChiDanDungThuoc.Text.Trim().Replace(Environment.NewLine, "")))
                    {
                        Utility.ShowMsg("Chỉ dẫn dùng thuốc(Cách dùng) không được để trống", "Thông báo");
               ...
               // }
                int id_kho = -1;
                if (chk_ketay.Checked)
                    id_kho = -1;
                else
                    id_kho = objDKho.IdKho;
                if (KIEU_THUOC_VT == "THUOC")
                {
                    if (STCT)
                    {
                ...
                if (noitru == 0 || objPhieudieutriNoitru == null)//Hiện chỉ cảnh báo cho kê đơn ngoại trú
                {
                    ...
                    DataRow[] dtRows = m_dtDonthuocChitiet.Select("id_thuoc = " + objThuoc.IdThuoc.ToString()+ " and id_kho="+ id_kho);
                    if (dtRows.Any())
                    {
                ...
                decimal num = 10000000;
                if(!chk_ketay.Checked) num = CommonLoadDuoc.SoLuongTonTrongKho(-1L, Utility.Int32Dbnull(objDKho.IdKho), Utility.Int32Dbnull(txtDrugID.Text, -1), -1,
                ...
                if (!chk_ketay.Checked &&  THU_VIEN_CHUNG.Laygiatrithamsohethong("THUOC_CANHBAOKHI_SLKE_LONHON_SLTON", "0", false) == "1")//Không kê tay mới cảnh báo
                {
                ...
                if (!chk_ketay.Checked)
                {
                    foreach (DataRow thuockho in listdata.Rows)
                    {
                        ...
                        if (soluong > 0)
                        {
                            if (Utility.ByteDbnull(objDoituongKCB.GiathuocQuanhe) == 0)//Giá thuốc kho
                            {
                                phuthu_dungtuyen = Utility.DecimaltoDbnull(thuockho[TThuockho.Columns.PhuthuDungtuyen], 0);
                                phuthu_traituyen = Utility.DecimaltoDbnull(thuockho[TThuockho.Columns.PhuthuTraituyen], 0);
                            }
                            string filter_cachdung = "id_kho=" + id_kho + " AND " + TThuockho.Columns.IdThuockho + "=" + Utility.sDbnull(thuockho[TThuockho.Columns.IdThuockho]) + " AND tu_tuc=" + (chkTutuc.Checked ? 1 : tu_tuc);
                            if (tachthuoc_theocachdung)
                                filter_cachdung += " AND CACH_DUNG='" + txtCachDung.Text + "'";
                            if (chk_tachthuoc.Checked)
                                filter_cachdung += " AND 1=2";//Ép để thêm dòng mới
                            DataRow[] rowArray = m_dtDonthuocChitiet.Select(filter_cachdung);
                            if (rowArray.Length > 0)//Dòng thuốc kho đã tồn tại và giờ kê thêm số lượng
                            {
                                ...
                                rowArray[0]["TT"] = tongsoluong * (Utility.DecimaltoDbnull(rowArray[0]["don_gia"], 0) + Utility.DecimaltoDbnull(rowArray[0]["phu_thu"], 0));
                                //rowArray[0]["tt_bhyt"] = tongsoluong * Utility.DecimaltoDbnull(rowArray[0]["don_gia"], 0);
                            ...
                            else//Kê 1 dòng thuốc kho mới
                            {
                                ...
                                row[KcbDonthuocChitiet.Columns.PhuthuDungtuyen] = phuthu_dungtuyen;

                                row[KcbDonthuocChitiet.Columns.PhuthuTraituyen] = phuthu_traituyen;
                                ...
                                row[TDmucKho.Columns.TenKho] = Utility.sDbnull(cbo_khokedon.Text, -1);
                                ...
                                row[KcbDonthuocChitiet.Columns.TyleTt] = Utility.DecimaltoDbnull(objThuoc.TyleTt, 100);
                                row[KcbDonthuocChitiet.Columns.TrangthaiThanhtoan] = 0;
                                row[KcbDonthuocChitiet.Columns.SttIn] = GetMaxSTT(m_dtDonthuocChitiet, 1);
                                row[KcbDonthuocChitiet.Columns.TuTuc] = !THU_VIEN_CHUNG.IsBaoHiem(objLuotkham.IdLoaidoituongKcb) ? 0 : (chkTutuc.Checked || Utility.Byte2Bool(objThuoc.TuTuc) ? 1 : tu_tuc);
                                row[KcbDonthuocChitiet.Columns.DonGia] = Utility.ByteDbnull(objDoituongKCB.GiathuocQuanhe) == 0 ? Utility.DecimaltoDbnull(thuockho[TThuockho.Columns.GiaBan], 0) : Utility.DecimaltoDbnull(this.txtPrice.Text, 0);
                                //REM 240822 để áp theo kiểu áp giá trong danh mục đối tượng KCB
                                ...
                                //row[KcbDonthuocChitiet.Columns.DonGia] = Utility.DecimaltoDbnull(row[KcbDonthuocChitiet.Columns.DonGia]) * (1 + Utility.DecimaltoDbnull(objDoituongKCB.MotaThem, 0) / 100);
                                ...
                                //Code more here
                                THU_VIEN_CHUNG.Bhyt_PhantichGia(objLuotkham, row);
                                m_dtDonthuocChitiet.Rows.Add(row);
                                AddtoView(row, soluong);
                                lstChitietdonthuoc.Add(getNewItem(row));
                                if (Utility.DecimaltoDbnull(row[KcbDonthuocChitiet.Columns.DonGia], 0) <= 0)
                                {
                                    Utility.ShowMsg(string.Format("Cảnh báo thuốc {0} có đơn giá <=0.\nVui lòng liên hệ các bộ phận quản lý Thuốc để xác nhận(nếu có nghi ngờ)", Utility.sDbnull(txtDrug_Name.Text, "")));
                                    Utility.Log(this.Name, globalVariables.UserName, string.Format("Cảnh báo thuốc {0} có đơn giá <=0 với thông tin giá quan hệ ={1}, giá thuốc kho ={2}, loại áp giá đối tượng ={3}. Vui lòng liên hệ các bộ phận quản lý Thuốc để xác nhận(nếu có nghi ngờ)", Utility.sDbnull(txtDrug_Name.Text, ""), Utility.DecimaltoDbnull(thuockho[TThuockho.Columns.GiaBan], 0), Utility.DecimaltoDbnull(this.txtPrice.Text, 0), Utility.ByteDbnull(objDoituongKCB.GiathuocQuanhe)), newaction.Check, this.GetType().Assembly.ManifestModule.Name);
                                }
                            ...
                            if (IdThuockho > 0)//100%
                            {
                                ...
                                THU_VIEN_CHUNG.UpdateKeTam(id_donthuocchitiet, id_donthuoc, GUID, privateguid, IdThuockho, objThuoc.IdThuoc, Utility.Int16Dbnull(objDKho.IdKho), tongsoluong, (byte)LoaiTamKe.KEDONTHUOC,
                                    txt_maluotkham.Text, Utility.Int32Dbnull(txt_idbenhnhan.Text), noitru, ngay_kedon, ngay_nhaton, "Thêm mới thuốc");
                            }
                ...
                else//Kê tay không cần quan tâm đến kho, id thuốc kho
                {
                    decimal soluong = Utility.DecimaltoDbnull(txtSoluong.Text);
                    int IdThuockho = -1;
                    long id_donthuoc = -1;
                    long id_donthuocchitiet = -1;
                    decimal sl_cu = 0;
                    ...
                    if (soluong > 0)
                    {
                        ...
                        phuthu_dungtuyen = 0;
                        phuthu_traituyen = 0;
                        string filter_cachdung = "id_kho=-1 AND id_thuockho=-1 AND id_thuoc=" + objThuoc.IdThuoc;
                        if (tachthuoc_theocachdung)
                            filter_cachdung += " AND CACH_DUNG='" + txtCachDung.Text + "'";
                        if (chk_tachthuoc.Checked)
                            filter_cachdung += " AND 1=2";//Ép để thêm dòng mới
                        ...
                        else//Kê 1 dòng thuốc kho mới
                        {
                            ...
                            row[KcbDonthuocChitiet.Columns.IdThuockho] = -1;
                            row[KcbDonthuocChitiet.Columns.GiaNhap] = 0;
                            row[KcbDonthuocChitiet.Columns.GiaBan] = 0;
                            row[KcbDonthuocChitiet.Columns.GiaBhyt] = 0;
                            row[KcbDonthuocChitiet.Columns.Vat] = 0;
                            row[KcbDonthuocChitiet.Columns.SoLo] = "";
                            row[KcbDonthuocChitiet.Columns.SoDky] = "";
                            row[KcbDonthuocChitiet.Columns.SoQdinhthau] = "";
                            row[KcbDonthuocChitiet.Columns.MaNhacungcap] = "";
                            row["ten_donvitinh"] = txtDonViTinh.Text;
                            row["sNgay_hethan"] = globalVariables.SysDate.AddDays(1000).ToString("dd/MM/yyyy");
                            row["sNgay_nhap"] = DateTime.Now.AddDays(1000).ToString("dd/MM/yyyy");
                            row[KcbDonthuocChitiet.Columns.NgayHethan] = globalVariables.SysDate.AddDays(1000);

                            row[KcbDonthuocChitiet.Columns.NgayNhap] = globalVariables.SysDate.AddDays(1000);
                            row[KcbDonthuocChitiet.Columns.IdKho] = -1;
                            row[TDmucKho.Columns.TenKho] = "Mua ngoài";
                            row[KcbDonthuocChitiet.Columns.DonviTinh] = txtDonViTinh.Text;
                            row[DmucThuoc.Columns.MaHoatchat] = txtBietduoc.Text;
                            row[KcbDonthuocChitiet.Columns.ChidanThem] = txtChiDanThem.Text;
                            row[KcbDonthuocChitiet.Columns.MotaThem] =
                            ...
                            row[KcbDonthuocChitiet.Columns.PtramBhytGoc] = objLuotkham.PtramBhytGoc;
                            row[KcbDonthuocChitiet.Columns.MaDoituongKcb] = MaDoiTuong;
                            row[KcbDonthuocChitiet.Columns.KieuBiendong] = "FIFO";
                            //Kiểm tra tương tác thuốc ở đây
                            //Code more here
                            THU_VIEN_CHUNG.Bhyt_PhantichGia(objLuotkham, row);
                            m_dtDonthuocChitiet.Rows.Add(row);
                        ...
                        Utility.GotoNewRowJanus(grdPresDetail, "guid", privateguid);
                    }
                }    

                ClearControl(new List<Control>() { txtKieuthuocVT, cbo_khokedon, cboKieuKedonthuocVT });
                this.txtdrug.Focus();
                ...
                foreach (DataRow thuockho in listdata.Rows)
                {
                    ...
                    if (_soluong > 0)
                    {
                        ...
                        if (rowArray.Length > 0)
                        {
                            ...
                            rowArray[0]["TT"] = SlMoi * (Utility.DecimaltoDbnull(rowArray[0]["don_gia"],0)+Utility.DecimaltoDbnull(rowArray[0]["phu_thu"],0));
                            //rowArray[0]["tt_bhyt"] = SlMoi * Utility.DecimaltoDbnull(rowArray[0]["don_gia"], 0);
                            ...
                            THU_VIEN_CHUNG.UpdateKeTam(Utility.Int32Dbnull(rowArray[0][KcbDonthuocChitiet.Columns.IdChitietdonthuoc]), Utility.Int32Dbnull(rowArray[0][KcbDonthuocChitiet.Columns.IdDonthuoc]), GUID, Utility.sDbnull(rowArray[0]["guid"]), IdThuockho, objThuoc.IdThuoc, Utility.Int16Dbnull(objDKho.IdKho), Utility.DecimaltoDbnull(moi), (byte)LoaiTamKe.KEDONTHUOC,
                                       txt_maluotkham.Text, Utility.Int32Dbnull(txt_idbenhnhan.Text), 0, ngay_kedon,ngay_nhaton, "Cập nhật lại số lượng cho chi tiết cũ(đã kê đơn)");
                        ...
                        else//Id thuốc kho mới
                        {
                            ...
                            if (IdThuockho > 0)//100%
                            {
                                ...
                                THU_VIEN_CHUNG.UpdateKeTam(-1, -1, GUID,privateguid, IdThuockho, objThuoc.IdThuoc, Utility.Int16Dbnull(objDKho.IdKho), Utility.DecimaltoDbnull(_soluong), (byte)LoaiTamKe.KEDONTHUOC,
                                       txt_maluotkham.Text, Utility.Int32Dbnull(txt_idbenhnhan.Text), 0, ngay_kedon,ngay_nhaton, "Cập nhật số lượng với id thuốc kho mới");
                            }
                        }
        ...
        private void AddtoView(DataRow newDr, decimal newQuantity)
        {
            ...
            else
            {
                ...
                rowArray[0]["TT"] = Utility.DecimaltoDbnull(rowArray[0][KcbDonthuocChitiet.Columns.SoLuong], 0) * (Utility.DecimaltoDbnull(rowArray[0]["don_gia"], 0) + Utility.DecimaltoDbnull(rowArray[0]["phu_thu"], 0));
               // rowArray[0]["tt_bhyt"] = Utility.DecimaltoDbnull(rowArray[0][KcbDonthuocChitiet.Columns.SoLuong], 0) * Utility.DecimaltoDbnull(rowArray[0]["don_gia"], 0);
        ...
        bool isAllowedChonKho = false;
        private void cboStock_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (!isAllowedChonKho) return;
                if (!chk_ketay.Checked && (blnHasLoaded && (cbo_khokedon.Items.Count > 0)) && ((cbo_khokedon.SelectedValue == null) || (cbo_khokedon.SelectedValue.ToString() != "-1")))
                {
                    globalVariables.KHOKEDON = Utility.Int32Dbnull(cbo_khokedon.SelectedValue, -1);
                    txtdrug.SetId(-1);
                    txtDrugID.Text = "-1";
                    txtdrug.RaiseEnterEvents();
                    int num = Utility.Int32Dbnull(cbo_khokedon.SelectedValue, -1);
                    if ((num > 0) && ((cbo_khokedon.Items.Count > 0)))
                    {
                        AutoSaveKhoID();
        ...
        private void cmdAddDetail_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Utility.Int32Dbnull(cbo_khokedon.SelectedValue, -1)<=0)
                {
        ...
        private void cmdDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Utility.AcceptQuestion("Bạn Có muốn xóa các " + THU_VIEN_CHUNG.laytenthuoc_vattu(KIEU_THUOC_VT) + " đang chọn hay không?", "thông báo xóa", true))
                {
                    ...
                    Utility.Log(this.Name, globalVariables.UserName, string.Format("Xóa thuốc khỏi đơn của bệnh nhân ID={0}, PID={1}, Tên={2}, DS thuốc bị xóa={3} thành công ", objLuotkham.IdBenhnhan.ToString(), objLuotkham.MaLuotkham, txt_tenbenhnhan.Text, _deleteitems), newaction.Delete, this.GetType().Assembly.ManifestModule.Name);
                    m_dtDonthuocChitiet.AcceptChanges();
                    m_blnCancel = false;
                    UpdateDataWhenChanged();
                }
        ...
        private void cmdSavePres_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (objCongkham != null )//|| (objCongkham == null && noitru == 0))
                {
                    ...
                    {
                        Utility.ShowMsg("Công khám bác sĩ đang kê đơn không tồn tại(Có thể bị xóa trong lúc bác sĩ đang kê và chưa kịp lưu).\nVui lòng liên hệ quầy tiếp đón và IT bệnh viện để xác minh");
                        return ;
                    }
                }
        ...
        private KcbDonthuocChitiet[] CreateArrayPresDetail()
        {
            ...
            try
            {
                foreach (DataRowView view in m_dtDonthuocChitiet.DefaultView)
                {
                    ...
                    chitietArray[index].BhytChitra = Utility.DecimaltoDbnull(view[KcbDonthuocChitiet.Columns.BhytChitra], 0);
                    chitietArray[index].BnhanChitra = Utility.DecimaltoDbnull(view[KcbDonthuocChitiet.Columns.BnhanChitra], 0);
                    chitietArray[index].BhytGiaTyle = Utility.DecimaltoDbnull(view[KcbDonthuocChitiet.Columns.BhytGiaTyle], 0);
                    chitietArray[index].BnCct = Utility.DecimaltoDbnull(view[KcbDonthuocChitiet.Columns.BnCct], 0);
                    chitietArray[index].BnTtt = Utility.DecimaltoDbnull(view[KcbDonthuocChitiet.Columns.BnTtt], 0);
                    #region REM
        ...
        private KcbDonthuoc TaoDonthuoc()
        {
            ...
            else
            {
                ...
                donthuoc.SongayNhaton = chk_ketay.Checked ? 100 : objDKho.SongayNhaton;
        ...
        void AutoSaveKhoID()
        {
            ...
            {
                globalVariables.KHOKEDON = Utility.Int32Dbnull(cbo_khokedon.SelectedValue, -1);
                string khokedon = Utility.sDbnull(cbo_khokedon.SelectedValue, "-1");
                if (noitru==1)
                {
                    if (KIEU_THUOC_VT == "THUOC")
                        Utility.SaveValue2File(string.Format(@"{0}\khothuoc_noitru.txt", AppPath), khokedon);
                    else
                        Utility.SaveValue2File(string.Format(@"{0}\khovt_noitru.txt", AppPath), khokedon);
                }
                else
                {
                    if (KIEU_THUOC_VT == "THUOC")
                        Utility.SaveValue2File(string.Format(@"{0}\khothuoc_ngoaitru.txt", AppPath), khokedon);
                    else
                        Utility.SaveValue2File(string.Format(@"{0}\khovt_ngoaitru.txt", AppPath), khokedon);
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (!isSaved)
                {
                    ...
                    else
                    {
                        ...
                        ActionResult actionResult = new KCB_DANGKY().PerformActionDeletePatientExam(null, objLuotkham.MaLuotkham,
                                                                                                      objLuotkham.IdBenhnhan, ref ErrMgs);
                    }    
                }
            }
            catch (Exception ex)
        ...
        private bool IsValidDeleteData()
        {
            ...
            string v_MaLuotkham = objLuotkham.MaLuotkham;

            long v_Patient_ID = objLuotkham.IdBenhnhan;
        ...
        private void frm_KCB_KE_DONTHUOC_TAIQUAY_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    cboStock_SelectedIndexChanged(cbo_khokedon, new EventArgs());
                }
        ...
        void TudongXacdinhkhokedon_vuasudung()
        {
            if (noitru == 1)
            {
                if (KIEU_THUOC_VT == "THUOC")
                    globalVariables.KHOKEDON = Utility.Int32Dbnull(Utility.GetFirstValueFromFile(string.Format(@"{0}\khothuoc_noitru.txt", AppPath)), -1);
                else
                    globalVariables.KHOKEDON = Utility.Int32Dbnull(Utility.GetFirstValueFromFile(string.Format(@"{0}\khovt_noitru.txt", AppPath)), -1);
            }
            else//Ngoại trú
            {
                if (KIEU_THUOC_VT == "THUOC")
                    globalVariables.KHOKEDON = Utility.Int32Dbnull(Utility.GetFirstValueFromFile(string.Format(@"{0}\khothuoc_ngoaitru.txt", AppPath)), -1);
                else
                    globalVariables.KHOKEDON = Utility.Int32Dbnull(Utility.GetFirstValueFromFile(string.Format(@"{0}\khovt_ngoaitru.txt", AppPath)), -1);
            }
            if (dtStockList.Select(TDmucKho.Columns.IdKho + "= " + globalVariables.KHOKEDON).Length > 0)
            {
                cbo_khokedon.SelectedIndex = Utility.GetSelectedIndex(cbo_khokedon, globalVariables.KHOKEDON.ToString());
                cboStock_SelectedIndexChanged(cbo_khokedon, new EventArgs());
            }
            else
            {
                cbo_khokedon.SelectedIndex = -1;
            }
            if (cbo_khokedon.SelectedIndex < 0 && dtStockList.Rows.Count == 1)
            {
                cbo_khokedon.SelectedIndex = 0;
                cboStock_SelectedIndexChanged(cbo_khokedon, new EventArgs());
            }
            if (cbo_khokedon.Items.Count == 0)
            {
        ...
        private void frm_KCB_KE_DONTHUOC_TAIQUAY_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                else if(objPhieudieutriNoitru!=null) txtBacsi.SetId(objPhieudieutriNoitru.IdBacsi);
                LoadBenh();
                ...
                AllowTextChanged = true;
                blnHasLoaded = true;
                TudongXacdinhkhokedon_vuasudung();
                //if (dtStockList.Select(TDmucKho.Columns.IdKho + "= " + globalVariables.KHOKEDON).Length > 0)
                //{
            ...
            {
                isAllowedChonKho = true;
                cboStock_SelectedIndexChanged(cbo_khokedon, e);
                ...
                if (Iddonthuocmau > 0)
                {
                    ...
                    if (objMau != null)
                    {
                        ...
                        if (dtChitietnhom.Rows.Count > 0)
                        {
                            ...
                            Utility.Log(this.Name, globalVariables.UserName, string.Format("Chọn kê đơn thuốc theo mẫu {0}-{1} cho người bệnh id={2}, mã lượt khám ={3}, id phiếu điều trị ={4}", Iddonthuocmau, objMau.TenDonthuoc,objLuotkham.IdBenhnhan,objLuotkham.MaLuotkham,objPhieudieutriNoitru!=null?objPhieudieutriNoitru.IdPhieudieutri:-1), newaction.Select, this.GetType().Assembly.ManifestModule.Name);
                        }
                    }
                }
                ClearControl(new List<Control>() { txtKieuthuocVT, cbo_khokedon });
                prgBar.SendToBack();
                prgBar.Visible = false;
            }
            catch (Exception ex)
        ...
        private void FillThongtinLuotkham()
        {
            if (objLuotkham != null)
            {
                ...
                txt_diachi.Text = Utility.sDbnull(objLuotkham.DiaChi);
               ...
               objDoituongKCB=DmucDoituongkcb.FetchByID(objLuotkham.IdDoituongKcb);
                if (objDoituongKCB != null)
                {
                    ...
                    chkTutuc.Visible = THU_VIEN_CHUNG.IsBaoHiem(objDoituongKCB.IdLoaidoituongKcb);
                    if (donthuoctaiquay == 1 && chkTutuc.Visible)
                    {
                        chkTutuc.Enabled = false;
                        chkTutuc.Checked = true;
                    }
                    else
        ...
        private KcbDonthuocChitiet getNewItem(DataRow drv)
        {
            return new KcbDonthuocChitiet
            {
                ...
                BhytGiaTyle = Utility.DecimaltoDbnull(drv[KcbDonthuocChitiet.Columns.BhytGiaTyle], 0),
                BnCct = Utility.DecimaltoDbnull(drv[KcbDonthuocChitiet.Columns.BnCct], 0),
                BnTtt = Utility.DecimaltoDbnull(drv[KcbDonthuocChitiet.Columns.BnTtt], 0),
        ...
        private void LaydanhsachKhotheoBs()
        {
            try
            {
                ...
                if (KieuDonthuoc == 3)//Đơn thuốc ra viện
                {
                    if (KIEU_THUOC_VT == "THUOC")
                    {
                        dtStockList = CommonLoadDuoc.LAYDANHMUCKHO_THEOKHOA(id_khoa>0?id_khoa: Utility.Int16Dbnull(globalVariables.idKhoatheoMay, -1), -1, "THUOC,THUOCVT", 0, 0,objLuotkham.MaDoituongKcb);// CommonLoadDuoc.LAYTHONGTIN_KHOTHUOC_LE_NGOAI_TRU_KEDON(objLuotkham.MaDoituongKcb);
                    }
                    else
                    {
                        dtStockList = CommonLoadDuoc.LAYDANHMUCKHO_THEOKHOA(id_khoa > 0 ? id_khoa : Utility.Int16Dbnull(globalVariables.idKhoatheoMay, -1), -1, "VT,THUOCVT", 0, 0, objLuotkham.MaDoituongKcb);// CommonLoadDuoc.LAYTHONGTIN_KHOTHUOC_LE_NGOAI_TRU_KEDON(objLuotkham.MaDoituongKcb);
                    }
                ...
                else if (KieuDonthuoc == 4)//Đơn vật tư tiêu hao phòng chức năng
                {
                    if (KIEU_THUOC_VT == "THUOC")
                    {
                        dtStockList = CommonLoadDuoc.LAYDANHMUCKHO_THEOKHOA(Utility.Int16Dbnull(globalVariables.idKhoatheoMay, -1), -1, "THUOC,THUOCVT", 100, 0, objLuotkham.MaDoituongKcb);// CommonLoadDuoc.LAYTHONGTIN_KHOTHUOC_LE_NGOAI_TRU_KEDON(objLuotkham.MaDoituongKcb);
                    }
                    else
                    {
                        dtStockList = CommonLoadDuoc.LAYDANHMUCKHO_THEOKHOA(Utility.Int16Dbnull(globalVariables.idKhoatheoMay, -1), -1, "VT,THUOCVT", 100, 0, objLuotkham.MaDoituongKcb);// CommonLoadDuoc.LAYTHONGTIN_KHOTHUOC_LE_NGOAI_TRU_KEDON(objLuotkham.MaDoituongKcb);
                    }
                ...
                else//Đơn thường
                {
                    if (noitru == 0)// Kê đơn ngoại trú tại phòng khám
                    {
                        ...
                        if (KIEU_THUOC_VT == "THUOC")
                        {
                            dtStockList = CommonLoadDuoc.LAYDANHMUCKHO_THEOKHOA(Utility.Int16Dbnull(globalVariables.idKhoatheoMay, -1), -1, "THUOC,THUOCVT", 100,donthuoctaiquay, capcuu, objLuotkham.MaDoituongKcb);// CommonLoadDuoc.LAYTHONGTIN_KHOTHUOC_LE_NGOAI_TRU_KEDON(objLuotkham.MaDoituongKcb);
                        }
                        else
                        {
                            dtStockList = CommonLoadDuoc.LAYDANHMUCKHO_THEOKHOA(Utility.Int16Dbnull(globalVariables.idKhoatheoMay, -1), -1, "VT,THUOCVT", 100,donthuoctaiquay, capcuu, objLuotkham.MaDoituongKcb);// CommonLoadDuoc.LAYTHONGTIN_KHOTHUOC_LE_NGOAI_TRU_KEDON(objLuotkham.MaDoituongKcb);
                        }
                        //}
                    }
                    else //Nội trú
                    {
                        ...
                        {
                            if (laytatca)
                                dtStockList = CommonLoadDuoc.LAYDANHMUCKHO_THEOKHOA(-1, Utility.Int16Dbnull(cboTang.SelectedValue), "NOITRU", "THUOC,THUOCVT", loaikho, 100, objLuotkham.MaDoituongKcb);// CommonLoadDuoc.LAYTHONGTIN_KHOTHUOC_NOITRU();
                            else
                                dtStockList = CommonLoadDuoc.LAYDANHMUCKHO_THEOKHOA(Utility.Int16Dbnull(objPhieudieutriNoitru.IdKhoanoitru, -1), Utility.Int16Dbnull(cboTang.SelectedValue), "NOITRU", "THUOC,THUOCVT", loaikho, 100, 100, objLuotkham.MaDoituongKcb);
                        }
                        else
                        {
                            if (laytatca)
                                dtStockList = CommonLoadDuoc.LAYDANHMUCKHO_THEOKHOA(-1, Utility.Int16Dbnull(cboTang.SelectedValue), "NOITRU", "THUOCVT,VT", loaikho, 100, objLuotkham.MaDoituongKcb);// CommonLoadDuoc.LAYTHONGTIN_KHOVATTU_NOITRU();
                            else
                                dtStockList = CommonLoadDuoc.LAYDANHMUCKHO_THEOKHOA(Utility.Int16Dbnull(objPhieudieutriNoitru.IdKhoanoitru, -1), Utility.Int16Dbnull(cboTang.SelectedValue), "NOITRU", "THUOCVT,VT", loaikho, 100, objLuotkham.MaDoituongKcb);// CommonLoadDuoc.LAYTHONGTIN_VATTU_KHOA((int)objLuotkham.IdKhoanoitru);
                        }
                ...
                DataBinding.BindDataCombobox(cbo_khokedon, dtStockList, TDmucKho.Columns.IdKho, TDmucKho.Columns.TenKho);
        ...
        private void LaydanhsachKhotheoBs_bak()
        {
            try
            {
                ...
                DataBinding.BindDataCombobox(cbo_khokedon, dtStockList, TDmucKho.Columns.IdKho, TDmucKho.Columns.TenKho);
                cbo_khokedon.SelectedIndex = Utility.GetSelectedIndex(cbo_khokedon,
                    ...
                    if (sluong_moi != sluong_cu)
                    {
                        ...
                        else//Giảm số lượng kê-->Không cần kiểm tra tồn--Cập nhật lại trong bảng tạm kê
                        {
                            ...
                            THU_VIEN_CHUNG.UpdateKeTam(_IdChitietdonthuoc, _IdDonthuoc, GUID, privateguid, _IdThuockho, idThuoc, Utility.Int16Dbnull(objDKho.IdKho), Utility.DecimaltoDbnull(sluong_moi), (byte)LoaiTamKe.KEDONTHUOC,
                               txt_maluotkham.Text, Utility.Int32Dbnull(txt_idbenhnhan.Text), 0, ngay_kedon,ngay_nhaton, string.Format("Hiệu chỉnh đơn thuốc từ số lượng {0} thành số lượng {1}", sluong_cu.ToString(), sluong_moi.ToString()));
                            if (objChitiet != null)
                            {
                                int rcount = new Update(KcbDonthuocChitiet.Schema).Set(KcbDonthuocChitiet.Columns.SoLuong).EqualTo(sluong_moi).Where(KcbDonthuocChitiet.Columns.IdChitietdonthuoc).IsEqualTo(objChitiet.IdChitietdonthuoc).Execute();
                                if (rcount > 0) Utility.Log(this.Name, globalVariables.UserName, string.Format("Cập nhật số lượng IdChitietdonthuoc={0}, số lượng cũ={1}, số lượng mới={2} thành công", objChitiet.IdChitietdonthuoc, sluong_cu, sluong_moi), newaction.Update, this.GetType().Assembly.ManifestModule.Name);
                        ...
                        foreach (DataRow row2 in rowArray2)
                        {
                            ...
                            if (num10 > 0)
                            {
                                row2["TT"] = num10 * (Utility.DecimaltoDbnull(row2["don_gia"], 0) + Utility.DecimaltoDbnull(row2["phu_thu"], 0));
                                //row2["tt_bhyt"] = num10 * Utility.DecimaltoDbnull(row2["don_gia"], 0);
        ...
        private void InitEvents()
        {
            ...
            cbo_khokedon.SelectedIndexChanged += cboStock_SelectedIndexChanged;
                ...
                if (arrPresDetail != null)
                {
                    ...
                    switch (_actionResult)
                    {
                        ...
                        case ActionResult.Success:
                            ...
                            if (!chk_ketay.Checked ) THU_VIEN_CHUNG.XoaKeTam(GUID);
                            UpdateChiDanThem();
        ...
        void Capnhatthongtinkhachhang()
        {
            if (objLuotkham != null && objBenhnhan != null)
            {
                objBenhnhan.TenBenhnhan = Utility.sDbnull(txt_tenbenhnhan.Text);
                objBenhnhan.DiaChi = Utility.sDbnull(txt_diachi.Text);
                objBenhnhan.IdGioitinh = Utility.ByteDbnull(cbo_gioitinh.SelectedValue);
...
//        }
        ...
        private void txtDrugID_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (rowArray.Length > 0)
                {
                    ...
                    txtTonKho.Text =chk_ketay.Checked?"1000":
                    ...
                    decimal giathuoc = 0;
                    if (Utility.ByteDbnull(objDoituongKCB.GiathuocQuanhe) == 1) giathuoc = LaygiaQhe(objThuoc.IdThuoc); // Utility.DecimaltoDbnull(rowArray[0]["gia_qhe"], 0);
                    else if (Utility.ByteDbnull(objDoituongKCB.GiathuocQuanhe) == 0)//Giá thuốc kho
                        giathuoc = 0;//Sẽ lấy trong thuốc kho khi bấm lưu. Vì lúc đó mới xác định chính xác id thuốc kho nào được kê
                    else //Giá danh mục
                    {
                        phuthu_dungtuyen =Utility.DecimaltoDbnull( objThuoc.PhuthuDungtuyen);
                        phuthu_traituyen = Utility.DecimaltoDbnull(objThuoc.PhuthuTraituyen);
                        giathuoc = Utility.DecimaltoDbnull(THU_VIEN_CHUNG.IsBaoHiem(objLuotkham.IdLoaidoituongKcb) ? objThuoc.GiaBhyt : objThuoc.GiaDv);
                    }
        ...
        private void txtSoluong_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Utility.DoTrim(txtTonKho.Text) != "" && !chk_ketay.Checked)
                {
                    if (Utility.Int32Dbnull(objDKho.KtraTon) == 1 &&
            ...
            if (arrPresDetail != null)
            {
                ...
                switch (_actionResult)
                {
                    ...
                    case ActionResult.Success:
                        ...
                        THU_VIEN_CHUNG.XoaKeTam(GUID);
                        UpdateChiDanThem();
                        Utility.Log(this.Name, globalVariables.UserName, string.Format("Sửa đơn thuốc cho người bệnh id={0}, mã lượt khám ={1}, id phiếu điều trị ={2}, id đơn thuốc={3} thành công", objLuotkham.IdBenhnhan, objLuotkham.MaLuotkham, objPhieudieutriNoitru != null ? objPhieudieutriNoitru.IdPhieudieutri : -1, IdDonthuoc), newaction.Insert, this.GetType().Assembly.ManifestModule.Name);
                        setMsg(lblMsg,
                            "Bạn thực hiện lưu đơn " + THU_VIEN_CHUNG.laytenthuoc_vattu(KIEU_THUOC_VT) + " thành công",
                            false);
                        UpdateDetailId(lstChitietDonthuoc);
        ...
        {
            if (Utility.Int64Dbnull(txt_idbenhnhan.Text, -1) > 0)
            {
                DataTable dtListDonThuoc =
                    SPs.KcbLaydanhsachDonthuocOld(Utility.Int64Dbnull(txt_idbenhnhan.Text, -1),KIEU_THUOC_VT).GetDataSet().Tables[0];
        ...
        private void grdListDonThuocCu_ColumnButtonClick(object sender, ColumnActionEventArgs e)
        {
            if (e.Column.Key == "Chon")
            {
                ...
                frm.txtPatientCode.Text = Utility.sDbnull(txt_maluotkham.Text);
                frm.txtPatientID.Text = Utility.sDbnull(txt_idbenhnhan.Text);
                frm.txtPatientName.Text = Utility.sDbnull(txt_tenbenhnhan.Text);
                frm.txtSex.Text = Utility.sDbnull(cbo_gioitinh.Text);
                frm.txtPres_ID.Text = Utility.sDbnull(grdListDonThuocCu.GetValue(KcbDonthuoc.Columns.IdDonthuoc));
                frm.txtYearBirth.Text = Utility.sDbnull(dtpBOD.Text);
        ...
        private void chk_ketay_CheckedChanged(object sender, EventArgs e)
        {
            if (chk_ketay.Checked)
            {
                LoadThuocKeTay();
        ...
        void LoadThuocKeTay()
        {
            try
            {
                ...
                //if (objLuotkham.DungTuyen != null)
                m_dtDanhmucthuoc = _kedonthuoc.LayThuoctrongkhokedonTay(-1,-1, KIEU_THUOC_VT,
                    Utility.sDbnull(objLuotkham.MaDoituongKcb, "DV"),
                    Utility.Int32Dbnull(objLuotkham.DungTuyen.Value, 0), noitru, globalVariables.MA_KHOA_THIEN);

                rowFilter = "1=1";
                txtdrug.AllowedSelectPrice = false;
                txtdrug.dtData = m_dtDanhmucthuoc;
                txtdrug.ChangeDataSource();
                txtdrug.Focus();
                txtdrug.SelectAll();
        ...
        private void cmd_LuuBenhnhan_Click(object sender, EventArgs e)
        {
            Capnhatthongtinkhachhang();
            objBenhnhan.IsNew = false;
            objBenhnhan.MarkOld();
