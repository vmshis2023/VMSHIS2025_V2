...
namespace VNS.HIS.UCs
{
    ...
    public class EasyCompletionComboBox : ComboBox
    {
        ...
        public Control Next_Control { get; set; }
        public bool RaiseEnterEventWhenInvisible { get; set; }
        /// <summary>How do we match user input to strings?</summary>
        private StringMatchingMethod m_matchingMethod;
        #endregion
        public delegate void OnEnterMe(Control Next_Control);
        ...
        public EasyCompletionComboBox()
        {
            ...
            m_suggestionList.SelectedIndexChanged += m_suggestionList_SelectedIndexChanged;
            m_suggestionList.MouseClick += m_suggestionList_MouseClick;
            m_dropDown.MouseClick += m_dropDown_MouseClick;
            onFontChanged(null, null);
        }

        void m_dropDown_MouseClick(object sender, MouseEventArgs e)
        {
            RaiseEnterEvents(); 
        }

        void m_suggestionList_MouseClick(object sender, MouseEventArgs e)
        {
            RaiseEnterEvents();
        }

        void m_suggestionList_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (_OnSelectionChanged != null) _OnSelectionChanged();
        }
        public void RaiseEnterEvents()
        {
            if (_OnEnterMe != null) _OnEnterMe(Next_Control);
        }
        public void SetId(object _value)
        {
            this.SelectedIndex = VNS.Libs.Utility.GetSelectedIndex(this,VNS.Libs.Utility.sDbnull( _value,"-1"));
        }
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            if (!m_dropDown.Visible)
            {
                if (RaiseEnterEventWhenInvisible && e.KeyCode == Keys.Enter)
                {
                    SendKeys.Send("{TAB}");
                }
                base.OnKeyDown(e);
            ...
            switch (e.KeyCode)
            {
                ...
                case Keys.Up:
                    ...
                    //if (m_suggestionList.SelectedIndex ==0)
                    //{
                    //    m_suggestionList.SelectedIndex= m_suggestionList.Items.Count;
                    //}
                    break;
                case Keys.Enter:
                    if (m_suggestionList.Text.Length != 0)
