    ...
    public partial class uc_donkinh : UserControl
    {
        ...
        void InDonKinh()
        {
            try
            {
                ...
                if (objDonkinh == null || objDonkinh.IdDonkinh <= 0)
                {
                    Utility.ShowMsg("Bạn cần lưu thông tin Đo thị lực trước khi thực hiện in");
                    return;
                }

                DataTable dtData = SPs.KcbDonkinhIn(objDonkinh.IdDonkinh, objDonkinh.IdBenhnhan, objDonkinh.MaLuotkham).GetDataSet().Tables[0];
                ...
                dicMF.Add("chkkinh2trong",Utility.Bool2byte( objDonkinh.Kinh2trong).ToString() == "1" ? "1" : "0");
                dicMF.Add("chkKinhdatrong", Utility.Bool2byte(objDonkinh.Kinhdatrong).ToString() == "1" ? "1" : "0");
                dicMF.Add("chkKinhnhingan", Utility.Bool2byte(objDonkinh.Kinhnhingan).ToString() == "1" ? "1" : "0");
                dicMF.Add("chkKinhdoimau", Utility.Bool2byte(objDonkinh.Kinhdoimau).ToString() == "1" ? "1" : "0");
                dicMF.Add("chkKinhpoly", Utility.Bool2byte(objDonkinh.Kinhpoly).ToString() == "1" ? "1" : "0");
                dicMF.Add("chkKinhaptrong", Utility.Bool2byte(objDonkinh.Kinhaptrong).ToString() == "1" ? "1" : "0");
                ...
                string fileKetqua = string.Format("{0}{1}{2}{3}{4}_{5}_{6}_{7}",
                               ...
                               Path.GetFileNameWithoutExtension(PathDoc), "DonKinh", objDonkinh.MaLuotkham, Utility.sDbnull(objDonkinh.IdDonkinh), Guid.NewGuid().ToString(), Path.GetExtension(PathDoc));
                ...
                if ((drData != null) && File.Exists(PathDoc))
                {
                    ...
                    if (builder.MoveToMergeField("logo") && globalVariables.SysLogo != null)
                        if (sysLogosize != null)
                        {
                            ...
                            builder.InsertImage(globalVariables.SysLogo);
                    byte[] QRCode = Utility.GetQRCode(objDonkinh.MaLuotkham);
