...
{
    public partial class frm_phatthuoc_tutruc : Form
    {
        ...
        public frm_phatthuoc_tutruc(string args)
        {
            InitializeComponent();
            try
            {
        ...
        void InitEvents()
        {
            ...
            this.Load += new System.EventHandler(this.frm_phatthuoc_tutruc_Load);
            Shown += frm_phatthuoc_tutruc_Shown;
            FormClosing += frm_phatthuoc_tutruc_FormClosing;
            this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.frm_phatthuoc_tutruc_KeyDown);
        ...
        void cboKhoanoitru_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //    string IDKhoa = cboKhoanoitru.SelectedValue.ToString();
            //    DataRow[] arrdr = m_dtKhothuoc.Select(string.Format(" ID_KHOAPHONG={0} or id_khoaphong_captren={0}" , IDKhoa ));
            //    DataTable _newTable = m_dtKhothuoc.Clone();
            ...
            try
            {
                ...
                //if (arrdr.Length > 0) _newTable = arrdr.CopyToDataTable();
                DataTable _newTable = CommonLoadDuoc.LAYDANHMUCKHO_THEOKHOA(IDKhoa, -1, "TATCA,NOITRU", kieuthuoc_vt, 1, 100);
                DataBinding.BindDataCombobox(cboKho, _newTable,
        ...
        void frm_phatthuoc_tutruc_FormClosing(object sender, FormClosingEventArgs e)
        {
            SaveUserConfigs();
        ...
        void frm_phatthuoc_tutruc_Shown(object sender, EventArgs e)
        {
            Try2Splitter();
        }
        ...
        private void HuyPhieuchi()
        {
            ...
            if (grdPhieuChi.CurrentRow != null)
            {
                ...
                long v_Payment_ID = Utility.Int32Dbnull(grdPhieuChi.CurrentRow.Cells[KcbThanhtoan.Columns.IdThanhtoan].Value, -1);
                KcbThanhtoan objPayment = KcbThanhtoan.FetchByID(v_Payment_ID);
        ...
        void grdPresDetail_UpdatingCell(object sender, UpdatingCellEventArgs e)
        {
            try
            {
                ...
                {
                    if (Utility.isValidGrid(grdPresDetail) && Utility.Int64Dbnull(grdPresDetail.CurrentRow.Cells["trangthai_thanhtoan"].Value, 1) == 1)
                    {
                    ...
                    else
                    {
                        if (e.Column.Key == "tile_chietkhau")
                        {
                            ...
                            grdPresDetail.CurrentRow.Cells["tien_chietkhau"].Value = Utility.DecimaltoDbnull(grdPresDetail.CurrentRow.Cells["TT_BN"].Value, 0) * Utility.DecimaltoDbnull(e.Value, 0) / 100;

                        }
                        else
                        {

                            if (Utility.DecimaltoDbnull(e.Value, 0) > Utility.DecimaltoDbnull(grdPresDetail.CurrentRow.Cells["TT_BN"].Value, 0))
                            {
                                Utility.ShowMsg("Tiền chiết khấu không được lớn hơn(>) tiền Bệnh nhân chi trả(" + Utility.DecimaltoDbnull(grdPresDetail.CurrentRow.Cells["TT_BN"].Value, 0).ToString() + "). Mời bạn kiểm tra lại");
                                e.Cancel = true;
                                return;
                            }
                            grdPresDetail.CurrentRow.Cells["tile_chietkhau"].Value = (Utility.DecimaltoDbnull(e.Value, 0) / Utility.DecimaltoDbnull(grdPresDetail.CurrentRow.Cells["TT_BN"].Value, 0)) * 100;
                        }
                    }
                    grdPresDetail.CurrentRow.Cells["TT"].Value = Utility.DecimaltoDbnull(grdPresDetail.CurrentRow.Cells["TT_BN"].Value, 0) - Utility.DecimaltoDbnull(grdPresDetail.CurrentRow.Cells["tien_chietkhau"].Value, 0);
                }

                else if (e.Column.Key == "sluong_sua")
                {
                    //Check các trạng thái
                    long _IdChitietdonthuoc = Utility.Int64Dbnull(grdPresDetail.GetValue(KcbDonthuocChitiet.Columns.IdChitietdonthuoc));
                    string ten_thuoc = grdPresDetail.GetValue("ten_chitietdichvu").ToString();
        ...
        private void frm_phatthuoc_tutruc_Load(object sender, EventArgs e)
        {
            LoadUserConfigs();
            if (kieuthuoc_vt == "ALL")
                m_dtKhothuoc = CommonLoadDuoc.LAYDANHMUCKHO(-1, kieukho, "ALL", "CHANLE,LE", 1, 100, 1);// CommonLoadDuoc.LAYTHONGTIN_KHOVTTH_LE_NGOAITRU();
            else if (kieuthuoc_vt == "THUOC")
                m_dtKhothuoc = CommonLoadDuoc.LAYDANHMUCKHO(-1, kieukho, "THUOC,THUOCVT", "CHANLE,LE", 1, 100, 1);// CommonLoadDuoc.LAYTHONGTIN_KHOTHUOC_LE_NGOAITRU();
            else
                m_dtKhothuoc = CommonLoadDuoc.LAYDANHMUCKHO(-1, kieukho, "VT,THUOCVT", "CHANLE,LE", 1, 100, 1);// CommonLoadDuoc.LAYTHONGTIN_KHOLE_NGOAITRU();
            DataBinding.BindData(cboKho, m_dtKhothuoc,
                                     TDmucKho.Columns.IdKho, TDmucKho.Columns.TenKho);
            DataBinding.BindDataCombobox(cboKhoanoitru,
                                                THU_VIEN_CHUNG.LaydanhsachKhoanoitruTheoBacsi(globalVariables.UserName, Utility.Bool2byte(globalVariables.IsAdmin), noitru_ngoaitru,"ALL"),
                ...
                m_dtDonthuoc =
                    SPs.ThuocTimkiemdonthuocTutruc(Utility.Int32Dbnull(txtPres_ID.Text, -1), txtPID.Text,
                                                           ...
                                                           chkByDate.Checked ? dtToDate.Value.ToString("dd/MM/yyyy") : "01/01/1900", Status,
                                                            Utility.Int32Dbnull(cboKho.SelectedValue),1,100,kieuthuoc_vt).GetDataSet().Tables[0];
        ...
        private void ModifyCommand()
        {
            try
            {
                ...
                cmdCapnhatdonthuoc.Enabled = false;
                cmdXoadonthuoc.Enabled = false;
                cmdThanhtoan.Enabled = false;
                cmdHuythanhtoan.Enabled = false;
                cmdPhanboHTTT.Enabled = false;
                ...
                cmdThemdonthuoc.Enabled = false;
                cmdTraLaiTien.Enabled = false;
                cmdInPhieuChi.Enabled = false;
            }
            catch (Exception ex)
            {
                Utility.CatchException(ex);
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frm_phatthuoc_tutruc_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        void DuyetCapphatthuoc()
        {
            try
            {
                ...
                if (presId > 0 && stockId > 0)
                {
                    ...
                    //List<long> lstIdchitietcancaphat = (from p in grdPresDetail.da.AsEnumerable()
                    //                                    where Utility.ByteDbnull(p.Cells[KcbDonthuocChitiet.Columns.TrangThai].Value, -1) == 0
        ...
        private KcbDanhsachBenhnhan TaoBenhnhan()
        {
            ...
            objBenhnhan.TenBenhnhan = "Bệnh nhân vãng lai.";
            objBenhnhan.DiaChi = "";
            objBenhnhan.KieuBenhnhan = 1;//0= đăng kí tiếp đón;1= bệnh nhân vãng lai tại quầy thuốc
            ...
            objBenhnhan.GioiTinh = "KXĐ";
            objBenhnhan.IdGioitinh = 2;
            objBenhnhan.NamSinh = 2000;
            objBenhnhan.DanToc = "01";
            objBenhnhan.NguoiTiepdon = globalVariables.UserName;
            objBenhnhan.NgayTiepdon = DateTime.Now;
            objBenhnhan.NguoiTao = globalVariables.UserName;
                        ...
                        DataTable dtTemp =
                         SPs.ThuocTimkiemdonthuocCapphatngoaitru(intIdDonthuoc, txtPID.Text,
                                                                ...
                                                                chkByDate.Checked ? dtToDate.Value.ToString("dd/MM/yyyy") : "01/01/1900",-1,
                                                                 Utility.Int32Dbnull(cboKho.SelectedValue),100, kieuthuoc_vt,100,1).GetDataSet().Tables[0];
                        foreach (DataRow dr in dtTemp.Rows)
                        {
                            m_dtDonthuoc.ImportRow(dr);
                        }
                        ...
                        case ActionResult.Success:
                            ...
                            if (IN_HOADON && PropertyLib._MayInProperties.TudonginhoadonSaukhiThanhtoan)
                            {
                                ...
                                if (kcbThanhtoanKieuinhoadon == 2 || kcbThanhtoanKieuinhoadon == 3)
                                    new INPHIEU_THANHTOAN_NGOAITRU().InBienlai(false, v_Payment_ID,-1, objLuotkham, 0);
                            }
        ...
        void HuyCkChitiettruockhithanhtoan()
        {
            ...
            try
            {
                ...
                foreach (GridEXRow _row in grdPresDetail.GetDataRows())
                {
        ...
        private void SetSumTotalProperties()
        {
            ...
            try
            {
                ...
                GridEXColumn gridExColumntrangthaithanhtoan = getGridExColumn(grdPresDetail, "trangthai_thanhtoan");
                GridEXColumn gridExColumn = getGridExColumn(grdPresDetail, "TT_KHONG_PHUTHU");
                GridEXColumn gridExColumnTutuc = getGridExColumn(grdPresDetail, "TT_BN_KHONG_TUTUC");
                GridEXColumn gridExColumnTt = getGridExColumn(grdPresDetail, "TT");
                GridEXColumn gridExColumnTtChietkhau = getGridExColumn(grdPresDetail, KcbThanhtoanChitiet.Columns.TienChietkhau);
                GridEXColumn gridExColumnBhyt = getGridExColumn(grdPresDetail, "TT_BHYT");
                GridEXColumn gridExColumnTtbn = getGridExColumn(grdPresDetail, "TT_BN");
                GridEXColumn gridExColumntutuc = getGridExColumn(grdPresDetail, "tu_tuc");
                GridEXColumn gridExColumntrangthaiHuy = getGridExColumn(grdPresDetail, "trangthai_huy");
                GridEXColumn gridExColumnPhuThu = getGridExColumn(grdPresDetail,
                    "TT_PHUTHU");
                ...
                GridEXColumn gridExColumnBnct = getGridExColumn(grdPresDetail,
                    "bnhan_chitra");
        ...
        private void TinhToanSoTienPhaithu()
        {
            ...
            try
            {
                List<GridEXRow> query = (from thanhtoan in grdPresDetail.GetCheckedRows()
                ...
                List<GridEXRow> query1 = (from thanhtoan in grdPresDetail.GetCheckedRows()
                                          where Utility.Int32Dbnull(thanhtoan.Cells["trangthai_huy"].Value) == 0
                                          select thanhtoan).ToList();
        ...
        void Capnhattin_ttoan(long id_donthuoc, byte tthai_ttoan, DateTime? ngay_thanhtoan)
        {
            ...
            try
            {
                DataRow[] arrdr = m_dtDonthuoc.Select(string.Format("{0}={1}", KcbDonthuoc.Columns.IdDonthuoc, id_donthuoc));
                if (arrdr.Length > 0)
                {
        ...
        private void cmdXoadonthuoc_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (Utility.AcceptQuestion(string.Format("Bạn có chắc chắn muốn xóa đơn thuốc của người bệnh {0}?", grdPres.GetValue("ten_benhnhan").ToString()), "xác nhận xóa", true))
                {
                    ...
                    DataRow[] arrdr = m_dtDonthuoc.Select(string.Format("{0}={1}", KcbDonthuoc.Columns.IdDonthuoc, Pres_ID));
                    if (arrdr.Length > 0)
                        m_dtDonthuoc.Rows.Remove(arrdr[0]);
                    m_dtChitietdonthuoc.AcceptChanges();
                    grdPres_SelectionChanged(grdPres, e);
