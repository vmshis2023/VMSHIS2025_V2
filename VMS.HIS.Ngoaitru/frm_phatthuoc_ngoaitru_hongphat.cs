...
{
    public partial class frm_phatthuoc_ngoaitru_hongphat : Form
    {
        ...
        public frm_phatthuoc_ngoaitru_hongphat(string kieuthuocVt)
        {
            ...
            try
            {
                ...
                Tra_thuoc_only = kieuthuocVt.Split('-')[3] == "1";
                InitEvents();
        ...
        void setquyen()
        {
            try
            {
                if (Tra_thuoc_only)
                {
                    uiTabPageCapphat.TabVisible = false;
                }
                else
                {
                  
                }
                bool hienthi_thanhtoan = Loai_Nguoidung == "TNV" || Loai_Nguoidung == "ALL";// Utility.Coquyen("thuoc_quaythuoc_thanhtoan") || Loai_Nguoidung == "TNV" || Loai_Nguoidung == "ALL";
                grdLichsutrathuoc.RootTable.Columns["NHAN"].Visible = hienthi_thanhtoan;
                uiTabPagePhieuchi.TabVisible = cmdTraLaiTien.Visible = cmdThanhtoan.Visible = cmdThanhtoan2.Visible = hienthi_thanhtoan;
                cmdHuythanhtoan.Visible = cmdHuythanhtoan2.Visible = cmdPhanboHTTT.Visible = hienthi_thanhtoan;
              ...
              cmdInphieuhoantrathuoc.Visible=  cmdXacnhantralai.Visible = cmdThemdonthuoc.Visible = cmdCapnhatdonthuoc.Visible = cmdXoadonthuoc.Visible = cmdPhatThuoc.Visible = cmdHuycapphat.Visible = cmdDuyetcapphat.Visible = cmdHuyduyetCapphat.Visible = Loai_Nguoidung == "DUOC" || Loai_Nguoidung == "ALL";
                ...
                pnlThongtintien.Height = hienthi_thanhtoan ? 132 : 0;
        ...
        void InitEvents()
        {
            ...
            this.Load += new System.EventHandler(this.frm_phatthuoc_ngoaitru_hongphat_Load);
            Shown += frm_phatthuoc_ngoaitru_hongphat_Shown;
            FormClosing += frm_phatthuoc_ngoaitru_hongphat_FormClosing;
            this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.frm_phatthuoc_ngoaitru_hongphat_KeyDown);
        ...
        void frm_phatthuoc_ngoaitru_hongphat_FormClosing(object sender, FormClosingEventArgs e)
        {
            SaveUserConfigs();
        ...
        void frm_phatthuoc_ngoaitru_hongphat_Shown(object sender, EventArgs e)
        {
            Try2Splitter();
            setquyen();
        }
        ...
        private void setProperties()
        {
            try
            {
                ...
                foreach (Control control in pnlThanhtoan.Controls)
                {
        ...
        private void frm_phatthuoc_ngoaitru_hongphat_Load(object sender, EventArgs e)
        {
            LoadUserConfigs();
            ucThongtinnguoibenh_quaythuoc1.SetReadonly();
            if (kieuthuoc_vt == "ALL")
        ...
        private void grdPres_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Utility.isValidGrid(grd_Donthuoc))
                {
                    ...
                    objLuotkham = Utility.getKcbLuotkham(Utility.Int64Dbnull(grd_Donthuoc.GetValue(KcbLuotkham.Columns.IdBenhnhan), -1), Utility.sDbnull(grd_Donthuoc.GetValue(KcbLuotkham.Columns.MaLuotkham), "-1"));
                    ucThongtinnguoibenh_quaythuoc1.Refresh(objLuotkham!=null?objLuotkham.MaLuotkham:"UKN");
                    ucThongtinnguoibenh_quaythuoc1.setKhoaPhongBacSi(Utility.sDbnull(grd_Donthuoc.GetValue("ten_khoaphong")), Utility.sDbnull(grd_Donthuoc.GetValue("ten_nhanvien")));
                    string ngaythanhtoan = Utility.sDbnull(grd_Donthuoc.GetValue("ngay_thanhtoan"),"");
                    LaythongtinchandoanICD();
                    GetDataPresDetail();
                    GetChiPhiDaThanhToan();
                    LaydanhsachLichsuthanhtoan_phieuchi();
                ...
                else
                {
                    ...
                    txtChanDoanICD.Clear();
                    grdChitietDonthuoc.DataSource = null;
                    ucThongtinnguoibenh_quaythuoc1.ClearControls();
                }
        ...
        List<string> lstChanDoanICD = new List<string>();
        void LaythongtinchandoanICD()
        {
            try
            {
                lstChanDoanICD.Clear();
                if(objLuotkham==null)
                {
                    txtChanDoanICD.Clear();
                    return;
                }    
                DataTable dtData = SPs.KcbLaythongtinChandoanIcdPhongkham(-1, objLuotkham.IdBenhnhan, objLuotkham.MaLuotkham).GetDataSet().Tables[0];
                foreach (DataRow dr in dtData.Rows)
                {
                  LayThongtinChanDoanInPhieuTomTatDieuTriNgoaiTru(Utility.sDbnull(dr["chan_doan"]), Utility.sDbnull(dr["mabenh_chinh"]), Utility.sDbnull(dr["tenbenh_chinh"]), Utility.sDbnull(dr["mabenh_phu"]), Utility.sDbnull(dr["tenbenh_phu"]));
                }
                string sValue = "";
                ...
                sValue = string.Join(";", lstChanDoanICD.ToArray<string>());
                //if (GhepCHANDOAN_PHUVAOCHANDOAN) sValue = sValue + ";" + chandoan;
                txtChanDoanICD.Text = sValue;
            }
            catch (Exception)
            {
        ...
        void LayThongtinChanDoanInPhieuTomTatDieuTriNgoaiTru(string chandoan, string ma_benh_chinh, string ten_benh_chinh, string ma_benh_phu, string ten_benh_phu)
        {
            ...
            if (!string.IsNullOrEmpty(ma_benh_chinh))
            {
                ...
                foreach (string ma in arrMa)
                {
                    ...
                    lstChanDoanICD.Add(string.Format("{0}-{1}", ma, ten));
                    idx++;
                }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frm_phatthuoc_ngoaitru_hongphat_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void LaydanhsachLichsuthanhtoan_phieuchi()
        {
            try
            {
                ...
                pnl_PhieuThu.Height =pnlThongtintien.Height>0 && m_dtPayment.Rows.Count > 0 ? 100 : 0;
                cmdInBienlai.Visible = cmdInhoadon.Visible = pnl_PhieuThu.Height > 0;
        ...
        private void mnu_cauhinh_duoc_vtth_Click(object sender, EventArgs e)
        {
            try
            {
                frm_Properties frm = new frm_Properties(PropertyLib._HisDuocProperties);
        ...
        private void mnu_cauhinh_in_Click(object sender, EventArgs e)
        {
            try
            {
                frm_Properties frm = new frm_Properties(PropertyLib._MayInProperties);
        ...
        private void mnu_cauhinh_phieuxuatthuoc_Click(object sender, EventArgs e)
        {
            var frm = new frm_Properties(PropertyLib._PhieuxuatBNProperty);
            frm.ShowDialog();
            ResetItems();
