...
namespace VNS.HIS.UI.Forms.NGOAITRU
{
    public partial class frm_chuyendoituongkcb : Form
    {
        ...
        void grdDanhsachthe_ColumnButtonClick(object sender, Janus.Windows.GridEX.ColumnActionEventArgs e)
        {
            try
            {
                objCurrent = KcbLichsuDoituongKcb.FetchByID(Utility.Int64Dbnull(KcbLichsuDoituongKcb.Columns.IdLichsuDoituongKcb));
                if (objCurrent != null && THU_VIEN_CHUNG.IsBaoHiem(objCurrent.IdLoaidoituongKcb))
                {
                    ...
                    if (!string.IsNullOrEmpty(Utility.sDbnull(objCurrent.NgaybatdauBhyt)))
                        dtInsFromDate.Value = Convert.ToDateTime(objCurrent.NgaybatdauBhyt);
                    if (!string.IsNullOrEmpty(Utility.sDbnull(objCurrent.NgayketthucBhyt)))
                        dtInsToDate.Value = Convert.ToDateTime(objCurrent.NgayketthucBhyt);
                    ...
                    txtNoiDongtrusoKCBBD.Text = Utility.sDbnull(objCurrent.NoiDongtrusoKcbbd);
                    txtDiachi_bhyt._Text = Utility.sDbnull(objCurrent.DiachiBhyt);
                    txtMaDTsinhsong.SetCode(objCurrent.MadtuongSinhsong);
                    ...
                    txtMaDoituongKCB.SetCode(objCurrent.MaDoituongKcbBhyt);
                    txtNoiDKKCBBD.Text = Utility.sDbnull(objCurrent.MaKcbbd);
                    txtMaDoituongKCB.SetCode(objCurrent.MaDoituongKcbBhyt);
        ...
        private void Getdata()
        {
            ...
            if (objLuotkham != null)
            {
                ...
                if (THU_VIEN_CHUNG.IsBaoHiem(_objDoituongKcb.IdLoaidoituongKcb) && !string.IsNullOrEmpty(objLuotkham.MatheBhyt))//Thông tin BHYT
                {
                    ...
                    txtMaDoituongKCB.SetCode(objLuotkham.MaDoituongKcbBhyt);
        ...
        void cmdSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (THU_VIEN_CHUNG.IsBaoHiem(_objDoituongKcb.IdLoaidoituongKcb))
                {
                    ...
                    foreach (DataRow dr in _dtLichsuKcb.Rows)
                    {
                        ...
                        {
                            if ((dtInsFromDate.Value.Date >= objTest.NgaybatdauBhyt && dtInsFromDate.Value.Date <= objTest.NgayketthucBhyt) || (dtInsToDate.Value.Date >= objTest.NgaybatdauBhyt && dtInsToDate.Value.Date <= objTest.NgayketthucBhyt))
                            {
                                ...
                                Utility.ShowMsg(string.Format("Ngày hiệu lực của thẻ BHYT mới: {0}-{1} không được phép nằm trong khoảng ngày hiệu lực của một trong các thẻ cũ: {2}-{3}", dtInsFromDate.Text, dtInsToDate.Text, objTest.NgaybatdauBhyt.Value.ToString("dd/MM/yyyy"), objTest.NgayketthucBhyt.Value.ToString("dd/MM/yyyy")));
                                return;
                            }
        ...
        private bool isValidData()
        {
            ...
            if (string.IsNullOrEmpty(txtNoidangkyKCBBD.Text))
            {
            ...
            if (dtInsToDate.Value.Date <objLuotkham.NgayTiepdon.Date)
            {
        ...
        void LoadNoiDKKCBBD()
        {
            ...
            if (dataTable.Rows.Count > 0)
            {
                ...
                txtNoidangkyKCBBD.Text = strClinicName.Trim();
            }
        }
                    ...
                    if (data.Length > 0)
                    {
                        if (data.EndsWith("$"))
                        {
                                ...
                                (arrayBhyt[7] != null))
                            {
                                ...
                                //txtMadauthe.Text = thebhyt.Substring(0, 2);
                        ...
                        //else if (AutofillInforbyQRCode(data))//Bỏ phần quét CMT
                        //{
                        //    txtQRCode.Clear();
                        //    txtTEN_BN.Focus();
